            ...
            switch (args[i]) {
                ...
                case '--replace-author':
                    ...
                    if (replacement && replacement.includes(',')) {
                        ...
                        console.error('❌ Error: --replace-author requires format "oldAuthor,newAuthor"');
                        process.exit(1);
                    }
                    break;
                case '--description':
  ...
  npm run update-frontmatter -- -p docs/zh --progress 100 --dry-run --verbose
    ...
    validateConfig(config) {
        if (!config.path) {
            ...
            console.error('💡 Usage: npm run update-frontmatter -- -p "path/to/docs" -e "EditorName"');
            return false;
        }
        ...
        if (!hasAnyField) {
            console.error('❌ Error: At least one frontmatter field must be specified');
            console.error('💡 Available options: -e/--editor, -a/--authors, -t/--title, -d/--description, --progress, --state, etc.');
            console.error('💡 Example: npm run update-frontmatter -- -p "docs/zh" -e "YourName"');
    ...
    updateAuthors(parsed, config, changes) {
        ...
        if (config.authors) {
            if (!parsed.data.authors || config.force) {
                ...
                hasChanges = true;
                operations.push(`authors: ${JSON.stringify(oldValue)} → ${JSON.stringify(config.authors)}`);
        ...
        if (config.replaceAuthor) {
            ...
            if (index !== -1) {
                ...
                hasChanges = true;
                operations.push(`authors: replaced "${oldAuthor}" → "${newAuthor}"`);
            } else if (config.verbose) {
    ...
    async run(args = process.argv.slice(2)) {
        ...
        if (config.replaceAuthor) contentFields.push(`replace-author: "${config.replaceAuthor.old}" → "${config.replaceAuthor.new}"`);
