...
export class DirectoryMigrationService {
    ...
    private async findBestDirectoryMatch(
        ...
        if (!bestMatch) {
            for (const activeDir of activeDirs) {
                ...
                // Check for common patterns like singular â†?plural
    ...
    /**
     * Checks for common rename patterns like singular â†?plural.
     */
    private isLikelyRename(oldName: string, newName: string): boolean {
        // Check for singular â†?plural transformations
        if (oldName + 's' === newName) return true;
        if (oldName === newName + 's') return true;
        
        // Check for other common patterns
        const patterns = [
            [/tool$/, /tools$/], // tool â†?tools
            [/item$/, /items$/], // item â†?items
            [/doc$/, /docs$/],   // doc â†?docs
            [/page$/, /pages$/], // page â†?pages
        ];
