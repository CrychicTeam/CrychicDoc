...
export class SyncEngine {
    ...
    public async syncOverrideType(
        ...
        const processedItems = structuralItems.map((item) => ({
            ...item,
            items: item.items
                ? (JSON.parse(JSON.stringify(item.items)) as SidebarItem[])
                : undefined,
        }));
        ...
        const isKubeJSSection = structuralItems.some(
            (item) =>
                item._relativePathKey?.includes("1.20.1") ||
                item.text?.includes("KubeJS") ||
                (item.items &&
                    item.items.some((child) =>
                        child._relativePathKey?.includes("Upgrade")
                    ))
        );
        ...
        for (let i = 0; i < processedItems.length; i++) {
            ...
            if (itemType === "locales") {
                ...
                valueFromStructure = currentProcessedItem._isDirectory
                    ? currentProcessedItem.collapsed !== undefined
                        ? currentProcessedItem.collapsed
                        : true
            ...
            if (existingJsonData.hasOwnProperty(itemKey)) {
                // Key exists in current JSON
                if (
                    metadataManagerInstance.isEntryUserModified(
                        valueFromJson,
                        metadataForThisKey
                    )
                ) {
                    ...
                    if (
                        itemType === "locales" &&
                        valueFromStructure !== valueFromJson
                    ) {
                ...
                // New item in structure, not in JSON: create a stub.
                ...
                if (itemType === "locales") {
                    ...
                    if (currentProcessedItem._isDirectory) {
                        finalValueToStoreInJson =
                            currentProcessedItem.collapsed !== undefined
                                ? currentProcessedItem.collapsed
                                : true; // Default new directories to collapsed
                    } else {
            ...
            if (finalValueToStoreInJson !== undefined) {
                updatedMetadata[itemKey] =
                    metadataManagerInstance.createNewMetadataEntry(
                        updatedJsonData.hasOwnProperty(itemKey)
                            ? updatedJsonData[itemKey]
                            : valueFromStructure, // Hash what's in JSON, or structural value if no JSON entry
                        isUserSetValue,
            ...
            if (
                ...
                currentProcessedItem.collapsed = updatedJsonData[itemKey] as
                    | boolean
                    | undefined;
            }
        }
        ...
        for (const existingKey in existingJsonData) {
            if (!structuralItemKeys.has(existingKey)) {
                // Key is in JSON but not in current structure (orphaned)
