...
export class KeyMigrationService {
    ...
    public async migrateKeysToRelative(
        ...
        for (const type of overrideTypes) {
            try {
                const jsonData = await this.jsonFileHandler.readJsonFile(
                    type,
                    lang,
                    currentConfigDirSignature
                );
                const metadata = await this.metadataManager.readMetadata(
                    type,
                    lang,
                    currentConfigDirSignature
                );
                ...
                for (const item of items) {
                    const fullKey =
                        item._relativePathKey || item.text || "untitled";
                    const relativeKey =
                        this.pathProcessor.extractRelativeKeyForCurrentDir(
                            item,
                            currentConfigDirSignature
                        );
                ...
                for (const [existingKey, value] of Object.entries(jsonData)) {
                    ...
                    const newKey =
                        pathMigrationMap.get(existingKey) || existingKey;
                ...
                if (typeHadMigration) {
                    await this.jsonFileHandler.writeJsonFile(
                        type,
                        lang,
                        currentConfigDirSignature,
                        migratedData
                    );
                    await this.metadataManager.writeMetadata(
                        type,
                        lang,
                        currentConfigDirSignature,
                        migratedMetadata
    ...
    public async migrateKeysRecursively(
        ...
        const currentMigration = await this.migrateKeysToRelative(
            items,
            currentConfigDirSignature,
            lang
        );
        ...
        for (const item of items) {
            if (item._isDirectory || (item.items && item.items.length > 0)) {
                ...
                const nextConfigDirSignature =
                    currentConfigDirSignature === "_root"
                        ? itemKeyPart
                        : path
                              .join(currentConfigDirSignature, itemKeyPart)
                              .replace(/\\/g, "/");

                // Skip GitBook directories
                if (
                    !this.pathProcessor.isGitBookRoot(
    ...
    public async cleanupRenamedDirectoryMetadata(
        ...
        try {
            const basePath = path.join(
                this.jsonFileHandler.getBaseOverridesPath(),
                lang,
                rootConfigDirSignature
            );
            const metadataBasePath = path.join(
                this.jsonFileHandler.getBaseOverridesPath(),
                ".metadata",
                lang,
                rootConfigDirSignature
            ...
            await this.cleanupOrphanedMetadataDirectories(
                basePath,
                metadataBasePath,
                currentActiveSignatures
            );
        } catch (error) {}
    }
                    ...
                    if (!actualExists && !isActive) {
                        ...
                        if (migrationTarget) {
                            ...
                            const migrationConfirmed =
                                await this.verifySuccessfulMigration(
                                    metadataPath,
                                    targetMetadataPath
                                );
                        ...
                        const hasUserData = await this.checkForUserData(
                            metadataPath
                        );
     ...
     */
    private async findMigrationTarget(
        orphanedDir: string,
        activeSignatures: Set<string>
    ): Promise<string | null> {
        ...
        for (const activeDir of activeDirectories) {
            if (
                ...
                ((activeDir.includes(orphanedDir) ||
                    orphanedDir.includes(activeDir)) &&
                    !(
                        orphanedDir === "tool" ||
                        orphanedDir === "tools" ||
                        activeDir === "tool" ||
                ...
                (Math.abs(activeDir.length - orphanedDir.length) <= 2 &&
                    Math.abs(activeDir.length - orphanedDir.length) > 0)
            ) {
                return activeDir;
            }
        }
     ...
     */
    private async verifySuccessfulMigration(
        sourcePath: string,
        targetPath: string
    ): Promise<{
        hasMigrated: boolean;
        ...
        try {
            ...
            for (const type of overrideTypes) {
                ...
                try {
                    ...
                    for (const [key, sourceEntry] of Object.entries(
                        sourceData
                    )) {
                        if (
                            this.isValidMetadataEntry(sourceEntry) &&
                            sourceEntry.isUserSet &&
                            targetData[key]
                        ) {
                            ...
                            if (
                                this.isValidMetadataEntry(targetEntry) &&
                                targetEntry.isUserSet &&
                                targetEntry.valueHash ===
                                    sourceEntry.valueHash &&
                                targetEntry.lastSeen &&
                                sourceEntry.lastSeen &&
            ...
            for (const type of overrideTypes) {
                ...
                try {
                    ...
                    for (const [key, entry] of Object.entries(data)) {
                        if (
                            this.isValidMetadataEntry(entry) &&
                            entry.isUserSet
                        ) {
                            return true;
                        }
