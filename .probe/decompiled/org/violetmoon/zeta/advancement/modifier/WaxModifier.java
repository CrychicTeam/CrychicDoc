...
public class WaxModifier extends AdvancementModifier {
    ...
    public boolean apply(ResourceLocation res, IMutableAdvancement adv) {
        ...
        if (criterion != null && criterion.getTrigger() instanceof ItemUsedOnLocationTrigger.TriggerInstance iib) {
            iib.location.compositePredicates = res.equals(TARGET_ON) ? iib.location.compositePredicates.or(ItemUsedOnLocationTrigger.TriggerInstance.itemUsedOnBlock(LocationPredicate.Builder.location().setBlock(BlockPredicate.Builder.block().of(this.unwaxed).build()), ItemPredicate.Builder.item().of(Items.HONEYCOMB)).location.compositePredicates) : iib.location.compositePredicates.or(ItemUsedOnLocationTrigger.TriggerInstance.itemUsedOnBlock(LocationPredicate.Builder.location().setBlock(BlockPredicate.Builder.block().of(this.waxed).build()), ItemPredicate.Builder.item().of(ItemTags.AXES)).location.compositePredicates);
        }
        return true;
    }
}