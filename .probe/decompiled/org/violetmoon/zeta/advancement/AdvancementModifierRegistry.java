...
public class AdvancementModifierRegistry {
    ...
    public void addListeners(ZAddReloadListener event) {
        if (!this.gatheredAddons) {
            final IAdvancementModifierDelegate delegateImpl = new AdvancementModifierRegistry.DelegateImpl();
            this.zeta.loadBus.fireExternal(new ZGatherAdvancementModifiers() {

                @Override
                public void register(IAdvancementModifier modifier) {
                    AdvancementModifierRegistry.this.addModifier(modifier);
                }
        ...
        event.addListener((ResourceManagerReloadListener) mgr -> this.onAdvancementsLoaded(advancements));
    }
    ...
    private void onAdvancementsLoaded(ServerAdvancementManager manager) {
        for (ResourceLocation res : this.modifiers.keySet()) {
            ...
            if (adv != null) {
                Collection<IAdvancementModifier> found = this.modifiers.get(res);
                if (!found.isEmpty()) {
                    int modifications = 0;
                    ...
                    if (modifications > 0) {
                        this.zeta.log.info("Modified advancement {} with {} patches", adv.getId(), modifications);
                        mutable.commit();
                    }
                }
            }
