...
public class IForgeBlockBlockExtensions implements IZetaBlockExtensions {
    ...
    public boolean makesOpenTrapdoorAboveClimbableZeta(BlockState state, LevelReader level, BlockPos pos, BlockState trapdoorState) {
        return state.m_60734_().makesOpenTrapdoorAboveClimbable(state, level, pos, trapdoorState);
    }
    ...
    public BlockState getToolModifiedStateZeta(BlockState state, UseOnContext context, String toolActionType, boolean simulate) {
        ...
        return action == null ? null : state.getToolModifiedState(context, action, simulate);
    }

    @Override
    public boolean isScaffoldingZeta(BlockState state, LevelReader level, BlockPos pos, LivingEntity entity) {
        return state.m_60734_().isScaffolding(state, level, pos, entity);
    }
}