...
public class SearchTextFieldModel {
    ...
    public SearchTextFieldModel(Collection<OptionPage> pages, EmbeddiumVideoOptionsScreen mainScreen) {
        this.pages = pages;
        this.allOptions = (Set<Option<?>>) pages.stream().flatMap(p -> p.getOptions().stream()).collect(Collectors.toUnmodifiableSet());
    ...
    public Predicate<Option<?>> getOptionPredicate() {
        return (Predicate<Option<?>>) (this.selectedOptions == this.allOptions ? Predicates.alwaysTrue() : this.selectedOptions::contains);
    }
        ...
        if (this.editable && !newText.trim().isEmpty()) {
            List<Option<?>> fuzzy = StringUtils.fuzzySearch(() -> this.pages.stream().flatMap(p -> p.getOptions().stream()).iterator(), newText, 2, o -> {
                ...
                if (o.getControl() instanceof CyclingControl<?> cycler) {
                    name = name + " " + (String) Arrays.stream(cycler.getNames()).map(Component::getString).collect(Collectors.joining(" "));
                }
                return name;
            });
            this.selectedOptions = new HashSet(fuzzy);
        }
        Minecraft.getInstance().m_6937_(() -> this.mainScreen.rebuildUI());
    }
    ...
    public void setSelectionEnd(int index) {
        ...
        if (textRenderer != null) {
            ...
            if (this.selectionEnd == this.firstCharacterIndex) {
                this.firstCharacterIndex = this.firstCharacterIndex - textRenderer.plainSubstrByWidth(this.text, j, true).length();
            }
