...
public class CookingPotTooltip implements ClientTooltipComponent {
    ...
    public int getWidth(Font font) {
        if (!this.mealStack.isEmpty()) {
            MutableComponent textServingsOf = this.mealStack.getCount() == 1 ? TextUtils.getTranslation("tooltip.cooking_pot.single_serving") : TextUtils.getTranslation("tooltip.cooking_pot.many_servings", this.mealStack.getCount());
    ...
    public void renderText(Font font, int x, int y, Matrix4f matrix4f, MultiBufferSource.BufferSource bufferSource) {
        ...
        if (!this.mealStack.isEmpty()) {
            ...
            font.drawInBatch(this.mealStack.getHoverName(), (float) (x + 16 + 4), (float) (y + this.textSpacing + 4), -1, true, matrix4f, bufferSource, Font.DisplayMode.NORMAL, 0, 15728880);
        } else {
            MutableComponent textEmpty = TextUtils.getTranslation("tooltip.cooking_pot.empty");
            font.drawInBatch(textEmpty, (float) x, (float) y, gray, true, matrix4f, bufferSource, Font.DisplayMode.NORMAL, 0, 15728880);
        }
    }
