...
public class StatGetterEffectEfficiency implements IStatGetter {
    ...
    public double getValue(Player player, ItemStack itemStack) {
        return this.base + (Double) CastOptional.cast(itemStack.getItem(), IModularItem.class).map(item -> (double) item.getEffectEfficiency(itemStack, this.effect) * this.multiplier).orElse(0.0);
    }

    @Override
    public double getValue(Player player, ItemStack itemStack, String slot) {
        return (Double) CastOptional.cast(itemStack.getItem(), IModularItem.class).map(item -> item.getModuleFromSlot(itemStack, slot)).map(module -> (double) module.getEffectEfficiency(itemStack, this.effect) * this.multiplier).orElse(0.0);
    }
