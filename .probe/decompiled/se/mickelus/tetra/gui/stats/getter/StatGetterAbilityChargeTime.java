...
public class StatGetterAbilityChargeTime implements IStatGetter {
    ...
    public double getValue(Player player, ItemStack itemStack) {
        return (Double) CastOptional.cast(itemStack.getItem(), ItemModularHandheld.class).map(item -> this.ability.getChargeTime(item, itemStack)).map(ticks -> (double) ticks.intValue() / 20.0).orElse(0.0);
    }
