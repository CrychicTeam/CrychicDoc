...
public class CraftingContext {
    ...
    public CraftingContext(@Nullable Level world, @Nullable BlockPos pos, @Nullable BlockState blockState, @Nullable Player player, ItemStack targetStack, String slot, ResourceLocation[] unlocks) {
        ...
        this.targetModule = (ItemModule) CastOptional.cast(targetStack.getItem(), IModularItem.class).map(item -> item.getModuleFromSlot(targetStack, slot)).orElse(null);
        if (this.targetModule instanceof ItemModuleMajor targetMajorModule) {
