...
public class ModLoaderCondition extends StructureCondition {
    ...
    public static final Codec<ModLoaderCondition> CODEC = RecordCodecBuilder.create(builder -> builder.group(Codec.STRING.listOf().fieldOf("loaders").forGetter(conditon -> conditon.validLoaders)).apply(builder, ModLoaderCondition::new));

    private final List<String> validLoaders;

    public ModLoaderCondition(List<String> validLoaders) {
        this.validLoaders = (List<String>) validLoaders.stream().map(String::toLowerCase).collect(Collectors.toList());
    }
