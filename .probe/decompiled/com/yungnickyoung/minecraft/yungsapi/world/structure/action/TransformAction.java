...
public class TransformAction extends StructureAction {
    ...
    private static <T> DataResult<T> encodeTemplate(Either<ResourceLocation, StructureTemplate> either, DynamicOps<T> ops, T data) {
        return either.left().isEmpty() ? DataResult.error(() -> "yungsapi - Cannot serialize a runtime pool element") : ResourceLocation.CODEC.encode((ResourceLocation) either.left().get(), ops, data);
    }
    ...
    public void apply(StructureContext ctx, PieceEntry targetPieceEntry) {
        ...
        if (templateManager == null) {
            ...
            StructurePoolElement newElement = new YungJigsawSinglePoolElement(newTemplate, old.processors, old.m_210539_(), old.name, old.maxCount, old.minRequiredDepth, old.maxPossibleDepth, old.isPriority, old.ignoreBounds, old.condition, old.enhancedTerrainAdaptation, old.deadendPool, old.modifiers);
