...
public class StructureTypeModuleForge {
    ...
    private static void registerStructureTypes(RegisterEvent event) {
        event.register(Registries.STRUCTURE_TYPE, helper -> AutoRegistrationManager.STRUCTURE_TYPES.stream().filter(data -> !data.processed()).forEach(data -> registerStructureType(data, helper)));
    }
