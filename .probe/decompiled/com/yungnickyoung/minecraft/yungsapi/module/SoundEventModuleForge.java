...
public class SoundEventModuleForge {
    ...
    private static void registerSoundEvents(RegisterEvent event) {
        event.register(Registries.SOUND_EVENT, helper -> AutoRegistrationManager.SOUND_EVENTS.stream().filter(data -> !data.processed()).forEach(data -> registerSoundEvent(data, helper)));
    }
