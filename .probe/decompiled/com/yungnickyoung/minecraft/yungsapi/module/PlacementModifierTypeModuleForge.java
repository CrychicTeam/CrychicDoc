...
public class PlacementModifierTypeModuleForge {
    ...
    private static void registerPlacementModifierTypes(RegisterEvent event) {
        event.register(Registries.PLACEMENT_MODIFIER_TYPE, helper -> AutoRegistrationManager.PLACEMENT_MODIFIER_TYPES.stream().filter(data -> !data.processed()).forEach(data -> registerPlacementModifierType(data, helper)));
    }
