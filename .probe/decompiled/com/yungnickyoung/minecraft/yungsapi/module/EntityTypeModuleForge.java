...
public class EntityTypeModuleForge {
    ...
    private static void registerEntityTypes(RegisterEvent event) {
        event.register(Registries.ENTITY_TYPE, helper -> AutoRegistrationManager.ENTITY_TYPES.stream().filter(data -> !data.processed()).forEach(data -> registerEntityType(data, helper)));
    }
    ...
    private static void registerEntityAttributes(EntityAttributeCreationEvent event) {
        ENTITY_ATTRIBUTES.forEach((entityType, builderSupplier) -> {
            ...
            builder.add(ForgeMod.SWIM_SPEED.get()).add(ForgeMod.NAMETAG_DISTANCE.get()).add(ForgeMod.ENTITY_GRAVITY.get());
            event.put((EntityType<? extends LivingEntity>) entityType.get(), builder.build());
        });
    }
}