    ...
    private static void addPaxiPackSource(AddPackFindersEvent event) {
        switch(event.getPackType()) {
            case CLIENT_RESOURCES:
                event.addRepositorySource(new PaxiRepositorySource(PaxiCommon.RESOURCE_PACK_DIRECTORY, PackType.CLIENT_RESOURCES, PaxiCommon.RESOURCEPACK_ORDERING_FILE));
                break;
            case SERVER_DATA:
                event.addRepositorySource(new PaxiRepositorySource(PaxiCommon.DATA_PACK_DIRECTORY, PackType.SERVER_DATA, PaxiCommon.DATAPACK_ORDERING_FILE));
        }
    }
}