...
public class ReplaceLiquidOnlyProcessor extends StructureProcessor {
    ...
    public static final Codec<ReplaceLiquidOnlyProcessor> CODEC = RecordCodecBuilder.create(instance -> instance.group(BlockState.CODEC.listOf().xmap(Sets::newHashSet, Lists::newArrayList).optionalFieldOf("blocks_to_always_place", new HashSet()).forGetter(config -> config.blocksToAlwaysPlace)).apply(instance, instance.stable(ReplaceLiquidOnlyProcessor::new)));
    ...
    public StructureTemplate.StructureBlockInfo processBlock(LevelReader worldView, BlockPos pos, BlockPos blockPos, StructureTemplate.StructureBlockInfo structureBlockInfoLocal, StructureTemplate.StructureBlockInfo structureBlockInfoWorld, StructurePlaceSettings structurePlacementData) {
        if (!this.blocksToAlwaysPlace.contains(structureBlockInfoWorld.state())) {
            ...
            if (!worldState.m_60819_().isEmpty() && !structureBlockInfoWorld.state().m_155947_() && !aboveWorldState.m_155947_()) {
