...
public class EnhancedBeardifierHelper {
    ...
    public static Beardifier forStructuresInChunk(StructureManager structureManager, ChunkPos chunkPos, Beardifier original) {
        ...
        for (StructureStart structureStart : structureManager.startsForStructure(chunkPos, structure -> structure instanceof JigsawStructure)) {
            ...
            for (StructurePiece structurePiece : structureStart.getPieces()) {
                if (structurePiece instanceof PoolElementStructurePiece) {
                    ...
                    if (poolElementPiece instanceof IAPoolElement) {
                        IAPoolElement iaElement = (IAPoolElement) poolElementPiece;
                        if (iaElement.getEnhancedTerrainAdaptation().isPresent()) {
                            kernelRadius = Math.max(kernelRadius, ((EnhancedTerrainAdaptation) iaElement.getEnhancedTerrainAdaptation().get()).getKernelRadius());
                        }
                    }
                }
            }
                ...
                for (StructurePiece nearbyPiece : structureStart.getPieces().stream().filter(structurePiecex -> structurePiecex.isCloseToChunk(chunkPos, maxKernelRadius)).toList()) {
                    if (nearbyPiece instanceof PoolElementStructurePiece) {
                        ...
                        if (poolElementPiece.getElement() instanceof IAPoolElement iaElement && iaElement.getEnhancedTerrainAdaptation().isPresent()) {
                            pieceTerrainAdaptation = (EnhancedTerrainAdaptation) iaElement.getEnhancedTerrainAdaptation().get();
                        }
