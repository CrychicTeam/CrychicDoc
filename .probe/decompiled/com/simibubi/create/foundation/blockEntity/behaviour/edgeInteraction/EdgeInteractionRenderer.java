                ...
                if (behaviour != null) {
                    if (behaviour.requiredItem.orElse(heldItem.getItem()) == heldItem.getItem()) {
                        ...
                        if (!connectiveSides.isEmpty()) {
                            Direction closestEdge = (Direction) connectiveSides.get(0);
                            ...
                            Vec3 offset = Vec3.atLowerCornerOf(closestEdge.getNormal()).scale(0.5).add(Vec3.atLowerCornerOf(face.getNormal()).scale(0.469)).add(VecHelper.CENTER_OF_ORIGIN);
                            ValueBox box = new ValueBox(Components.immutableEmpty(), bb, pos).passive(!hit).transform(new EdgeInteractionRenderer.EdgeValueBoxTransform(offset)).wideOutline();
                            CreateClient.OUTLINER.showValueBox("edge", box).highlightFace(face);
                            if (hit) {
                                List<MutableComponent> tip = new ArrayList();
                                tip.add(Lang.translateDirect("logistics.crafter.connected"));
                                tip.add(Lang.translateDirect(CrafterHelper.areCraftersConnected(world, pos, pos.relative(closestEdge)) ? "logistics.crafter.click_to_separate" : "logistics.crafter.click_to_merge"));
