...
public class ContinuousOBBCollider extends OBBCollider {
    ...
    public static ContinuousOBBCollider.ContinuousSeparationManifold separateBBs(Vec3 cA, Vec3 cB, Vec3 eA, Vec3 eB, Matrix3d m, Vec3 motion) {
        ContinuousOBBCollider.ContinuousSeparationManifold mf = new ContinuousOBBCollider.ContinuousSeparationManifold();
        ...
        return !separate(mf, uA0, diff.x, eA.x, a00 * eB.x + a01 * eB.y + a02 * eB.z, motion.x, true) && !separate(mf, uA1, diff.y, eA.y, a10 * eB.x + a11 * eB.y + a12 * eB.z, motion.y, true) && !separate(mf, uA2, diff.z, eA.z, a20 * eB.x + a21 * eB.y + a22 * eB.z, motion.z, true) && !separate(mf, uB0, diff2.x, eA.x * a00 + eA.y * a10 + eA.z * a20, eB.x, motion2.x, false) && !separate(mf, uB1, diff2.y, eA.x * a01 + eA.y * a11 + eA.z * a21, eB.y, motion2.y, false) && !separate(mf, uB2, diff2.z, eA.x * a02 + eA.y * a12 + eA.z * a22, eB.z, motion2.z, false) ? mf : null;
    }

    static boolean separate(ContinuousOBBCollider.ContinuousSeparationManifold mf, Vec3 axis, double TL, double rA, double rB, double projectedMotion, boolean axisOfObjA) {
        checkCount++;
        ...
        if (!discreteCollision && Math.signum(projectedMotion) == Math.signum(TL)) {
            ...
            boolean isBestSeperation = distance != 0.0 && -diff <= Math.abs(mf.separation);
            ...
            if (isBestSeperation) {
                ...
                mf.collisionPosition = normalizedAxis.scale(Math.signum(TL) * (axisOfObjA ? -rB : -rB) - Math.signum(seperation) * 0.125);
            }
            return false;
        }
    }
    ...
    public static class ContinuousSeparationManifold extends OBBCollider.SeparationManifold {
        ...
        public Vec3 asSeparationVec(double obbStepHeight) {
            if (this.isDiscreteCollision) {
                return this.stepSeparation <= obbStepHeight ? this.createSeparationVec(this.stepSeparation, this.stepSeparationAxis) : super.asSeparationVec();
            } else {
                double t = this.getTimeOfImpact();
                return t == -1.0 ? null : Vec3.ZERO;
            }
        }
