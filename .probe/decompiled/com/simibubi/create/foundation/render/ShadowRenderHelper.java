...
public class ShadowRenderHelper {
    ...
    public static void renderShadow(PoseStack matrixStack, MultiBufferSource buffer, float opacity, float radius) {
        ...
        opacity /= 2.0F;
        shadowVertex(entry, builder, opacity, -1.0F * radius, 0.0F, -1.0F * radius, 0.0F, 0.0F);
        shadowVertex(entry, builder, opacity, -1.0F * radius, 0.0F, 1.0F * radius, 0.0F, 1.0F);
        shadowVertex(entry, builder, opacity, 1.0F * radius, 0.0F, 1.0F * radius, 1.0F, 1.0F);
        shadowVertex(entry, builder, opacity, 1.0F * radius, 0.0F, -1.0F * radius, 1.0F, 0.0F);
    }
    ...
    public static void renderShadow(PoseStack matrixStack, MultiBufferSource buffer, LevelReader world, Vec3 pos, float opacity, float radius) {
        ...
        double d0 = pos.y();
        double d1 = pos.z();
        int i = Mth.floor(d2 - (double) radius);
        int j = Mth.floor(d2 + (double) radius);
        int k = Mth.floor(d0 - (double) radius);
        int l = Mth.floor(d0);
        int i1 = Mth.floor(d1 - (double) radius);
        int j1 = Mth.floor(d1 + (double) radius);
        PoseStack.Pose entry = matrixStack.last();
        VertexConsumer builder = buffer.getBuffer(SHADOW_LAYER);
    ...
    private static void renderBlockShadow(PoseStack.Pose entry, VertexConsumer builder, LevelReader world, BlockPos pos, double x, double y, double z, float radius, float opacity) {
        ...
        if (blockstate.m_60799_() != RenderShape.INVISIBLE && world.getMaxLocalRawBrightness(pos) > 3 && blockstate.m_60838_(world, blockpos)) {
            VoxelShape voxelshape = blockstate.m_60808_(world, pos.below());
            if (!voxelshape.isEmpty()) {
                ...
                if (f >= 0.0F) {
                    ...
                    double d0 = (double) pos.m_123341_() + AABB.minX;
                    double d1 = (double) pos.m_123341_() + AABB.maxX;
                    double d2 = (double) pos.m_123342_() + AABB.minY;
                    double d3 = (double) pos.m_123343_() + AABB.minZ;
                    double d4 = (double) pos.m_123343_() + AABB.maxZ;
                    float f1 = (float) (d0 - x);
                    float f2 = (float) (d1 - x);
    ...
    private static void shadowVertex(PoseStack.Pose entry, VertexConsumer builder, float alpha, float x, float y, float z, float u, float v) {
        builder.vertex(entry.pose(), x, y, z).color(1.0F, 1.0F, 1.0F, alpha).uv(u, v).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(15728880).normal(entry.normal(), 0.0F, 1.0F, 0.0F).endVertex();
    }
}