...
public class Debug {
    ...
    public static void debugChatAndShowStack(String message, int depth) {
        if (Minecraft.getInstance().player != null) {
            Minecraft.getInstance().player.displayClientMessage(Components.literal(message).append("@").append(debugStack(depth)), false);
        }
    }
    ...
    public static Component debugStack(int depth) {
        ...
        MutableComponent text = Components.literal("[").append(Components.literal(getLogicalSide()).withStyle(ChatFormatting.GOLD)).append("] ");
        for (int i = 1; i < depth + 2 && i < stackTraceElements.length; i++) {
            ...
            if (!e.getClassName().equals(Debug.class.getName())) {
                text.append(Components.literal(e.getMethodName()).withStyle(ChatFormatting.YELLOW)).append(", ");
            }
        }
