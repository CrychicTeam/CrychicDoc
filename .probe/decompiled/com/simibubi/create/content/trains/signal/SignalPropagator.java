...
public class SignalPropagator {
    ...
    public static void walkSignals(TrackGraph graph, SignalBoundary signal, boolean front, Predicate<Pair<TrackNode, SignalBoundary>> boundaryCallback, Predicate<EdgeData> nonBoundaryCallback, boolean forCollection) {
        ...
        Couple<TrackEdge> startEdges = startNodes.mapWithParams((l1, l2) -> (TrackEdge) graph.getConnectionsFrom(l1).get(l2), startNodes.swap());
    ...
    private static void walkSignals(TrackGraph graph, List<Couple<TrackNode>> frontier, Predicate<Pair<TrackNode, SignalBoundary>> boundaryCallback, Predicate<EdgeData> nonBoundaryCallback, boolean forCollection) {
        ...
        while (!frontier.isEmpty()) {
            Couple<TrackNode> couple = (Couple<TrackNode>) frontier.remove(0);
            TrackNode currentNode = couple.getFirst();
            TrackNode prevNode = couple.getSecond();
            label65: for (Entry<TrackNode, TrackEdge> entry : graph.getConnectionsFrom(currentNode).entrySet()) {
                TrackNode nextNode = (TrackNode) entry.getKey();
                TrackEdge edge = (TrackEdge) entry.getValue();
                if (nextNode != prevNode && visited.add(edge) && (!forCollection || ((TrackEdge) graph.getConnectionsFrom(prevNode).get(currentNode)).canTravelTo(edge))) {
