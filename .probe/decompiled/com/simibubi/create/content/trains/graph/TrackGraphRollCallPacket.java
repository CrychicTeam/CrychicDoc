...
public class TrackGraphRollCallPacket extends SimplePacketBase {
    ...
    public TrackGraphRollCallPacket(FriendlyByteBuf buffer) {
        this.ints = new int[buffer.readVarInt()];
    ...
    public void write(FriendlyByteBuf buffer) {
        buffer.writeVarInt(this.ints.length);
        for (int i : this.ints) {
            buffer.writeInt(i);
        }
    }
    ...
    public boolean handle(NetworkEvent.Context context) {
        context.enqueueWork(() -> {
            ...
            for (int i = 0; i < this.ints.length; i += 2) {
                UUID uuid = (UUID) idByNetId.get(this.ints[i]);
                if (uuid == null) {
                    failedIds.add(this.ints[i]);
                } else {
                    unusedIds.remove(uuid);
                    TrackGraph trackGraph = (TrackGraph) manager.trackNetworks.get(uuid);
                    if (trackGraph.getChecksum() != this.ints[i + 1]) {
                        Create.LOGGER.warn("Track network: " + uuid.toString().substring(0, 6) + " failed its checksum; Requesting refresh");
                        failedIds.add(this.ints[i]);
                    }
                }
            }
