...
public class WorldshaperItem extends ZapperItem {
    ...
    public Component validateUsage(ItemStack item) {
        return (Component) (!item.getOrCreateTag().contains("BrushParams") ? Lang.translateDirect("terrainzapper.shiftRightClickToSet") : super.validateUsage(item));
    }
    ...
    protected boolean activate(Level world, Player player, ItemStack stack, BlockState stateToUse, BlockHitResult raytrace, CompoundTag data) {
        ...
        targetPos = targetPos.offset(brush.getOffset(player.m_20154_(), raytrace.getDirection(), option));
        ...
        brush.redirectTool(tool).run(world, affectedPositions, raytrace.getDirection(), stateToUse, data, player);
        return true;
    }
