...
public class KineticSpeedDisplaySource extends NumericSingleLineDisplaySource {
    ...
    protected MutableComponent provideLine(DisplayLinkContext context, DisplayTargetStats stats) {
        if (context.getSourceBlockEntity() instanceof SpeedGaugeBlockEntity speedGauge) {
            ...
            return Lang.number((double) speed).space().translate("generic.unit.rpm").component();
        } else {
            return ZERO.copy();
        }
    }
    ...
    public void initConfigurationWidgets(DisplayLinkContext context, ModularGuiLineBuilder builder, boolean isFirstLine) {
        ...
        if (!isFirstLine) {
            builder.addSelectionScrollInput(0, 95, (selectionScrollInput, label) -> selectionScrollInput.forOptions(Lang.translatedOptions("display_source.kinetic_speed", "absolute", "directional")), "Directional");
        }
    }
