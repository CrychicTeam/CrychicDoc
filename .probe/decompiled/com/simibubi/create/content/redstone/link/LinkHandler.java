                    ...
                    if (!AllItems.LINKED_CONTROLLER.isIn(heldItem)) {
                        if (!AllItems.WRENCH.isIn(heldItem)) {
                            ...
                            if (fakePlayer) {
                                ...
                                Vec3 localHit = ray.m_82450_().subtract(Vec3.atLowerCornerOf(pos)).add(Vec3.atLowerCornerOf(ray.getDirection().getNormal()).scale(0.25));
                                fakePlayerChoice = localHit.distanceToSqr(behaviour.firstSlot.getLocalOffset(blockState)) > localHit.distanceToSqr(behaviour.secondSlot.getLocalOffset(blockState));
                            }
                            for (boolean first : Arrays.asList(false, true)) {
                                if (behaviour.testHit(first, ray.m_82450_()) || fakePlayer && fakePlayerChoice == first) {
