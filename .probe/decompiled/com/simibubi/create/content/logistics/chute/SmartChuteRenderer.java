...
public class SmartChuteRenderer extends SmartBlockEntityRenderer<SmartChuteBlockEntity> {
    ...
    protected void renderSafe(SmartChuteBlockEntity blockEntity, float partialTicks, PoseStack ms, MultiBufferSource buffer, int light, int overlay) {
        super.renderSafe(blockEntity, partialTicks, ms, buffer, light, overlay);
        if (!blockEntity.item.isEmpty()) {
            if (!(blockEntity.itemPosition.getValue(partialTicks) > 0.0F)) {
