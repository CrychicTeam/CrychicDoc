            ...
            if (world.isClientSide) {
                ...
                if (player == null || !player.isSpectator()) {
                    ...
                    if (player != null) {
                        ...
                        Lang.builder().translate(mode.getTranslationKey(), Lang.blockName(state).style(ChatFormatting.WHITE)).color(mode.getColor()).sendStatus(player);
                    }
    ...
    public static void flushSettings(BlockPos pos) {
        if (currentSelection != null) {
            ...
            while (iterator.hasNext()) {
                ArmInteractionPoint point = (ArmInteractionPoint) iterator.next();
                if (!point.getPos().m_123314_(pos, (double) ArmBlockEntity.getRange())) {
                    iterator.remove();
                    removed++;
                }
            }
            ...
            if (removed > 0) {
                Lang.builder().translate("mechanical_arm.points_outside_range", removed).style(ChatFormatting.RED).sendStatus(player);
            } else {
                int inputs = 0;
                int outputs = 0;
                ...
                if (inputs + outputs > 0) {
                    Lang.builder().translate("mechanical_arm.summary", inputs, outputs).style(ChatFormatting.WHITE).sendStatus(player);
                }
            }
        ...
        while (iterator.hasNext()) {
            ...
            if (!point.isValid()) {
                ...
                if (!shape.isEmpty()) {
                    ...
                    CreateClient.OUTLINER.showAABB(point, shape.bounds().move(pos)).colored(color).lineWidth(0.0625F);
                }
            }
        }
    }
