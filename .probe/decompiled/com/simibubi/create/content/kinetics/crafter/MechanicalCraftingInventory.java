    ...
    public MechanicalCraftingInventory(RecipeGridHandler.GroupedItems items) {
        ...
        for (int y = 0; y < items.height; y++) {
            for (int x = 0; x < items.width; x++) {
                ItemStack stack = (ItemStack) items.grid.get(Pair.of(x + items.minX, y + items.minY));
                this.m_6836_(x + (items.height - y - 1) * items.width, stack == null ? ItemStack.EMPTY : stack.copy());
            }
        }
    }
}