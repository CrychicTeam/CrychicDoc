...
public class MechanicalCraftingRecipe extends ShapedRecipe {
    ...
    private static MechanicalCraftingRecipe fromShaped(ShapedRecipe recipe, boolean acceptMirrored) {
        return new MechanicalCraftingRecipe(recipe.getId(), recipe.getGroup(), recipe.getWidth(), recipe.getHeight(), recipe.getIngredients(), recipe.getResultItem(null), acceptMirrored);
    }
    ...
    public boolean matches(CraftingContainer inv, Level worldIn) {
        if (!(inv instanceof MechanicalCraftingInventory)) {
            ...
            for (int i = 0; i <= inv.getWidth() - this.m_44220_(); i++) {
                for (int j = 0; j <= inv.getHeight() - this.m_44221_(); j++) {
    ...
    private boolean matchesSpecific(CraftingContainer inv, int p_77573_2_, int p_77573_3_) {
        ...
        for (int i = 0; i < inv.getWidth(); i++) {
            for (int j = 0; j < inv.getHeight(); j++) {
                ...
                if (!ingredient.test(inv.m_8020_(i + j * inv.getWidth()))) {
                    return false;
                }
            }
        }
    ...
    public static class Serializer extends ShapedRecipe.Serializer {
        ...
        public ShapedRecipe fromJson(ResourceLocation recipeId, JsonObject json) {
            return MechanicalCraftingRecipe.fromShaped(super.fromJson(recipeId, json), GsonHelper.getAsBoolean(json, "acceptMirrored", true));
        }

        @Override
        public ShapedRecipe fromNetwork(ResourceLocation recipeId, FriendlyByteBuf buffer) {
            return MechanicalCraftingRecipe.fromShaped(super.fromNetwork(recipeId, buffer), buffer.readBoolean() && buffer.readBoolean());
        }
