...
public class ClientSchematicLoader {
    ...
    private List<Component> availableSchematics = new ArrayList();
    ...
    public static boolean validateSizeLimitation(long size) {
        if (Minecraft.getInstance().hasSingleplayerServer()) {
            ...
            if (size > (long) (maxSize * 1000)) {
                ...
                if (player != null) {
                    player.displayClientMessage(Lang.translateDirect("schematics.uploadTooLarge").append(" (" + size / 1000L + " KB)."), false);
    ...
    private void continueUpload(String schematic) {
        if (this.activeUploads.containsKey(schematic)) {
            ...
            try {
                int status = ((InputStream) this.activeUploads.get(schematic)).read(data);
                if (status != -1) {
        ...
        try {
            Files.list(Paths.get("schematics/")).filter(f -> !Files.isDirectory(f, new LinkOption[0]) && f.getFileName().toString().endsWith(".nbt")).forEach(path -> {
                if (!Files.isDirectory(path, new LinkOption[0])) {
                    this.availableSchematics.add(Components.literal(path.getFileName().toString()));
                }
            });
