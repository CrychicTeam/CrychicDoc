...
public class PotionFluidHandler {
    ...
    public static FluidIngredient potionIngredient(Potion potion, int amount) {
        return FluidIngredient.fromFluidStack(FluidHelper.copyStackWithAmount(getFluidFromPotionItem(PotionUtils.setPotion(new ItemStack(Items.POTION), potion)), amount));
    }
    ...
    public static ItemStack fillBottle(ItemStack stack, FluidStack availableFluid) {
        ...
        ItemStack potionStack = new ItemStack(itemFromBottleType(NBTHelper.readEnum(tag, "Bottle", PotionFluid.BottleType.class)));
            ...
            for (MobEffectInstance effectinstance : list) {
                ...
                if (!map.isEmpty()) {
                    for (Entry<Attribute, AttributeModifier> entry : map.entrySet()) {
                        ...
                        list1.add(new Tuple<>(((Attribute) entry.getKey()).getDescriptionId(), attributemodifier1));
                    }
                }
                if (effectinstance.getAmplifier() > 0) {
                    textcomponent.append(" ").append(Components.translatable("potion.potency." + effectinstance.getAmplifier()).getString());
                }
                if (effectinstance.getDuration() > 20) {
                    textcomponent.append(" (").append(MobEffectUtil.formatDuration(effectinstance, p_185182_2_)).append(")");
                }
                tooltip.add(textcomponent.withStyle(effect.getCategory().getTooltipFormatting()));
            }
        }
        if (!list1.isEmpty()) {
            ...
            for (Tuple<String, AttributeModifier> tuple : list1) {
                ...
                if (d0 > 0.0) {
                    tooltip.add(Components.translatable("attribute.modifier.plus." + attributemodifier2.getOperation().toValue(), ItemStack.ATTRIBUTE_MODIFIER_FORMAT.format(d1), Components.translatable(tuple.getA())).withStyle(ChatFormatting.BLUE));
                } else if (d0 < 0.0) {
                    d1 *= -1.0;
                    tooltip.add(Components.translatable("attribute.modifier.take." + attributemodifier2.getOperation().toValue(), ItemStack.ATTRIBUTE_MODIFIER_FORMAT.format(d1), Components.translatable(tuple.getA())).withStyle(ChatFormatting.RED));
                }
            }
        }
    }
