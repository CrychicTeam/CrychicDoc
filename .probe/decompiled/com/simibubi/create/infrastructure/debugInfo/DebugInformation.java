...
public class DebugInformation {
    ...
    public static String getVersionOfMod(String id) {
        return (String) ModList.get().getModContainerById(id).map(mod -> mod.getModInfo().getVersion().toString()).orElse("None");
    }
    ...
    public static String getMcSystemInfo(String key) {
        return (String) mcSystemInfo.get(key);
    }

    public static String getIndent(int depth) {
        return (String) Stream.generate(() -> "\t").limit((long) depth).collect(Collectors.joining());
    }
    ...
    static {
        DebugInfoSection.builder("Create").put("Mod Version", "0.5.1f").put("Forge Version", getVersionOfMod("forge")).put("Minecraft Version", SharedConstants.getCurrentVersion().getName()).buildTo(DebugInformation::registerBothInfo);
        DistExecutor.unsafeRunWhenOn(Dist.CLIENT, () -> () -> DebugInfoSection.builder("Graphics").put("Flywheel Version", Flywheel.getVersion().toString()).put("Flywheel Backend", (Supplier<String>) (() -> Backend.getBackendType().toString())).put("OpenGL Renderer", GlUtil::m_84820_).put("OpenGL Version", GlUtil::m_84821_).put("Graphics Mode", (Supplier<String>) (() -> Minecraft.getInstance().options.graphicsMode().toString())).buildTo(DebugInformation::registerClientInfo));
        DebugInfoSection.builder("System Information").put("Operating System", SystemReportAccessor.getOPERATING_SYSTEM()).put("Java Version", SystemReportAccessor.getJAVA_VERSION()).put("JVM Flags", getMcSystemInfo("JVM Flags")).put("Memory", (Supplier<String>) (() -> getMcSystemInfo("Memory"))).put("CPU", getCpuInfo()).putAll(listAllGraphicsCards()).buildTo(DebugInformation::registerBothInfo);
