...
public class AllPlacedFeatures {
    ...
    public static void bootstrap(BootstapContext<PlacedFeature> ctx) {
        ...
        PlacementUtils.register(ctx, ZINC_ORE, zincOre, placement(CountPlacement.of(8), -63, 70));
        PlacementUtils.register(ctx, STRIATED_ORES_OVERWORLD, striatedOresOverworld, placement(RarityFilter.onAverageOnceEvery(18), -30, 70));
        PlacementUtils.register(ctx, STRIATED_ORES_NETHER, striatedOresNether, placement(RarityFilter.onAverageOnceEvery(18), 40, 90));
    }

    private static List<PlacementModifier> placement(PlacementModifier frequency, int minHeight, int maxHeight) {
        return List.of(frequency, InSquarePlacement.spread(), HeightRangePlacement.uniform(VerticalAnchor.absolute(minHeight), VerticalAnchor.absolute(maxHeight)), ConfigPlacementFilter.INSTANCE);
    }
}