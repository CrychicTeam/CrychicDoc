...
public class CouplingCommand {
    ...
    public static final SimpleCommandExceptionType ONLY_MINECARTS_ALLOWED = new SimpleCommandExceptionType(Components.literal("Only Minecarts can be coupled"));

    public static final SimpleCommandExceptionType SAME_DIMENSION = new SimpleCommandExceptionType(Components.literal("Minecarts have to be in the same Dimension"));
    ...
    public static ArgumentBuilder<CommandSourceStack, ?> register() {
        return ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) Commands.literal("coupling").requires(cs -> cs.hasPermission(2))).then(((LiteralArgumentBuilder) Commands.literal("add").then(Commands.argument("cart1", EntityArgument.entity()).then(Commands.argument("cart2", EntityArgument.entity()).executes(ctx -> {
        ...
        })))).then(Commands.argument("carts", EntityArgument.entities()).executes(ctx -> {
        ...
        })))).then(Commands.literal("remove").then(Commands.argument("cart1", EntityArgument.entity()).then(Commands.argument("cart2", EntityArgument.entity()).executes(ctx -> {
            Entity cart1 = EntityArgument.getEntity(ctx, "cart1");
            if (!(cart1 instanceof AbstractMinecart)) {
                throw ONLY_MINECARTS_ALLOWED.create();
            } else {
                ...
                if (!(cart2 instanceof AbstractMinecart)) {
                    ...
                    if (!cart1Capability.isPresent()) {
                        ((CommandSourceStack) ctx.getSource()).sendSuccess(() -> Components.literal("Minecart has no Couplings Attached"), true);
                        return 0;
                    } else {
                        ...
                        if (cart1Couplings == 0) {
                            ...
                            ((CommandSourceStack) ctx.getSource()).sendSuccess(() -> Components.literal("The specified Carts are not coupled"), true);
                            return 0;
                        }
                    }
                }
            ...
            if (!(cart instanceof AbstractMinecart)) {
                ...
                if (!capability.isPresent()) {
                    ...
                    if (couplings == 0) {
                        ...
                        controller.decouple();
                        ((CommandSourceStack) ctx.getSource()).sendSuccess(() -> Components.literal("Removed " + couplings + " couplings from the Minecart"), true);
                        return couplings;
                    }
                }
            }
