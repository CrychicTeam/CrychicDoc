...
public class DumpRailwaysCommand {
    ...
    static ArgumentBuilder<CommandSourceStack, ?> register() {
        return ((LiteralArgumentBuilder) Commands.literal("trains").requires(cs -> cs.hasPermission(2))).executes(ctx -> {
            CommandSourceStack source = (CommandSourceStack) ctx.getSource();
            fillReport(source.getLevel(), source.getPosition(), (s, f) -> source.sendSuccess(() -> Components.literal(s).withStyle(st -> st.withColor(f)), false), c -> source.sendSuccess(() -> c, false));
            return 1;
        });
    }
    ...
    static void fillReport(ServerLevel level, Vec3 location, BiConsumer<String, Integer> chat, Consumer<Component> chatRaw) {
        ...
        chat.accept("", white);
        List<TrackGraph> nearest = railways.trackNetworks.values().stream().sorted((tg1, tg2) -> Float.compare(tg1.distanceToLocationSqr(level, location), tg2.distanceToLocationSqr(level, location))).limit(5L).toList();
        if (graphCount > 0) {
        ...
        List<Train> nearestTrains = railways.trains.values().stream().sorted((t1, t2) -> Float.compare(t1.distanceToLocationSqr(level, location), t2.distanceToLocationSqr(level, location))).limit(5L).toList();
        if (trainCount > 0 && !nearestTrains.isEmpty()) {
            ...
            for (Train train : nearestTrains) {
                ...
                if (currentStation != null) {
                    ...
                    chat.accept(" -> Travelling to " + train.navigation.destination.name + " (" + Mth.floor(train.navigation.distanceToDestination) + "m away)", darkBlue);
                }
    ...
    private static Component createDeleteButton(Train train) {
        return ComponentUtils.wrapInSquareBrackets(Components.literal("Remove").withStyle(p_180514_ -> p_180514_.withColor(16756064).withClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/c killTrain " + train.id.toString())).withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, Components.literal("Click to remove ").append(train.name))).withInsertion("/c killTrain " + train.id.toString())));
    }
}