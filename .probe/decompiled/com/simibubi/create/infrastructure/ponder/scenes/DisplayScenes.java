...
public class DisplayScenes {
    ...
    public static void link(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.idle(20);
        scene.overlay.showSelectionWithText(depot, 70).text("...then attach it to the block to read from").pointAt(util.vector.centerOf(linkPos)).colored(PonderPalette.INPUT).placeNearTarget();
        scene.idle(60);
        ...
        scene.world.flashDisplayLink(linkPos);
        scene.idle(20);
        scene.overlay.showText(80).text("The display will now receive information from the link").pointAt(target.add(-2.45F, -0.5, 0.0)).attachKeyFrame().placeNearTarget();
        scene.idle(30);
        scene.world.removeItemsFromBelt(depotPos);
        ...
        scene.idle(10);
        scene.world.setDisplayBoardText(board, 1, Components.literal("1024 ").append(Lang.translateDirect("generic.unit.stress")));
        scene.world.flashDisplayLink(linkPos);
        scene.idle(40);
        ...
        scene.idle(10);
        scene.world.setDisplayBoardText(board, 1, Components.literal("6:00 ").append(Lang.translateDirect("generic.daytime.pm")));
    ...
    public static void board(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.world.showSection(link, Direction.EAST);
        scene.idle(15);
        scene.overlay.chaseBoundingBoxOutline(PonderPalette.INPUT, depot, new AABB(linkPos).contract(-0.5, 0.0, 0.0), 60);
        scene.idle(5);
        scene.overlay.chaseBoundingBoxOutline(PonderPalette.OUTPUT, link, new AABB(board).expandTowards(-2.0, -1.0, 0.0).deflate(0.0, 0.0, 0.1875), 60);
        scene.idle(20);
    ...
    public static void redstone(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.idle(20);
        scene.overlay.showText(80).colored(PonderPalette.RED).attachKeyFrame().pointAt(util.vector.topOf(linkPos)).placeNearTarget().text("When powered by Redstone, Display Links stop sending updates");
        scene.idle(50);
        scene.world.removeItemsFromBelt(depotPos);
        ...
        scene.world.setDisplayBoardText(board, 1, item.getHoverName());
        scene.idle(15);
        scene.overlay.showText(100).colored(PonderPalette.GREEN).attachKeyFrame().pointAt(target.add(-2.45, -0.5, 0.0)).placeNearTarget().text("Once unpowered, the Timer is reset and new info is sent immediately");
        scene.idle(100);
        scene.world.hideSection(depot, Direction.SOUTH);
        ...
        scene.world.moveSection(redstoneBlock, util.vector.of(-1.0, 0.0, 1.0), 0);
        scene.idle(10);
        scene.world.flashDisplayLink(linkPos);
        scene.world.setDisplayBoardText(board, 1, Components.immutableEmpty());
        scene.idle(25);
        scene.overlay.showSelectionWithText(depot, 80).colored(PonderPalette.RED).attachKeyFrame().pointAt(util.vector.topOf(linkPos)).placeNearTarget().text("Signals emitted from the source do not affect the Link");
        scene.idle(70);
    }
}