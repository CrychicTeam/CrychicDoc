...
public class DetectorScenes {
    ...
    public static void smartObserver(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.idle(10);
        scene.overlay.showText(60).text("Smart Observers can be used to detect a variety of events").pointAt(util.vector.blockSurface(observerPos, Direction.WEST)).placeNearTarget();
        scene.idle(50);
        ...
        scene.world.moveSection(chestLink, util.vector.of(0.0, 0.0, 1.0), 0);
        scene.idle(15);
        ItemStack copperIngot = new ItemStack(Items.COPPER_INGOT);
        ItemStack amethystItem = new ItemStack(Blocks.AMETHYST_BLOCK);
        scene.overlay.showControls(new InputWindowElement(util.vector.blockSurface(observerPos.north(), Direction.NORTH), Pointing.RIGHT).withItem(copperIngot), 40);
        scene.idle(7);
        ...
        scene.idle(15);
        scene.overlay.showText(60).text("It can detect items or fluids inside of generic containers").attachKeyFrame().pointAt(util.vector.blockSurface(observerPos, Direction.WEST)).placeNearTarget();
        scene.idle(65);
        scene.overlay.showCenteredScrollInput(observerPos, Direction.UP, 10);
        scene.idle(5);
        scene.overlay.showControls(new InputWindowElement(util.vector.blockSurface(observerPos, Direction.UP), Pointing.DOWN).rightClick().withItem(amethystItem), 60);
        scene.idle(7);
        scene.world.setFilterData(util.select.position(observerPos), SmartObserverBlockEntity.class, amethystItem);
        scene.world.toggleRedstonePower(util.select.position(observerPos));
        scene.world.toggleRedstonePower(redstoneDust);
        scene.idle(25);
        scene.overlay.showText(60).text("The filter slot can be used to look for specific contents only").attachKeyFrame().pointAt(util.vector.blockSurface(observerPos, Direction.UP)).placeNearTarget();
        scene.idle(50);
        ...
        scene.world.moveSection(amethystLink, util.vector.of(-1.0, 0.0, 1.0), 0);
        scene.idle(15);
        scene.world.toggleRedstonePower(util.select.position(observerPos));
        scene.world.toggleRedstonePower(redstoneDust);
        scene.effects.indicateRedstone(observerPos);
        scene.idle(15);
        scene.overlay.showText(50).text("It also activates when the block itself matches the filter").attachKeyFrame().pointAt(util.vector.blockSurface(observerPos.north(), Direction.WEST)).placeNearTarget();
        scene.idle(45);
        ...
        scene.idle(10);
        scene.overlay.showText(60).text("Additionally, smart observers can monitor belts, chutes and pipes").attachKeyFrame().pointAt(util.vector.blockSurface(observerPos, Direction.UP)).placeNearTarget();
        scene.idle(60);
        ...
        scene.world.moveSection(observerLink, util.vector.of(1.0, 1.0, 0.0), 10);
        scene.world.hideSection(redstoneDust, Direction.EAST);
        scene.idle(20);
        scene.overlay.showText(60).text("...and will emit a pulse, if an item enters or exits a funnel").attachKeyFrame().pointAt(util.vector.blockSurface(observerPos.above().east(), Direction.WEST)).placeNearTarget();
        scene.idle(60);
        for (int i = 0; i < 3; i++) {
            scene.world.createItemOnBelt(util.grid.at(3, 1, 1), Direction.EAST, amethystItem);
            scene.world.toggleRedstonePower(util.select.position(observerPos));
    ...
    public static void thresholdSwitch(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.world.moveSection(stripLink, util.vector.of(2.0, 0.0, 0.0), 0);
        scene.idle(5);
        ...
        scene.idle(5);
        scene.overlay.showText(60).text("Threshold Switches monitor the fill level of containers").pointAt(util.vector.blockSurface(switchPos, Direction.NORTH)).placeNearTarget();
        scene.idle(60);
        ...
        scene.idle(10);
        Vec3 upper = util.vector.blockSurface(switchPos, Direction.NORTH).add(0.0, 0.1875, 0.0);
        scene.overlay.showLine(PonderPalette.RED, upper.add(0.125, 0.0, 0.0), upper.subtract(0.125, 0.0, 0.0), 60);
        scene.overlay.showText(70).text("When the inventory content exceeds the upper threshold...").colored(PonderPalette.RED).pointAt(upper.subtract(0.125, 0.0, 0.0)).placeNearTarget();
        scene.idle(60);
        scene.world.removeItemsFromBelt(util.grid.at(3, 0, 2));
        scene.world.flapFunnel(util.grid.at(3, 1, 2), false);
        scene.world.cycleBlockProperty(switchPos, ThresholdSwitchBlock.LEVEL);
        ...
        scene.idle(20);
        scene.overlay.showText(50).text("...the switch will change its redstone output").pointAt(util.vector.blockSurface(switchPos.north(), Direction.DOWN)).placeNearTarget();
        scene.idle(50);
        ...
        for (int ix = 0; ix < 5; ix++) {
            ...
            if (ix % 3 == 1) {
                scene.world.modifyBlock(switchPos, s -> (BlockState) s.m_61124_(ThresholdSwitchBlock.LEVEL, (Integer) s.m_61143_(ThresholdSwitchBlock.LEVEL) - 1), false);
            }
        }
        scene.world.multiplyKineticSpeed(util.select.everywhere(), 0.125F);
        Vec3 lower = util.vector.blockSurface(switchPos, Direction.NORTH).add(0.0, -0.1875, 0.0);
        scene.overlay.showLine(PonderPalette.GREEN, lower.add(0.125, 0.0, 0.0), lower.subtract(0.125, 0.0, 0.0), 60);
        scene.overlay.showText(70).text("The signal stays until the lower threshold is reached").attachKeyFrame().colored(PonderPalette.GREEN).pointAt(lower.subtract(0.125, 0.0, 0.0)).placeNearTarget();
        scene.idle(30);
        ...
        scene.world.toggleRedstonePower(redstone);
        scene.idle(40);
        scene.overlay.showText(90).text("The redstone output can now be used to control item supply, keeping the buffer filled").pointAt(util.vector.blockSurface(switchPos.north(), Direction.DOWN)).attachKeyFrame().placeNearTarget();
        scene.idle(100);
        scene.addKeyframe();
        scene.overlay.showLine(PonderPalette.GREEN, lower.add(0.125, 0.0, 0.0), lower.subtract(0.125, 0.0, 0.0), 105);
        scene.idle(5);
        scene.overlay.showLine(PonderPalette.RED, upper.add(0.125, 0.0, 0.0), upper.subtract(0.125, 0.0, 0.0), 100);
        scene.idle(15);
        scene.overlay.showControls(new InputWindowElement(util.vector.blockSurface(switchPos, Direction.UP), Pointing.DOWN).rightClick(), 60);
        scene.idle(7);
        scene.overlay.showText(70).text("The specific thresholds can be changed in the UI").pointAt(upper.subtract(0.125, 0.0, 0.0)).placeNearTarget();
        scene.idle(80);
        scene.overlay.showCenteredScrollInput(switchPos, Direction.UP, 70);
        scene.overlay.showText(70).text("A filter can help to only count specific contents toward the total").pointAt(util.vector.blockSurface(switchPos, Direction.UP)).attachKeyFrame().placeNearTarget();
        scene.idle(80);
        ...
        scene.world.moveSection(tankLink, util.vector.of(1.0, 0.0, -2.0), 0);
        scene.idle(10);
        scene.world.cycleBlockProperty(switchPos, ThresholdSwitchBlock.LEVEL);
        scene.world.cycleBlockProperty(switchPos, ThresholdSwitchBlock.LEVEL);
        scene.idle(15);
        scene.overlay.showText(70).text("Fluid buffers can be monitored in a similar fashion").pointAt(util.vector.blockSurface(switchPos, Direction.NORTH)).attachKeyFrame().placeNearTarget();
        scene.idle(80);
        ...
        scene.idle(10);
        scene.world.moveSection(switchLink, util.vector.of(0.0, 1.0, 0.0), 15);
        scene.world.modifyBlock(switchPos, s -> (BlockState) s.m_61124_(ThresholdSwitchBlock.LEVEL, 0), false);
        scene.idle(5);
        ...
        scene.overlay.showText(70).text("...as well as, curiously, the length of an extended rope pulley").pointAt(util.vector.blockSurface(switchPos.above(), Direction.NORTH)).attachKeyFrame().placeNearTarget();
        scene.idle(10);
        scene.world.setKineticSpeed(pulley, 32.0F);
        scene.world.movePulley(pulleyPos, 15.0F, 205);
        ...
        scene.idle(5);
        scene.world.cycleBlockProperty(switchPos, ThresholdSwitchBlock.LEVEL);
        scene.world.setKineticSpeed(pulley, 0.0F);
    }
}