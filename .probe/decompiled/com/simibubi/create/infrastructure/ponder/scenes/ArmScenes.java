...
public class ArmScenes {
    ...
    public static void setup(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.overlay.chaseBoundingBoxOutline(PonderPalette.INPUT, new Object(), depotBounds.move(4.0, 2.0, 1.0), 400);
        ...
        scene.overlay.chaseBoundingBoxOutline(PonderPalette.INPUT, second, depotBounds.move(0.0, 2.0, 1.0), 100);
        scene.idle(25);
        scene.overlay.showControls(input, 30);
        scene.idle(7);
        scene.overlay.chaseBoundingBoxOutline(PonderPalette.OUTPUT, second, depotBounds.move(0.0, 2.0, 1.0), 280);
        scene.overlay.showText(70).colored(PonderPalette.OUTPUT).text("Right-Click again to toggle between Input (Blue) and Output (Yellow)").pointAt(util.vector.blockSurface(outputDepot, Direction.WEST)).placeNearTarget();
        scene.idle(80);
        scene.world.showSection(util.select.position(1, 1, 0), Direction.DOWN);
        scene.idle(15);
        scene.overlay.chaseBoundingBoxOutline(PonderPalette.INPUT, new Object(), depotBounds.move(1.0, 1.0, 0.0), 43);
        ...
        scene.world.showSection(armSel, Direction.DOWN);
        scene.idle(10);
        Vec3 armTop = armSurface.add(0.5, 1.5, 0.0);
        ...
        scene.idle(5);
        scene.overlay.chaseBoundingBoxOutline(PonderPalette.OUTPUT, new Object(), depotBounds.move(0.0, 2.0, 1.0), 60);
        scene.idle(5);
        scene.overlay.chaseBoundingBoxOutline(PonderPalette.INPUT, new Object(), depotBounds.move(4.0, 2.0, 1.0), 60);
        scene.idle(5);
        scene.overlay.chaseBoundingBoxOutline(PonderPalette.OUTPUT, new Object(), depotBounds.move(1.0, 1.0, 0.0), 60);
        scene.idle(5);
        scene.overlay.chaseBoundingBoxOutline(PonderPalette.INPUT, new Object(), depotBounds.move(1.0, 3.0, 4.0), 60);
        scene.idle(5);
        scene.overlay.chaseBoundingBoxOutline(PonderPalette.INPUT, new Object(), depotBounds.move(4.0, 1.0, 2.0), 60);
        scene.idle(5);
        ...
        scene.world.createItemOnBeltLike(inputDepot, Direction.SOUTH, copper);
        scene.idle(20);
        scene.world.instructArm(armPos, ArmBlockEntity.Phase.MOVE_TO_INPUT, ItemStack.EMPTY, 2);
        scene.idle(24);
        scene.world.removeItemsFromBelt(inputDepot);
        scene.world.instructArm(armPos, ArmBlockEntity.Phase.SEARCH_OUTPUTS, copper, -1);
        scene.idle(20);
        ...
        AABB chestBounds = new AABB(0.0625, 0.0, 0.0625, 0.9375, 0.875, 0.9375);
        AABB funnelBounds = new AABB(0.0, 0.0, 0.5, 1.0, 1.0, 1.0);
        scene.overlay.chaseBoundingBoxOutline(PonderPalette.RED, in, chestBounds.move(4.0, 2.0, 3.0), 120);
        scene.overlay.chaseBoundingBoxOutline(PonderPalette.RED, out, chestBounds.move(0.0, 2.0, 3.0), 120);
        ...
        scene.idle(10);
        scene.overlay.chaseBoundingBoxOutline(PonderPalette.INPUT, in, depotBounds.move(4.0, 1.0, 2.0), 80);
        scene.idle(5);
        scene.overlay.chaseBoundingBoxOutline(PonderPalette.OUTPUT, out, funnelBounds.move(0.0, 2.0, 2.0), 80);
        scene.idle(5);
        scene.overlay.showText(60).text("Funnels and Depots can help to Bridge that gap").colored(PonderPalette.OUTPUT).placeNearTarget().pointAt(util.vector.topOf(util.grid.at(0, 2, 2)).add(0.0, 0.0, 0.25));
        scene.idle(70);
    ...
    public static void filtering(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        for (int y = 0; y < 3; y++) {
            for (int x = 0; x < 3; x++) {
                ...
                scene.overlay.showControls(new InputWindowElement(filterSlot.add((double) (2 - x), (double) (-y), 0.0), Pointing.LEFT).rightClick().withItem(item), 5);
                scene.idle(7);
        ...
        for (int i = 0; i < 4; i++) {
            ...
            scene.world.instructArm(armPos, ArmBlockEntity.Phase.SEARCH_INPUTS, i == 3 ? ItemStack.EMPTY : sand, -1);
            scene.world.modifyBlockEntity(funnelPos.north(), MechanicalCrafterBlockEntity.class, mct -> mct.getInventory().insertItem(0, sand.copy(), false));
            scene.idle(10);
        }
        scene.world.instructArm(armPos, ArmBlockEntity.Phase.MOVE_TO_INPUT, ItemStack.EMPTY, 0);
        scene.idle(24);
        ...
        for (int i = 0; i < 5; i++) {
            ...
            scene.world.flapFunnel(funnelPos, false);
            scene.world.instructArm(armPos, ArmBlockEntity.Phase.SEARCH_INPUTS, i == 4 ? ItemStack.EMPTY : sulphur, -1);
            scene.world.modifyBlockEntity(funnelPos.north(), MechanicalCrafterBlockEntity.class, mct -> mct.getInventory().insertItem(0, sulphur.copy(), false));
            scene.idle(10);
        }
        scene.idle(120);
    }
    ...
    public static void modes(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        AABB beltBox = depotBox.contract(0.0, -0.1875, 0.0).inflate(1.0, 0.0, 0.0);
        BlockPos depotPos = util.grid.at(1, 1, 4);
        BlockPos armPos = util.grid.at(3, 1, 4);
        scene.overlay.chaseBoundingBoxOutline(PonderPalette.INPUT, depotBox, depotBox.move(1.0, 1.0, 4.0), 60);
        ...
        scene.idle(40);
        scene.overlay.chaseBoundingBoxOutline(PonderPalette.OUTPUT, depotBox, beltBox.move(2.0, 1.0, 2.0), 40);
        ...
        scene.idle(70);
        Vec3 scrollSlot = util.vector.of(3.5, 1.1875, 4.0);
