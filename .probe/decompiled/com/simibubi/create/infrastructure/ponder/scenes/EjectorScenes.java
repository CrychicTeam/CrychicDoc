...
public class EjectorScenes {
    ...
    public static void ejector(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.idle(10);
        scene.overlay.chaseBoundingBoxOutline(PonderPalette.GREEN, slot, bb.expandTowards(-15.0, 15.0, 0.0), 100);
        scene.idle(10);
        ...
        scene.idle(70);
        scene.overlay.chaseBoundingBoxOutline(PonderPalette.RED, new Object(), bb.move(-2.0, 0.0, -1.0), 60);
        scene.idle(10);
        ...
        scene.overlay.showControls(new InputWindowElement(util.vector.topOf(ejectorPos).add(0.5, 0.0, 0.0), Pointing.RIGHT).withItem(copperBlock), 30);
        scene.idle(7);
        ...
        Vec3 input = util.vector.blockSurface(ejectorPos, Direction.WEST).add(0.0, -0.125, 0.0);
        Vec3 topOfSlot = input.add(0.0, 0.125, 0.0);
        ...
        scene.idle(10);
        scene.overlay.showText(80).attachKeyFrame().text("Using the value panel, a required Stack Size can be configured").pointAt(input.add(0.0, 0.0, 0.125)).placeNearTarget();
        scene.world.modifyBlockEntityNBT(ejectorS, EjectorBlockEntity.class, nbt -> nbt.putInt("ScrollValue", 10));
        scene.idle(90);
        ...
        scene.world.modifyEntities(ItemEntity.class, Entity::m_146870_);
        scene.addKeyframe();
        ElementLink<ParrotElement> birb = scene.special.createBirb(util.vector.topOf(ejectorPos).add(0.0, -0.1875, 0.0), ParrotElement.FlappyPose::new);
        scene.idle(15);
    ...
    public static void splitY(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.overlay.showControls(new InputWindowElement(util.vector.topOf(tunnel), Pointing.DOWN).showing(AllIcons.I_TUNNEL_PREFER_NEAREST), 80);
        scene.idle(10);
        ...
        scene.idle(110);
        Vec3 input = util.vector.blockSurface(ejectorPos, Direction.NORTH).subtract(0.0, 0.125, 0.0);
        Vec3 topOfSlot = input.add(0.0, 0.125, 0.0);
        ...
        scene.world.modifyBlockEntityNBT(util.select.position(2, 1, 2), EjectorBlockEntity.class, nbt -> nbt.putInt("ScrollValue", 10));
        scene.idle(90);
    ...
    public static void redstone(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        for (int ix = 0; ix < 6; ix++) {
            ...
            if (ix == 1) {
                scene.overlay.showText(60).attachKeyFrame().pointAt(util.vector.blockSurface(util.grid.at(4, 1, 1), Direction.NORTH)).placeNearTarget().text("Observers can detect when Ejectors activate");
            }
        }
    }
}