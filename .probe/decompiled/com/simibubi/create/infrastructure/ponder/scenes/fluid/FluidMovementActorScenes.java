...
public class FluidMovementActorScenes {
    ...
    public static void transfer(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        FluidStack chocolate = new FluidStack(FluidHelper.convertToStill((Fluid) AllFluids.CHOCOLATE.get()), 1000);
        ...
        ItemStack bucket = ((ForgeFlowingFluid.Flowing) AllFluids.CHOCOLATE.get()).getFluidType().getBucket(chocolate);
        scene.world.modifyBlock(pumpPos, s -> (BlockState) s.m_61124_(PumpBlock.FACING, Direction.NORTH), false);
        scene.world.modifyBlockEntity(st, type, be -> be.getCapability(fhc).ifPresent(ifh -> ifh.fill(FluidHelper.copyStackWithAmount(chocolate, 10000), IFluidHandler.FluidAction.EXECUTE)));
        ...
        scene.world.modifyBlockEntityNBT(both, psiClass, nbt -> {
            nbt.putFloat("Distance", 1.0F);
            nbt.putFloat("Timer", 14.0F);
        });
        scene.idle(17);
        ...
        for (int i = 0; i < 16; i++) {
            if (i == 8) {
                scene.overlay.showControls(new InputWindowElement(util.vector.blockSurface(util.grid.at(5, 3, 2), Direction.WEST).add(0.0, 0.5, 0.0), Pointing.LEFT).withItem(bucket), 30);
            }
            scene.world.modifyBlockEntity(st, type, be -> be.getCapability(fhc).ifPresent(ifh -> ifh.drain(1000, IFluidHandler.FluidAction.EXECUTE)));
            scene.world.modifyBlockEntity(ct1, type, be -> be.getCapability(fhc).ifPresent(ifh -> ifh.fill(chocolate, IFluidHandler.FluidAction.EXECUTE)));
            scene.idle(2);
        }
        for (int i = 0; i < 8; i++) {
            scene.world.modifyBlockEntity(st, type, be -> be.getCapability(fhc).ifPresent(ifh -> ifh.drain(1000, IFluidHandler.FluidAction.EXECUTE)));
            scene.world.modifyBlockEntity(ct2, type, be -> be.getCapability(fhc).ifPresent(ifh -> ifh.fill(chocolate, IFluidHandler.FluidAction.EXECUTE)));
            scene.idle(2);
        }
        scene.idle(50);
        scene.overlay.showText(40).placeNearTarget().pointAt(util.vector.topOf(pumpPos)).text("...or extracted from the contraption");
        scene.world.modifyBlock(pumpPos, s -> (BlockState) s.m_61124_(PumpBlock.FACING, Direction.SOUTH), true);
        scene.world.propagatePipeChange(pumpPos);
        scene.idle(30);
        for (int i = 0; i < 8; i++) {
            scene.world.modifyBlockEntity(ct2, type, be -> be.getCapability(fhc).ifPresent(ifh -> ifh.drain(1000, IFluidHandler.FluidAction.EXECUTE)));
            scene.world.modifyBlockEntity(st, type, be -> be.getCapability(fhc).ifPresent(ifh -> ifh.fill(chocolate, IFluidHandler.FluidAction.EXECUTE)));
            scene.idle(2);
        }
        for (int i = 0; i < 16; i++) {
            scene.world.modifyBlockEntity(ct1, type, be -> be.getCapability(fhc).ifPresent(ifh -> ifh.drain(1000, IFluidHandler.FluidAction.EXECUTE)));
            scene.world.modifyBlockEntity(st, type, be -> be.getCapability(fhc).ifPresent(ifh -> ifh.fill(chocolate, IFluidHandler.FluidAction.EXECUTE)));
            scene.idle(2);
        }
        scene.world.modifyBlockEntity(util.grid.at(2, 2, 3), type, be -> be.getCapability(fhc).ifPresent(ifh -> ifh.drain(8000, IFluidHandler.FluidAction.EXECUTE)));
        scene.idle(50);
        ...
        scene.world.modifyBlockEntityNBT(both, psiClass, nbt -> nbt.putFloat("Timer", 2.0F));
        scene.idle(15);
