...
public class ChuteScenes {
    ...
    public static void downward(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.idle(15);
        scene.world.modifyEntity(remove, Entity::m_146870_);
        ...
        scene.idle(7);
        scene.world.modifyBlock(util.grid.at(3, 3, 3), s -> (BlockState) s.m_61124_(ChuteBlock.SHAPE, ChuteBlock.Shape.WINDOW), false);
        ...
        scene.idle(60);
        scene.overlay.showControls(new InputWindowElement(util.vector.blockSurface(util.grid.at(2, 2, 2), Direction.NORTH), Pointing.RIGHT).rightClick().withItem(AllBlocks.INDUSTRIAL_IRON_BLOCK.asStack()), 40);
        scene.idle(7);
        scene.world.modifyBlock(util.grid.at(3, 2, 3), s -> (BlockState) s.m_61124_(ChuteBlock.SHAPE, ChuteBlock.Shape.ENCASED), false);
        scene.overlay.showText(50).pointAt(util.vector.blockSurface(util.grid.at(2, 2, 2), Direction.WEST)).placeNearTarget().text("Using Industrial Iron Blocks, chutes can be encased");
        scene.idle(10);
        for (int i = 0; i < 8; i++) {
            scene.idle(10);
        ...
        scene.world.modifyBlock(util.grid.at(2, 2, 1), s -> (BlockState) s.m_61124_(ChuteBlock.SHAPE, ChuteBlock.Shape.NORMAL), false);
        scene.world.modifyBlock(util.grid.at(2, 3, 2), s -> (BlockState) s.m_61124_(ChuteBlock.SHAPE, ChuteBlock.Shape.INTERSECTION), false);
        ...
        for (int i = 0; i < 3; i++) {
            remove = scene.world.createItemEntity(util.vector.centerOf(util.grid.at(2, 6, 3).relative(offset)), util.vector.of(0.0, 0.1, 0.0).add(Vec3.atLowerCornerOf(offset.getNormal()).scale(-0.1)), stack);
            scene.idle(12);
    ...
    public static void upward(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.overlay.showText(70).text("On the 'blocked' end, items will have to be inserted/taken from the sides").attachKeyFrame().pointAt(util.vector.centerOf(util.grid.at(3, 1, 2)).add(0.0, 0.1875, 0.0)).placeNearTarget();
        scene.idle(32);
    ...
    public static void smart(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.idle(70);
        Vec3 filter = util.vector.blockSurface(smarty, Direction.NORTH).add(0.0, 0.1875, 0.0);
        scene.overlay.showFilterSlotInput(filter, Direction.NORTH, 70);
        scene.idle(10);
        scene.rotateCameraY(20.0F);
        scene.overlay.showText(60).text("Items in the filter slot specify what to extract or transfer").attachKeyFrame().pointAt(filter.add(0.0, 0.0, 0.125)).placeNearTarget();
        scene.idle(60);
        scene.world.showSection(util.select.position(2, 4, 2), Direction.DOWN);
        scene.idle(15);
        ItemStack copper = new ItemStack(Items.IRON_INGOT);
        scene.overlay.showControls(new InputWindowElement(filter.add(0.0, 0.125, 0.0), Pointing.DOWN).rightClick().withItem(copper), 40);
        scene.idle(7);
        ...
        for (int i = 0; i < 18; i++) {
            ...
            if (i == 8) {
                scene.rotateCameraY(-20.0F);
                scene.overlay.showControls(new InputWindowElement(filter.add(0.0, 0.125, 0.0), Pointing.DOWN).rightClick(), 40);
                scene.overlay.showText(50).text("Use the value panel to specify the extracted stack size").attachKeyFrame().pointAt(filter.add(0.0, 0.0, 0.125)).placeNearTarget();
            }
            if (i == 13) {
