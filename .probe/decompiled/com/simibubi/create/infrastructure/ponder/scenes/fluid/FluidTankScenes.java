...
public class FluidTankScenes {
    ...
    public static void storage(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.idle(5);
        FluidStack content = new FluidStack(((ForgeFlowingFluid.Flowing) AllFluids.CHOCOLATE.get()).m_5613_(), 16000);
        ...
        scene.world.modifyBlock(pumpPos, s -> (BlockState) s.m_61124_(PumpBlock.FACING, Direction.SOUTH), false);
        scene.idle(10);
        ...
        scene.world.modifyBlockEntity(util.grid.at(2, 0, 5), FluidTankBlockEntity.class, be -> be.getTankInventory().fill(content, IFluidHandler.FluidAction.EXECUTE));
        scene.idle(20);
        for (int i = 0; i < 4; i++) {
            scene.world.modifyBlockEntity(tankPos, FluidTankBlockEntity.class, be -> be.getTankInventory().drain(2000, IFluidHandler.FluidAction.EXECUTE));
            scene.idle(5);
        }
        ...
        scene.idle(40);
        scene.world.modifyBlock(pumpPos, s -> (BlockState) s.m_61124_(PumpBlock.FACING, Direction.NORTH), true);
        scene.world.propagatePipeChange(pumpPos);
        for (int i = 0; i < 4; i++) {
            scene.world.modifyBlockEntity(tankPos, FluidTankBlockEntity.class, be -> be.getTankInventory().fill(FluidHelper.copyStackWithAmount(content, 2000), IFluidHandler.FluidAction.EXECUTE));
            scene.idle(5);
        }
        scene.idle(40);
        ...
        scene.overlay.showText(50).text("The contained fluid can be measured by a Comparator").attachKeyFrame().placeNearTarget().pointAt(util.vector.blockSurface(util.grid.at(2, 1, 1), Direction.DOWN).add(0.0, 0.125, 0.0));
        scene.idle(50);
        ...
        scene.idle(80);
        scene.world.modifyBlockEntity(util.grid.at(4, 3, 0), SpoutBlockEntity.class, be -> be.getCapability(ForgeCapabilities.FLUID_HANDLER).ifPresent(ifh -> ifh.fill(content, IFluidHandler.FluidAction.EXECUTE)));
        ...
        scene.idle(90);
        ItemStack chocBucket = ((ForgeFlowingFluid.Flowing) AllFluids.CHOCOLATE.get()).getFluidType().getBucket(new FluidStack(FluidHelper.convertToStill((Fluid) AllFluids.CHOCOLATE.get()), 1000));
        scene.world.createItemOnBeltLike(util.grid.at(3, 1, 0), Direction.WEST, chocBucket);
        scene.idle(40);
        scene.world.modifyBlockEntityNBT(util.select.position(util.grid.at(4, 3, 0)), SpoutBlockEntity.class, nbt -> nbt.putInt("ProcessingTicks", 20));
        scene.idle(20);
        ...
        for (int i = 0; i < 10; i++) {
            scene.effects.emitParticles(util.vector.topOf(3, 1, 1).add(0.0, 0.0625, 0.0), EmitParticlesInstruction.Emitter.simple(FluidFX.getFluidParticle(content), VecHelper.offsetRandomly(Vec3.ZERO, random, 0.1F)), 1.0F, 1);
        }
    }
    ...
    public static void sizes(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.idle(7);
        scene.world.modifyBlocks(full2, s -> (BlockState) s.m_61124_(FluidTankBlock.SHAPE, FluidTankBlock.Shape.PLAIN), false);
        scene.idle(30);
    ...
    public static void creative(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.idle(7);
        scene.world.modifyBlockEntity(cTankPos, CreativeFluidTankBlockEntity.class, be -> ((CreativeFluidTankBlockEntity.CreativeSmartFluidTank) be.getTankInventory()).setContainedFluid(new FluidStack(Fluids.FLOWING_LAVA, 1000)));
        scene.idle(5);
        ...
        scene.world.propagatePipeChange(pumpPos);
        scene.idle(40);
        scene.overlay.showText(70).text("Pipe Networks can now endlessly draw the assigned fluid from the tank").attachKeyFrame().pointAt(util.vector.blockSurface(util.grid.at(3, 1, 2), Direction.WEST));
        scene.idle(120);
        scene.world.modifyBlock(pumpPos, s -> (BlockState) s.m_61124_(PumpBlock.FACING, Direction.NORTH), true);
        scene.world.propagatePipeChange(pumpPos);
        scene.idle(40);
