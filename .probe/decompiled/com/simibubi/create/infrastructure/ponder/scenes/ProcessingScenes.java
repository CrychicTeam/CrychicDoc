...
public class ProcessingScenes {
    ...
    public static void millstone(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.world.modifyBlockEntity(millstone, MillstoneBlockEntity.class, ms -> ms.inputInv.setStackInSlot(0, itemStack));
        scene.idle(10);
        ...
        scene.idle(60);
        scene.world.modifyBlockEntity(millstone, MillstoneBlockEntity.class, ms -> ms.inputInv.setStackInSlot(0, ItemStack.EMPTY));
        ...
        scene.idle(20);
        scene.overlay.showText(50).text("The outputs can also be extracted by automation").pointAt(util.vector.blockSurface(millstone, Direction.WEST).add(-0.5, 0.4, 0.0)).placeNearTarget();
        scene.idle(60);
    }
    ...
    public static void crushingWheels(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.idle(18);
        scene.world.modifyEntity(entity1, Entity::m_146870_);
        EmitParticlesInstruction.Emitter blockSpace = EmitParticlesInstruction.Emitter.withinBlockSpace(new ItemParticleOption(ParticleTypes.ITEM, input), util.vector.of(0.0, 0.0, 0.0));
        scene.effects.emitParticles(util.vector.centerOf(center).add(0.0, -0.2, 0.0), blockSpace, 3.0F, 40);
        scene.idle(10);
        ...
        scene.idle(60);
        scene.world.createItemEntity(centerTop.add(0.0, -1.4, 0.0), util.vector.of(0.0, 0.0, 0.0), output);
        scene.idle(10);
        scene.world.createItemEntity(centerTop.add(0.0, -1.4, 0.0), util.vector.of(0.0, 0.0, 0.0), output);
        scene.overlay.showControls(new InputWindowElement(centerTop.add(0.0, -2.0, 0.0), Pointing.UP).withItem(output), 30);
        scene.idle(40);
        ...
        scene.idle(5);
        scene.overlay.showText(50).attachKeyFrame().text("Items can be inserted and picked up through automated means as well").pointAt(centerTop.add(0.0, 0.5, 0.0)).placeNearTarget();
        scene.idle(40);
        for (int i = 0; i < 5; i++) {
            if (i < 4) {
            ...
            if (i < 4) {
                scene.effects.emitParticles(util.vector.centerOf(center).add(0.0, -0.2, 0.0), blockSpace, 3.0F, 28);
            }
            if (i == 0) {
    ...
    public static void pressing(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.idle(70);
        scene.overlay.showText(60).pointAt(pressSide.subtract(0.0, 2.0, 0.0)).placeNearTarget().text("The Input items can be dropped or placed on a Depot under the Press");
        scene.idle(50);
    ...
    public static void mixing(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.idle(80);
        scene.world.modifyBlockEntityNBT(util.select.position(basin), BasinBlockEntity.class, nbt -> nbt.put("VisualizedItems", NBTHelper.writeCompoundList(ImmutableList.of(IntAttached.with(1, purple)), ia -> ((ItemStack) ia.getValue()).serializeNBT())));
        scene.idle(4);
        ...
        scene.idle(10);
        scene.world.setBlock(util.grid.at(1, 1, 2), (BlockState) AllBlocks.BLAZE_BURNER.getDefaultState().m_61124_(BlazeBurnerBlock.HEAT_LEVEL, BlazeBurnerBlock.HeatLevel.KINDLED), true);
        scene.idle(10);
        scene.overlay.showText(80).pointAt(basinSide.subtract(0.0, 1.0, 0.0)).placeNearTarget().text("Some of those recipes may require the heat of a Blaze Burner");
        scene.idle(40);
        scene.rotateCameraY(30.0F);
        scene.idle(60);
    ...
    public static void compacting(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.world.modifyBlockEntityNBT(util.select.position(basin), BasinBlockEntity.class, nbt -> nbt.put("VisualizedItems", NBTHelper.writeCompoundList(ImmutableList.of(IntAttached.with(1, copperBlock)), ia -> ((ItemStack) ia.getValue()).serializeNBT())));
        scene.idle(4);
        ...
        scene.world.modifyBlockEntityNBT(util.select.position(basin), BasinBlockEntity.class, nbt -> nbt.put("VisualizedItems", NBTHelper.writeCompoundList(ImmutableList.of(IntAttached.with(1, bark)), ia -> ((ItemStack) ia.getValue()).serializeNBT())));
        scene.idle(4);
    ...
    public static void emptyBlazeBurner(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.world.createEntity(w -> {
            ...
            blazeEntity.m_20343_(v.x, v.y, v.z);
            blazeEntity.m_146922_(blazeEntity.f_19859_ = 180.0F);
            return blazeEntity;
        });
        scene.idle(20);
        ...
        scene.idle(50);
        scene.world.modifyEntities(Blaze.class, Entity::m_146870_);
        scene.idle(20);
        scene.world.showSection(util.select.position(2, 1, 2), Direction.DOWN);
        scene.idle(20);
        ...
        scene.world.showSection(util.select.position(1, 1, 2), Direction.DOWN);
        scene.idle(20);
        scene.world.modifyBlock(util.grid.at(1, 1, 2), s -> (BlockState) s.m_61124_(BlazeBurnerBlock.HEAT_LEVEL, BlazeBurnerBlock.HeatLevel.KINDLED), false);
        ...
        scene.idle(7);
        scene.world.modifyBlock(util.grid.at(3, 1, 2), s -> (BlockState) s.m_61124_(LitBlazeBurnerBlock.FLAME_TYPE, LitBlazeBurnerBlock.FlameType.SOUL), false);
    ...
    public static void blazeBurner(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.idle(7);
        scene.world.modifyBlock(burner, s -> (BlockState) s.m_61124_(BlazeBurnerBlock.HEAT_LEVEL, BlazeBurnerBlock.HeatLevel.KINDLED), false);
        scene.idle(20);
        ...
        scene.world.modifyBlockEntityNBT(util.select.position(4, 1, 2), teType, nbt -> nbt.put("HeldItem", AllItems.BLAZE_CAKE.asStack().serializeNBT()));
    ...
    public static void basin(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        for (int i = 0; i < 4; i++) {
            scene.world.createItemEntity(util.vector.centerOf(basinPos.above(3)), util.vector.of(0.0, 0.0, 0.0), stack);
            scene.idle(10);
        }
        scene.idle(10);
        ...
        scene.idle(10);
        scene.overlay.showText(80).attachKeyFrame().colored(PonderPalette.GREEN).text("When a valid component is present, the Basin will show an output faucet").pointAt(basinSide.add(0.15, 0.0, -0.5)).placeNearTarget();
        scene.idle(90);
        ...
        scene.world.modifyBlockEntityNBT(util.select.position(basinPos), BasinBlockEntity.class, nbt -> nbt.put("VisualizedItems", NBTHelper.writeCompoundList(ImmutableList.of(IntAttached.with(1, new ItemStack(Blocks.BRICKS))), ia -> ((ItemStack) ia.getValue()).serializeNBT())));
        scene.idle(4);
        ...
        scene.overlay.showText(60).attachKeyFrame().colored(PonderPalette.GREEN).text("Outputs will be caught by the inventory below").pointAt(basinSide.add(0.0, -1.0, -1.0)).placeNearTarget();
        scene.idle(70);
        scene.world.hideIndependentSection(depot, Direction.NORTH);
        scene.idle(10);
        scene.world.modifyBlock(basinPos, s -> (BlockState) s.m_61124_(BasinBlock.FACING, Direction.DOWN), false);
        ...
        scene.idle(40);
        scene.overlay.showText(70).text("Desired outputs will then have to be extracted from the basin").pointAt(util.vector.topOf(util.grid.at(3, 1, 2)).subtract(0.0, 0.1875, 0.0)).placeNearTarget();
        scene.idle(80);
