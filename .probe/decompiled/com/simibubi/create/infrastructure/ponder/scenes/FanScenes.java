...
public class FanScenes {
    ...
    public static void processing(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        ElementLink<EntityElement> entityLink = scene.world.createItemEntity(util.vector.centerOf(blockPos.west(2).above(2)), util.vector.of(0.0, 0.1, 0.0), stack);
        scene.idle(15);
        scene.world.modifyEntity(entityLink, e -> e.setDeltaMovement(-0.2F, 0.0, 0.0));
        Vec3 itemVec = util.vector.blockSurface(util.grid.at(1, 1, 2), Direction.EAST).add(0.1, 0.0, 0.0);
        scene.overlay.showControls(new InputWindowElement(itemVec, Pointing.DOWN).withItem(stack), 20);
        scene.idle(20);
        scene.effects.emitParticles(itemVec.add(0.0, 0.2F, 0.0), EmitParticlesInstruction.Emitter.simple(ParticleTypes.LARGE_SMOKE, Vec3.ZERO), 1.0F, 60);
        ...
        entityLink = scene.world.createItemEntity(util.vector.centerOf(blockPos.west(2).above(2)), util.vector.of(0.0, 0.1, 0.0), stack);
        scene.idle(15);
        scene.world.modifyEntity(entityLink, e -> e.setDeltaMovement(-0.2F, 0.0, 0.0));
        scene.overlay.showControls(new InputWindowElement(itemVec, Pointing.DOWN).withItem(stack), 20);
        scene.idle(20);
        scene.effects.emitParticles(itemVec.add(0.0, 0.2F, 0.0), EmitParticlesInstruction.Emitter.simple(ParticleTypes.SPIT, Vec3.ZERO), 1.0F, 60);
        ...
        scene.idle(10);
        Vec3 depotTop = util.vector.topOf(2, 1, 2).add(0.0, 0.25, 0.0);
        scene.effects.emitParticles(depotTop, EmitParticlesInstruction.Emitter.simple(ParticleTypes.SPIT, Vec3.ZERO), 0.5F, 30);
        scene.idle(30);
        scene.world.modifyBlockEntityNBT(util.select.position(depos), DepotBlockEntity.class, nbt -> nbt.put("HeldItem", new TransportedItemStack(clay).serializeNBT()));
        scene.effects.emitParticles(depotTop, EmitParticlesInstruction.Emitter.simple(ParticleTypes.SPIT, Vec3.ZERO), 0.5F, 30);
        ...
        scene.idle(60);
        scene.effects.emitParticles(depotTop, EmitParticlesInstruction.Emitter.simple(ParticleTypes.SPIT, Vec3.ZERO), 0.5F, 25);
        scene.idle(25);
