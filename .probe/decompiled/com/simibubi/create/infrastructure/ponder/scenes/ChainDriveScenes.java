...
public class ChainDriveScenes {
    ...
    public static void chainDriveAsRelay(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.idle(7);
        scene.world.modifyBlock(rotatedECD, s -> (BlockState) s.m_61124_(ChainDriveBlock.AXIS, Direction.Axis.Y), true);
        scene.idle(40);
    ...
    public static void adjustableChainGearshift(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.world.modifyKineticSpeed(util.select.fromTo(westGauge.below(), middleGauge), f -> 2.0F * f);
        scene.idle(10);
        AABB bb = new AABB(eastDrive);
        scene.overlay.chaseBoundingBoxOutline(PonderPalette.MEDIUM, eastDrive, bb, 160);
        scene.overlay.chaseBoundingBoxOutline(PonderPalette.FAST, eastDrive.west(), bb.move(-2.0, 0.0, 0.0).expandTowards(0.9375, 0.0, 0.0), 160);
        scene.idle(20);
        ...
        scene.overlay.chaseBoundingBoxOutline(PonderPalette.MEDIUM, eastDrive, bb.expandTowards(-0.9375, 0.0, 0.0), 160);
        scene.overlay.chaseBoundingBoxOutline(PonderPalette.SLOW, eastDrive.west(), bb.move(-2.0, 0.0, 0.0), 160);
        scene.idle(20);
        ...
        scene.world.modifyBlock(analogPos.south(), s -> (BlockState) s.m_61124_(RedStoneWireBlock.POWER, 8), false);
        scene.world.toggleRedstonePower(util.select.position(1, 1, 4));
        ...
        for (BlockPos gauge : new BlockPos[] { eastGauge, middleGauge, westGauge }) {
            scene.idle(5);
            TextWindowElement.Builder builder = scene.overlay.showText(180).colored(gauge == westGauge ? PonderPalette.SLOW : PonderPalette.MEDIUM).placeNearTarget().pointAt(util.vector.blockSurface(gauge, Direction.NORTH));
            if (gauge == westGauge) {
                builder.text("12 RPM");
            } else {
