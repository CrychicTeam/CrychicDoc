...
public class MechanicalSawScenes {
    ...
    public static void processing(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.world.setBlock(shaftPos, (BlockState) AllBlocks.SHAFT.getDefaultState().m_61124_(ShaftBlock.AXIS, Direction.Axis.Z), false);
        ...
        scene.idle(10);
        scene.world.modifyEntity(logItem, e -> e.setDeltaMovement(util.vector.of(0.05, 0.2, 0.0)));
        scene.idle(12);
        scene.world.modifyEntity(logItem, Entity::m_146870_);
        scene.world.createItemOnBeltLike(sawPos, Direction.WEST, log);
        scene.idle(50);
        logItem = scene.world.createItemEntity(util.vector.topOf(sawPos).add(0.5, -0.1, 0.0), util.vector.of(0.05, 0.18, 0.0), strippedLog);
        scene.idle(12);
        scene.overlay.showControls(new InputWindowElement(itemSpawn.add(2.0, 0.0, 0.0), Pointing.DOWN).withItem(strippedLog), 20);
        scene.idle(30);
        ...
        scene.idle(25);
        logItem = scene.world.createItemEntity(util.vector.topOf(sawPos).add(-0.5, -0.1, 0.0), util.vector.of(-0.05, 0.18, 0.0), planks);
        scene.idle(22);
        ...
        scene.world.modifyEntity(logItem, Entity::m_146870_);
        scene.world.setBlock(shaftPos, (BlockState) AllBlocks.COGWHEEL.getDefaultState().m_61124_(ShaftBlock.AXIS, Direction.Axis.Z), true);
        scene.idle(3);
        scene.addKeyframe();
        ...
        scene.idle(40);
        Vec3 filter = util.vector.of(2.5, 1.8125, 2.3125);
    ...
    public static void treeCutting(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        for (int ix = 0; ix < 5; ix++) {
            Vec3 dropPos = util.vector.centerOf(breakingPos.above(ix));
            float distance = (float) dropPos.distanceTo(util.vector.centerOf(breakingPos));
            scene.world.createItemEntity(dropPos, util.vector.of((double) (-distance / 20.0F), 0.0, 0.0), new ItemStack(Items.OAK_LOG));
        }
        scene.idle(35);
        ...
        for (int ix = 0; ix < 30; ix++) {
            scene.world.replaceBlocks(util.select.fromTo(2, ix + 1, 2, 1, ix + 1, 3), Blocks.AIR.defaultBlockState(), true);
            for (int x = 1; x <= 2; x++) {
                for (int z = 2; z <= 3; z++) {
                    Vec3 dropPos = util.vector.centerOf(x, ix + 1, z);
                    float distance = (float) dropPos.distanceTo(util.vector.centerOf(breakingPos));
                    scene.world.createItemEntity(dropPos, util.vector.of((double) (-distance / 20.0F), 0.0, 0.0), new ItemStack(Items.JUNGLE_LOG));
                }
            }
            scene.idle(1);
        }
