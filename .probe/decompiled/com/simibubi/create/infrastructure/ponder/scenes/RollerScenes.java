...
public class RollerScenes {
    ...
    public static void clearAndPave(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.idle(5);
        ElementLink<ParrotElement> birbLink = scene.special.createBirb(util.vector.centerOf(8, 3, 4), ParrotElement.FacePointOfInterestPose::new);
        ...
        scene.overlay.showControls(new InputWindowElement(util.vector.topOf(util.grid.at(2, 2, 4)), Pointing.DOWN).showing(AllIcons.I_ROLLER_PAVE), 70);
        ...
        Vec3 filterSlot = util.vector.of(6.6875, 3.0, 3.3125);
        scene.overlay.showFilterSlotInput(filterSlot, Direction.UP, 60);
        scene.overlay.showText(60).pointAt(filterSlot.add(-0.125, 0.0, 0.0)).attachKeyFrame().text("While disassembled, a suitable paving material can be specified").placeNearTarget();
        scene.idle(50);
        ...
        scene.idle(10);
        scene.overlay.showSelectionWithText(util.select.fromTo(5, 0, 3, 0, 0, 5), 90).pointAt(util.vector.topOf(util.grid.at(3, 0, 4))).attachKeyFrame().colored(PonderPalette.RED).text("Note that any block destroyed by a roller has a chance not to yield drops").placeNearTarget();
        scene.idle(100);
    ...
    public static void fill(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.idle(5);
        ElementLink<WorldSectionElement> trainLink = scene.world.showIndependentSection(train, Direction.DOWN);
        ElementLink<ParrotElement> birbLink = scene.special.createBirb(util.vector.centerOf(8, 7, 4), ParrotElement.FacePointOfInterestPose::new);
        scene.idle(5);
        scene.world.showSectionAndMerge(rollers, Direction.EAST, trainLink);
        scene.idle(15);
        Vec3 filterSlot = util.vector.of(6.6875, 7.0, 3.6875);
        scene.overlay.showFilterSlotInput(filterSlot, Direction.UP, 60);
        scene.overlay.showText(60).pointAt(filterSlot.add(-0.125, 0.0, 0.0)).attachKeyFrame().text("While disassembled, rollers can be set to other modes").placeNearTarget();
        scene.idle(70);
        scene.overlay.showSelectionWithText(util.select.fromTo(5, 3, 3, 0, 1, 5), 90).attachKeyFrame().colored(PonderPalette.BLUE).text("The 'fill' modes can help to bridge gaps between pavement and terrain").placeNearTarget();
        scene.idle(100);
        scene.overlay.showControls(new InputWindowElement(filterSlot, Pointing.DOWN).showing(AllIcons.I_ROLLER_FILL), 50);
        scene.idle(15);
        Block paveMaterial = Blocks.COARSE_DIRT;
        ItemStack paveItem = new ItemStack(paveMaterial);
        scene.overlay.showControls(new InputWindowElement(filterSlot.add(0.0, 0.0, -0.375), Pointing.UP).withItem(paveItem), 35);
        scene.idle(7);
        scene.world.setFilterData(rollers, RollerBlockEntity.class, paveItem);
        scene.idle(10);
        scene.world.cycleBlockProperty(stationPos, StationBlock.ASSEMBLING);
        ...
        for (int i = 0; i < 3; i++) {
            scene.world.modifyBlockEntity(util.grid.at(6, 6, 3 + i), RollerBlockEntity.class, rte -> rte.setAnimatedSpeed(-100.0F));
        }
        for (int j = 0; j < 5; j++) {
            for (int i = 0; i < 3; i++) {
            ...
            if (j == 2) {
                scene.overlay.showText(90).attachKeyFrame().pointAt(util.vector.blockSurface(util.grid.at(3, 2, 3), Direction.NORTH)).text("On 'straight fill', they will place simple columns down to the surface").placeNearTarget();
            }
            scene.idle(14);
        }
        for (int i = 0; i < 3; i++) {
        ...
        scene.idle(4);
        for (int i = 0; i < 3; i++) {
            scene.world.modifyBlockEntity(util.grid.at(6, 6, 3 + i), RollerBlockEntity.class, rte -> rte.setAnimatedSpeed(0.0F));
        }
        scene.idle(15);
        ...
        scene.world.showSectionAndMerge(rollers, Direction.EAST, trainLink);
        scene.idle(25);
        scene.overlay.showControls(new InputWindowElement(filterSlot, Pointing.DOWN).showing(AllIcons.I_ROLLER_WIDE_FILL), 40);
        scene.idle(45);
        scene.world.cycleBlockProperty(stationPos, StationBlock.ASSEMBLING);
        scene.world.animateTrainStation(stationPos, false);
        scene.world.toggleControls(controlsPos);
        ...
        for (int j = 0; j < 5; j++) {
            ...
            if (j == 2) {
                scene.overlay.showText(90).attachKeyFrame().pointAt(util.vector.blockSurface(util.grid.at(3, 2, 3), Direction.NORTH)).text("On 'sloped fill', layers placed further down will increase in size").placeNearTarget();
            }
            scene.idle(14);
        }
        for (int i = 0; i < 3; i++) {
