...
public class TestItems {
    ...
    public static void armPurgatory(CreateGameTestHelper helper) {
        ...
        DepotBlockEntity depot1 = helper.getBlockEntity((BlockEntityType<DepotBlockEntity>) AllBlockEntityTypes.DEPOT.get(), depot1Pos);
        BlockPos depot2Pos = new BlockPos(1, 2, 1);
        DepotBlockEntity depot2 = helper.getBlockEntity((BlockEntityType<DepotBlockEntity>) AllBlockEntityTypes.DEPOT.get(), depot2Pos);
        helper.m_177421_(lever);
        helper.m_177361_(() -> {
    ...
    public static void attributeFilters(CreateGameTestHelper helper) {
        ...
        Map<BlockPos, ItemStack> outputs = Map.of(new BlockPos(3, 2, 1), new ItemStack((ItemLike) AllBlocks.BRASS_BLOCK.get()), new BlockPos(4, 2, 1), new ItemStack(Items.APPLE), new BlockPos(5, 2, 1), new ItemStack(Items.WATER_BUCKET), new BlockPos(6, 2, 1), EnchantedBookItem.createForEnchantment(new EnchantmentInstance(Enchantments.ALL_DAMAGE_PROTECTION, 1)), new BlockPos(7, 2, 1), Util.make(new ItemStack(Items.NETHERITE_SWORD), s -> s.setDamageValue(1)), new BlockPos(8, 2, 1), new ItemStack(Items.IRON_HELMET), new BlockPos(9, 2, 1), new ItemStack(Items.COAL), new BlockPos(10, 2, 1), new ItemStack(Items.POTATO));
        helper.m_177421_(lever);
        helper.m_177361_(() -> {
    ...
    public static void brassTunnelFiltering(CreateGameTestHelper helper) {
        Map<BlockPos, ItemStack> outputs = Map.of(new BlockPos(3, 2, 2), new ItemStack(Items.COPPER_INGOT, 13), new BlockPos(4, 2, 3), new ItemStack((ItemLike) AllItems.ZINC_INGOT.get(), 4), new BlockPos(4, 2, 4), new ItemStack(Items.IRON_INGOT, 2), new BlockPos(4, 2, 5), new ItemStack(Items.GOLD_INGOT, 24), new BlockPos(3, 2, 6), new ItemStack(Items.DIAMOND, 17));
        BlockPos lever = new BlockPos(2, 3, 2);
        helper.m_177421_(lever);
        helper.m_177361_(() -> outputs.forEach(helper::assertContainerContains));
    }
    ...
    public static void brassTunnelPreferNearest(CreateGameTestHelper helper) {
        ...
        BlockPos lever = new BlockPos(2, 3, 2);
        helper.m_177421_(lever);
        tunnels.forEach(tunnel -> helper.setTunnelMode(tunnel, BrassTunnelBlockEntity.SelectionMode.PREFER_NEAREST));
        helper.m_177361_(() -> out.forEach(pos -> helper.assertContainerContains(pos, (ItemLike) AllBlocks.BRASS_CASING.get())));
    }
    ...
    public static void brassTunnelRoundRobin(CreateGameTestHelper helper) {
        ...
        brassTunnelModeTest(helper, BrassTunnelBlockEntity.SelectionMode.ROUND_ROBIN, outputs);
    }
    ...
    public static void brassTunnelSplit(CreateGameTestHelper helper) {
        ...
        brassTunnelModeTest(helper, BrassTunnelBlockEntity.SelectionMode.SPLIT, outputs);
    }
    ...
    public static void brassTunnelSyncInput(CreateGameTestHelper helper) {
        ...
        List<BlockPos> outputs = List.of(new BlockPos(7, 2, 1), new BlockPos(7, 2, 2), new BlockPos(7, 2, 3));
        helper.m_177421_(lever);
        tunnels.forEach(tunnel -> helper.setTunnelMode(tunnel, BrassTunnelBlockEntity.SelectionMode.SYNCHRONIZE));
        helper.m_177361_(() -> {
            if (helper.secondsPassed() < 9L) {
                helper.m_177245_((BlockPos) redstoneBlocks.get(0), Blocks.AIR);
                helper.assertSecondsPassed(3);
                outputs.forEach(helper::m_177440_);
                helper.m_177245_((BlockPos) redstoneBlocks.get(1), Blocks.AIR);
                helper.assertSecondsPassed(6);
                outputs.forEach(helper::m_177440_);
                helper.m_177245_((BlockPos) redstoneBlocks.get(2), Blocks.AIR);
                helper.assertSecondsPassed(9);
            } else {
                outputs.forEach(out -> helper.assertContainerContains(out, (ItemLike) AllBlocks.BRASS_CASING.get()));
            }
        });
    }
    ...
    public static void smartObserverBeltAndFunnel(CreateGameTestHelper helper) {
        ...
        helper.m_177361_(() -> {
            helper.assertSecondsPassed(9);
            targets.forEach(pos -> helper.m_177208_(Blocks.DIAMOND_BLOCK, pos));
            overflows.forEach(pos -> helper.m_177208_(Blocks.AIR, pos));
        });
    }
    ...
    public static void smartObserverChutes(CreateGameTestHelper helper) {
        ...
        helper.m_177421_(lever);
        helper.m_177361_(() -> helper.m_177208_(Blocks.DIAMOND_BLOCK, output));
    }
    ...
    public static void smartObserverCounting(CreateGameTestHelper helper) {
        ...
        NixieTubeBlockEntity chestNixie = helper.getBlockEntity((BlockEntityType<NixieTubeBlockEntity>) AllBlockEntityTypes.NIXIE_TUBE.get(), chestNixiePos);
        ...
        NixieTubeBlockEntity doubleChestNixie = helper.getBlockEntity((BlockEntityType<NixieTubeBlockEntity>) AllBlockEntityTypes.NIXIE_TUBE.get(), doubleChestNixiePos);
        helper.m_177361_(() -> {
    ...
    public static void smartObserverStorage(CreateGameTestHelper helper) {
        ...
        helper.m_177421_(lever);
        helper.m_177361_(() -> helper.m_177255_(lamp, RedstoneLampBlock.LIT, true));
    }
    ...
    public static void depotDisplay(CreateGameTestHelper helper) {
        ...
        List<DepotBlockEntity> depots = Stream.of(new BlockPos(2, 2, 1), new BlockPos(1, 2, 1)).map(pos -> helper.getBlockEntity((BlockEntityType<DepotBlockEntity>) AllBlockEntityTypes.DEPOT.get(), pos)).toList();
        List<BlockPos> levers = List.of(new BlockPos(2, 5, 0), new BlockPos(1, 5, 0));
        levers.forEach(helper::m_177421_);
        FlapDisplayBlockEntity display = helper.<FlapDisplayBlockEntity>getBlockEntity((BlockEntityType<FlapDisplayBlockEntity>) AllBlockEntityTypes.FLAP_DISPLAY.get(), displayPos).getController();
        helper.m_177361_(() -> {
            for (int i = 0; i < 2; i++) {
                FlapDisplayLayout line = (FlapDisplayLayout) display.getLines().get(i);
                MutableComponent textComponent = Components.empty();
                line.getSections().stream().map(FlapDisplaySection::getText).forEach(textComponent::m_7220_);
