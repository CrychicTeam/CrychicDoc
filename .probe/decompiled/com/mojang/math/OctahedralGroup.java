...
public enum OctahedralGroup implements StringRepresentable {
    ...
    private static final OctahedralGroup[][] cayleyTable = Util.make(new OctahedralGroup[values().length][values().length], p_56533_ -> {
        Map<Pair<SymmetricGroup3, BooleanList>, OctahedralGroup> $$1 = (Map<Pair<SymmetricGroup3, BooleanList>, OctahedralGroup>) Arrays.stream(values()).collect(Collectors.toMap(p_174952_ -> Pair.of(p_174952_.permutation, p_174952_.packInversions()), p_174950_ -> p_174950_));
        for (OctahedralGroup $$2 : values()) {
            for (OctahedralGroup $$3 : values()) {
                ...
                p_56533_[$$2.ordinal()][$$3.ordinal()] = (OctahedralGroup) $$1.get(Pair.of($$6, $$7));
            }
        }
    });

    private static final OctahedralGroup[] inverseTable = (OctahedralGroup[]) Arrays.stream(values()).map(p_56536_ -> (OctahedralGroup) Arrays.stream(values()).filter(p_174947_ -> p_56536_.compose(p_174947_) == IDENTITY).findAny().get()).toArray(OctahedralGroup[]::new);
    ...
    public Direction rotate(Direction p_56529_) {
        if (this.rotatedDirections == null) {
            ...
            for (Direction $$2 : Direction.values()) {
                ...
                this.rotatedDirections.put($$2, $$7);
            }
        }
        return (Direction) this.rotatedDirections.get(p_56529_);
    }
