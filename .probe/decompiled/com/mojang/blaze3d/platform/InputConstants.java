...
public class InputConstants {
    ...
    public static InputConstants.Key getKey(int int0, int int1) {
        return int0 == -1 ? InputConstants.Type.SCANCODE.getOrCreate(int1) : InputConstants.Type.KEYSYM.getOrCreate(int0);
    }

    public static InputConstants.Key getKey(String string0) {
        if (InputConstants.Key.NAME_MAP.containsKey(string0)) {
            return (InputConstants.Key) InputConstants.Key.NAME_MAP.get(string0);
        } else {
            for (InputConstants.Type $$1 : InputConstants.Type.values()) {
                if (string0.startsWith($$1.defaultPrefix)) {
                    String $$2 = string0.substring($$1.defaultPrefix.length() + 1);
                    int $$3 = Integer.parseInt($$2);
                    if ($$1 == InputConstants.Type.MOUSE) {
                        $$3--;
                    }
                    return $$1.getOrCreate($$3);
                }
            }
            throw new IllegalArgumentException("Unknown key name: " + string0);
        }
    }

    public static boolean isKeyDown(long long0, int int1) {
        return GLFW.glfwGetKey(long0, int1) == 1;
    }

    public static void setupKeyboardCallbacks(long long0, GLFWKeyCallbackI gLFWKeyCallbackI1, GLFWCharModsCallbackI gLFWCharModsCallbackI2) {
        GLFW.glfwSetKeyCallback(long0, gLFWKeyCallbackI1);
        GLFW.glfwSetCharModsCallback(long0, gLFWCharModsCallbackI2);
    }

    public static void setupMouseCallbacks(long long0, GLFWCursorPosCallbackI gLFWCursorPosCallbackI1, GLFWMouseButtonCallbackI gLFWMouseButtonCallbackI2, GLFWScrollCallbackI gLFWScrollCallbackI3, GLFWDropCallbackI gLFWDropCallbackI4) {
        GLFW.glfwSetCursorPosCallback(long0, gLFWCursorPosCallbackI1);
        GLFW.glfwSetMouseButtonCallback(long0, gLFWMouseButtonCallbackI2);
        GLFW.glfwSetScrollCallback(long0, gLFWScrollCallbackI3);
        GLFW.glfwSetDropCallback(long0, gLFWDropCallbackI4);
    }

    public static void grabOrReleaseMouse(long long0, int int1, double double2, double double3) {
        GLFW.glfwSetCursorPos(long0, double2, double3);
        GLFW.glfwSetInputMode(long0, 208897, int1);
    }
    ...
    public static void updateRawMouseInput(long long0, boolean boolean1) {
        if (isRawMouseInputSupported()) {
            GLFW.glfwSetInputMode(long0, GLFW_RAW_MOUSE_MOTION, boolean1 ? 1 : 0);
        }
    }
    ...
    public static final class Key {
        ...
        Key(String string0, InputConstants.Type inputConstantsType1, int int2) {
            this.name = string0;
            this.type = inputConstantsType1;
            this.value = int2;
            this.displayName = new LazyLoadedValue<>(() -> (Component) inputConstantsType1.displayTextSupplier.apply(int2, string0));
            NAME_MAP.put(string0, this);
        }
        ...
        public boolean equals(Object object0) {
            if (this == object0) {
                ...
                InputConstants.Key $$1 = (InputConstants.Key) object0;
    ...
    public static enum Type {
        ...
        KEYSYM("key.keyboard", (p_288232_, p_288233_) -> {
            if ("key.keyboard.unknown".equals(p_288233_)) {
                ...
                String $$2 = GLFW.glfwGetKeyName(p_288232_, -1);
                return $$2 != null ? Component.literal($$2.toUpperCase(Locale.ROOT)) : Component.translatable(p_288233_);
            }
        }), SCANCODE("scancode", (p_84911_, p_84912_) -> {
            String $$2 = GLFW.glfwGetKeyName(-1, p_84911_);
            return $$2 != null ? Component.literal($$2) : Component.translatable(p_84912_);
        }), MOUSE("key.mouse", (p_84904_, p_84905_) -> Language.getInstance().has(p_84905_) ? Component.translatable(p_84905_) : Component.translatable("key.mouse", p_84904_ + 1));
        ...
        private static void addKey(InputConstants.Type p_84900_, String p_84901_, int p_84902_) {
            InputConstants.Key $$3 = new InputConstants.Key(p_84901_, p_84900_, p_84902_);
            p_84900_.map.put(p_84902_, $$3);
        }
        ...
        public InputConstants.Key getOrCreate(int p_84896_) {
            return (InputConstants.Key) this.map.computeIfAbsent(p_84896_, p_84907_ -> {
                int $$1 = p_84907_;
                if (this == MOUSE) {
                    $$1 = p_84907_ + 1;
                }
                String $$2 = this.defaultPrefix + "." + $$1;
                return new InputConstants.Key($$2, this, p_84907_);
            });
        }
