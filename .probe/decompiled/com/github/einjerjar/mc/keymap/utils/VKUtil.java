...
public class VKUtil {
    ...
    public static List<VirtualKeyboardWidget> genLayout(KeyLayout layout, int x, int y, EWidget.SimpleWidgetAction<VirtualKeyboardWidget> onClick, VirtualKeyboardWidget.SpecialVKKeyClicked onSpecialClick) {
        ...
        VirtualKeyboardWidget vkExtra = new VirtualKeyboardWidget(layout.keys().extra(), vkBasic.left(), vkBasic.bottom() + 4, 0, 0);
        VirtualKeyboardWidget vkMouse = new VirtualKeyboardWidget(layout.keys().mouse(), vkExtra.left(), vkExtra.bottom() + 2, 0, 0);
        VirtualKeyboardWidget vkNumpad = new VirtualKeyboardWidget(layout.keys().numpad(), vkMouse.right() + 4, vkBasic.bottom() + 4, 0, 0);
