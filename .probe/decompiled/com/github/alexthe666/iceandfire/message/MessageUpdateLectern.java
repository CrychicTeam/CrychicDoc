...
public class MessageUpdateLectern {
    ...
    public static class Handler {
        ...
        public static void handle(MessageUpdateLectern message, Supplier<NetworkEvent.Context> ctx) {
            ((NetworkEvent.Context) ctx.get()).enqueueWork(() -> DistExecutor.unsafeRunWhenOn(Dist.DEDICATED_SERVER, () -> () -> handlePacket(message, ctx)));
            ((NetworkEvent.Context) ctx.get()).enqueueWork(() -> DistExecutor.unsafeRunWhenOn(Dist.CLIENT, () -> () -> handlePacket(message, ctx)));
            ((NetworkEvent.Context) ctx.get()).setPacketHandled(true);
        }

        public static void handlePacket(MessageUpdateLectern message, Supplier<NetworkEvent.Context> contextSupplier) {
            ...
            context.enqueueWork(() -> {
                ...
                if (player != null) {
                    ...
                    if (player.m_9236_().m_46805_(pos) && player.m_9236_().getBlockEntity(pos) instanceof TileEntityLectern lectern) {
