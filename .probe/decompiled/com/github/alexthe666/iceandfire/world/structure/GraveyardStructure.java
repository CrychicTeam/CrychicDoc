...
public class GraveyardStructure extends IafStructure {
    ...
    protected Optional<Structure.GenerationStub> findGenerationPoint(Structure.GenerationContext pContext) {
        if (!IafConfig.generateGraveyards) {
            ...
            return !this.isBiomeValid(pContext, BiomeConfig.graveyardBiomes, blockpos) ? Optional.empty() : JigsawPlacement.addPieces(pContext, this.startPool, this.startJigsawName, this.size, blockpos, false, this.projectStartToHeightmap, this.maxDistanceFromCenter);
        }
    }
    ...
    public static GraveyardStructure buildStructureConfig(BootstapContext<Structure> context) {
        ...
        return new GraveyardStructure(new Structure.StructureSettings(context.lookup(Registries.BIOME).getOrThrow(IafBiomeTagGenerator.HAS_GRAVEYARD), new HashMap(), GenerationStep.Decoration.SURFACE_STRUCTURES, TerrainAdjustment.BEARD_THIN), graveyardHolder, Optional.empty(), 1, ConstantHeight.ZERO, Optional.of(Heightmap.Types.WORLD_SURFACE_WG), 16);
    }
}