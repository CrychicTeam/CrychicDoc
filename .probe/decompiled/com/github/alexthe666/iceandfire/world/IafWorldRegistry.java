...
public class IafWorldRegistry {
    ...
    public static final DeferredRegister<Feature<?>> FEATURES = DeferredRegister.create(ForgeRegistries.FEATURES, "iceandfire");
    ...
    public static final RegistryObject<Feature<NoneFeatureConfiguration>> SPAWN_DRAGON_SKELETON_L = register("spawn_dragon_skeleton_lightning", () -> new SpawnDragonSkeleton(IafEntityRegistry.LIGHTNING_DRAGON.get(), NoneFeatureConfiguration.CODEC));

    public static final RegistryObject<Feature<NoneFeatureConfiguration>> SPAWN_DRAGON_SKELETON_F = register("spawn_dragon_skeleton_fire", () -> new SpawnDragonSkeleton(IafEntityRegistry.FIRE_DRAGON.get(), NoneFeatureConfiguration.CODEC));

    public static final RegistryObject<Feature<NoneFeatureConfiguration>> SPAWN_DRAGON_SKELETON_I = register("spawn_dragon_skeleton_ice", () -> new SpawnDragonSkeleton(IafEntityRegistry.ICE_DRAGON.get(), NoneFeatureConfiguration.CODEC));
    ...
    public static boolean isFarEnoughFromSpawn(LevelAccessor level, BlockPos position) {
        ...
        BlockPos spawnRelative = new BlockPos(spawnPoint.getXSpawn(), position.m_123342_(), spawnPoint.getYSpawn());
    ...
    public static void addFeatures(Holder<Biome> biome, HashMap<String, Holder<PlacedFeature>> features, ModifiableBiomeInfo.BiomeInfo.Builder builder) {
        ...
        if (!ADDED_FEATURES.isEmpty()) {
            ...
            IceAndFire.LOGGER.debug("Added the following features to the biome [{}]: {}", ((ResourceKey) biome.unwrapKey().get()).location(), featureList);
        }
        ADDED_FEATURES = null;
    }
    ...
    private static void addFeatureToBiome(ResourceKey<PlacedFeature> featureResource, HashMap<String, Holder<PlacedFeature>> features, ModifiableBiomeInfo.BiomeInfo.Builder builder, GenerationStep.Decoration step) {
        ...
        Holder<PlacedFeature> feature = (Holder<PlacedFeature>) features.get(identifier);
        if (feature != null) {
