...
public class DragonAIWander extends Goal {
    ...
    public boolean canUse() {
        if (!this.dragon.canMove() || this.dragon.isFuelingForge()) {
            return false;
        } else if (!this.dragon.isFlying() && !this.dragon.isHovering()) {
            if (!this.mustUpdate && this.dragon.m_217043_().nextInt(this.executionChance) != 0) {
                return false;
            } else {
    ...
    public boolean canContinueToUse() {
        return !this.dragon.m_21573_().isDone();
    }

    @Override
    public void start() {
        this.dragon.m_21573_().moveTo(this.xPosition, this.yPosition, this.zPosition, this.speed);
    }
