...
public abstract class DragonTabulaModelAnimator<T extends EntityDragonBase> extends IceAndFireTabulaModelAnimator implements ITabulaModelAnimator<T> {
    ...
    public void setRotationAngles(TabulaModel model, T entity, float limbSwing, float limbSwingAmount, float ageInTicks, float rotationYaw, float rotationPitch, float scale) {
        ...
        if (entity.isModelDead()) {
            ...
            if (entity.m_20205_() >= 2.0F && entity.flyProgress == 0.0F && entity.hoverProgress == 0.0F) {
                LegArticulator.articulateQuadruped(entity, entity.legSolver, model.getCube("BodyUpper"), model.getCube("BodyLower"), model.getCube("Neck1"), model.getCube("ThighL"), model.getCube("LegL"), this.toesPartsL, model.getCube("ThighR"), model.getCube("LegR"), this.toesPartsR, model.getCube("armL1"), model.getCube("armL2"), this.clawL, model.getCube("armR1"), model.getCube("armR2"), this.clawR, 1.0F, 0.5F, 0.5F, -0.15F, -0.15F, 0.0F, Minecraft.getInstance().getFrameTime());
            }
        }
    }
    ...
    private void setRotationsLoop(TabulaModel model, T entity, float limbSwingAmount, boolean walking, TabulaModel currentPosition, TabulaModel prevPosition, float partialTick, float deltaTicks, AdvancedModelBox cube) {
        ...
        if (entity.sleepProgress > 0.0F && !this.isRotationEqual(cube, this.getModel(EnumDragonPoses.SLEEPING_POSE).getCube(cube.boxName))) {
            this.transitionTo(cube, this.getModel(EnumDragonPoses.SLEEPING_POSE).getCube(cube.boxName), Mth.lerp(partialTick, entity.prevAnimationProgresses[1], entity.sleepProgress), 20.0F, false);
        }
        if (entity.hoverProgress > 0.0F && !this.isRotationEqual(cube, this.getModel(EnumDragonPoses.HOVERING_POSE).getCube(cube.boxName)) && !this.isWing(model, cube) && !cube.boxName.contains("Tail")) {
            this.transitionTo(cube, this.getModel(EnumDragonPoses.HOVERING_POSE).getCube(cube.boxName), Mth.lerp(partialTick, entity.prevAnimationProgresses[2], entity.hoverProgress), 20.0F, false);
        }
        if (entity.flyProgress > 0.0F && !this.isRotationEqual(cube, this.getModel(EnumDragonPoses.FLYING_POSE).getCube(cube.boxName))) {
            this.transitionTo(cube, this.getModel(EnumDragonPoses.FLYING_POSE).getCube(cube.boxName), Mth.lerp(partialTick, entity.prevAnimationProgresses[3], entity.flyProgress) - Mth.lerp(partialTick, entity.prevDiveProgress, entity.diveProgress) * 2.0F, 20.0F, false);
        }
        if (entity.sitProgress > 0.0F && !entity.m_20159_() && !this.isRotationEqual(cube, this.getModel(EnumDragonPoses.SITTING_POSE).getCube(cube.boxName))) {
            this.transitionTo(cube, this.getModel(EnumDragonPoses.SITTING_POSE).getCube(cube.boxName), Mth.lerp(partialTick, entity.prevAnimationProgresses[0], entity.sitProgress), 20.0F, false);
        }
        if (entity.ridingProgress > 0.0F && !this.isHorn(cube) && !this.isRotationEqual(cube, this.getModel(EnumDragonPoses.SIT_ON_PLAYER_POSE).getCube(cube.boxName))) {
            this.transitionTo(cube, this.getModel(EnumDragonPoses.SIT_ON_PLAYER_POSE).getCube(cube.boxName), Mth.lerp(partialTick, entity.prevAnimationProgresses[5], entity.ridingProgress), 20.0F, false);
            if (cube.boxName.equals("BodyUpper")) {
                cube.offsetZ = cube.offsetZ + (-12.0F - cube.offsetZ) / 20.0F * Mth.lerp(partialTick, entity.prevAnimationProgresses[5], entity.ridingProgress);
            }
        }
        if (entity.tackleProgress > 0.0F && !this.isRotationEqual(this.getModel(EnumDragonPoses.TACKLE).getCube(cube.boxName), this.getModel(EnumDragonPoses.FLYING_POSE).getCube(cube.boxName)) && !this.isWing(model, cube)) {
            this.transitionTo(cube, this.getModel(EnumDragonPoses.TACKLE).getCube(cube.boxName), Mth.lerp(partialTick, entity.prevAnimationProgresses[6], entity.tackleProgress), 5.0F, false);
        }
        if (entity.diveProgress > 0.0F && !this.isRotationEqual(cube, this.getModel(EnumDragonPoses.DIVING_POSE).getCube(cube.boxName))) {
            this.transitionTo(cube, this.getModel(EnumDragonPoses.DIVING_POSE).getCube(cube.boxName), Mth.lerp(partialTick, entity.prevDiveProgress, entity.diveProgress), 10.0F, false);
        }
        if (entity.fireBreathProgress > 0.0F && !this.isRotationEqual(cube, this.getModel(EnumDragonPoses.STREAM_BREATH).getCube(cube.boxName)) && !this.isWing(model, cube) && !cube.boxName.contains("Finger")) {
            if (entity.prevFireBreathProgress <= entity.fireBreathProgress) {
                this.transitionTo(cube, this.getModel(EnumDragonPoses.BLAST_CHARGE3).getCube(cube.boxName), Mth.clamp(Mth.lerp(partialTick, entity.prevFireBreathProgress, entity.fireBreathProgress), 0.0F, 5.0F), 5.0F, false);
            }
            this.transitionTo(cube, this.getModel(EnumDragonPoses.STREAM_BREATH).getCube(cube.boxName), Mth.clamp(Mth.lerp(partialTick, entity.prevFireBreathProgress, entity.fireBreathProgress) - 5.0F, 0.0F, 5.0F), 5.0F, false);
        }
        if (!walking) {
            AdvancedModelBox flightPart = this.getModel(EnumDragonPoses.FLYING_POSE).getCube(cube.boxName);
            AdvancedModelBox prevPositionCubex = prevPosition.getCube(cube.boxName);
            AdvancedModelBox currPositionCubex = currentPosition.getCube(cube.boxName);
            float prevX = prevPositionCubex.rotateAngleX;
            float prevY = prevPositionCubex.rotateAngleY;
            float prevZ = prevPositionCubex.rotateAngleZ;
            float x = currPositionCubex.rotateAngleX;
            float y = currPositionCubex.rotateAngleY;
            float z = currPositionCubex.rotateAngleZ;
    ...
    public void setRotationsLoopDeath(TabulaModel model, T entity, float limbSwingAmount, boolean walking, TabulaModel currentPosition, TabulaModel prevPosition, float partialTick, float deltaTicks, AdvancedModelBox cube) {
        if (entity.modelDeadProgress > 0.0F) {
            ...
            if (!this.isRotationEqual(cube, pose.getCube(cube.boxName))) {
                this.transitionTo(cube, pose.getCube(cube.boxName), entity.prevModelDeadProgress + (entity.modelDeadProgress - entity.prevModelDeadProgress) * Minecraft.getInstance().getFrameTime(), 20.0F, cube.boxName.equals("ThighR") || cube.boxName.equals("ThighL"));
            }
            if (this instanceof IceDragonTabulaModelAnimator && cube.boxName.equals("BodyUpper")) {
                cube.rotationPointY = cube.rotationPointY + 0.35F * Mth.lerp(partialTick, entity.prevModelDeadProgress, entity.modelDeadProgress);
            }
        }
    }
