...
public abstract class ModelBipedBase<T extends LivingEntity> extends AdvancedEntityModel<T> implements ICustomStatueModel, BasicHeadedModel, ArmedModel {
    ...
    public void progressRotationInterp(AdvancedModelBox model, float progress, float rotX, float rotY, float rotZ, float max) {
        model.rotateAngleX = model.rotateAngleX + progress * (rotX - model.defaultRotationX) / max;
        model.rotateAngleY = model.rotateAngleY + progress * (rotY - model.defaultRotationY) / max;
        model.rotateAngleZ = model.rotateAngleZ + progress * (rotZ - model.defaultRotationZ) / max;
    }
    ...
    public void progressPosition(AdvancedModelBox model, float progress, float x, float y, float z) {
        model.rotationPointX = model.rotationPointX + progress * (x - model.defaultPositionX) / 20.0F;
        model.rotationPointY = model.rotationPointY + progress * (y - model.defaultPositionY) / 20.0F;
        model.rotationPointZ = model.rotationPointZ + progress * (z - model.defaultPositionZ) / 20.0F;
    }
    ...
    public void setupAnim(T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        float f = 1.0F;
        this.armRight.rotateAngleX = this.armRight.rotateAngleX + Mth.cos(limbSwing * 0.6662F + (float) Math.PI) * 2.0F * limbSwingAmount * 0.5F / f;
        this.armLeft.rotateAngleX = this.armLeft.rotateAngleX + Mth.cos(limbSwing * 0.6662F) * 2.0F * limbSwingAmount * 0.5F / f;
        ...
        if (entityIn.m_20159_()) {
            this.armRight.rotateAngleX += (float) (-Math.PI / 5);
            this.armLeft.rotateAngleX += (float) (-Math.PI / 5);
            this.legRight.rotateAngleX = -1.4137167F;
            this.legRight.rotateAngleY = (float) (Math.PI / 10);
            this.legRight.rotateAngleZ = 0.07853982F;
            this.legLeft.rotateAngleX = -1.4137167F;
            this.legLeft.rotateAngleY = (float) (-Math.PI / 10);
        ...
        if (this.f_102608_ > 0.0F) {
            ...
            this.body.rotateAngleY = Mth.sin(Mth.sqrt(f1) * (float) (Math.PI * 2)) * 0.2F;
            ...
            this.armRight.rotateAngleY = this.armRight.rotateAngleY + this.body.rotateAngleY;
            this.armLeft.rotateAngleY = this.armLeft.rotateAngleY + this.body.rotateAngleY;
            this.armLeft.rotateAngleX = this.armLeft.rotateAngleX + this.body.rotateAngleX;
            f1 = 1.0F - this.f_102608_;
            f1 *= f1;
            f1 *= f1;
            f1 = 1.0F - f1;
            float f2 = Mth.sin(f1 * (float) Math.PI);
            float f3 = Mth.sin(this.f_102608_ * (float) Math.PI) * -(this.head.rotateAngleX - 0.7F) * 0.75F;
            ...
            modelrenderer.rotateAngleZ = modelrenderer.rotateAngleZ + Mth.sin(this.f_102608_ * (float) Math.PI) * -0.4F;
        }
        if (this.isSneak) {
        ...
        this.armRight.rotateAngleZ = this.armRight.rotateAngleZ + Mth.cos(ageInTicks * 0.09F) * 0.05F + 0.05F;
        this.armLeft.rotateAngleZ = this.armLeft.rotateAngleZ - (Mth.cos(ageInTicks * 0.09F) * 0.05F + 0.05F);
        this.armRight.rotateAngleX = this.armRight.rotateAngleX + Mth.sin(ageInTicks * 0.067F) * 0.05F;
        this.armLeft.rotateAngleX = this.armLeft.rotateAngleX - Mth.sin(ageInTicks * 0.067F) * 0.05F;
    }
