...
public class CoeffTransformer {
    ...
    static int[][] dequantCoef = new int[][] { { 10, 13, 10, 13, 13, 16, 13, 16, 10, 13, 10, 13, 13, 16, 13, 16 }, { 11, 14, 11, 14, 14, 18, 14, 18, 11, 14, 11, 14, 14, 18, 14, 18 }, { 13, 16, 13, 16, 16, 20, 16, 20, 13, 16, 13, 16, 16, 20, 16, 20 }, { 14, 18, 14, 18, 18, 23, 18, 23, 14, 18, 14, 18, 18, 23, 18, 23 }, { 16, 20, 16, 20, 20, 25, 20, 25, 16, 20, 16, 20, 20, 25, 20, 25 }, { 18, 23, 18, 23, 23, 29, 23, 29, 18, 23, 18, 23, 23, 29, 23, 29 } };

    static int[][] dequantCoef8x8 = new int[6][64];

    static int[][] initDequantCoeff8x8 = new int[][] { { 20, 18, 32, 19, 25, 24 }, { 22, 19, 35, 21, 28, 26 }, { 26, 23, 42, 24, 33, 31 }, { 28, 25, 45, 26, 35, 33 }, { 32, 28, 51, 30, 40, 38 }, { 36, 32, 58, 34, 46, 43 } };
    ...
    static {
        ...
        for (int g = 0; g < 6; g++) {
            ...
            for (int ix = 0; ix < 8; ix += 4) {
                for (int j = 0; j < 8; j += 4) {
                    dequantCoef8x8[g][(ix << 3) + j] = initDequantCoeff8x8[g][0];
                }
            }
            for (int ix = 1; ix < 8; ix += 2) {
                for (int j = 1; j < 8; j += 2) {
                    dequantCoef8x8[g][(ix << 3) + j] = initDequantCoeff8x8[g][1];
                }
            }
            for (int ix = 2; ix < 8; ix += 4) {
                for (int j = 2; j < 8; j += 4) {
                    dequantCoef8x8[g][(ix << 3) + j] = initDequantCoeff8x8[g][2];
                }
            }
            for (int ix = 0; ix < 8; ix += 4) {
                for (int j = 1; j < 8; j += 2) {
                    dequantCoef8x8[g][(ix << 3) + j] = initDequantCoeff8x8[g][3];
                }
            }
            for (int ix = 1; ix < 8; ix += 2) {
                for (int j = 0; j < 8; j += 4) {
            ...
            for (int ix = 0; ix < 8; ix += 4) {
                for (int j = 2; j < 8; j += 4) {
                    dequantCoef8x8[g][(ix << 3) + j] = initDequantCoeff8x8[g][4];
                }
            }
            for (int ix = 2; ix < 8; ix += 4) {
                for (int j = 0; j < 8; j += 4) {
