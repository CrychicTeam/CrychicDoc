...
public class CitadelShaderRenderTypes extends RenderType {
    ...
    public static RenderType getRainbowAura(ResourceLocation locationIn) {
        return m_173215_("rainbow_aura", DefaultVertexFormat.POSITION_COLOR_TEX, VertexFormat.Mode.QUADS, 256, false, true, RenderType.CompositeState.builder().setShaderState(RENDERTYPE_RAINBOW_AURA_SHADER).setCullState(f_110110_).setTextureState(new RenderStateShard.TextureStateShard(locationIn, false, false)).setTransparencyState(RenderStateShard.TRANSLUCENT_TRANSPARENCY).setDepthTestState(f_110113_).setOutputState(RAINBOW_AURA_OUTPUT).createCompositeState(true));
    }
}