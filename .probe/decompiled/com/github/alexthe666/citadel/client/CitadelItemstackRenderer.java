...
public class CitadelItemstackRenderer extends BlockEntityWithoutLevelRenderer {
    ...
    public void renderByItem(ItemStack stack, ItemDisplayContext transformType, PoseStack matrixStack, MultiBufferSource buffer, int combinedLight, int combinedOverlay) {
        ...
        int id = Minecraft.getInstance().player == null ? 0 : Minecraft.getInstance().player.m_19879_();
        if (stack.getItem() == Citadel.FANCY_ITEM.get()) {
            ...
            if (animateAnyways || stack.getTag() != null && stack.getTag().contains("DisplayBob") && stack.getTag().getBoolean("DisplayBob")) {
                matrixStack.translate(0.0F, 0.05F + 0.1F * Mth.sin(0.3F * ticksExisted), 0.0F);
            }
            if (stack.getTag() != null && stack.getTag().contains("DisplaySpin") && stack.getTag().getBoolean("DisplaySpin")) {
                matrixStack.mulPose(Axis.YP.rotationDegrees(6.0F * ticksExisted));
            }
        ...
        if (stack.getItem() == Citadel.EFFECT_ITEM.get()) {
            ...
            if (stack.getTag() != null && stack.getTag().contains("DisplayEffect")) {
                ...
                effect = (MobEffect) mobEffectList.get(time % size);
                if (effect == null) {
            ...
            int br = 255;
            bufferbuilder.m_252986_(mx, 1.0F, 1.0F, 0.0F).uv(sprite.getU1(), sprite.getV0()).color(br, br, br, 255).uv2(combinedLight).endVertex();
            bufferbuilder.m_252986_(mx, 0.0F, 1.0F, 0.0F).uv(sprite.getU0(), sprite.getV0()).color(br, br, br, 255).uv2(combinedLight).endVertex();
            bufferbuilder.m_252986_(mx, 0.0F, 0.0F, 0.0F).uv(sprite.getU0(), sprite.getV1()).color(br, br, br, 255).uv2(combinedLight).endVertex();
            bufferbuilder.m_252986_(mx, 1.0F, 0.0F, 0.0F).uv(sprite.getU1(), sprite.getV1()).color(br, br, br, 255).uv2(combinedLight).endVertex();
            tessellator.end();
            matrixStack.popPose();
        }
        if (stack.getItem() == Citadel.ICON_ITEM.get()) {
            ...
            if (stack.getTag() != null && stack.getTag().contains("IconLocation")) {
                ...
                if (LOADED_ICONS.containsKey(iconLocationStr)) {
                    texture = (ResourceLocation) LOADED_ICONS.get(iconLocationStr);
                } else {
            ...
            Matrix4f mx = matrixStack.last().pose();
            int br = 255;
            bufferbuilder.m_252986_(mx, 1.0F, 1.0F, 0.0F).uv(1.0F, 0.0F).color(br, br, br, 255).uv2(combinedLight).endVertex();
            bufferbuilder.m_252986_(mx, 0.0F, 1.0F, 0.0F).uv(0.0F, 0.0F).color(br, br, br, 255).uv2(combinedLight).endVertex();
            bufferbuilder.m_252986_(mx, 0.0F, 0.0F, 0.0F).uv(0.0F, 1.0F).color(br, br, br, 255).uv2(combinedLight).endVertex();
            bufferbuilder.m_252986_(mx, 1.0F, 0.0F, 0.0F).uv(1.0F, 1.0F).color(br, br, br, 255).uv2(combinedLight).endVertex();
            tessellator.end();
            matrixStack.popPose();
        }
    }
