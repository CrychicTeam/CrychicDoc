...
public class AMWorldData extends SavedData {
    ...
    private static final Predicate<BlockState> IS_WATER = state -> state.m_60713_(Blocks.WATER);

    public static AMWorldData get(Level world) {
        if (world instanceof ServerLevel) {
            ServerLevel overworld = world.getServer().getLevel(Level.OVERWORLD);
            AMWorldData fromMap = (AMWorldData) dataMap.get(overworld);
            if (fromMap == null) {
                DimensionDataStorage storage = overworld.getDataStorage();
                AMWorldData data = storage.computeIfAbsent(AMWorldData::load, AMWorldData::new, "alexsmobs_world_data");
                if (data != null) {
    ...
    public boolean isInPupfishChunk(BlockPos pos) {
        return this.pupfishChunk == null ? false : pos.m_123341_() >= this.pupfishChunk.getMinBlockX() && pos.m_123341_() <= this.pupfishChunk.getMaxBlockX() && pos.m_123343_() >= this.pupfishChunk.getMinBlockZ() && pos.m_123343_() <= this.pupfishChunk.getMaxBlockZ();
    }
    ...
    private void searchForPupfishChunk() {
        if (this.level != null && this.level.getChunkSource().getGenerator() instanceof NoiseBasedChunkGenerator chunkGenerator) {
            Random random = new Random(this.level.getSeed() + (long) this.pupfishSeedAddition);
            ...
            int maxWater = this.getWaterHeight(chunkGenerator, this.level.getChunkSource().randomState(), center.m_123341_(), center.m_123343_(), this.level);
            if (maxWater > 31 && maxWater < 63) {
                this.pupfishChunk = checkPos;
                AlexsMobs.LOGGER.info("Found Pupfish chunk at " + this.pupfishChunk.getMaxBlockX() + " ~ " + this.pupfishChunk.getMinBlockZ() + " after " + this.pupfishSeedAddition + " tries");
            }
        }
