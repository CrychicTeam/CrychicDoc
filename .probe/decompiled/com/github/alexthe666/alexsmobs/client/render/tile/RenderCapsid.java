...
public class RenderCapsid<T extends TileEntityCapsid> implements BlockEntityRenderer<T> {
    ...
    public void render(T entity, float partialTicks, PoseStack matrixStackIn, MultiBufferSource bufferIn, int combinedLightIn, int combinedOverlayIn) {
        ...
        if (!stack.isEmpty()) {
            ...
            matrixStackIn.mulPose(new Quaternionf().rotateY(Maths.rad((double) (entity.getBlockAngle() + yaw))));
            matrixStackIn.pushPose();
            matrixStackIn.translate(0.0F, -0.1F, 0.0F);
            if (entity.vibratingThisTick && entity.m_58904_() != null) {
                float vibrate = 0.05F;
                matrixStackIn.translate((entity.m_58904_().random.nextFloat() - 0.5F) * vibrate, (entity.m_58904_().random.nextFloat() - 0.5F) * vibrate, (entity.m_58904_().random.nextFloat() - 0.5F) * vibrate);
            }
            matrixStackIn.scale(1.3F, 1.3F, 1.3F);
            BakedModel ibakedmodel = Minecraft.getInstance().getItemRenderer().getModel(stack, entity.m_58904_(), (LivingEntity) null, 0);
