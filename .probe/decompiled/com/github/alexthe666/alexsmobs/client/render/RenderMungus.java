...
public class RenderMungus extends MobRenderer<EntityMungus, ModelMungus> {
    ...
    protected void scale(EntityMungus entitylivingbaseIn, PoseStack matrixStackIn, float partialTickTime) {
        String s = ChatFormatting.stripFormatting(entitylivingbaseIn.m_7755_().getString());
        if (s != null && s.toLowerCase().contains("drip")) {
            matrixStackIn.translate(0.0F, entitylivingbaseIn.m_6162_() ? -0.075F : -0.15F, 0.0F);
        }
    }
    ...
    public void render(EntityMungus entityIn, float entityYaw, float partialTicks, PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn) {
        ...
        if (target != null) {
            ...
            float f9 = 0.2F;
            float f10 = 0.282F;
            float f11 = Mth.cos((float) (Math.PI * 3.0 / 4.0)) * 0.8F;
            float f12 = Mth.sin((float) (Math.PI * 3.0 / 4.0)) * 0.8F;
            float f13 = Mth.cos((float) (Math.PI / 4)) * 0.8F;
            float f14 = Mth.sin((float) (Math.PI / 4)) * 0.8F;
            float f15 = Mth.cos((float) Math.PI * 5.0F / 4.0F) * 0.8F;
            float f16 = Mth.sin((float) Math.PI * 5.0F / 4.0F) * 0.8F;
            float f17 = Mth.cos((float) Math.PI * 7.0F / 4.0F) * 0.8F;
            float f18 = Mth.sin((float) Math.PI * 7.0F / 4.0F) * 0.8F;
            float f19 = Mth.cos((float) Math.PI) * 0.4F;
            float f20 = Mth.sin((float) Math.PI) * 0.4F;
            float f21 = Mth.cos(0.0F) * 0.4F;
            float f22 = Mth.sin(0.0F) * 0.4F;
            float f23 = Mth.cos((float) (Math.PI / 2)) * 0.4F;
            float f24 = Mth.sin((float) (Math.PI / 2)) * 0.4F;
            float f25 = Mth.cos((float) (Math.PI * 3.0 / 2.0)) * 0.4F;
            float f26 = Mth.sin((float) (Math.PI * 3.0 / 2.0)) * 0.4F;
            float f27 = 0.0F;
            float f28 = 0.4999F;
            float f29 = -1.0F + f2;
        ...
        public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, EntityMungus entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
            ...
            if (blockstate != null) {
                ...
                if (mushroomCount == 1 && !altOrder || mushroomCount >= 2) {
                    matrixStackIn.pushPose();
                    matrixStackIn.translate(0.2F, -1.4F, 0.15);
                ...
                if (mushroomCount == 1 && altOrder || mushroomCount >= 2) {
                    matrixStackIn.pushPose();
                    matrixStackIn.translate(-0.2F, -1.5, -0.2);
                    matrixStackIn.scale(-1.0F, -1.0F, 1.0F);
                    matrixStackIn.translate(-0.5, -0.5, -0.5);
                    blockrendererdispatcher.renderSingleBlock(blockstate, matrixStackIn, bufferIn, packedLightIn, i);
                    matrixStackIn.popPose();
                ...
                if (mushroomCount >= 3) {
                    matrixStackIn.pushPose();
                    matrixStackIn.translate(0.76F, -0.4F, 0.1);
                ...
                if (mushroomCount >= 4) {
                    matrixStackIn.pushPose();
                    matrixStackIn.translate(-0.76F, -1.0, 0.1);
                ...
                if (mushroomCount >= 5) {
                    matrixStackIn.pushPose();
                    matrixStackIn.translate(-0.76F, -0.1F, 0.1);
    ...
    static class MungusSackLayer extends RenderLayer<EntityMungus, ModelMungus> {
        ...
        public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, EntityMungus entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
            VertexConsumer lead = bufferIn.getBuffer(AMRenderTypes.getEyesFlickering(RenderMungus.TEXTURE_SACK_OVERLAY, 0.0F));
            float alpha = 0.75F + (Mth.cos(ageInTicks * 0.2F) + 1.0F) * 0.125F;
            ((ModelMungus) this.m_117386_()).renderToBuffer(matrixStackIn, lead, 240, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, alpha);
            if (entitylivingbaseIn.getBeamTarget() != null) {
                VertexConsumer beam = bufferIn.getBuffer(AMRenderTypes.getGhost(RenderMungus.TEXTURE_BEAM_OVERLAY));
                float beamAlpha = 0.75F + (Mth.cos(ageInTicks * 1.0F) + 1.0F) * 0.125F;
                ((ModelMungus) this.m_117386_()).renderToBuffer(matrixStackIn, beam, 240, LivingEntityRenderer.getOverlayCoords(entitylivingbaseIn, 0.0F), 1.0F, 1.0F, 1.0F, beamAlpha);
            }
            String s = ChatFormatting.stripFormatting(entitylivingbaseIn.m_7755_().getString());
            if (s != null && s.toLowerCase().contains("drip")) {
                VertexConsumer shoeBuffer = bufferIn.getBuffer(AMRenderTypes.m_110458_(RenderMungus.TEXTURE_SHOES));
                matrixStackIn.pushPose();
                ((ModelMungus) this.m_117386_()).renderShoes();
                ((ModelMungus) this.m_117386_()).renderToBuffer(matrixStackIn, shoeBuffer, packedLightIn, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F);
