    ...
    class LayerShiny extends RenderLayer<EntityBlueJay, ModelBlueJay> {
        ...
        public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, EntityBlueJay entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
            if (entitylivingbaseIn.getFeedTime() > 0) {
                VertexConsumer ivertexbuilder = bufferIn.getBuffer(RenderType.entityTranslucent(RenderBlueJay.TEXTURE_SHINY));
                float alpha = (float) (1.0 + Math.sin((double) (ageInTicks * 0.3F))) * 0.1F + 0.8F;
                ((ModelBlueJay) this.m_117386_()).renderToBuffer(matrixStackIn, ivertexbuilder, packedLightIn, LivingEntityRenderer.getOverlayCoords(entitylivingbaseIn, 0.0F), 1.0F, 1.0F, 1.0F, alpha);
            }
        }
    }
}