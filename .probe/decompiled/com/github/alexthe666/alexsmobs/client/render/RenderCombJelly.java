...
public class RenderCombJelly extends MobRenderer<EntityCombJelly, ModelCombJelly> {
    ...
    static class RainbowLayer extends RenderLayer<EntityCombJelly, ModelCombJelly> {
        ...
        public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, EntityCombJelly entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
            VertexConsumer rainbow = AMRenderTypes.createMergedVertexConsumer(bufferIn.getBuffer(AMRenderTypes.COMBJELLY_RAINBOW_GLINT), bufferIn.getBuffer(RenderType.entityCutoutNoCull(RenderCombJelly.TEXTURE_OVERLAY)));
            RenderCombJelly.STRIPES_MODEL.setupAnim(entitylivingbaseIn, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch);
            RenderCombJelly.STRIPES_MODEL.m_7695_(matrixStackIn, rainbow, packedLightIn, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F);
        }
    }
}