    ...
    class LayerSnow extends RenderLayer<EntityMoose, ModelMoose> {
        ...
        public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, EntityMoose entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
            if (entitylivingbaseIn.isSnowy()) {
                VertexConsumer ivertexbuilder = bufferIn.getBuffer(RenderType.entityCutoutNoCull(entitylivingbaseIn.isAntlered() && !entitylivingbaseIn.m_6162_() ? RenderMoose.TEXTURE_SNOWY_ANTLERED : RenderMoose.TEXTURE_SNOWY));
                ((ModelMoose) this.m_117386_()).renderToBuffer(matrixStackIn, ivertexbuilder, packedLightIn, LivingEntityRenderer.getOverlayCoords(entitylivingbaseIn, 0.0F), 1.0F, 1.0F, 1.0F, 1.0F);
            }
        }
    }
}