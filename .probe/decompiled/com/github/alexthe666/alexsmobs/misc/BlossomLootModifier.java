...
public class BlossomLootModifier implements IGlobalLootModifier {
    ...
    public static final Supplier<Codec<BlossomLootModifier>> CODEC = () -> RecordCodecBuilder.create(inst -> inst.group(LOOT_CONDITIONS_CODEC.fieldOf("conditions").forGetter(lm -> lm.conditions)).apply(inst, BlossomLootModifier::new));
    ...
    protected ObjectArrayList<ItemStack> doApply(ObjectArrayList<ItemStack> generatedLoot, LootContext context) {
        if (AMConfig.acaciaBlossomsDropFromLeaves) {
            ...
            int bananaStep = (int) Math.min((float) AMConfig.blossomChance * 0.1F, 0.0F);
