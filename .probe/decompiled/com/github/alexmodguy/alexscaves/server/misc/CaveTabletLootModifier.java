...
public class CaveTabletLootModifier implements IGlobalLootModifier {
    ...
    public static final Supplier<Codec<CaveTabletLootModifier>> CODEC = () -> RecordCodecBuilder.create(inst -> inst.group(ENTRY_CODEC.forGetter(configuration -> configuration.biome), Codec.BOOL.fieldOf("replace").forGetter(configuration -> configuration.replace), LOOT_CONDITIONS_CODEC.fieldOf("conditions").forGetter(lm -> lm.conditions)).apply(inst, CaveTabletLootModifier::new));
    ...
    protected ObjectArrayList<ItemStack> doApply(ObjectArrayList<ItemStack> generatedLoot, LootContext context) {
        if (context.getRandom().nextFloat() < this.getChance()) {
            ...
            generatedLoot.add(this.getTablet());
        }
    ...
    private float getChance() {
        if (this.biome == null || BiomeGenerationConfig.isBiomeDisabledCompletely(this.biome)) {
            return 0.0F;
        } else if (this.biome.equals(ACBiomeRegistry.MAGNETIC_CAVES)) {
            return AlexsCaves.COMMON_CONFIG.magneticTabletLootChance.get().floatValue();
        } else if (this.biome.equals(ACBiomeRegistry.PRIMORDIAL_CAVES)) {
            return AlexsCaves.COMMON_CONFIG.primordialTabletLootChance.get().floatValue();
        } else if (this.biome.equals(ACBiomeRegistry.TOXIC_CAVES)) {
            return AlexsCaves.COMMON_CONFIG.toxicTabletLootChance.get().floatValue();
        } else if (this.biome.equals(ACBiomeRegistry.ABYSSAL_CHASM)) {
            return AlexsCaves.COMMON_CONFIG.abyssalTabletLootChance.get().floatValue();
        } else {
            return this.biome.equals(ACBiomeRegistry.FORLORN_HOLLOWS) ? AlexsCaves.COMMON_CONFIG.forlornTabletLootChance.get().floatValue() : 0.0F;
        }
    }
