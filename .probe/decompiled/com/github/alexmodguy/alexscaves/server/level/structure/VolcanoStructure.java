...
public class VolcanoStructure extends Structure {
    ...
    public static final Codec<VolcanoStructure> CODEC = m_226607_(settings -> new VolcanoStructure(settings));
    ...
    public Optional<Structure.GenerationStub> findGenerationPoint(Structure.GenerationContext context) {
        ...
        for (Holder<Biome> holder : ACMath.getBiomesWithinAtY(context.biomeSource(), i, -34, j, 30, context.randomState().sampler())) {
    ...
    public void generatePieces(StructurePiecesBuilder builder, Structure.GenerationContext context) {
        ...
        int volcanoWidthRadius = 30 + context.random().m_188503_(15);
        int volcanoHeight = volcanoWidthRadius + 8 + context.random().m_188503_(5);
        int i = context.chunkPos().getMinBlockX();
        int j = context.chunkPos().getMinBlockZ();
        int k = context.chunkGenerator().getSeaLevel();
        BlockPos xzCoords = new BlockPos(i, -34, j);
        int widthChunks = (int) Math.ceil((double) ((float) volcanoWidthRadius / 16.0F));
