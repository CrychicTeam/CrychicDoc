...
public class PeeringCoprolithFeature extends Feature<NoneFeatureConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        int j = 2;
        double d4 = (double) (blockpos.m_123342_() + randomsource.nextInt(3) - 2);
        double d5 = (double) (blockpos.m_123342_() + randomsource.nextInt(3) - 2);
        int k = blockpos.m_123341_() - Mth.ceil(f1) - i;
        int l = blockpos.m_123342_() - 2 - i;
        int i1 = blockpos.m_123343_() - Mth.ceil(f1) - i;
    ...
    protected boolean doPlace(WorldGenLevel worldGenLevel, RandomSource randomSource, double xMin, double xMax, double zMin, double zMax, double yMin, double yMax, int sizeX, int sizeY, int sizeZ, int length, int width) {
        ...
        for (int k = 0; k < j; k++) {
            ...
            double d3 = randomSource.nextDouble() * (double) j / 16.0;
                            ...
                            for (int j2 = l; j2 <= k1; j2++) {
                                ...
                                if (d5 * d5 + d6 * d6 < 1.0) {
                                    for (int k2 = i1; k2 <= l1; k2++) {
                                        ...
                                        if (d5 * d5 + d6 * d6 + d7 * d7 < 1.0 && !worldGenLevel.m_151562_(j2)) {
                                            ...
                                            if (!bitset.get(l2)) {
                                                ...
                                                if (worldGenLevel.ensureCanWrite(blockpos$mutableblockpos)) {
                                                    ...
                                                    if (levelchunksection != null) {
                                                        ...
                                                        if (canPlaceOre(blockstate, bulksectionaccess::m_156110_, randomSource, blockpos$mutableblockpos)) {
    ...
    public static boolean canPlaceOre(BlockState blockState, Function<BlockPos, BlockState> blockStateFunction, RandomSource randomSource, BlockPos.MutableBlockPos blockPos) {
        return !blockState.m_60713_(ACBlockRegistry.COPROLITH.get()) && !blockState.m_60713_(ACBlockRegistry.GUANOSTONE.get()) ? false : m_159750_(blockStateFunction, blockPos);
    }
}