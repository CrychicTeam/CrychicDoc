...
public class CoveredBlockBlobConfiguration implements FeatureConfiguration {
    ...
    public static final Codec<CoveredBlockBlobConfiguration> CODEC = RecordCodecBuilder.create(config -> config.group(BlockStateProvider.CODEC.fieldOf("block").forGetter(otherConfig -> otherConfig.block), BlockStateProvider.CODEC.fieldOf("cover").forGetter(otherConfig -> otherConfig.coverBlock)).apply(config, CoveredBlockBlobConfiguration::new));
