...
public class ResistorShieldItem extends ShieldItem {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand interactionHand) {
        ...
        player.m_216990_(ACSoundRegistry.RESITOR_SHIELD_SPIN.get());
    ...
    public void onUseTick(Level level, LivingEntity living, ItemStack stack, int timeUsing) {
        ...
        if (level.isClientSide) {
            setUseTime(stack, i);
            if (i == 10) {
                living.m_216990_(ACSoundRegistry.RESITOR_SHIELD_SLAM.get());
            }
            if (i >= 10 && i % 5 == 0) {
                AlexsCaves.PROXY.playWorldSound(living, (byte) (scarlet ? 9 : 10));
                Vec3 particlesFrom = living.m_20182_().add(0.0, 0.2, 0.0);
                float particleMax = (float) (5 + living.getRandom().nextInt(5));
                for (int particles = 0; (float) particles < particleMax; particles++) {
                    Vec3 vec3 = new Vec3(((double) living.getRandom().nextFloat() - 0.5) * 0.3F, ((double) living.getRandom().nextFloat() - 0.5) * 0.3F, (double) (range * 0.5F + range * 0.5F * living.getRandom().nextFloat())).yRot((float) ((double) ((float) particles / particleMax) * Math.PI * 2.0)).add(particlesFrom);
                    if (scarlet) {
        ...
        if (i >= 10 && i % 5 == 0) {
            AABB bashBox = living.m_20191_().inflate(5.0, 1.0, 5.0);
            for (LivingEntity entity : living.m_9236_().m_45976_(LivingEntity.class, bashBox)) {
                if (!living.m_7307_(entity) && !entity.equals(living) && entity.m_20270_(living) <= range) {
                    entity.hurt(living.m_269291_().mobAttack(living), firstHit ? (float) (6 + slamEnchantAmount * 3) : 2.0F);
                    if (scarlet) {
                        entity.knockback(firstHit ? 0.5 : 0.2, entity.m_20185_() - living.m_20185_(), entity.m_20189_() - living.m_20189_());
                    } else {
                        entity.knockback(firstHit ? 0.5 : 0.2, living.m_20185_() - living.m_20185_(), living.m_20189_() - entity.m_20189_());
                    }
                }
            }
        }
        if (i == 10 && !level.isClientSide) {
            stack.hurtAndBreak(1, living, player1 -> player1.broadcastBreakEvent(player1.getUsedItemHand()));
        }
    }
    ...
    public static float getLerpedSwitchTime(ItemStack stack, float f) {
        CompoundTag compoundtag = stack.getTag();
        float prev = compoundtag != null ? (float) compoundtag.getInt("PrevSwitchTime") : 0.0F;
        float current = compoundtag != null ? (float) compoundtag.getInt("SwitchTime") : 0.0F;
        return prev + f * (current - prev);
    }
