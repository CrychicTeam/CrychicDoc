...
public class RadiationRemovingFoodItem extends Item {
    ...
    public RadiationRemovingFoodItem(Item.Properties properties) {
        super(properties);
    }
    ...
    public ItemStack finishUsingItem(ItemStack stack, Level level, LivingEntity livingEntity) {
        ...
        if (mobEffectInstance != null && foodProperties != null) {
            float f = Math.min(livingEntity.getMaxHealth(), livingEntity.getHealth() + (float) Math.ceil((double) ((float) foodProperties.getNutrition() * 1.5F + 1.0F)));
