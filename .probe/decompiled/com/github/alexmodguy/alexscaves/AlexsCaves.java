...
public class AlexsCaves {
    ...
    public static final SimpleChannel NETWORK_WRAPPER = NetworkRegistry.ChannelBuilder.named(PACKET_NETWORK_NAME).clientAcceptedVersions(PROTOCOL_VERSION::equals).serverAcceptedVersions(PROTOCOL_VERSION::equals).networkProtocolVersion(() -> PROTOCOL_VERSION).simpleChannel();
    ...
    public AlexsCaves() {
        ModLoadingContext.get().registerConfig(Type.COMMON, COMMON_CONFIG_SPEC, "alexscaves-general.toml");
        ModLoadingContext.get().registerConfig(Type.CLIENT, CLIENT_CONFIG_SPEC, "alexscaves-client.toml");
    ...
    static {
        Pair<ACServerConfig, ForgeConfigSpec> serverPair = new ForgeConfigSpec.Builder().configure(ACServerConfig::new);
        COMMON_CONFIG = (ACServerConfig) serverPair.getLeft();
        COMMON_CONFIG_SPEC = (ForgeConfigSpec) serverPair.getRight();
        Pair<ACClientConfig, ForgeConfigSpec> clientPair = new ForgeConfigSpec.Builder().configure(ACClientConfig::new);
        CLIENT_CONFIG = (ACClientConfig) clientPair.getLeft();
        CLIENT_CONFIG_SPEC = (ForgeConfigSpec) clientPair.getRight();
    }
}