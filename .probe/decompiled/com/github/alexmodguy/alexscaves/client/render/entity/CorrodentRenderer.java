...
public class CorrodentRenderer extends MobRenderer<CorrodentEntity, CorrodentModel> {
    ...
    public CorrodentRenderer(EntityRendererProvider.Context renderManagerIn) {
        ...
        this.m_115326_(new CorrodentRenderer.LayerGlow());
    }
    ...
    public void render(CorrodentEntity entityIn, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn) {
        ...
        if (digAmount > 0.0F) {
            ...
            for (BlockPos mutableBlockPos : BlockPos.betweenClosed((int) Math.floor(x - digEffectDistance), (int) Math.floor(y - digEffectDistance), (int) Math.floor(z - digEffectDistance), (int) Math.floor(x + digEffectDistance), (int) Math.floor(y + digEffectDistance), (int) Math.floor(z + digEffectDistance))) {
                ...
                if (amount >= 0) {
                    allDugBlocksOnScreen.put(mutableBlockPos.immutable(), Math.max((Integer) allDugBlocksOnScreen.getOrDefault(mutableBlockPos, -1), amount));
                }
            }
        }
    }
    ...
    public static void renderEntireBatch(LevelRenderer levelRenderer, PoseStack poseStack, int renderTick, Camera camera, float partialTick) {
        if (!allDugBlocksOnScreen.isEmpty()) {
            ...
            for (Entry<BlockPos, Integer> posAndInt : allDugBlocksOnScreen.entrySet()) {
                int progress = (Integer) posAndInt.getValue() - 1;
                if (progress >= 0 && progress < 10) {
                    poseStack.pushPose();
                    BlockPos pos = (BlockPos) posAndInt.getKey();
                    poseStack.translate((double) pos.m_123341_(), (double) pos.m_123342_(), (double) pos.m_123343_());
                    PoseStack.Pose posestack$pose1 = poseStack.last();
                    VertexConsumer vertexconsumer1 = new SheetedDecalTextureGenerator(multibuffersource$buffersource.getBuffer((RenderType) ModelBakery.DESTROY_TYPES.get(progress)), posestack$pose1.pose(), posestack$pose1.normal(), 1.0F);
                    ModelData modelData = Minecraft.getInstance().level.getModelDataManager().getAt(pos);
                    Minecraft.getInstance().getBlockRenderer().renderBreakingTexture(Minecraft.getInstance().level.m_8055_(pos), pos, Minecraft.getInstance().level, poseStack, vertexconsumer1, modelData == null ? ModelData.EMPTY : modelData);
                    poseStack.popPose();
                }
            }
            poseStack.popPose();
    ...
    class LayerGlow extends RenderLayer<CorrodentEntity, CorrodentModel> {
        ...
        public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, CorrodentEntity entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
            VertexConsumer ivertexbuilder = bufferIn.getBuffer(RenderType.eyes(CorrodentRenderer.TEXTURE_EYES));
            float alpha = 1.0F;
            ((CorrodentModel) this.m_117386_()).m_7695_(matrixStackIn, ivertexbuilder, packedLightIn, LivingEntityRenderer.getOverlayCoords(entitylivingbaseIn, 0.0F), 1.0F, 1.0F, 1.0F, alpha);
        }
    }
}