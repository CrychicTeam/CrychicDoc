...
public class AbyssalAltarBlockRenderer<T extends AbyssalAltarBlockEntity> implements BlockEntityRenderer<T> {
    ...
    public void render(T altar, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int combinedLightIn, int combinedOverlayIn) {
        ...
        if (!itemStack.isEmpty()) {
            ...
            float slideBy = altar.getItem(0).isEmpty() ? 0.5F * (1.0F - altar.getSlideProgress(partialTicks)) : 0.5F * altar.getSlideProgress(partialTicks);
            poseStack.translate(0.0F, 0.0F, slideBy);
            poseStack.mulPose(Axis.XP.rotationDegrees(90.0F));
            poseStack.scale(0.5F, 0.5F, 0.5F);
            ItemRenderer renderer = Minecraft.getInstance().getItemRenderer();
            BakedModel bakedModel = renderer.getModel(itemStack, altar.m_58904_(), null, 0);
            ...
            for (int k = 0; k < j; k++) {
                ...
                if (k > 0) {
                    if (flag) {
                        ...
                        poseStack.translate(f11, f13, f10 - (float) k * 0.1F);
                    } else {
