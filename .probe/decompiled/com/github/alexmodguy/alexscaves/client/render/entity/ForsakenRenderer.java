...
public class ForsakenRenderer extends MobRenderer<ForsakenEntity, ForsakenModel> {
    ...
    public ForsakenRenderer(EntityRendererProvider.Context renderManagerIn) {
        ...
        this.m_115326_(new ForsakenRenderer.LayerGlow());
    ...
    public void render(ForsakenEntity entity, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource source, int packedLight) {
        ...
        mouthParticlePositions.put(entity.m_19879_(), ((ForsakenModel) this.f_115290_).getMouthPosition(Vec3.ZERO));
    }
    ...
    class LayerGlow extends RenderLayer<ForsakenEntity, ForsakenModel> {
        ...
        public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, ForsakenEntity entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
            VertexConsumer darnkessConsumer = bufferIn.getBuffer(RenderType.entityTranslucent(ForsakenRenderer.TEXTURE_DARKNESS));
            ((ForsakenModel) this.m_117386_()).m_7695_(matrixStackIn, darnkessConsumer, packedLightIn, LivingEntityRenderer.getOverlayCoords(entitylivingbaseIn, 0.0F), 1.0F, 1.0F, 1.0F, entitylivingbaseIn.getDarknessAmount(partialTicks));
            VertexConsumer eyesConsumer = bufferIn.getBuffer(RenderType.eyes(ForsakenRenderer.TEXTURE_EYES));
            ((ForsakenModel) this.m_117386_()).m_7695_(matrixStackIn, eyesConsumer, packedLightIn, LivingEntityRenderer.getOverlayCoords(entitylivingbaseIn, 0.0F), 1.0F, 1.0F, 1.0F, 1.0F);
        }
    }
}