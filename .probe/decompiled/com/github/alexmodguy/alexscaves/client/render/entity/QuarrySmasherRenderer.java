...
public class QuarrySmasherRenderer extends EntityRenderer<QuarrySmasherEntity> {
    ...
    public void render(QuarrySmasherEntity entity, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn) {
        ...
        QUARRY_SMASHER_MODEL.m_7695_(poseStack, textureVertexConsumer, packedLightIn, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F);
        VertexConsumer activeVertexConsumer = bufferIn.getBuffer(ACRenderTypes.getEyesAlphaEnabled(TEXTURE_GLOW));
        QUARRY_SMASHER_MODEL.m_7695_(poseStack, activeVertexConsumer, 240, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F - inactive);
        poseStack.popPose();
        if (!entity.isInactive() && entity.lastMiningArea != null) {
            ...
            Vec3 corner1 = new Vec3(entity.lastMiningArea.minX + 0.5, entity.lastMiningArea.minY + 0.5, entity.lastMiningArea.minZ + 0.5);
            Vec3 corner2 = new Vec3(entity.lastMiningArea.maxX + 0.5, entity.lastMiningArea.minY + 0.5, entity.lastMiningArea.minZ + 0.5);
            Vec3 corner3 = new Vec3(entity.lastMiningArea.minX + 0.5, entity.lastMiningArea.minY + 0.5, entity.lastMiningArea.maxZ + 0.5);
            Vec3 corner4 = new Vec3(entity.lastMiningArea.maxX + 0.5, entity.lastMiningArea.minY + 0.5, entity.lastMiningArea.maxZ + 0.5);
            int segCount = Mth.clamp((int) Math.ceil(0.3F * corner1.horizontalDistance()), 3, 30);
            LightningBoltData bolt1 = new LightningBoltData(LIGHTNING_BOLT_INFO, corner1, position.add(-0.7F, (double) armStickyOuty, -0.7F), segCount).size(0.3F).lifespan(1).spawn(LightningBoltData.SpawnFunction.CONSECUTIVE).fade(LightningBoltData.FadeFunction.NONE);
            LightningBoltData bolt2 = new LightningBoltData(LIGHTNING_BOLT_INFO, corner2, position.add(0.7F, (double) armStickyOuty, -0.7F), segCount).size(0.3F).lifespan(1).spawn(LightningBoltData.SpawnFunction.CONSECUTIVE).fade(LightningBoltData.FadeFunction.NONE);
            LightningBoltData bolt3 = new LightningBoltData(LIGHTNING_BOLT_INFO, corner3, position.add(-0.7F, (double) armStickyOuty, 0.7F), segCount).size(0.3F).lifespan(1).spawn(LightningBoltData.SpawnFunction.CONSECUTIVE).fade(LightningBoltData.FadeFunction.NONE);
            LightningBoltData bolt4 = new LightningBoltData(LIGHTNING_BOLT_INFO, corner4, position.add(0.7F, (double) armStickyOuty, 0.7F), segCount).size(0.3F).lifespan(1).spawn(LightningBoltData.SpawnFunction.CONSECUTIVE).fade(LightningBoltData.FadeFunction.NONE);
        ...
        if (entity.headPart != null && entity.f_19797_ > 0) {
            Vec3 boundroidTop = entity.headPart.m_20318_(partialTicks).subtract(entity.m_20318_(partialTicks));
            poseStack.pushPose();
            ...
            BOUNDROID_MODEL.m_7695_(poseStack, boundroidVertexConsumer, packedLightIn, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F);
            poseStack.popPose();
            Vec3 chainOffset = new Vec3(0.0, -0.75, 0.0);
            Vec3 chainTo = entity.m_20318_(partialTicks).add(chainOffset).subtract(entity.headPart.m_20318_(partialTicks).add(0.0, -0.25, 0.0));
    ...
    private LightningRender getLightingRender(UUID uuid) {
        ...
        return (LightningRender) lightningRenderMap.get(uuid);
    }
