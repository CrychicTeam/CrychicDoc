...
public class SeaPigRenderer extends MobRenderer<SeaPigEntity, SeaPigModel> implements CustomBookEntityRenderer {
    ...
    public SeaPigRenderer(EntityRendererProvider.Context renderManagerIn) {
        ...
        this.m_115326_(new SeaPigRenderer.LayerOutside(renderManagerIn.getItemInHandRenderer()));
    }
    ...
    protected RenderType getRenderType(SeaPigEntity seaPig, boolean normal, boolean translucent, boolean outline) {
        return this.sepia ? ACRenderTypes.getBookWidget(TEXTURE_INNARDS, true) : super.m_7225_(seaPig, normal, translucent, outline);
    }
    ...
    class LayerOutside extends RenderLayer<SeaPigEntity, SeaPigModel> {
        ...
        public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, SeaPigEntity entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
            if (entitylivingbaseIn.isDigesting()) {
                ...
                matrixStackIn.translate(0.0F, 0.25F - invProgress * 0.1F, -0.5F + progress * 0.2F);
                matrixStackIn.scale(invProgress, invProgress, invProgress);
                matrixStackIn.mulPose(Axis.XN.rotationDegrees(200.0F));
                matrixStackIn.mulPose(Axis.ZN.rotationDegrees((float) (Math.sin((double) (progress * 15.0F)) * 4.0)));
            ...
            VertexConsumer ivertexbuilder1 = bufferIn.getBuffer(SeaPigRenderer.this.sepia ? ACRenderTypes.getBookWidget(SeaPigRenderer.TEXTURE, true) : RenderType.entityTranslucent(SeaPigRenderer.TEXTURE));
            ((SeaPigModel) this.m_117386_()).m_7695_(matrixStackIn, ivertexbuilder1, packedLightIn, LivingEntityRenderer.getOverlayCoords(entitylivingbaseIn, 0.0F), 1.0F, 1.0F, 1.0F, 1.0F);
        }
    }
}