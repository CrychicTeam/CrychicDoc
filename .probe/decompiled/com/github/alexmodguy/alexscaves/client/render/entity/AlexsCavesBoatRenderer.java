...
public class AlexsCavesBoatRenderer<T extends Boat & AlexsCavesBoat> extends EntityRenderer<T> {
    ...
    public void render(T entity, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn) {
        ACBoatModel model = (ACBoatModel) this.modelMap.get(entity.getACBoatType());
        poseStack.pushPose();
        ...
        if (this.isChest) {
            ...
            CHEST_MODEL.m_7695_(poseStack, bufferIn.getBuffer(RenderType.entityCutoutNoCull(CHEST_TEXTURE)), packedLightIn, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F);
            poseStack.popPose();
        }
        model.m_6973_(entity, partialTicks, 0.0F, -0.1F, 0.0F, 0.0F);
        VertexConsumer vertexconsumer = bufferIn.getBuffer(RenderType.entityCutoutNoCull(this.getTextureLocation(entity)));
        model.m_7695_(poseStack, vertexconsumer, packedLightIn, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F);
    ...
    public ResourceLocation getTextureLocation(T entity) {
        return (ResourceLocation) this.textureMap.get(entity.getACBoatType());
    }
}