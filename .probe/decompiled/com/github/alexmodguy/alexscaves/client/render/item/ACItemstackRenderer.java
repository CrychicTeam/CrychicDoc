...
public class ACItemstackRenderer extends BlockEntityWithoutLevelRenderer {
    ...
    private static final ResourceLocation SIREN_LIGHT_TEXTURE = new ResourceLocation("alexscaves", "textures/entity/siren_light.png");

    private static final ResourceLocation SIREN_LIGHT_COLOR_TEXTURE = new ResourceLocation("alexscaves", "textures/entity/siren_light_color.png");
    ...
    private static final ResourceLocation COPPER_VALVE_TEXTURE = new ResourceLocation("alexscaves", "textures/entity/copper_valve.png");
    ...
    private static final ResourceLocation BEHOLDER_TEXTURE = new ResourceLocation("alexscaves", "textures/entity/beholder.png");

    private static final ResourceLocation BEHOLDER_TEXTURE_EYE = new ResourceLocation("alexscaves", "textures/entity/beholder_eye.png");
    ...
    public void renderByItem(ItemStack itemStackIn, ItemDisplayContext transformType, PoseStack poseStack, MultiBufferSource bufferIn, int combinedLightIn, int combinedOverlayIn) {
        ...
        if (itemStackIn.is(ACItemRegistry.CAVE_MAP.get())) {
            ...
            if (transformType.firstPerson() && done) {
                ...
                ItemStack offhandHeldItem = player.m_21120_(InteractionHand.OFF_HAND);
                boolean renderingSmallMap = !offhandHeldItem.isEmpty();
                boolean offhand = offhandHeldItem.equals(itemStackIn);
                poseStack.pushPose();
                if (renderingSmallMap) {
                    poseStack.translate(left ? 0.5 : -0.5, 0.35, 0.0);
                    CaveMapRenderHelper.renderOneHandedCaveMap(poseStack, bufferIn, combinedLightIn, 0.0F, offhand ? player.getMainArm().getOpposite() : player.getMainArm(), 0.0F, itemStackIn);
                } else {
                    poseStack.translate(left ? 0.55F : -0.55F, 0.525F, 0.75F);
                    CaveMapRenderHelper.renderTwoHandedCaveMap(poseStack, bufferIn, combinedLightIn, partialTick, 0.0F, 0.0F, itemStackIn);
                }
                poseStack.popPose();
                ...
                this.renderStaticItemSpriteWithLighting(spriteItem, transformType, combinedLightIn, combinedOverlayIn, poseStack, bufferIn, level);
            }
        }
        if (itemStackIn.is(ACItemRegistry.GALENA_GAUNTLET.get())) {
            ...
            float openAmount = GalenaGauntletItem.getLerpedUseTime(itemStackIn, partialTick) / 5.0F;
            float closeAmount = 1.0F - openAmount;
            float ageInTicks = Minecraft.getInstance().player == null ? 0.0F : (float) Minecraft.getInstance().player.f_19797_ + partialTick;
            if (!left && transformType != ItemDisplayContext.GUI) {
                GALENA_GAUNTLET_RIGHT_MODEL.setupAnim(null, openAmount, 0.0F, ageInTicks, 0.0F, 0.0F);
                GALENA_GAUNTLET_RIGHT_MODEL.m_7695_(poseStack, getVertexConsumerFoil(bufferIn, RenderType.entityCutoutNoCull(GALENA_GAUNTLET_TEXTURE), GALENA_GAUNTLET_TEXTURE, itemStackIn.hasFoil()), combinedLightIn, combinedOverlayIn, 1.0F, 1.0F, 1.0F, 1.0F);
                GALENA_GAUNTLET_RIGHT_MODEL.m_7695_(poseStack, getVertexConsumer(bufferIn, ACRenderTypes.getEyesAlphaEnabled(GALENA_GAUNTLET_BLUE_TEXTURE), GALENA_GAUNTLET_BLUE_TEXTURE), combinedLightIn, combinedOverlayIn, 1.0F, 1.0F, 1.0F, openAmount);
                GALENA_GAUNTLET_RIGHT_MODEL.m_7695_(poseStack, getVertexConsumer(bufferIn, ACRenderTypes.getEyesAlphaEnabled(GALENA_GAUNTLET_RED_TEXTURE), GALENA_GAUNTLET_RED_TEXTURE), combinedLightIn, combinedOverlayIn, 1.0F, 1.0F, 1.0F, closeAmount);
            } else {
                GALENA_GAUNTLET_LEFT_MODEL.setupAnim(null, openAmount, 0.0F, ageInTicks, 0.0F, 0.0F);
                GALENA_GAUNTLET_LEFT_MODEL.m_7695_(poseStack, getVertexConsumerFoil(bufferIn, RenderType.entityCutoutNoCull(GALENA_GAUNTLET_TEXTURE), GALENA_GAUNTLET_TEXTURE, itemStackIn.hasFoil()), combinedLightIn, combinedOverlayIn, 1.0F, 1.0F, 1.0F, 1.0F);
                GALENA_GAUNTLET_LEFT_MODEL.m_7695_(poseStack, getVertexConsumer(bufferIn, ACRenderTypes.getEyesAlphaEnabled(GALENA_GAUNTLET_BLUE_TEXTURE), GALENA_GAUNTLET_BLUE_TEXTURE), combinedLightIn, combinedOverlayIn, 1.0F, 1.0F, 1.0F, openAmount);
                GALENA_GAUNTLET_LEFT_MODEL.m_7695_(poseStack, getVertexConsumer(bufferIn, ACRenderTypes.getEyesAlphaEnabled(GALENA_GAUNTLET_RED_TEXTURE), GALENA_GAUNTLET_RED_TEXTURE), combinedLightIn, combinedOverlayIn, 1.0F, 1.0F, 1.0F, closeAmount);
            }
            poseStack.popPose();
        }
        if (itemStackIn.is(ACItemRegistry.RESISTOR_SHIELD.get())) {
            ...
            float switchProgress = ResistorShieldItem.getLerpedSwitchTime(itemStackIn, partialTick) / 5.0F;
            float leftOffset = left ? -1.0F : 1.0F;
            if (transformType.firstPerson()) {
                poseStack.translate(useProgressTurn * 0.2F * leftOffset, useProgressUp, 0.0F);
                poseStack.mulPose(Axis.XP.rotationDegrees(useProgressTurn * -10.0F));
            } else if (heldIn3d) {
                poseStack.translate(useProgressTurn * 0.4F * leftOffset, useProgress * -0.1F, useProgressTurn * -0.2F);
                poseStack.mulPose(Axis.ZP.rotationDegrees(useProgressTurn * 10.0F * leftOffset));
                poseStack.mulPose(Axis.YP.rotationDegrees(useProgressTurn * 80.0F * leftOffset));
            }
            poseStack.mulPose(Axis.XP.rotationDegrees(-180.0F));
            RESISTOR_SHIELD_MODEL.setupAnim(null, useProgress, switchProgress, 0.0F, 0.0F, 0.0F);
            RESISTOR_SHIELD_MODEL.m_7695_(poseStack, getVertexConsumerFoil(bufferIn, RenderType.entityCutoutNoCull(RESISTOR_SHIELD_TEXTURE), RESISTOR_SHIELD_TEXTURE, itemStackIn.hasFoil()), combinedLightIn, combinedOverlayIn, 1.0F, 1.0F, 1.0F, 1.0F);
            RESISTOR_SHIELD_MODEL.m_7695_(poseStack, getVertexConsumer(bufferIn, ACRenderTypes.getEyesAlphaEnabled(RESISTOR_SHIELD_RED_TEXTURE), RESISTOR_SHIELD_RED_TEXTURE), combinedLightIn, combinedOverlayIn, 1.0F, 1.0F, 1.0F, switchProgress);
            RESISTOR_SHIELD_MODEL.m_7695_(poseStack, getVertexConsumer(bufferIn, ACRenderTypes.getEyesAlphaEnabled(RESISTOR_SHIELD_BLUE_TEXTURE), RESISTOR_SHIELD_BLUE_TEXTURE), combinedLightIn, combinedOverlayIn, 1.0F, 1.0F, 1.0F, 1.0F - switchProgress);
            poseStack.popPose();
        }
        if (itemStackIn.is(ACItemRegistry.PRIMITIVE_CLUB.get())) {
            poseStack.translate(0.5F, 0.5F, 0.5F);
            ItemStack spriteItemx = new ItemStack(ACItemRegistry.PRIMITIVE_CLUB_SPRITE.get());
            spriteItemx.setTag(itemStackIn.getTag());
            if (heldIn3d) {
                poseStack.pushPose();
                poseStack.mulPose(Axis.XP.rotationDegrees(-180.0F));
                poseStack.translate(0.0F, -1.15F, -0.1F);
                if (transformType.firstPerson()) {
                ...
                PRIMITIVE_CLUB_MODEL.m_7695_(poseStack, vertexconsumer, combinedLightIn, combinedOverlayIn, 1.0F, 1.0F, 1.0F, 1.0F);
                poseStack.popPose();
            } else {
                this.renderStaticItemSprite(spriteItemx, transformType, combinedLightIn, combinedOverlayIn, poseStack, bufferIn, level);
            }
        }
        if (itemStackIn.is(ACItemRegistry.LIMESTONE_SPEAR.get())) {
            poseStack.translate(0.5F, 0.5F, 0.5F);
            ItemStack spriteItemx = new ItemStack(ACItemRegistry.LIMESTONE_SPEAR_SPRITE.get());
            spriteItemx.setTag(itemStackIn.getTag());
            if (heldIn3d) {
                poseStack.pushPose();
                poseStack.mulPose(Axis.XP.rotationDegrees(-180.0F));
                poseStack.translate(0.0F, -0.85F, -0.1F);
                if (transformType.firstPerson()) {
                ...
                LIMESTONE_SPEAR_MODEL.m_7695_(poseStack, vertexconsumer, combinedLightIn, combinedOverlayIn, 1.0F, 1.0F, 1.0F, 1.0F);
                poseStack.popPose();
            } else {
                this.renderStaticItemSprite(spriteItemx, transformType, combinedLightIn, combinedOverlayIn, poseStack, bufferIn, level);
            }
        ...
        if (itemStackIn.is(ACItemRegistry.EXTINCTION_SPEAR.get())) {
            poseStack.translate(0.5F, 0.5F, 0.5F);
            ItemStack spriteItemx = new ItemStack(ACItemRegistry.EXTINCTION_SPEAR_SPRITE.get());
            spriteItemx.setTag(itemStackIn.getTag());
            if (heldIn3d) {
                poseStack.pushPose();
                poseStack.mulPose(Axis.XP.rotationDegrees(-180.0F));
                ...
                EXTINCTION_SPEAR_MODEL.m_7695_(poseStack, vertexconsumer1, 240, combinedOverlayIn, 1.0F, 1.0F, 1.0F, 1.0F);
                ...
                EXTINCTION_SPEAR_MODEL.m_7695_(poseStack, vertexconsumer2, 240, combinedOverlayIn, 1.0F, 1.0F, 1.0F, 1.0F);
                poseStack.popPose();
            } else {
                this.renderStaticItemSprite(spriteItemx, transformType, combinedLightIn, combinedOverlayIn, poseStack, bufferIn, level);
            }
        ...
        if (itemStackIn.is(ACBlockRegistry.SIREN_LIGHT.get().asItem())) {
            ...
            SIREN_LIGHT_MODEL.m_7695_(poseStack, getVertexConsumer(bufferIn, RenderType.entityCutoutNoCull(SIREN_LIGHT_TEXTURE), SIREN_LIGHT_TEXTURE), combinedLightIn, combinedOverlayIn, 1.0F, 1.0F, 1.0F, 1.0F);
            SIREN_LIGHT_MODEL.m_7695_(poseStack, getVertexConsumer(bufferIn, RenderType.entityTranslucent(SIREN_LIGHT_COLOR_TEXTURE), SIREN_LIGHT_COLOR_TEXTURE), combinedLightIn, combinedOverlayIn, 0.0F, 1.0F, 0.0F, 1.0F);
            poseStack.popPose();
        }
        if (itemStackIn.is(ACItemRegistry.RAYGUN.get())) {
            float ageInTicks = Minecraft.getInstance().player == null ? 0.0F : (float) Minecraft.getInstance().player.f_19797_ + partialTick;
            float useAmount = RaygunItem.getLerpedUseTime(itemStackIn, partialTick) / 5.0F;
            float pulseAlpha = useAmount * (0.25F + 0.25F * (float) (1.0 + Math.sin((double) (ageInTicks * 0.8F))));
            poseStack.pushPose();
            poseStack.translate(0.5F, 1.5F, 0.5F);
            poseStack.mulPose(Axis.XP.rotationDegrees(-180.0F));
            poseStack.mulPose(Axis.YP.rotationDegrees(180.0F));
            ...
            RAYGUN_MODEL.setupAnim(null, useAmount, ageInTicks, 0.0F, 0.0F, 0.0F);
            ...
            RAYGUN_MODEL.m_7695_(poseStack, getVertexConsumerFoil(bufferIn, RenderType.entityCutoutNoCull(texture), texture, itemStackIn.hasFoil()), combinedLightIn, combinedOverlayIn, 1.0F, 1.0F, 1.0F, 1.0F);
            RAYGUN_MODEL.m_7695_(poseStack, getVertexConsumer(bufferIn, ACRenderTypes.getEyesAlphaEnabled(textureActive), textureActive), combinedLightIn, combinedOverlayIn, 1.0F, 1.0F, 1.0F, pulseAlpha);
            poseStack.popPose();
            poseStack.popPose();
        }
        if (itemStackIn.is(ACItemRegistry.SEA_STAFF.get())) {
            poseStack.translate(0.5F, 0.5F, 0.5F);
            ItemStack spriteItemx = new ItemStack(ACItemRegistry.SEA_STAFF_SPRITE.get());
            spriteItemx.setTag(itemStackIn.getTag());
            if (heldIn3d) {
                poseStack.pushPose();
                poseStack.mulPose(Axis.XP.rotationDegrees(-180.0F));
                ...
                SEA_STAFF_MODEL.m_7695_(poseStack, vertexconsumer, combinedLightIn, combinedOverlayIn, 1.0F, 1.0F, 1.0F, 1.0F);
                poseStack.popPose();
            } else {
                this.renderStaticItemSprite(spriteItemx, transformType, combinedLightIn, combinedOverlayIn, poseStack, bufferIn, level);
            }
        ...
        if (itemStackIn.is(ACItemRegistry.ORTHOLANCE.get())) {
            poseStack.translate(0.5F, 0.5F, 0.5F);
            ItemStack spriteItemx = new ItemStack(ACItemRegistry.ORTHOLANCE_SPRITE.get());
            spriteItemx.setTag(itemStackIn.getTag());
            if (heldIn3d) {
                poseStack.pushPose();
                poseStack.mulPose(Axis.XP.rotationDegrees(-180.0F));
                ...
                ORTHOLANCE_MODEL.m_7695_(poseStack, vertexconsumer, combinedLightIn, combinedOverlayIn, 1.0F, 1.0F, 1.0F, 1.0F);
                poseStack.popPose();
            } else {
                this.renderStaticItemSprite(spriteItemx, transformType, combinedLightIn, combinedOverlayIn, poseStack, bufferIn, level);
            }
        ...
        if (itemStackIn.is(ACBlockRegistry.COPPER_VALVE.get().asItem())) {
            ...
            COPPER_VALVE_MODEL.m_7695_(poseStack, getVertexConsumer(bufferIn, RenderType.entityCutoutNoCull(COPPER_VALVE_TEXTURE), COPPER_VALVE_TEXTURE), combinedLightIn, combinedOverlayIn, 1.0F, 1.0F, 1.0F, 1.0F);
            poseStack.popPose();
        }
        if (itemStackIn.is(ACBlockRegistry.BEHOLDER.get().asItem())) {
            ...
            poseStack.translate(0.5F, 1.5F, 0.5F);
            poseStack.mulPose(Axis.XP.rotationDegrees(-180.0F));
            BEHOLDER_MODEL.setupAnim(null, 0.0F, 45.0F, ageInTicks, 0.0F, 0.0F);
            BEHOLDER_MODEL.m_7695_(poseStack, getVertexConsumer(bufferIn, RenderType.entityCutoutNoCull(BEHOLDER_TEXTURE), BEHOLDER_TEXTURE), combinedLightIn, combinedOverlayIn, 1.0F, 1.0F, 1.0F, 1.0F);
            BEHOLDER_MODEL.m_7695_(poseStack, getVertexConsumer(bufferIn, RenderType.eyes(BEHOLDER_TEXTURE_EYE), BEHOLDER_TEXTURE_EYE), combinedLightIn, combinedOverlayIn, 1.0F, 1.0F, 1.0F, 1.0F);
            poseStack.popPose();
        }
        if (itemStackIn.is(ACItemRegistry.DREADBOW.get())) {
            ...
            poseStack.translate(0.5F, 0.5F, 0.5F);
            ItemStack spriteItemx = new ItemStack(pullAmount >= 0.8F ? ACItemRegistry.DREADBOW_PULLING_2_SPRITE.get() : (pullAmount >= 0.5F ? ACItemRegistry.DREADBOW_PULLING_1_SPRITE.get() : (pullAmount > 0.0F ? ACItemRegistry.DREADBOW_PULLING_0_SPRITE.get() : ACItemRegistry.DREADBOW_SPRITE.get())));
            spriteItemx.setTag(itemStackIn.getTag());
            if (heldIn3d) {
                poseStack.pushPose();
                if (transformType.firstPerson()) {
                    ...
                    poseStack.mulPose(Axis.YP.rotationDegrees(left ? 7.0F : -7.0F));
                }
                EntityType type = DreadbowItem.getTypeOfArrow(itemStackIn);
                DREADBOW_MODEL.setupAnim(null, pullAmount, ageInTicks, 0.0F, 0.0F, 0.0F);
                if (Minecraft.getInstance().player != null && Minecraft.getInstance().player.isUsingItem() && pullAmount > 0.0F && type != null && Minecraft.getInstance().level != null) {
                    if (this.renderedDreadbowArrow == null || this.renderedDreadbowArrow.getType() != type) {
                        this.renderedDreadbowArrow = type.create(Minecraft.getInstance().level);
                    }
                    ...
                    manager.render(this.renderedDreadbowArrow, 0.0, 0.0, 0.0, 0.0F, 1.0F, poseStack, bufferIn, DreadbowItem.isConvertibleArrow(this.renderedDreadbowArrow) ? Math.round(240.0F * (1.0F - pullAmount)) : combinedLightIn);
                    poseStack.popPose();
                }
                ...
                DREADBOW_MODEL.m_7695_(poseStack, vertexconsumer, combinedLightIn, combinedOverlayIn, 1.0F, 1.0F, 1.0F, 1.0F);
                ...
                DREADBOW_MODEL.m_7695_(poseStack, getVertexConsumer(bufferIn, RenderType.eyes(eyeTexture), eyeTexture), combinedLightIn, combinedOverlayIn, 1.0F, 1.0F, 1.0F, 1.0F);
                poseStack.popPose();
            } else {
                this.renderStaticItemSprite(spriteItemx, transformType, combinedLightIn, combinedOverlayIn, poseStack, bufferIn, level);
            }
