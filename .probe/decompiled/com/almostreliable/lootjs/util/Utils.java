...
public class Utils {
    ...
    public static StatePropertiesPredicate.Builder createProperties(Block block, Map<String, String> propertyMap) {
        ...
        if (propertyMap.isEmpty()) {
            ...
            for (Property<?> property : block.defaultBlockState().m_61147_()) {
    ...
    public static String formatEntity(Entity entity) {
        return String.format("Type=%s, Id=%s, Dim=%s, x=%.2f, y=%.2f, z=%.2f", quote(LootJSPlatform.INSTANCE.getRegistryName(entity.getType())), entity.getId(), quote(entity.level().dimension().location()), entity.getX(), entity.getY(), entity.getZ());
    }
    ...
    public static String quote(String prefix, Collection<?> objects) {
        return prefix + "[" + (String) objects.stream().map(Object::toString).map(Utils::quote).collect(Collectors.joining(",")) + "]";
    }
}