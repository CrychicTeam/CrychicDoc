...
public class ComponentHeatwave extends PotionEffectComponent {

    public ComponentHeatwave(ResourceLocation guiIcon) {
        super(guiIcon, EffectInit.HEATWAVE, new AttributeValuePair(Attribute.MAGNITUDE, 0.0F, 0.0F, 5.0F, 1.0F, 20.0F), new AttributeValuePair(Attribute.DURATION, 10.0F, 5.0F, 60.0F, 5.0F, 20.0F), new AttributeValuePair(Attribute.RADIUS, 3.0F, 1.0F, 15.0F, 1.0F, 5.0F), new AttributeValuePair(Attribute.DAMAGE, 5.0F, 1.0F, 20.0F, 1.0F, 5.0F));
    }

    @Override
    public ComponentApplicationResult ApplyEffect(SpellSource source, SpellTarget target, IModifiedSpellPart<SpellEffect> modificationData, SpellContext context) {
        if (target.isLivingEntity() && ((LivingEntity) target.getEntity()).hasEffect(EffectInit.CHILL.get())) {
            ((LivingEntity) target.getEntity()).removeEffect(EffectInit.CHILL.get());
            context.getServerLevel().m_6249_(source.getCaster(), target.getEntity().getBoundingBox().inflate((double) modificationData.getValue(Attribute.RADIUS)), e -> e instanceof LivingEntity && !this.isTargetFriendlyToCaster(source, e)).stream().map(e -> (LivingEntity) e).forEach(e -> e.hurt(e.m_269291_().inFire(), modificationData.getValue(Attribute.DAMAGE)));
