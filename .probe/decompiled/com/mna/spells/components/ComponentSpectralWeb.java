...
public class ComponentSpectralWeb extends SpellEffect {

    public ComponentSpectralWeb(ResourceLocation guiIcon) {
        super(guiIcon);
    }
    ...
    public ComponentApplicationResult ApplyEffect(SpellSource source, SpellTarget target, IModifiedSpellPart<SpellEffect> modificationData, SpellContext context) {
        ...
        if (against.m_60734_() == BlockInit.CHALK_RUNE.get() && (Boolean) against.m_61143_(ChalkRuneBlock.METAL) && source.isPlayerCaster()) {
            ...
            if (!ForgeEventFactory.onBlockPlace(source.getPlayer(), BlockSnapshot.create(world.m_46472_(), world, targetBlock), targetBlockFace)) {
                BlockState placementState = (BlockState) BlockInit.SPECTRAL_WEB.get().m_49966_().m_61124_(SpectralCraftingTableBlock.PERMANENT, true);
                world.m_46597_(targetBlock, placementState);
                placed = true;
            }
            return ComponentApplicationResult.fromBoolean(placed);
        } else {
            return ComponentApplicationResult.fromBoolean(this.tryPlaceBlock(source.getPlayer(), world, BlockInit.SPECTRAL_WEB.get(), targetBlock, targetBlockFace, true, null).success);
        }
    }
