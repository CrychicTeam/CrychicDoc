...
public class GenericRadialMenu {
    ...
    private GenericRadialMenu.State state = GenericRadialMenu.State.INITIALIZING;
    ...
    public float radiusOffset = 30.0F;
    ...
    public boolean isClosed() {
        return this.state == GenericRadialMenu.State.CLOSED;
    }

    public boolean isReady() {
        return this.state == GenericRadialMenu.State.NORMAL;
    }
    ...
    public void close() {
        this.state = GenericRadialMenu.State.CLOSING;
    ...
    public void tick() {
        if (this.state == GenericRadialMenu.State.INITIALIZING) {
            this.startAnimation = (double) this.minecraft.level.m_46467_() + (double) this.minecraft.getFrameTime();
            this.state = GenericRadialMenu.State.OPENING;
    ...
    public void draw(GuiGraphics pGuiGraphics, float partialTicks, int mouseX, int mouseY) {
        ...
        if (!this.isClosed()) {
            ...
            boolean animated = this.state == GenericRadialMenu.State.OPENING || this.state == GenericRadialMenu.State.CLOSING;
            this.radiusIn = animated ? Math.max(0.1F, this.radiusOffset * this.animProgress) : this.radiusOffset;
        ...
        switch(this.state) {
            case OPENING:
                ...
                if ((double) openAnimation >= 1.0 || this.getVisibleItemCount() == 0) {
                    openAnimation = 1.0F;
                    this.state = GenericRadialMenu.State.NORMAL;
                }
                break;
            case CLOSING:
                ...
                if (openAnimation <= 0.0F || this.getVisibleItemCount() == 0) {
                    openAnimation = 0.0F;
                    this.state = GenericRadialMenu.State.CLOSED;
                }
        }
