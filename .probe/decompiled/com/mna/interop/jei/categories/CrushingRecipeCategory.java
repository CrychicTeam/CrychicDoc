...
public class CrushingRecipeCategory implements IRecipeCategory<CrushingRecipe> {
    ...
    public CrushingRecipeCategory(IGuiHelper guiHelper) {
        this.background = guiHelper.drawableBuilder(GuiTextures.Jei.CRUSHING, 0, 0, this.xSize, this.ySize).setTextureSize(152, 177).build();
        this.localizedName = I18n.get("gui.mna.jei.crushing");
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, CrushingRecipe recipe, IFocusGroup focuses) {
        builder.addSlot(RecipeIngredientRole.INPUT, 22, 51).addIngredients(Ingredient.of(MATags.smartLookupItem(recipe.getInputResource()).stream().map(i -> new ItemStack(i))));
        builder.addSlot(RecipeIngredientRole.OUTPUT, 114, 51).addIngredient(VanillaTypes.ITEM_STACK, recipe.getResultItem());
    }

    public void draw(CrushingRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics pGuiGraphics, double mouseX, double mouseY) {
        ...
        if (mc.font != null) {
            ...
            GuiRenderUtils.renderByproducts(pGuiGraphics, this.xSize / 2 - 24, 32, recipe, false);
        }
    }
}