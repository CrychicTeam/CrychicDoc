...
public class MMCQ {
    ...
    private static final Comparator<MMCQ.VBox> COMPARATOR_PRODUCT = new Comparator<MMCQ.VBox>() {
        ...
        public int compare(MMCQ.VBox a, MMCQ.VBox b) {
            ...
            return aCount == bCount ? aVolume - bVolume : Long.compare((long) aCount * (long) aVolume, (long) bCount * (long) bVolume);
        }
    };
    ...
    public static class CMap {
        ...
        public int[] nearest(int[] color) {
            ...
            for (int i = 0; i < numVBoxes; i++) {
                ...
                double d2 = Math.sqrt(Math.pow((double) (color[0] - vbColor[0]), 2.0) + Math.pow((double) (color[1] - vbColor[1]), 2.0) + Math.pow((double) (color[2] - vbColor[2]), 2.0));
                if (d2 < d1) {
                    d1 = d2;
                    pColor = vbColor;
                }
