...
public class TooltipsUtils {
    ...
    private static int indexOfInternal(List<Component> lines, Predicate<String> comparator) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            } else if (component.getContents() instanceof LiteralContents literalContents && comparator.test(literalContents.text())) {
                return i;
            }
        }
        return -1;
    ...
    public static List<MutableComponent> formatActiveSpellTooltip(ItemStack stack, SpellData spellData, CastSource castSource, @Nonnull LocalPlayer player) {
        ...
        MutableComponent manaCost = getManaCostComponent(spell.getCastType(), spell.getManaCost(spellLevel)).withStyle(ChatFormatting.BLUE);
        MutableComponent cooldownTime = Component.translatable("tooltip.irons_spellbooks.cooldown_length_seconds", Utils.timeFromTicks((float) MagicManager.getEffectiveSpellCooldown(spell, player, castSource), 2)).withStyle(ChatFormatting.BLUE);
        List<MutableComponent> lines = new ArrayList();
        lines.add(Component.empty());
        lines.add(title);
        uniqueInfo.forEach(line -> lines.add(Component.literal(" ").append(line.withStyle(getStyleFor(player, spell)))));
        if (spell.getCastType() != CastType.INSTANT) {
            lines.add(Component.literal(" ").append(getCastTimeComponent(spell.getCastType(), Utils.timeFromTicks((float) spell.getEffectiveCastTime(spellLevel, player), 2)).withStyle(ChatFormatting.BLUE)));
        }
        if (castSource != CastSource.SWORD || ServerConfigs.SWORDS_CONSUME_MANA.get()) {
            lines.add(manaCost);
        }
        if (castSource != CastSource.SWORD || ServerConfigs.SWORDS_CD_MULTIPLIER.get().floatValue() > 0.0F) {
    ...
    public static List<Component> formatScrollTooltip(ItemStack stack, @Nonnull LocalPlayer player) {
        if (stack.getItem() instanceof Scroll) {
            ...
            if (spellList.isEmpty()) {
                ...
                MutableComponent title = Component.translatable("tooltip.irons_spellbooks.level", levelText).append(" ").append(Component.translatable("tooltip.irons_spellbooks.rarity", spell.getRarity(spellData.getLevel()).getDisplayName()).withStyle(spell.getRarity(spellData.getLevel()).getDisplayName().getStyle())).withStyle(ChatFormatting.GRAY);
                List<MutableComponent> uniqueInfo = spell.getUniqueInfo(spellLevel, player);
                ...
                MutableComponent manaCost = getManaCostComponent(spell.getCastType(), spell.getManaCost(spellLevel)).withStyle(ChatFormatting.BLUE);
                MutableComponent cooldownTime = Component.translatable("tooltip.irons_spellbooks.cooldown_length_seconds", Utils.timeFromTicks((float) MagicManager.getEffectiveSpellCooldown(spell, player, CastSource.SCROLL), 2)).withStyle(ChatFormatting.BLUE);
    ...
    public static MutableComponent getLevelComponenet(SpellData spellData, LivingEntity caster) {
        ...
        if (diff > 0) {
            ...
            return diff < 0 ? Component.translatable("tooltip.irons_spellbooks.level_minus", levelTotal, diff) : Component.literal(String.valueOf(levelTotal));
        }
    }
    ...
    public static MutableComponent getCastTimeComponent(CastType type, String castTime) {
        return switch(type) {
            case CONTINUOUS ->
                Component.translatable("tooltip.irons_spellbooks.cast_continuous", castTime);
            case LONG ->
                Component.translatable("tooltip.irons_spellbooks.cast_long", castTime);
            default ->
    ...
    public static MutableComponent getManaCostComponent(CastType castType, int manaCost) {
        return castType == CastType.CONTINUOUS ? Component.translatable("tooltip.irons_spellbooks.mana_cost_per_second", manaCost * 2) : Component.translatable("tooltip.irons_spellbooks.mana_cost", manaCost);
    }

    public static MutableComponent getTitleComponent(SpellData spellData, @NotNull LocalPlayer player) {
        MutableComponent levelText = getLevelComponenet(spellData, player);
        AbstractSpell spell = spellData.getSpell();
        return Component.translatable("tooltip.irons_spellbooks.selected_spell", spell.getDisplayName(player), levelText).withStyle(spell.getSchoolType().getDisplayName().getStyle());
    }
