...
public class ClientSetup {
    ...
    private static void addLayerToPlayerSkin(EntityRenderersEvent.AddLayers event, String skinName) {
        ...
        if (render instanceof LivingEntityRenderer livingRenderer) {
            ...
            livingRenderer.addLayer(new EnergySwirlLayer.Vanilla(livingRenderer, EnergySwirlLayer.EVASION_TEXTURE, 2L));
            livingRenderer.addLayer(new EnergySwirlLayer.Vanilla(livingRenderer, EnergySwirlLayer.CHARGE_TEXTURE, 64L));
        ...
        for (Entry<EntityType<?>, EntityRenderer<?>> entry : Minecraft.getInstance().getEntityRenderDispatcher().renderers.entrySet()) {
            EntityRenderer<?> livingEntityRendererTest = (EntityRenderer<?>) entry.getValue();
    ...
    public static void clientSetup(FMLClientSetupEvent e) {
        e.enqueueWork(() -> {
            ItemProperties.register(ItemRegistry.WAYWARD_COMPASS.get(), new ResourceLocation("angle"), new CompassItemPropertyFunction((level, itemStack, entity) -> WaywardCompass.getCatacombsLocation(entity, itemStack.getOrCreateTag())));
            ...
            ItemProperties.register(ItemRegistry.AUTOLOADER_CROSSBOW.get(), new ResourceLocation("pull"), (itemStack, clientLevel, livingEntity, i) -> CrossbowItem.isCharged(itemStack) ? 0.0F : (float) AutoloaderCrossbow.getLoadingTicks(itemStack) / (float) AutoloaderCrossbow.getChargeDuration(itemStack));
            ...
            ItemRegistry.getIronsItems().stream().filter(item -> item.get() instanceof SpellBook).forEach(item -> CuriosRendererRegistry.register((Item) item.get(), SpellBookCurioRenderer::new));
        });
        PlayerAnimationFactory.ANIMATION_DATA_FACTORY.registerFactory(SpellAnimations.ANIMATION_RESOURCE, 42, player -> {
            ...
            animation.addModifierLast(new AdjustmentModifier(partName -> {
                switch(partName) {
                    case "rightArm":
                    case "leftArm":
                        return Optional.of(new AdjustmentModifier.PartModifier(new Vec3f(player.m_146909_() * (float) (Math.PI / 180.0), (float) (Math.PI / 180.0) * (player.f_20885_ - player.f_20883_), 0.0F), Vec3f.ZERO));
                    default:
    ...
    public static void registerSpecialModels(ModelEvent.RegisterAdditional event) {
        ...
        for (SchoolType schoolType : ((IForgeRegistry) SchoolRegistry.REGISTRY.get()).getValues()) {
