...
public class InkItem extends Item {
    ...
    public static InkItem getInkForRarity(SpellRarity rarity) {
        return switch(rarity) {
            case COMMON ->
                (InkItem) ItemRegistry.INK_COMMON.get();
            case UNCOMMON ->
                (InkItem) ItemRegistry.INK_UNCOMMON.get();
            case RARE ->
                (InkItem) ItemRegistry.INK_RARE.get();
            case EPIC ->
                (InkItem) ItemRegistry.INK_EPIC.get();
            case LEGENDARY ->
                (InkItem) ItemRegistry.INK_LEGENDARY.get();
            default ->
                (InkItem) ItemRegistry.INK_COMMON.get();
        };
    }
    ...
    public void appendHoverText(ItemStack pStack, @Nullable Level pLevel, List<Component> lines, TooltipFlag pIsAdvanced) {
        ...
        lines.add(Component.translatable("tooltip.irons_spellbooks.ink_tooltip", this.rarity.getDisplayName()).withStyle(ChatFormatting.GRAY));
    }
}