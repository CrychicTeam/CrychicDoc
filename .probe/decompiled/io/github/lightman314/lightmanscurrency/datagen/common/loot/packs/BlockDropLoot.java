...
public class BlockDropLoot extends LootTableProviderTemplate {
    ...
    protected void simpleBlock(@Nonnull RegistryObject<? extends Block> block) {
        this.simpleBlock(block.get());
    }

    protected void tallBlock(@Nonnull RegistryObject<? extends Block> block) {
        this.tallBlock(block.get());
    }

    protected void tallBlock(@Nonnull Block block) {
        this.define(this.getBlockTable(block), LootTable.lootTable().withPool(LootPool.lootPool().add(LootItem.lootTableItem(block).m_79080_(LootItemBlockStatePropertyCondition.hasBlockStateProperties(block).setProperties(StatePropertiesPredicate.Builder.properties().hasProperty(ITallBlock.ISBOTTOM, true)))).when(ExplosionCondition.survivesExplosion())));
    }

    protected void coinPilesAndBlocks(@Nonnull RegistryObject<? extends ItemLike> coin, @Nonnull RegistryObject<? extends Block> pile, @Nonnull RegistryObject<? extends Block> block) {
        this.coinPilesAndBlocks(coin.get().asItem(), pile.get(), block.get());
    }

    protected void coinPilesAndBlocks(@Nonnull Item coin, @Nonnull Block pile, @Nonnull Block block) {
        this.define(this.getBlockTable(pile), LootTable.lootTable().withPool(LootPool.lootPool().add(AlternativesEntry.alternatives(LootItem.lootTableItem(pile).m_79080_(MatchTool.toolMatches(ItemPredicate.Builder.item().hasEnchantment(new EnchantmentPredicate(Enchantments.SILK_TOUCH, MinMaxBounds.Ints.atLeast(1))))), LootItem.lootTableItem(coin).apply(SetItemCountFunction.setCount(ConstantValue.exactly(9.0F)))))));
        this.define(this.getBlockTable(block), LootTable.lootTable().withPool(LootPool.lootPool().add(AlternativesEntry.alternatives(LootItem.lootTableItem(block).m_79080_(MatchTool.toolMatches(ItemPredicate.Builder.item().hasEnchantment(new EnchantmentPredicate(Enchantments.SILK_TOUCH, MinMaxBounds.Ints.atLeast(1))))), LootItem.lootTableItem(pile).apply(SetItemCountFunction.setCount(ConstantValue.exactly(4.0F)))))));
    }
}