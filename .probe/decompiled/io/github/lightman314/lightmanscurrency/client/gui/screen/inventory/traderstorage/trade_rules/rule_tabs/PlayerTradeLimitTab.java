...
public class PlayerTradeLimitTab extends TradeRuleSubTab<PlayerTradeLimit> {
    ...
    public void initialize(ScreenArea screenArea, boolean firstOpen) {
        ...
        this.buttonClearMemory = this.addChild(new EasyTextButton(screenArea.pos.offset(10, 50), screenArea.width - 20, 20, LCText.BUTTON_CLEAR_MEMORY.get(), this::PressClearMemoryButton).withAddons(EasyAddonHelper.tooltip(LCText.TOOLTIP_TRADE_LIMIT_CLEAR_MEMORY)));
    ...
    public void renderBG(@Nonnull EasyGuiGraphics gui) {
        PlayerTradeLimit rule = this.getRule();
        if (rule != null) {
            gui.drawString(LCText.GUI_TRADE_LIMIT_INFO.get(rule.getLimit()), 10, 9, 16777215);
        }
        Component text = this.getRule().getTimeLimit() > 0L ? LCText.GUI_PLAYER_TRADE_LIMIT_DURATION.get(new TimeUtil.TimeData(this.getRule().getTimeLimit()).getShortString()) : LCText.GUI_PLAYER_TRADE_LIMIT_NO_DURATION.get();
        TextRenderUtil.drawCenteredText(gui, text, this.screen.getXSize() / 2, 75, 16777215);
    }
