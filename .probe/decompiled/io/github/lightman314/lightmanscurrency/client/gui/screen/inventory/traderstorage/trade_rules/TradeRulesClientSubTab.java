...
public abstract class TradeRulesClientSubTab extends EasyTab {
    ...
    public List<TradeRule> getTradeRules() {
        ...
        return (List<TradeRule>) (host != null ? host.getRules() : new ArrayList());
    }

    @Nonnull
    public List<TradeRule> getFilteredRules() {
        return this.filterRules(this.getTradeRules());
    }

    protected final List<TradeRule> filterRules(@Nonnull List<TradeRule> rules) {
        return (List<TradeRule>) rules.stream().filter(TradeRule::canActivate).collect(Collectors.toList());
    }
