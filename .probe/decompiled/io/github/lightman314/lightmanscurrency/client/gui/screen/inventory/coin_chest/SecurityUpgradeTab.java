...
public class SecurityUpgradeTab extends CoinChestTab.Upgrade {
    ...
    public boolean isVisible() {
        ...
        return data != null && data.upgrade instanceof CoinChestSecurityUpgrade upgrade ? upgrade.isAdmin(this.screen.be, data, ((CoinChestMenu) this.screen.m_6262_()).player) : false;
    }
    ...
    public void initialize(ScreenArea screenArea, boolean firstOpen) {
        ...
        this.setPlayerButton = this.addChild(new EasyTextButton(screenArea.pos.offset(10, 60), screenArea.width - 20, 20, LCText.BUTTON_OWNER_SET_PLAYER.get(), this::SetPlayerOwner).withAddons(EasyAddonHelper.tooltip(LCText.TOOLTIP_WARNING_CANT_BE_UNDONE.getWithStyle(ChatFormatting.YELLOW, ChatFormatting.BOLD))));
    ...
    protected OwnerData getCurrentOwner() {
        CoinChestUpgradeData data = this.getUpgradeData();
        return data != null && data.upgrade instanceof CoinChestSecurityUpgrade upgrade ? upgrade.parseOwnerData(this.menu.be, data) : null;
    }

    private Component getOwnerName() {
        OwnerData data = this.getCurrentOwner();
        return data != null ? data.getName() : LCText.GUI_OWNER_NULL.get();
    }

    @Override
    public void renderBG(@Nonnull EasyGuiGraphics gui) {
        gui.drawString(TextRenderUtil.fitString(LCText.GUI_OWNER_CURRENT.get(this.getOwnerName()), this.screen.getXSize() - 20), 8, 6, 4210752);
    }
