...
public class TraderSelectClientTab extends TraderInterfaceClientTab<TraderSelectTab> implements IScrollable {
    ...
    public MutableComponent getTooltip() {
        return LCText.TOOLTIP_INTERFACE_TRADER_SELECT.get();
    }
    ...
    private List<TraderData> filterTraders(List<TraderData> allTraders) {
        ...
        if (be == null) {
            ...
            for (TraderData trader : allTraders) {
                if (be.validTraderType(trader) && (interaction.trades && trader.hasValidTrade() || interaction.requiresPermissions && be.hasTraderPermissions(trader))) {
                    traders.add(trader);
                }
            }
            return traders;
    ...
    private void SelectTrader(EasyButton button) {
        ...
        if (index >= 0 && index < this.filteredTraderList.size()) {
            long traderID = ((TraderData) this.filteredTraderList.get(index)).getID();
    ...
    private int scrollOf(TraderData trader) {
        if (this.filteredTraderList != null) {
            ...
            return index >= 0 ? Math.min(index, this.getMaxScroll()) : this.scroll;
        } else {
            return this.scroll;
        }
    }
