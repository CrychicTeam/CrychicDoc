...
public class EjectionMenuButton extends InventoryButton {
    ...
    public static final ResourceLocation GUI_TEXTURE = new ResourceLocation("lightmanscurrency", "textures/gui/misc.png");

    private static EjectionMenuButton lastButton = null;

    public static final int SIZE = 9;

    public static final Sprite SPRITE = Sprite.SimpleSprite(GUI_TEXTURE, 0, 0, 9, 9);

    public static final ScreenPosition OFFSET = ScreenPosition.of(-10, 0);

    private Player getPlayer() {
        return this.inventoryScreen.getMinecraft().player;
    }
    ...
    protected ScreenPosition getPositionOffset(boolean isCreativeScreen) {
        return isCreativeScreen ? LCConfig.CLIENT.notificationAndTeamButtonCreativePosition.get().offset(OFFSET) : LCConfig.CLIENT.notificationAndTeamButtonPosition.get().offset(OFFSET);
    }

    @Override
    protected boolean canShow() {
        return !EjectionSaveData.GetValidEjectionData(true, this.getPlayer()).isEmpty();
    }

    public static void tryRenderTooltip(EasyGuiGraphics gui) {
        if (lastButton != null && lastButton.m_5953_((double) gui.mousePos.x, (double) gui.mousePos.y)) {
