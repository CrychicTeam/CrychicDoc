...
public class CoinManagementScreen extends EasyMenuScreen<CoinManagementMenu> {
    ...
    private CoinManagementTab currentTab() {
        return (CoinManagementTab) this.allTabs.get(this.selectedTab);
    }

    public EditableChainData getSelectedChain() {
        return (EditableChainData) this.editableDataMap.get(this.selectedChain);
    }
    ...
    public void ChangeChainID(@Nonnull String oldID, @Nonnull String newID, boolean allowOverride) {
        if (this.editableDataMap.containsKey(newID) && !allowOverride) {
            ...
            if (oldID.equals("main")) {
                ...
                EditableChainData copy = ((EditableChainData) this.editableDataMap.get(oldID)).copy();
                this.editableDataMap.put(newID, copy);
            } else {
                EditableChainData data = (EditableChainData) this.editableDataMap.get(oldID);
                data.chainID = newID;
