...
public class SelectionTab extends ATMTab {
    ...
    public Team selectedTeam() {
        return this.getBankReference() instanceof TeamBankReference teamBankReference ? TeamSaveData.GetTeam(true, teamBankReference.teamID) : null;
    }
    ...
    public void renderBG(@Nonnull EasyGuiGraphics gui) {
        ...
        if (this.adminMode) {
            ...
            for (int i = 0; i < lines.size(); i++) {
                gui.drawString(((FormattedText) lines.get(i)).getString(), 7, 70 + 9 * i, 4210752);
            }
        }
    }
    ...
    public void renderAfterWidgets(@Nonnull EasyGuiGraphics gui) {
        if (this.getTeamList().isEmpty() && !this.adminMode) {
            gui.pushOffsetZero();
            TextRenderUtil.drawVerticallyCenteredMultilineText(gui, LCText.GUI_BANK_NO_TEAMS_AVAILABLE.get(), this.teamSelection.m_252754_() + 1, TeamButton.Size.NARROW.width - 2, this.teamSelection.m_252907_() + 1, this.teamSelection.m_93694_() - 2, 16777215);
            gui.popOffset();
        }
    }
