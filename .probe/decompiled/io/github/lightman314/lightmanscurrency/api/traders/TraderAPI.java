...
public final class TraderAPI {
    ...
    public static TraderType<?> getTraderType(@Nonnull ResourceLocation type) {
        return (TraderType<?>) traderRegistry.get(type.toString());
    }
    ...
    public static TradeRuleType<?> getTradeRuleType(@Nonnull ResourceLocation type) {
        return (TradeRuleType<?>) tradeRuleRegistry.get(type.toString());
    }
    ...
    public static boolean filterTrader(@Nonnull TraderData data, @Nonnull String searchText) {
        UnmodifiableIterator var2 = ImmutableList.copyOf(searchFilters).iterator();
        while (var2.hasNext()) {
            ITraderSearchFilter filter = (ITraderSearchFilter) var2.next();
            try {
                if (filter.filter(data, searchText)) {
                    return true;
                }
            } catch (Throwable var5) {
                LightmansCurrency.LogError("Error filtering traders: ", var5);
            }
        }
        return false;
    }
