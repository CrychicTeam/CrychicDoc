...
public final class EasyGuiGraphics {
    ...
    public EasyGuiGraphics pushOffset(@Nonnull AbstractWidget widget) {
        this.offsetStack.add(0, ScreenPosition.of(widget.getX(), widget.getY()));
        return this.refactorOffset();
    }
    ...
    private EasyGuiGraphics refactorOffset() {
        this.offset = this.offsetStack.size() > 0 ? (ScreenPosition) this.offsetStack.get(0) : ScreenPosition.ZERO;
        return this;
    }

    private EasyGuiGraphics(@Nonnull GuiGraphics gui, Font font, int mouseX, int mouseY, float partialTicks) {
        this.gui = gui;
        this.font = font;
    ...
    public static EasyGuiGraphics create(@Nonnull GuiGraphics gui, int mouseX, int mouseY, float partialTicks) {
        return create(gui, Minecraft.getInstance().font, mouseX, mouseY, partialTicks);
    }

    public static EasyGuiGraphics create(@Nonnull GuiGraphics gui, Font font, int mouseX, int mouseY, float partialTicks) {
    ...
    public static EasyGuiGraphics create(@Nonnull ScreenEvent.Render event) {
        return new EasyGuiGraphics(event.getGuiGraphics(), event.getScreen().getMinecraft().font, event.getMouseX(), event.getMouseY(), event.getPartialTick());
    }

    public static EasyGuiGraphics create(@Nonnull ContainerScreenEvent.Render event) {
        return new EasyGuiGraphics(event.getGuiGraphics(), event.getContainerScreen().getMinecraft().font, event.getMouseX(), event.getMouseY(), 0.0F);
    }
    ...
    public void setColor(@Nonnull Vector4f color) {
        this.gui.setColor(color.x, color.y, color.z, color.w);
    }
    ...
    public void blit(@Nonnull ResourceLocation image, int x, int y, int u, int v, int width, int height) {
        this.gui.blit(image, this.offset.x + x, this.offset.y + y, u, v, width, height);
    }
    ...
    public void renderButtonBG(int x, int y, int width, int height, float alpha, int textureY) {
        this.renderButtonBG(x, y, width, height, alpha, textureY, 16777215);
    }
    ...
    public void fill(int x, int y, int width, int height, int color) {
        this.gui.fill(this.offset.x + x, this.offset.y + y, this.offset.x + x + width, this.offset.y + y + height, color);
    }
    ...
    public void renderTooltip(@Nonnull Component tooltip, int x, int y) {
        this.gui.renderTooltip(this.font, tooltip, this.offset.x + x, this.offset.y + y);
    }
    ...
    public void renderComponentTooltip(@Nonnull List<Component> tooltip, int x, int y) {
        if (tooltip.size() != 0) {
            this.gui.renderComponentTooltip(this.font, tooltip, this.offset.x + x, this.offset.y + y);
        }
    }
    ...
    public void renderTooltip(@Nonnull ItemStack item, int x, int y) {
        this.gui.renderTooltip(this.font, item, x, y);
    }

    public void drawString(String text, int x, int y, int color) {
        this.gui.drawString(this.font, text, this.offset.x + x, this.offset.y + y, color, false);
    }
    ...
    public void drawShadowed(String text, int x, int y, int color) {
        this.gui.drawString(this.font, text, this.offset.x + x, this.offset.y + y, color, true);
    }
    ...
    public void drawWordWrap(String text, int x, int y, int columnWidth, int color) {
        this.gui.drawWordWrap(this.font, EasyText.literal(text), this.offset.x + x, this.offset.y + y, columnWidth, color);
    }

    public void drawWordWrap(Component text, int x, int y, int columnWidth, int color) {
        this.gui.drawWordWrap(this.font, text, this.offset.x + x, this.offset.y + y, columnWidth, color);
    }
    ...
    public void renderSlotBackground(Pair<ResourceLocation, ResourceLocation> background, int x, int y) {
        if (background != null) {
            TextureAtlasSprite textureatlassprite = (TextureAtlasSprite) Minecraft.getInstance().getTextureAtlas((ResourceLocation) background.getFirst()).apply((ResourceLocation) background.getSecond());
            this.gui.blit(this.offset.x + x, this.offset.y + y, 0, 16, 16, textureatlassprite);
        }
    }

    public void renderSlotHighlight(int x, int y) {
        this.gui.fillGradient(RenderType.guiOverlay(), this.offset.x + x, this.offset.y + y, this.offset.x + x + 16, this.offset.y + y + 16, -2130706433, -2130706433, 0);
    }
    ...
    public EasyGuiGraphics pushPose() {
        this.gui.pose().pushPose();
        return this;
    }

    public void TranslateToForeground() {
        this.gui.pose().translate(0.0, 0.0, 250.0);
    }

    public void popPose() {
        this.gui.pose().popPose();
    }
}