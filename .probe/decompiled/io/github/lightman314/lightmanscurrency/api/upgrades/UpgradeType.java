...
public abstract class UpgradeType {
    ...
    public static boolean hasUpgrade(@Nonnull UpgradeType type, @Nonnull Container upgradeContainer) {
        for (int i = 0; i < upgradeContainer.getContainerSize(); i++) {
            ...
            if (stack.getItem() instanceof UpgradeItem upgradeItem && upgradeItem.getUpgradeType() == type) {
                return true;
            }
        }
        return false;
    ...
    public final void addTarget(@Nonnull ItemLike target) {
        this.addTarget(formatTarget(target));
    }

    public final void addTarget(@Nonnull RegistryObject<? extends ItemLike> target) {
        this.addTarget(formatTarget(target));
    }
    ...
    protected static Component formatTarget(@Nonnull RegistryObject<? extends ItemLike> target) {
        return formatTarget(target.get());
    }
    ...
    public static class Simple extends UpgradeType {
        ...
        public Simple(@Nonnull Component... tooltips) {
        ...
        public final UpgradeType.Simple withTarget(@Nonnull Component target) {
            this.targets.add(target);
            return this;
        }

        public final UpgradeType.Simple withTarget(@Nonnull ItemLike target) {
            this.targets.add(formatTarget(target));
            return this;
        }

        public final UpgradeType.Simple withTarget(@Nonnull RegistryObject<? extends ItemLike> target) {
            this.targets.add(formatTarget(target));
            return this;
        }
    }
