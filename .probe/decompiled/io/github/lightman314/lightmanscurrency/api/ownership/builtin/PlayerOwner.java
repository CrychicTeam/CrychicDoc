...
public class PlayerOwner extends Owner {
    ...
    public static final OwnerType TYPE = OwnerType.create(new ResourceLocation("lightmanscurrency", "player"), tag -> of(PlayerReference.load(tag.getCompound("Player"))));
    ...
    public boolean isOnline() {
        return this.isClient() ? false : this.player.isOnline();
    }
    ...
    public BankReference asBankReference() {
        return PlayerBankReference.of(this.player).flagAsClient(this.isClient());
    }
    ...
    public void pushNotification(@Nonnull NonNullSupplier<? extends Notification> notificationSource, int notificationLevel, boolean sendToChat) {
        NotificationAPI.PushPlayerNotification(this.player.id, notificationSource.get(), sendToChat);
    }
    ...
    public boolean matches(@Nonnull Owner other) {
        if (other instanceof PlayerOwner po && po.player.is(this.player)) {
            return true;
        }
        return false;
    }
