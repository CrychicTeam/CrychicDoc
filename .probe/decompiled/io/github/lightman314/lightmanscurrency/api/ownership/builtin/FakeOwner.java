...
public class FakeOwner extends Owner {
    ...
    public static final OwnerType TYPE = OwnerType.create(new ResourceLocation("lightmanscurrency", "fake"), tag -> of(Component.Serializer.fromJson(tag.getString("Name"))));
    ...
    public static FakeOwner of(@Nonnull String name) {
        return of(EasyText.literal(name));
    }
    ...
    public boolean matches(@Nonnull Owner other) {
        if (other instanceof FakeOwner fo && fo.name.equals(this.name)) {
            return true;
        }
        return false;
    }
