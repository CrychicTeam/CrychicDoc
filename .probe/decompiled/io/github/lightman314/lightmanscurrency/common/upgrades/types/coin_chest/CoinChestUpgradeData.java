...
public class CoinChestUpgradeData {
    ...
    public void tick(@Nonnull CoinChestBlockEntity be) {
        if (this.ticks && this.upgrade instanceof TickableCoinChestUpgrade tickable && --this.tickTimer <= 0) {
            this.tickTimer = tickable.getTickFrequency();
            try {
                tickable.OnServerTick(be, this);
            } catch (Throwable var4) {
                LightmansCurrency.LogError("Error ticking a Tickable CoinChestUpgrade", var4);
            }
        }
    }
}