...
public class EquipmentRestriction extends ItemTradeRestriction {
    ...
    private boolean equippable(ItemStack item) {
        try {
            return item.canEquip(this.equipmentType, safeGetDummyArmorStand()) || this.vanillaEquippable(item);
    ...
    private boolean vanillaEquippable(ItemStack item) {
        try {
            return Mob.m_147233_(item) == this.equipmentType;
        } catch (Throwable var3) {
    ...
    public Pair<ResourceLocation, ResourceLocation> getEmptySlotBG() {
        return switch(this.equipmentType) {
            case HEAD ->
                Pair.of(InventoryMenu.BLOCK_ATLAS, InventoryMenu.EMPTY_ARMOR_SLOT_HELMET);
            case CHEST ->
                Pair.of(InventoryMenu.BLOCK_ATLAS, InventoryMenu.EMPTY_ARMOR_SLOT_CHESTPLATE);
            case LEGS ->
                Pair.of(InventoryMenu.BLOCK_ATLAS, InventoryMenu.EMPTY_ARMOR_SLOT_LEGGINGS);
            case FEET ->
                Pair.of(InventoryMenu.BLOCK_ATLAS, InventoryMenu.EMPTY_ARMOR_SLOT_BOOTS);
            case OFFHAND ->
    ...
    private static ArmorStand safeGetDummyArmorStand() throws Exception {
        return new ArmorStand(LightmansCurrency.PROXY.safeGetDummyLevel(), 0.0, 0.0, 0.0);
    }
}