...
public class AuctionTradeButtonRenderer extends TradeRenderManager<AuctionTradeData> {
    ...
    public List<DisplayEntry> getInputDisplays(TradeContext context) {
        return Lists.newArrayList(new DisplayEntry[] { DisplayEntry.of(this.trade.getLastBidAmount(), this.getBidInfo(), true) });
    }
    ...
    public List<Component> getAdditionalTooltips(TradeContext context, int mouseX, int mouseY) {
        TimeUtil.TimeData time = new TimeUtil.TimeData(this.trade.getRemainingTime(TimeUtil.getCurrentTime()));
        return Lists.newArrayList(new Component[] { LCText.TOOLTIP_TRADER_AUCTION_TIME_REMAINING.get(EasyText.literal(time.getString()).withStyle(s -> s.withColor(this.getTextColor(time)))) });
    }

    private int getTextColor(TimeUtil.TimeData remainingTime) {
        if (remainingTime.miliseconds < 3600000L) {
            return remainingTime.miliseconds < 300000L ? 16711680 : 16776960;
        } else {
            return 65280;
        }
    }
