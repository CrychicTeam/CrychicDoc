...
public class ItemTraderDataArmor extends ItemTraderData {
    ...
    public static final TraderType<ItemTraderDataArmor> TYPE = new TraderType<>(new ResourceLocation("lightmanscurrency", "item_trader_armor"), ItemTraderDataArmor::new);

    private ItemTraderDataArmor() {
        super(TYPE);
    }

    public ItemTraderDataArmor(Level level, BlockPos pos) {
        super(TYPE, 4, level, pos);
    }
    ...
    protected ItemTradeRestriction getTradeRestriction(int tradeIndex) {
        return switch(tradeIndex % 4) {
            case 0 ->
                EquipmentRestriction.HEAD;
            case 1 ->
                EquipmentRestriction.CHEST;
            case 2 ->
                EquipmentRestriction.LEGS;
            default ->
                EquipmentRestriction.FEET;
        };
    }
}