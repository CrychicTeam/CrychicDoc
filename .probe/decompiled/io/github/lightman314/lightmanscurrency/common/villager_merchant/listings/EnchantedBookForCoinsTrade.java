...
public class EnchantedBookForCoinsTrade implements VillagerTrades.ItemListing {
    ...
    public static final EnchantedBookForCoinsTrade.Serializer SERIALIZER = new EnchantedBookForCoinsTrade.Serializer();
    ...
    public MerchantOffer getOffer(@NotNull Entity trader, RandomSource rand) {
        ...
        if (chain == null) {
            ...
            if (CoinValue.fromNumber(chain.chain, value) instanceof CoinValue coinValue) {
                List coins = coinValue.getAsSeperatedItemList();
                ItemStack price1 = ItemStack.EMPTY;
                ItemStack price2 = ItemStack.EMPTY;
                if (!coins.isEmpty()) {
                    price1 = (ItemStack) coins.get(0);
                }
                if (coins.size() > 1) {
                    price2 = (ItemStack) coins.get(1);
                }
