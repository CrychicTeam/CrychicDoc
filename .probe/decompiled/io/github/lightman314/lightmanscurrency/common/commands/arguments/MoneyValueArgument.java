...
public class MoneyValueArgument implements ArgumentType<MoneyValue> {
    ...
    public static MoneyValueArgument argument(CommandBuildContext context) {
        return new MoneyValueArgument(context.holderLookup(ForgeRegistries.ITEMS.getRegistryKey()));
    }

    public static MoneyValueArgument safeArgument(RegisterCommandsEvent event) {
        return argument(event.getBuildContext());
    }
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
        ...
        if (arg.contains(";")) {
            ...
            for (CurrencyType typex : MoneyAPI.API.AllCurrencyTypes()) {
                MoneyValueParser parser = typex.getValueParser();
