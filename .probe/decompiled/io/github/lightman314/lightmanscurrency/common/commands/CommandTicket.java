...
public class CommandTicket {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        LiteralArgumentBuilder<CommandSourceStack> command = (LiteralArgumentBuilder<CommandSourceStack>) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) Commands.literal("tickets").requires(commandSource -> commandSource.isPlayer() && commandSource.hasPermission(2))).then(Commands.literal("changeColor").then(Commands.argument("color", ColorArgument.argument()).executes(CommandTicket::changeColor)))).then(((LiteralArgumentBuilder) Commands.literal("create").executes(CommandTicket::createTicketNonColored)).then(Commands.argument("color", ColorArgument.argument()).executes(CommandTicket::createTicketColored)));
    ...
    static int createTicketNonColored(CommandContext<CommandSourceStack> commandContext) throws CommandSyntaxException {
        ServerPlayer player = ((CommandSourceStack) commandContext.getSource()).getPlayerOrException();
    ...
    private static void giveItemToPlayer(ServerPlayer player, ItemStack item) {
        ...
        if (inv.getItem(inv.selected).isEmpty()) {
            player.m_150109_().setItem(inv.selected, item);
        } else {
