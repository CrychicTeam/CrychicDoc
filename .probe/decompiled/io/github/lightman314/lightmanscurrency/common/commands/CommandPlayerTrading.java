...
public class CommandPlayerTrading {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        LiteralArgumentBuilder<CommandSourceStack> requestTradeCommand = (LiteralArgumentBuilder<CommandSourceStack>) ((LiteralArgumentBuilder) Commands.literal("lctrade").requires(CommandSourceStack::m_230897_)).then(Commands.argument("player", EntityArgument.player()).executes(CommandPlayerTrading::requestPlayerTrade));
        LiteralArgumentBuilder<CommandSourceStack> acceptTradeCommand = (LiteralArgumentBuilder<CommandSourceStack>) ((LiteralArgumentBuilder) Commands.literal("lctradeaccept").requires(CommandSourceStack::m_230897_)).then(Commands.argument("tradeID", TradeIDArgument.argument()).executes(CommandPlayerTrading::acceptPlayerTrade));
    ...
    private static int requestPlayerTrade(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        if (guest == host) {
            ...
            host.sendSystemMessage(LCText.COMMAND_TRADE_HOST_NOTIFY.get(guest.m_7755_()));
            guest.sendSystemMessage(LCText.COMMAND_TRADE_GUEST_NOTIFY.get(host.m_7755_(), LCText.COMMAND_TRADE_GUEST_NOTIFY_PROMPT.getWithStyle(ChatFormatting.BOLD, ChatFormatting.GREEN).withStyle(s -> s.withClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/lctradeaccept " + tradeID)))).withStyle(ChatFormatting.GOLD));
            return 1;
        }
    }
        ...
        if (trade != null && trade.isGuest(guest)) {
            ...
            if (rangeResult > 0) {
                TextEntry entry = switch(rangeResult) {
                    case 1 ->
                        LCText.COMMAND_TRADE_ACCEPT_FAIL_OFFLINE;
                    case 2 ->
                        LCText.COMMAND_TRADE_ACCEPT_FAIL_DISTANCE;
                    case 3 ->
                        LCText.COMMAND_TRADE_ACCEPT_FAIL_DIMENSION;
                    default ->
                        null;
                };
                if (entry != null) {
                ...
                return 0;
            } else if (trade.requestAccepted(guest)) {
                return 1;
            } else {
