...
public class LowBalanceNotification extends Notification {
    ...
    public static final NotificationType<LowBalanceNotification> TYPE = new NotificationType<>(new ResourceLocation("lightmanscurrency", "bank_low_balance"), LowBalanceNotification::new);
    ...
    public static NonNullSupplier<Notification> create(@Nonnull MutableComponent accountName, @Nonnull MoneyValue value) {
        return () -> new LowBalanceNotification(accountName, value);
    }

    @Nonnull
    @Override
    protected NotificationType<LowBalanceNotification> getType() {
        return TYPE;
    }
    ...
    public MutableComponent getMessage() {
        return LCText.NOTIFICATION_BANK_LOW_BALANCE.get(this.value.getString());
    }
    ...
    protected boolean canMerge(@Nonnull Notification other) {
        if (other instanceof LowBalanceNotification lbn) {
            return !lbn.accountName.getString().equals(this.accountName.getString()) ? false : lbn.value.equals(this.value);
        } else {
            return false;
        }
    }
