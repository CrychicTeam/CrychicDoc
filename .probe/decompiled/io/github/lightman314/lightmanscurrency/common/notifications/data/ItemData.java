...
public class ItemData {
    ...
    public static MutableComponent format(@Nonnull ItemData d1, @Nonnull ItemData d2) {
        ...
        } else if (d1.stack.isEmpty()) {
            return d2.format();
        } else {
            return d2.stack.isEmpty() ? d1.format() : d1.formatWith(d2);
        }
    }
    ...
    public boolean matches(@Nonnull ItemData other) {
        return this.deprecatedName == null && other.deprecatedName == null ? this.customName.equals(other.customName) && InventoryUtil.ItemsFullyMatch(this.stack, other.stack) : false;
    }
