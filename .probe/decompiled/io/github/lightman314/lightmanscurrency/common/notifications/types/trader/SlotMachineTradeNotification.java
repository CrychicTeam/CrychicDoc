...
public class SlotMachineTradeNotification extends TaxableNotification {
    ...
    public static final NotificationType<SlotMachineTradeNotification> TYPE = new NotificationType<>(new ResourceLocation("lightmanscurrency", "slot_machine_trade"), SlotMachineTradeNotification::new);
    ...
    public static NonNullSupplier<Notification> create(SlotMachineEntry entry, MoneyValue cost, PlayerReference customer, TraderCategory traderData, MoneyValue taxesPaid) {
    ...
    protected boolean canMerge(@Nonnull Notification other) {
        if (other instanceof SlotMachineTradeNotification smtn) {
            if (!smtn.traderData.matches(this.traderData)) {
                return false;
            } else if (smtn.items.size() != this.items.size()) {
                return false;
            } else {
                for (int i = 0; i < this.items.size(); i++) {
                    if (!((ItemData) this.items.get(i)).matches((ItemData) smtn.items.get(i))) {
                        return false;
                    }
                }
                if (!smtn.money.equals(this.money)) {
                    return false;
                } else if (!smtn.cost.equals(this.cost)) {
                    return false;
                } else {
                    return !smtn.customer.equals(this.customer) ? false : this.TaxesMatch(smtn);
                }
            }
        } else {
            return false;
