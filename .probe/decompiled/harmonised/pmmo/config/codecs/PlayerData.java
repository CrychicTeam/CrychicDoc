...
public record PlayerData(boolean override, boolean ignoreReq, Map<String, Double> bonuses) implements DataSource<PlayerData> {
    ...
    public static final Codec<PlayerData> CODEC = RecordCodecBuilder.create(instance -> instance.group(Codec.BOOL.optionalFieldOf("override").forGetter(pd -> Optional.of(pd.override())), Codec.BOOL.optionalFieldOf("ignoreReq").forGetter(pd -> Optional.of(pd.ignoreReq())), CodecTypes.DOUBLE_CODEC.optionalFieldOf("bonuses").forGetter(pd -> Optional.of(pd.bonuses()))).apply(instance, (override, reqIgnore, bonus) -> new PlayerData((Boolean) override.orElse(false), (Boolean) reqIgnore.orElse(false), DataSource.<String, Double>clearEmptyValues((Map<String, Double>) bonus.orElse(new HashMap())))));
    ...
    public PlayerData combine(PlayerData two) {
        ...
        BiConsumer<PlayerData, PlayerData> bothOrNeither = (o, t) -> {
            ...
            t.bonuses().forEach((skill, mod) -> mergedBonus.merge(skill, mod, (o1, t1) -> o1 > t1 ? o1 : t1));
        };
    ...
    public Map<String, Double> mergeWithPlayerBonuses(Map<String, Double> map) {
        ...
        this.bonuses().forEach((string, ratio) -> newMap.merge(string, ratio, (one, two) -> one + two));
        return newMap;
    }
}