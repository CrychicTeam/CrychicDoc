...
public class CommonSetup {
    ...
    public static void onServerStartup(ServerStartingEvent event) {
        MsLoggy.INFO.log(MsLoggy.LOG_CODE.LOADING, "Loading PMMO Saved Data");
        Core.get(LogicalSide.SERVER).getData();
        MsLoggy.INFO.log(MsLoggy.LOG_CODE.LOADING, "Computing data for cache");
        Core.get(LogicalSide.SERVER).getData().computeLevelsForCache();
        MsLoggy.INFO.log(MsLoggy.LOG_CODE.LOADING, "PMMO Server loading process complete");
    }

    @SubscribeEvent
    public static void onConfigReload(ModConfigEvent.Reloading event) {
        if (event.getConfig().getType().equals(Type.SERVER)) {
    ...
    public static void gatherData(GatherDataEvent event) {
        ...
        if (event.includeServer()) {
            ...
            generator.addProvider(true, new ItemTagProvider(generator.getPackOutput(), event.getLookupProvider(), blockProvider.m_274426_(), event.getExistingFileHelper()));
