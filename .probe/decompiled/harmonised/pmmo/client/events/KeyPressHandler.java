...
public class KeyPressHandler {
    ...
    public static void keyPressEvent(InputEvent.Key event) {
        ...
        if (mc.player != null) {
            if (ClientSetup.VEIN_KEY.isDown() && mc.hitResult != null && mc.hitResult.getType().equals(HitResult.Type.BLOCK)) {
                BlockHitResult bhr = (BlockHitResult) mc.hitResult;
                Block block = mc.player.m_9236_().getBlockState(bhr.getBlockPos()).m_60734_();
                if (!Core.get(LogicalSide.CLIENT).getLoader().DIMENSION_LOADER.getData(mc.player.m_9236_().dimension().location()).veinBlacklist().contains(RegistryUtil.getId(block)) && !Core.get(LogicalSide.CLIENT).getLoader().BIOME_LOADER.getData(RegistryUtil.getId(mc.player.m_9236_().m_204166_(mc.player.m_20183_()))).veinBlacklist().contains(RegistryUtil.getId(block))) {
            ...
            if (ClientSetup.ADD_VEIN.isDown()) {
                Config.VEIN_LIMIT.set(Integer.valueOf(Config.VEIN_LIMIT.get() + 1));
            ...
            if (mc.screen == null && ClientSetup.OPEN_MENU.isDown()) {
                if (mc.hitResult != null && !mc.player.isCrouching()) {
                    if (mc.hitResult.getType().equals(HitResult.Type.BLOCK)) {
                        mc.setScreen(new StatsScreen(((BlockHitResult) mc.hitResult).getBlockPos()));
                    } else if (mc.hitResult.getType().equals(HitResult.Type.ENTITY)) {
                    ...
                    } else if (mc.hitResult.getType().equals(HitResult.Type.MISS)) {
