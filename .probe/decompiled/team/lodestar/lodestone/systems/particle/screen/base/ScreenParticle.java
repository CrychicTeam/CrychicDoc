...
public abstract class ScreenParticle {
    ...
    public ScreenParticle(ClientLevel pLevel, double pX, double pY, double pXSpeed, double pYSpeed) {
        this(pLevel, pX, pY);
        this.xMotion = pXSpeed + (Math.random() * 2.0 - 1.0) * 0.4F;
        this.yMotion = pYSpeed + (Math.random() * 2.0 - 1.0) * 0.4F;
        double d0 = (Math.random() + Math.random() + 1.0) * 0.15F;
        double d1 = Math.sqrt(this.xMotion * this.xMotion + this.yMotion * this.yMotion);
        this.xMotion = this.xMotion / d1 * d0 * 0.4F;
        this.yMotion = this.yMotion / d1 * d0 * 0.4F + 0.1F;
    }
    ...
    public void tick() {
        ...
        if (this.age++ >= this.lifetime) {
            ...
            this.yMotion = this.yMotion - 0.04 * (double) this.gravity;
            this.xMotion = this.xMotion * (double) this.friction;
            this.yMotion = this.yMotion * (double) this.friction;
            this.x = this.x + this.xMotion;
            this.y = this.y + this.yMotion;
            this.xMoved = this.xMoved + this.xMotion;
            this.yMoved = this.yMoved + this.yMotion;
        }
    }
