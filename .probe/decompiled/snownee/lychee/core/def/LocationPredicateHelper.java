...
public interface LocationPredicateHelper {
    ...
    static void toNetwork(LocationPredicate predicate, FriendlyByteBuf pBuffer) {
        ...
        ResourceLocation dim = (ResourceLocation) Optional.ofNullable(access.getDimension()).map(ResourceKey::m_135782_).orElse(null);
        CommonProxy.writeNullableRL(dim, pBuffer);
        ResourceLocation biome = (ResourceLocation) Optional.ofNullable(access.getBiome()).map(ResourceKey::m_135782_).orElse(null);
        CommonProxy.writeNullableRL(biome, pBuffer);
        ResourceLocation feature = (ResourceLocation) Optional.ofNullable(access.getStructure()).map(ResourceKey::m_135782_).orElse(null);
    ...
    void lychee$setBiomeTag(TagKey<Biome> var1);
