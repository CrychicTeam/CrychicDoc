...
public abstract class LycheeRecipe<C extends LycheeContext> extends ContextualHolder implements ILycheeRecipe<C>, Recipe<C> {
    ...
    public int getRandomRepeats(int max, C ctx) {
        ...
        if (!this.maxRepeats.m_55327_()) {
            times = IntBoundsHelper.random(this.maxRepeats, ctx.getRandom());
        }
    ...
    public abstract static class Serializer<R extends LycheeRecipe<?>> implements RecipeSerializer<R> {
        ...
        public final R fromJson(ResourceLocation pRecipeId, JsonObject jsonObject) {
            ...
            R recipe = (R) this.factory.apply(pRecipeId);
            ...
            if (jsonObject.has("max_repeats")) {
                recipe.maxRepeats = MinMaxBounds.Ints.fromJson(jsonObject.get("max_repeats"));
        ...
        public final R fromNetwork(ResourceLocation id, FriendlyByteBuf buf) {
            try {
                R recipe = (R) this.factory.apply(id);
                if (LycheeConfig.debug) {
                ...
                Lychee.LOGGER.error("Exception while reading Lychee recipe: " + id, var4);
                return null;
            }
        }
