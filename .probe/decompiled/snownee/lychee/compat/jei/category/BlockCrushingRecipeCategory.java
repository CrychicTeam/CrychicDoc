...
public class BlockCrushingRecipeCategory extends BaseJEICategory<BlockCrushingContext, BlockCrushingRecipe> {
    ...
    public void draw(BlockCrushingRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics graphics, double mouseX, double mouseY) {
        ...
        if (landingBlock.m_60791_() < 5) {
            ...
            matrixStack.translate((double) x + 10.5, (double) (y + (anyLandingBlock ? 1 : 16)), 0.0);
            float shadow = 0.6F;
            if (anyLandingBlock) {
        ...
        matrixStack.pushPose();
        matrixStack.translate((float) x, (float) (y - 13), 0.0F);
        GuiGameElement.of(fallingBlock).scale(15.0).atLocal(0.0, (double) ticks * 1.3 - 1.3, 0.0).rotateBlock(20.0, 225.0, 0.0).lighting(JEIREI.BLOCK_LIGHTING).<RenderElement>at(0.0F, 0.0F, 300.0F).render(graphics);
        if (!landingBlock.m_60795_()) {
            GuiGameElement.of(landingBlock).scale(15.0).atLocal(0.0, 1.0, 0.0).rotateBlock(20.0, 225.0, 0.0).lighting(JEIREI.BLOCK_LIGHTING).render(graphics);
        }
        matrixStack.popPose();
    }
    ...
    public List<Component> getTooltipStrings(BlockCrushingRecipe recipe, IRecipeSlotsView recipeSlotsView, double mouseX, double mouseY) {
        ...
        } else if (recipe.getLandingBlock() != BlockPredicate.ANY && landingBlockRect.contains(x, y)) {
            BlockState landingBlock = CommonProxy.getCycledItem(BlockPredicateHelper.getShowcaseBlockStates(recipe.getLandingBlock()), Blocks.AIR.defaultBlockState(), 2000);
