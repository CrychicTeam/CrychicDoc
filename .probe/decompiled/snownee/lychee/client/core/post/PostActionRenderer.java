...
public interface PostActionRenderer<T extends PostAction> {
    ...
    PostActionRenderer<PostAction> DEFAULT = new PostActionRenderer<PostAction>() {
    };
    ...
    static List<Component> getTooltipsFromRandom(RandomSelect randomSelect, PostAction child) {
        ...
        List<Component> list = (List<Component>) (randomSelect.entries.length == 1 && randomSelect.emptyWeight == 0 ? Lists.newArrayList(new Component[] { randomSelect.getDisplayName() }) : of(child).getBaseTooltips(child));
        if (index == -1) {
            return list;
        } else {
    ...
    default List<Component> getBaseTooltips(T action) {
        return Lists.newArrayList(new Component[] { action.getDisplayName() });
    }
