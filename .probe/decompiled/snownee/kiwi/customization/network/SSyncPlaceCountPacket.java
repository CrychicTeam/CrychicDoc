...
public class SSyncPlaceCountPacket extends PacketHandler {
    ...
    @Override
    public CompletableFuture<FriendlyByteBuf> receive(Function<Runnable, CompletableFuture<FriendlyByteBuf>> executor, FriendlyByteBuf friendlyByteBuf, @Nullable ServerPlayer serverPlayer) {
        int count = friendlyByteBuf.readVarInt();
        return (CompletableFuture<FriendlyByteBuf>) executor.apply((Runnable) () -> ((KPlayer) Objects.requireNonNull((KPlayer) Minecraft.getInstance().player)).kiwi$setPlaceCount(count));
    }
