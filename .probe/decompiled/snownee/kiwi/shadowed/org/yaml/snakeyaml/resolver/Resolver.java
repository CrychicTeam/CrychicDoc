package snownee.kiwi.shadowed.org.yaml.snakeyaml.resolver;
...
public class Resolver {
    ...
    public void addImplicitResolver(Tag tag, Pattern regexp, String first, int limit) {
        if (first == null) {
            List<ResolverTuple> curr = (List<ResolverTuple>) this.yamlImplicitResolvers.get(null);
            if (curr == null) {
                curr = new ArrayList();
            ...
            for (int j = chrs.length; i < j; i++) {
                ...
                List<ResolverTuple> curr = (List<ResolverTuple>) this.yamlImplicitResolvers.get(theC);
                if (curr == null) {
                    curr = new ArrayList();
    ...
    public Tag resolve(NodeId kind, String value, boolean implicit) {
        if (kind == NodeId.scalar && implicit) {
            ...
            if (value.length() == 0) {
                resolvers = (List<ResolverTuple>) this.yamlImplicitResolvers.get('\u0000');
            } else {
                resolvers = (List<ResolverTuple>) this.yamlImplicitResolvers.get(value.charAt(0));
            }
            if (resolvers != null) {
            ...
            if (this.yamlImplicitResolvers.containsKey(null)) {
                for (ResolverTuple vx : (List) this.yamlImplicitResolvers.get(null)) {
                    Tag tag = vx.getTag();
