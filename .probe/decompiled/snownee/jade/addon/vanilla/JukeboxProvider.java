...
public enum JukeboxProvider implements IBlockComponentProvider, IServerDataProvider<BlockAccessor> {
    ...
    public void appendTooltip(ITooltip tooltip, BlockAccessor accessor, IPluginConfig config) {
        ...
        if ((Boolean) state.m_61143_(JukeboxBlock.HAS_RECORD) && accessor.getServerData().contains("Record")) {
            try {
    ...
    public void appendServerData(CompoundTag data, BlockAccessor accessor) {
        ItemStack stack = ((JukeboxBlockEntity) accessor.getBlockEntity()).m_272036_();
        if (!stack.isEmpty()) {
