...
public class ArmorSwap extends SingleSwapItem implements ItemOnBackItem {
    ...
    public static boolean isValidItem(ItemStack stack) {
        return stack.getItem().canFitInsideContainerItems() && !(stack.getItem() instanceof BaseBagItem) && LivingEntity.getEquipmentSlotForItem(stack).getType() == EquipmentSlot.Type.ARMOR;
    }
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> list, TooltipFlag flag) {
        LangData.addInfo(list, LangData.Info.ARMORBAG_INFO, LangData.Info.INHERIT);
    }

    @Nullable
    @Override
    public IQuickSwapToken<?> getTokenOfType(ItemStack stack, LivingEntity player, QuickSwapType type) {
        return type != QuickSwapTypes.ARMOR ? null : new SingleSwapToken(this, stack, type);
    }
