...
public class ForgeLootCrateOpenerBlockEntity extends LootCrateOpenerBlockEntity {
    ...
    private final ForgeLootCrateOpenerBlockEntity.LootCrateHandler lootCrateHandler = new ForgeLootCrateOpenerBlockEntity.LootCrateHandler();

    private final LazyOptional<IItemHandler> itemCap = LazyOptional.of(() -> this.lootCrateHandler);
    ...
    public <T> LazyOptional<T> getCapability(@NotNull Capability<T> cap, @Nullable Direction side) {
        return cap == ForgeCapabilities.ITEM_HANDLER ? this.itemCap.cast() : super.getCapability(cap, side);
    }
