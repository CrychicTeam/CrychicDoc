...
public class AddTaskButton extends Button {
    ...
    public void onClicked(MouseButton button) {
        ...
        for (TaskType type : TaskTypes.TYPES.values()) {
            contextMenu.add(new ContextMenuItem(type.getDisplayName(), type.getIconSupplier(), b -> {
                this.playClickSound();
                type.getGuiProvider().openCreationGui(this.parent, this.quest, task -> {
        ...
        if (!clip.isEmpty()) {
            QuestObjectBase.parseHexId(clip).ifPresent(taskId -> {
                if (FTBQuestsAPI.api().getQuestFile(true).get(taskId) instanceof Task task) {
                    ...
                    contextMenu.add(new AddTaskButton.PasteTaskMenuItem(task, b -> this.copyAndCreateTask(task)));
                }
            });
        }
    ...
    public static class PasteTaskMenuItem extends TooltipContextMenuItem {
        ...
        public PasteTaskMenuItem(Task task, @Nullable Consumer<Button> callback) {
            super(Component.translatable("ftbquests.gui.paste_task"), Icons.ADD, callback, Component.literal("\"").append(task.getTitle()).append("\""), Component.literal(QuestObjectBase.getCodeString(task.id)).withStyle(ChatFormatting.DARK_GRAY));
        }
    }
}