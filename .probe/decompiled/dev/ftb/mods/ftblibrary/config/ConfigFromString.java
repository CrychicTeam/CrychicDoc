...
public abstract class ConfigFromString<T> extends ConfigValue<T> {
    ...
    public abstract boolean parse(@Nullable Consumer<T> var1, String var2);
    ...
    public Component getStringForGUI(@Nullable T v) {
        return Component.literal(this.getStringFromValue(v));
    }

    @Override
    public void onClicked(Widget clicked, MouseButton button, ConfigCallback callback) {
        BaseScreen gui = clicked.getGui();
        EditStringConfigOverlay.PosProvider.Offset offset = clicked instanceof EditStringConfigOverlay.PosProvider p ? p.getOverlayOffset() : EditStringConfigOverlay.PosProvider.Offset.NONE;
