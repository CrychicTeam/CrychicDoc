...
public class UITesting {
    ...
    public static void openTestScreen() {
        ConfigGroup group = new ConfigGroup("test", accepted -> Minecraft.getInstance().player.displayClientMessage(Component.literal("Accepted: " + accepted), false));
        ...
        new UITesting.TestConfigScreen(group).setAutoclose(true).openGuiLater();
    }

    private static class TestButtonListScreen extends AbstractButtonListScreen {

        private final UITesting.TestConfigScreen parent;

        public TestButtonListScreen(UITesting.TestConfigScreen parent) {
            this.parent = parent;
            this.setBorder(2, 2, 1);
            ...
            for (int i = 0; i < 50; i++) {
                panel.add(new SimpleTextButton(panel, Component.literal("Button " + i), Icons.ACCEPT) {
                    ...
                    public void onClicked(MouseButton button) {
                        SimpleToast.info(Component.literal("Selected " + this.getTitle().getString()), Component.literal(" "));
                        this.closeGui();
                    }
                });
            }
    ...
    public static class TestConfigScreen extends EditConfigScreen {
        ...
        protected Panel createTopPanel() {
            return new UITesting.TestConfigScreen.TestTopPanel();
        }

        private void openTestContextMenu(Panel parent) {
            ContextMenu menu = new ContextMenu(parent, List.of(ContextMenuItem.title(Component.literal("Title")), ContextMenuItem.SEPARATOR, new ContextMenuItem(Component.literal("Select item"), Icons.ADD, button -> new SelectItemStackScreen(new ItemStackConfig(1L), accepted -> parent.getGui().run()).openGui()), ContextMenuItem.subMenu(Component.literal("line 2 >"), Icons.REMOVE, List.of(ContextMenuItem.title(Component.literal("Submenu")), ContextMenuItem.SEPARATOR, new ContextMenuItem(Component.literal("line 2a"), Icons.BELL, button1 -> {
            }))), new ContextMenuItem(Component.literal("Test Search"), Icon.empty(), button -> this.openTestButtonList()) {
        ...
        private void openTestButtonList() {
            UITesting.TestButtonListScreen screen = new UITesting.TestButtonListScreen(this);
