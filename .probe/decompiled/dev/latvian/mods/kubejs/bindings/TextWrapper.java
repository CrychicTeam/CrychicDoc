        ...
        if (o == null) {
            ...
            if (s.startsWith("{") && s.endsWith("}")) {
                try {
                    ...
                    return Component.literal("Error: " + var13);
                }
            } else {
                return ofString(s);
            }
            ...
            MutableComponent textx = Component.empty();
            for (Object e1 : list) {
                textx.append(of(e1));
            }
            return textx;
    ...
    public static ClickEvent clickEventOf(Object o) {
        if (o == null) {
            ...
            if (json != null) {
                ...
                return new ClickEvent((ClickEvent.Action) Objects.requireNonNull(ClickEvent.Action.getByName(action), "Invalid click event action %s!".formatted(action)), value);
            } else {
                String s = o.toString();
                ...
                return switch(var4) {
                    case "command" ->
                        new ClickEvent(ClickEvent.Action.RUN_COMMAND, split[1]);
                    case "suggest_command" ->
                        new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, split[1]);
                    case "copy" ->
                        new ClickEvent(ClickEvent.Action.COPY_TO_CLIPBOARD, split[1]);
                    case "file" ->
                        new ClickEvent(ClickEvent.Action.OPEN_FILE, split[1]);
                    default ->
                        {
                            ClickEvent.Action action = ClickEvent.Action.getByName(split[0]);
                            ???;
                        }
                };
            }
