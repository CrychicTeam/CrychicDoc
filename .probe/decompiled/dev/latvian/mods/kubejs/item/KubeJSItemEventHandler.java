...
public class KubeJSItemEventHandler {
    ...
    private static CompoundEventResult<ItemStack> rightClick(Player player, InteractionHand hand) {
        if (!ItemEvents.RIGHT_CLICKED.hasListeners()) {
            ...
            return !player.getCooldowns().isOnCooldown(stack.getItem()) ? ItemEvents.RIGHT_CLICKED.post(player, stack.getItem(), new ItemClickedEventJS(player, hand, stack)).archCompound() : CompoundEventResult.pass();
        }
    }
    ...
    private static EventResult entityInteract(Player player, Entity entity, InteractionHand hand) {
        return ItemEvents.ENTITY_INTERACTED.hasListeners() ? ItemEvents.ENTITY_INTERACTED.post(player, player.m_21120_(hand).getItem(), new ItemEntityInteractedEventJS(player, entity, hand)).arch() : EventResult.pass();
    }
