...
public final class RegistryInfo<T> implements Iterable<BuilderBase<? extends T>>, TypeWrapperFactory<T> {
    ...
    private static final RegistrarManager REGISTRIES = RegistrarManager.get("kubejs");
    ...
    public static <T> RegistryInfo<T> of(ResourceKey<? extends Registry<?>> key, Class<T> type) {
        RegistryInfo<?> r = (RegistryInfo<?>) MAP.get(key);
        if (r == null) {
            RegistryInfo<T> reg = new RegistryInfo<>(UtilsJS.cast(key), type);
            MAP.put(key, reg);
            return reg;
        } else {
    ...
    public static RegistryInfo<?> of(ResourceKey<? extends Registry<?>> key) {
        return of(UtilsJS.cast(key), (Class<T>) Object.class);
    }
    ...
    public void addType(String type, Class<? extends BuilderBase<? extends T>> builderType, BuilderFactory factory, boolean isDefault) {
        BuilderType<T> b = new BuilderType<>(type, builderType, factory);
        this.types.put(type, b);
        if (isDefault) {
            if (this.defaultType != null) {
                ConsoleJS.STARTUP.warn("Previous default type '" + this.defaultType.type() + "' for registry '" + this.key.location() + "' replaced with '" + type + "'!");
            }
            this.defaultType = b;
        }
    }
    ...
    public void addBuilder(BuilderBase<? extends T> builder) {
        if (builder == null) {
            throw new IllegalArgumentException("Can't add null builder in registry '" + this.key.location() + "'!");
        } else {
            if (DevProperties.get().debugInfo) {
                ConsoleJS.STARTUP.info("~ " + this.key.location() + " | " + builder.id);
            }
            if (this.objects.containsKey(builder.id)) {
                throw new IllegalArgumentException("Duplicate key '" + builder.id + "' in registry '" + this.key.location() + "'!");
            } else {
    ...
    public BuilderType getDefaultType() {
        if (this.types.isEmpty()) {
            ...
            if (this.defaultType == null) {
                this.defaultType = (BuilderType<T>) this.types.values().iterator().next();
            }
    ...
    public boolean equals(Object obj) {
        if (obj == this) {
            ...
            if (obj instanceof RegistryInfo ri && this.key.equals(ri.key)) {
                return true;
            }
            return false;
        }
    ...
    public int registerObjects(RegistryCallback<T> function) {
        ...
        if (this.objects.isEmpty()) {
            ...
            int added = 0;
            for (BuilderBase<? extends T> builder : this) {
    ...
    public Registrar<T> getArchitecturyRegistrar() {
        if (this.architecturyRegistrar == null) {
            this.architecturyRegistrar = REGISTRIES.get((ResourceKey<Registry<T>>) this.key);
        }
        return this.architecturyRegistrar;
    }

    public Registry<T> getVanillaRegistry() {
        return (Registry<T>) BuiltInRegistries.REGISTRY.get(this.key);
    }
    ...
    public T getValue(ResourceLocation id) {
        return this.getArchitecturyRegistrar().get(id);
    }
    ...
    public T wrap(Context cx, Object o) {
        if (o == null) {
            return null;
        } else if (this.objectBaseClass.isInstance(o)) {
            return (T) o;
        } else {
    ...
    public void fireRegistryEvent() {
        RegistryEventJS<T> event = new RegistryEventJS<>(this);
