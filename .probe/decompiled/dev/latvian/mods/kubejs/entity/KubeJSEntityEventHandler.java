...
public class KubeJSEntityEventHandler {
    ...
    private static EventResult checkSpawn(LivingEntity entity, LevelAccessor la, double x, double y, double z, MobSpawnType type, @Nullable BaseSpawner spawner) {
        if (la instanceof Level level && (la.m_5776_() || UtilsJS.staticServer != null) && EntityEvents.CHECK_SPAWN.hasListeners()) {
            return EntityEvents.CHECK_SPAWN.post(level, entity.m_6095_(), new CheckLivingEntitySpawnEventJS(entity, level, x, y, z, type, spawner)).arch();
        }
        return EventResult.pass();
    }

    private static EventResult livingDeath(LivingEntity entity, DamageSource source) {
        return EntityEvents.DEATH.hasListeners() ? EntityEvents.DEATH.post(entity, entity.m_6095_(), new LivingEntityDeathEventJS(entity, source)).arch() : EventResult.pass();
    }

    private static EventResult livingHurt(LivingEntity entity, DamageSource source, float amount) {
        return EntityEvents.HURT.hasListeners() ? EntityEvents.HURT.post(entity, entity.m_6095_(), new LivingEntityHurtEventJS(entity, source, amount)).arch() : EventResult.pass();
    }

    private static EventResult entitySpawned(Entity entity, Level level) {
        return (level.isClientSide() || UtilsJS.staticServer != null) && EntityEvents.SPAWNED.hasListeners() ? EntityEvents.SPAWNED.post(level, entity.getType(), new EntitySpawnedEventJS(entity, level)).arch() : EventResult.pass();
    }
}