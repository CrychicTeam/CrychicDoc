...
public class UnitContext {
    ...
    public Unit parse(String input) {
        Unit u = (Unit) this.cache.get(input);
        if (u == null) {
            u = this.createStream(input).getUnit();
            this.cache.put(input, u);
        }
        return u;
    }
    ...
    public void debugInfo(String s) {
        if (this.debug >= 0) {
            if (this.debug >= 2) {
                System.out.println("  ".repeat(this.debug - 1) + s);
            } else {
    ...
    public void debugInfo(String s, Collection<?> values) {
        this.debugInfo(s + ": " + (String) values.stream().map(Object::toString).collect(Collectors.joining("  ")));
    }
