...
public class Placebo {
    ...
    public static final SimpleChannel CHANNEL = NetworkRegistry.ChannelBuilder.named(new ResourceLocation("placebo", "placebo")).clientAcceptedVersions(s -> true).serverAcceptedVersions(s -> true).networkProtocolVersion(() -> "1.0.0").simpleChannel();
    ...
    private <T> void checkAndPost(RegisterEvent e, Class<T> clazz, IForgeRegistry<? extends T> reg) {
        if (e.getForgeRegistry() == reg) {
            ModContainer ctr = ModLoadingContext.get().getActiveContainer();
            ModLoader.get().postEventWithWrapInModOrder(new RegistryEvent.Register(clazz, reg), (mc, ev) -> ModLoadingContext.get().setActiveContainer(mc), (mc, ev) -> ModLoadingContext.get().setActiveContainer(null));
            ModLoadingContext.get().setActiveContainer(ctr);
        }
    }

    @SubscribeEvent
    public void registerElse(RegistryEvent.Register<RecipeType<?>> e) {
