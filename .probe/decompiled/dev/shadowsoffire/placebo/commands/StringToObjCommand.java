...
public class StringToObjCommand {
    ...
    public static void register(LiteralArgumentBuilder<CommandSourceStack> builder, CommandBuildContext buildCtx) {
        builder.then(((LiteralArgumentBuilder) Commands.literal("string_to_obj").requires(src -> src.hasPermission(2))).then(Commands.argument("nbt_item", ItemArgument.item(buildCtx)).executes(ctx -> {
            try {
    ...
    public static String toJsonStr(CompoundTag tag) throws IOException {
        JsonElement json = (JsonElement) NBTAdapter.EITHER_CODEC.encodeStart(JsonOps.INSTANCE, tag).get().left().get();
