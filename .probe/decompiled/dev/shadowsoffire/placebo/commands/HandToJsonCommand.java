...
public class HandToJsonCommand {
    ...
    public static void register(LiteralArgumentBuilder<CommandSourceStack> builder) {
        builder.then(Commands.literal("hand").executes(ctx -> {
            try {
                String str = toJsonStr((ItemStack) ((CommandSourceStack) ctx.getSource()).getEntity().getHandSlots().iterator().next());
    ...
    public static String toJsonStr(ItemStack stack) throws IOException {
        JsonElement json = (JsonElement) ItemAdapter.CODEC.encodeStart(JsonOps.INSTANCE, stack).get().left().get();
