...
public class SetCurioAttributesFunction extends LootItemConditionalFunction {
    ...
    SetCurioAttributesFunction(LootItemCondition[] conditions, List<SetCurioAttributesFunction.Modifier> modifiers) {
        super(conditions);
        this.modifiers = ImmutableList.copyOf(modifiers);
    }

    public static void register() {
        TYPE = Registry.register(BuiltInRegistries.LOOT_FUNCTION_TYPE, new ResourceLocation("curios", "set_curio_attributes"), new LootItemFunctionType(new SetCurioAttributesFunction.Serializer()));
    }
    ...
    public Set<LootContextParam<?>> getReferencedContextParams() {
        return (Set<LootContextParam<?>>) this.modifiers.stream().flatMap(mod -> mod.amount.m_6231_().stream()).collect(ImmutableSet.toImmutableSet());
    }
    ...
    public ItemStack run(@Nonnull ItemStack stack, LootContext context) {
        ...
        for (SetCurioAttributesFunction.Modifier modifier : this.modifiers) {
            ...
            if (modifier.attribute instanceof SlotAttribute wrapper) {
                CuriosApi.addSlotModifier(stack, wrapper.getIdentifier(), modifier.name, uuid, (double) modifier.amount.getFloat(context), modifier.operation, slot);
            } else {
                CuriosApi.addModifier(stack, modifier.attribute, modifier.name, uuid, (double) modifier.amount.getFloat(context), modifier.operation, slot);
            }
        }
        return stack;
    }
    ...
    static class Modifier {
        ...
        public static SetCurioAttributesFunction.Modifier deserialize(JsonObject object, JsonDeserializationContext context) {
            String s = GsonHelper.getAsString(object, "name");
            ResourceLocation resourcelocation = new ResourceLocation(GsonHelper.getAsString(object, "attribute"));
            Attribute attribute;
            ...
            if (attribute == null) {
                ...
                AttributeModifier.Operation operation = operationFromString(GsonHelper.getAsString(object, "operation"));
                NumberProvider numberprovider = GsonHelper.getAsObject(object, "amount", context, NumberProvider.class);
                UUID uuid = null;
                String[] slots;
                ...
                return new SetCurioAttributesFunction.Modifier(s, attribute, operation, numberprovider, slots, uuid);
            }
        }
    ...
    public static class Serializer extends LootItemConditionalFunction.Serializer<SetCurioAttributesFunction> {
        ...
        public void serialize(@Nonnull JsonObject object, @Nonnull SetCurioAttributesFunction function, @Nonnull JsonSerializationContext context) {
            ...
            JsonArray jsonarray = new JsonArray();
            for (SetCurioAttributesFunction.Modifier modifier : function.modifiers) {
        ...
        public SetCurioAttributesFunction deserialize(@Nonnull JsonObject object, @Nonnull JsonDeserializationContext context, @Nonnull LootItemCondition[] conditions) {
            ...
            List<SetCurioAttributesFunction.Modifier> list = Lists.newArrayListWithExpectedSize(jsonarray.size());
            for (JsonElement jsonelement : jsonarray) {
                list.add(SetCurioAttributesFunction.Modifier.deserialize(GsonHelper.convertToJsonObject(jsonelement, "modifier"), context));
            }
            if (list.isEmpty()) {
