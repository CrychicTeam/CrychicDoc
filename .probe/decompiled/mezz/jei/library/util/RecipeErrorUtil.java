...
public final class RecipeErrorUtil {
    ...
    private static <T> String getIngredientInfo(IIngredientType<T> ingredientType, RecipeIngredientRole role, IIngredientSupplier ingredients, IIngredientManager ingredientManager) {
        ...
        return truncatedStream(stringStream, ingredientList.size(), 10).toList().toString();
    }

    public static String getNameForRecipe(Object recipe) {
        IPlatformRecipeHelper recipeHelper = Services.PLATFORM.getRecipeHelper();
        return (String) Optional.of(recipe).filter(Recipe.class::isInstance).map(Recipe.class::cast).flatMap(recipeHelper::getRegistryNameForRecipe).map(registryName -> {
            ...
            try {
                ...
                LOGGER.error("Failed recipe.toString", var2);
    ...
    private static Stream<String> truncatedStream(Stream<String> stream, int size, int limit) {
        return size + 1 > limit ? Stream.concat(stream.limit((long) limit), Stream.of(String.format("<truncated to %s elements, skipped %s>", limit, size - limit))) : stream;
    }
}