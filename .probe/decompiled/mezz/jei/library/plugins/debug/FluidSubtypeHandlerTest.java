...
public class FluidSubtypeHandlerTest<T> implements IIngredientSubtypeInterpreter<T> {
    ...
    public String apply(T fluidStack, UidContext context) {
        Fluid fluid = this.type.getBase(fluidStack);
        return (String) Services.PLATFORM.getRegistry(Registries.FLUID).getRegistryName(fluid).map(ResourceLocation::toString).orElseThrow(() -> new IllegalArgumentException("Fluid has no registry name: " + fluid));
    }
}