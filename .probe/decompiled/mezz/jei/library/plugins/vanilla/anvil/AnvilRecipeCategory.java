...
public class AnvilRecipeCategory implements IRecipeCategory<IJeiAnvilRecipe> {
    ...
    public AnvilRecipeCategory(IGuiHelper guiHelper) {
        ...
        this.icon = guiHelper.createDrawableItemStack(new ItemStack(Blocks.ANVIL));
    }
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, IJeiAnvilRecipe recipe, IFocusGroup focuses) {
        ...
        IRecipeSlotBuilder rightInputSlot = builder.addSlot(RecipeIngredientRole.INPUT, 37, 1).addItemStacks(rightInputs).setSlotName("rightSlot");
        IRecipeSlotBuilder outputSlot = builder.addSlot(RecipeIngredientRole.OUTPUT, 91, 1).addItemStacks(outputs);
    ...
    public void draw(IJeiAnvilRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY) {
        ...
        if (!leftStack.isEmpty() && !rightStack.isEmpty()) {
            ...
            int mainColor = playerHasEnoughLevels(player, cost) ? -8323296 : -40864;
    ...
    private static boolean playerHasEnoughLevels(@Nullable LocalPlayer player, int cost) {
        if (player == null) {
            ...
            return player.m_7500_() ? true : cost < 40 && cost <= player.f_36078_;
        }
    }
