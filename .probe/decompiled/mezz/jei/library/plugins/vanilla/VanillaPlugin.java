...
public class VanillaPlugin implements IModPlugin {
        ...
        registration.registerSubtypeInterpreter(Items.ENCHANTED_BOOK, (itemStack, context) -> {
            ...
            for (int i = 0; i < enchantments.size(); i++) {
                ...
                if (resourceLocation != null) {
                    enchantmentRegistry.getValue(resourceLocation).map(enchantment -> enchantment.getDescriptionId() + ".lvl" + compoundnbt.getShort("lvl")).ifPresent(enchantmentNames::add);
                }
            }
    ...
    public void registerCategories(IRecipeCategoryRegistration registration) {
        ...
        registration.addRecipeCategories(this.craftingCategory = new CraftingRecipeCategory(guiHelper), this.stonecuttingCategory = new StoneCuttingRecipeCategory(guiHelper), this.furnaceCategory = new FurnaceSmeltingCategory(guiHelper), this.smokingCategory = new SmokingCategory(guiHelper), this.blastingCategory = new BlastingCategory(guiHelper), this.campfireCategory = new CampfireCookingCategory(guiHelper), this.smithingCategory = new SmithingRecipeCategory(guiHelper), new CompostableRecipeCategory(guiHelper), new FurnaceFuelCategory(guiHelper, textures), new BrewingRecipeCategory(guiHelper), new AnvilRecipeCategory(guiHelper));
    }
    ...
    public void registerRecipes(IRecipeRegistration registration) {
        ...
        List<CraftingRecipe> handledCraftingRecipes = (List<CraftingRecipe>) craftingRecipes.get(true);
        List<CraftingRecipe> unhandledCraftingRecipes = (List<CraftingRecipe>) craftingRecipes.get(false);
    ...
    private static List<CraftingRecipe> replaceSpecialCraftingRecipes(List<CraftingRecipe> unhandledCraftingRecipes, IStackHelper stackHelper) {
        ...
        replacers.put(TippedArrowRecipe.class, (Supplier) () -> TippedArrowRecipeMaker.createRecipes(stackHelper));
        ...
        return unhandledCraftingRecipes.stream().map(Object::getClass).distinct().filter(replacers::containsKey).limit((long) replacers.size()).flatMap(recipeClass -> {
            Supplier<List<CraftingRecipe>> supplier = (Supplier<List<CraftingRecipe>>) replacers.get(recipeClass);
            try {
                List<CraftingRecipe> results = (List<CraftingRecipe>) supplier.get();
                return results.stream();
            } catch (RuntimeException var4) {
                LOGGER.error("Failed to create JEI recipes for {}", recipeClass, var4);
                return Stream.of();
            }
        }).toList();
    }
