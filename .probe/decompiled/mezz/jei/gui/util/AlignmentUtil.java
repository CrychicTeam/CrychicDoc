    ...
    public static ImmutableRect2i align(ImmutableSize2i size, ImmutableRect2i availableArea, HorizontalAlignment horizontalAlignment, VerticalAlignment verticalAlignment) {
        ...
        int x = switch(horizontalAlignment) {
            case LEFT ->
                availableArea.getX();
            case CENTER ->
                availableArea.getX() + (availableArea.getWidth() - width) / 2;
            case RIGHT ->
                availableArea.getX() + (availableArea.getWidth() - width);
        };
        ...
        int y = switch(verticalAlignment) {
            case TOP ->
                availableArea.getY();
            case CENTER ->
                availableArea.getY() + (availableArea.getHeight() - height) / 2;
            case BOTTOM ->
                availableArea.getY() + (availableArea.getHeight() - height);
        };
