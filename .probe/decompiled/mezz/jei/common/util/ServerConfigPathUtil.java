...
public final class ServerConfigPathUtil {
    ...
    public static Optional<Path> getWorldPath(Path basePath) {
        return getWorldPath().map(basePath::resolve);
    }
    ...
    private static Optional<Path> getWorldPath() {
        ...
        return Optional.ofNullable(minecraft.getConnection()).flatMap(clientPacketListener -> {
            ...
            return connection.isMemoryConnection() ? Optional.ofNullable(minecraft.getSingleplayerServer()).flatMap(minecraftServer -> reflectionUtil.getFieldWithClass(minecraftServer, LevelStorageSource.LevelStorageAccess.class).findFirst().map(LevelStorageSource.LevelStorageAccess::m_78277_).map(PathUtil::sanitizePathName).map(name -> worldDirPath.resolve("local").resolve(name))) : Optional.ofNullable(minecraft.getCurrentServer()).map(serverData -> {
