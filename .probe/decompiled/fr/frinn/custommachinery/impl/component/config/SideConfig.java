...
public class SideConfig {
    ...
    private Direction facing() {
        return (Direction) this.component.getManager().getTile().m_58900_().m_61143_(BlockStateProperties.HORIZONTAL_FACING);
    }
    ...
    public SideMode getSideMode(RelativeSide side) {
        return (SideMode) this.sides.get(side);
    }
    ...
    public static class Template {
        ...
        public static final NamedCodec<SideConfig.Template> CODEC = NamedCodec.record(templateInstance -> templateInstance.group(EnumMapCodec.of(RelativeSide.class, SideMode.CODEC, SideMode.BOTH).forGetter(template -> template.sides), NamedCodec.BOOL.optionalFieldOf("input", false).forGetter(template -> template.autoInput), NamedCodec.BOOL.optionalFieldOf("output", false).forGetter(template -> template.autoOutput), NamedCodec.BOOL.optionalFieldOf("enabled", true).forGetter(template -> template.enabled)).apply(templateInstance, SideConfig.Template::new), "Side Config Template");

        public static final SideConfig.Template DEFAULT_ALL_BOTH = makeDefault(SideMode.BOTH, true);

        public static final SideConfig.Template DEFAULT_ALL_INPUT = makeDefault(SideMode.INPUT, true);

        public static final SideConfig.Template DEFAULT_ALL_OUTPUT = makeDefault(SideMode.OUTPUT, true);

        public static final SideConfig.Template DEFAULT_ALL_NONE = makeDefault(SideMode.NONE, true);

        public static final SideConfig.Template DEFAULT_ALL_NONE_DISABLED = makeDefault(SideMode.NONE, false);
        ...
        private static SideConfig.Template makeDefault(SideMode defaultMode, boolean enabled) {
            EnumMap<RelativeSide, SideMode> map = Maps.newEnumMap(RelativeSide.class);
            for (RelativeSide side : RelativeSide.values()) {
                map.put(side, defaultMode);
            }
            return new SideConfig.Template(map, false, false, enabled);
        }
