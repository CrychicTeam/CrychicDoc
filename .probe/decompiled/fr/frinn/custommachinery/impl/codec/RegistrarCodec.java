...
public class RegistrarCodec<V> implements NamedCodec<V> {
    ...
    public static final NamedCodec<ResourceLocation> CM_LOC_CODEC = NamedCodec.STRING.comapFlatMap(s -> {
        try {
            return s.contains(":") ? DataResult.success(new ResourceLocation(s)) : DataResult.success(new ResourceLocation(ICustomMachineryAPI.INSTANCE.modid(), s));
    ...
    public <T> DataResult<Pair<V, T>> decode(DynamicOps<T> ops, T input) {
        return (this.isCM ? CM_LOC_CODEC : DefaultCodecs.RESOURCE_LOCATION).decode(ops, input).flatMap(keyValuePair -> !this.registrar.contains((ResourceLocation) keyValuePair.getFirst()) ? DataResult.error(() -> "Unknown registry key in " + this.registrar.key() + ": " + keyValuePair.getFirst()) : DataResult.success(keyValuePair.mapFirst(this.registrar::get)));
    }
