...
public class SidedEnergyStorage implements IEnergyStorage {
    ...
    public int receiveEnergy(int maxReceive, boolean simulate) {
        return ((SideMode) this.mode.get()).isInput() ? Utils.toInt(this.component.receiveEnergy((long) maxReceive, simulate)) : 0;
    }

    @Override
    public int extractEnergy(int maxExtract, boolean simulate) {
        return ((SideMode) this.mode.get()).isOutput() ? Utils.toInt(this.component.extractEnergy((long) maxExtract, simulate)) : 0;
    }
    ...
    public boolean canExtract() {
        return ((SideMode) this.mode.get()).isOutput();
    }

    @Override
    public boolean canReceive() {
        return ((SideMode) this.mode.get()).isInput();
    }
}