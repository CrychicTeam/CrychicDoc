...
public class FuelItemIngredientWrapper implements IJEIIngredientWrapper<ItemStack> {
    ...
    public boolean setupRecipe(IRecipeLayoutBuilder builder, int xOffset, int yOffset, IGuiElement element, IRecipeHelper helper) {
        if (element instanceof SlotGuiElement slotElement && element.getType() == Registration.SLOT_GUI_ELEMENT.get()) {
            return (Boolean) helper.getComponentForElement(slotElement).map(t -> {
                if (t instanceof ItemMachineComponent.Template template && template.getVariant() == FuelItemComponentVariant.INSTANCE) {
                    ...
                    builder.addSlot(RecipeIngredientRole.INPUT, element.getX() - xOffset + 1, element.getY() - yOffset + 1).addIngredients(VanillaTypes.ITEM_STACK, ingredients).addTooltipCallback((view, tooltips) -> view.getDisplayedIngredient(VanillaTypes.ITEM_STACK).ifPresent(stack -> tooltips.add(Component.translatable("custommachinery.jei.ingredient.fuel.burntime", FuelRegistry.get(stack)).withStyle(ChatFormatting.GRAY))));
                    return true;
                }
                return false;
            }).orElse(false);
