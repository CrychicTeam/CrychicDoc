...
public class FluidIngredientWrapper implements IJEIIngredientWrapper<FluidStack> {
    ...
    public boolean setupRecipe(IRecipeLayoutBuilder builder, int xOffset, int yOffset, IGuiElement element, IRecipeHelper helper) {
        if (element instanceof FluidGuiElement fluidElement && element.getType() == Registration.FLUID_GUI_ELEMENT.get()) {
            ...
            if (!fluidElement.getComponentId().equals(this.tank) && !(Boolean) template.map(t -> t.canAccept(ingredients, this.mode == RequirementIOMode.INPUT, helper.getDummyManager()) && (this.tank.isEmpty() || t.getId().equals(this.tank))).orElse(false)) {
                return false;
            }
            IRecipeSlotBuilder slot = builder.addSlot(this.roleFromMode(this.mode), element.getX() - xOffset + 1, element.getY() - yOffset + 1).setFluidRenderer(this.amount, false, element.getWidth() - 2, element.getHeight() - 2).addTooltipCallback((view, tooltips) -> {
                if (this.isPerTick) {
                ...
                if (this.chance == 0.0) {
                    ...
                    tooltips.add(Component.translatable("custommachinery.jei.ingredient.chance", (int) (this.chance * 100.0)));
                }
