...
public class ItemFilterIngredientWrapper implements IJEIIngredientWrapper<ItemStack> {
    ...
    public boolean setupRecipe(IRecipeLayoutBuilder builder, int xOffset, int yOffset, IGuiElement element, IRecipeHelper helper) {
        if (element instanceof SlotGuiElement slotElement && element.getType() == Registration.SLOT_GUI_ELEMENT.get()) {
            ...
            if (!slotElement.getComponentId().equals(this.slot) && !(Boolean) template.map(t -> {
                if (t instanceof ItemMachineComponent.Template itemComponentTemplate && itemComponentTemplate.getVariant() == FilterItemComponentVariant.INSTANCE && (this.slot.isEmpty() || t.getId().equals(this.slot))) {
                    return true;
                }
                return false;
            }).orElse(false)) {
            ...
            builder.addSlot(RecipeIngredientRole.INPUT, slotX - xOffset, slotY - yOffset).addIngredients(VanillaTypes.ITEM_STACK, ingredients);
            return true;
        }
        return false;
    }
