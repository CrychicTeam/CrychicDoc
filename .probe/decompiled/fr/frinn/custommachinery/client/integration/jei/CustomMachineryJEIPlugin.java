...
public class CustomMachineryJEIPlugin implements IModPlugin {
    ...
    public void registerItemSubtypes(ISubtypeRegistration registration) {
        registration.useNbtForSubtypes((Item) Registration.CUSTOM_MACHINE_ITEM.get());
    }
    ...
    public void registerRecipes(IRecipeRegistration registry) {
        if (Minecraft.getInstance().level != null) {
            Map<ResourceLocation, List<CustomMachineRecipe>> machineRecipes = (Map<ResourceLocation, List<CustomMachineRecipe>>) Minecraft.getInstance().level.getRecipeManager().getAllRecipesFor((net.minecraft.world.item.crafting.RecipeType) Registration.CUSTOM_MACHINE_RECIPE.get()).stream().filter(CustomMachineRecipe::showInJei).sorted(Comparators.JEI_PRIORITY_COMPARATOR.reversed()).collect(Collectors.groupingBy(CustomMachineRecipe::getMachineId));
            ...
            Map<ResourceLocation, List<CustomCraftRecipe>> craftRecipes = (Map<ResourceLocation, List<CustomCraftRecipe>>) Minecraft.getInstance().level.getRecipeManager().getAllRecipesFor((net.minecraft.world.item.crafting.RecipeType) Registration.CUSTOM_CRAFT_RECIPE.get()).stream().filter(CustomCraftRecipe::showInJei).sorted(Comparators.JEI_PRIORITY_COMPARATOR.reversed()).collect(Collectors.groupingBy(CustomCraftRecipe::getMachineId));
    ...
    public void registerGuiHandlers(IGuiHandlerRegistration registration) {
        registration.addGuiContainerHandler(CustomMachineScreen.class, new IGuiContainerHandler<CustomMachineScreen>() {
            ...
            public List<Rect2i> getGuiExtraAreas(CustomMachineScreen containerScreen) {
                return Minecraft.getInstance().screen instanceof MachineConfigScreen screen ? screen.popups().stream().map(popup -> new Rect2i(popup.x, popup.y, popup.xSize, popup.ySize)).toList() : Collections.emptyList();
            }
            ...
            public Collection<IGuiClickableArea> getGuiClickableAreas(CustomMachineScreen screen, double mouseX, double mouseY) {
                ...
                return elements.stream().filter(element -> element instanceof ProgressBarGuiElement).map(element -> {
                    ...
                    return CustomMachineryJEIPlugin.createBasic(posX, posY, width, height, screen.getMachine().getId(), progress.getTooltips().isEmpty());
                }).toList();
            }
        });
        registration.addGhostIngredientHandler(CustomMachineScreen.class, new IGhostIngredientHandler<CustomMachineScreen>() {

            public <I> List<IGhostIngredientHandler.Target<I>> getTargetsTyped(CustomMachineScreen screen, ITypedIngredient<I> ingredient, boolean doStart) {
                return ingredient.getIngredient() instanceof ItemStack stack ? (List) ((CustomMachineContainer) screen.m_6262_()).f_38839_.stream().filter(slot -> slot instanceof FilterSlotItemComponent).map(slot -> {
                    final FilterSlotItemComponent filterSlot = (FilterSlotItemComponent) slot;
