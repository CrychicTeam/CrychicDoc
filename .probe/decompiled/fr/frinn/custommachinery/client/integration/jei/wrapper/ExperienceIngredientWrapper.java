...
public class ExperienceIngredientWrapper implements IJEIIngredientWrapper<Experience> {
    ...
    public boolean setupRecipe(IRecipeLayoutBuilder builder, int xOffset, int yOffset, IGuiElement element, IRecipeHelper helper) {
        if (element instanceof ExperienceGuiElement experienceElement && element.getType() == Registration.EXPERIENCE_GUI_ELEMENT.get() && experienceElement.getMode().isDisplay()) {
            builder.addSlot(this.roleFromMode(this.mode), element.getX() - xOffset + 1, element.getY() - yOffset + 1).setCustomRenderer(CustomIngredientTypes.EXPERIENCE, new ExperienceJEIIngredientRenderer(experienceElement)).addIngredient(CustomIngredientTypes.EXPERIENCE, this.experience).addTooltipCallback((recipeSlotView, tooltip) -> {
                ...
                if (this.experience.isPoints()) {
                    ...
                    } else if (this.mode == RequirementIOMode.INPUT) {
