...
public class GroupWidget extends AbstractWidget implements ContainerEventHandler {
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double delta) {
        for (AbstractWidget children : this.children) {
            if (children.m_6050_(mouseX, mouseY, delta)) {
                return true;
            }
        }
        return false;
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double dragX, double dragY) {
        return this.focused != null ? this.focused.mouseDragged(mouseX, mouseY, button, dragX, dragY) : false;
    }

    @Override
    public void mouseMoved(double mouseX, double mouseY) {
        this.children.forEach(children -> children.m_94757_(mouseX, mouseY));
    }

    @Override
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        for (AbstractWidget children : this.children) {
            if (children.m_7933_(keyCode, scanCode, modifiers)) {
                return true;
            }
        }
        return false;
    ...
    public boolean keyReleased(int keyCode, int scanCode, int modifiers) {
        for (AbstractWidget children : this.children) {
            if (children.m_7920_(keyCode, scanCode, modifiers)) {
                return true;
            }
        }
        return false;
    ...
    public boolean charTyped(char codePoint, int modifiers) {
        for (AbstractWidget children : this.children) {
            if (children.m_5534_(codePoint, modifiers)) {
                return true;
            }
        }
        return false;
