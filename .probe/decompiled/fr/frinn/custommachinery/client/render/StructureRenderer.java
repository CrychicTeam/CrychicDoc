...
public class StructureRenderer {
    ...
    public void render(PoseStack matrix, MultiBufferSource buffer, Direction direction, Level world, BlockPos machinePos) {
        Map<BlockPos, IIngredient<PartialBlockState>> blocks = (Map<BlockPos, IIngredient<PartialBlockState>>) this.blocksGetter.apply(direction);
        this.timer.onDraw();
        blocks.forEach((pos, ingredient) -> {
            matrix.pushPose();
            matrix.translate((float) pos.m_123341_(), (float) pos.m_123342_(), (float) pos.m_123343_());
            if ((pos.m_123341_() != 0 || pos.m_123342_() != 0 || pos.m_123343_() != 0) && ingredient != BlockIngredient.ANY) {
                PartialBlockState state = this.timer.get(ingredient.getAll());
                BlockPos blockPos = machinePos.offset(pos);
                if (state != null && state != PartialBlockState.ANY && !state.getBlockState().m_60795_()) {
                    if (world.getBlockState(blockPos).m_60795_()) {
    ...
    private void renderTransparentBlock(PartialBlockState state, PoseStack matrix, MultiBufferSource buffer) {
        ...
        if (model != Minecraft.getInstance().getModelManager().getMissingModel()) {
            Arrays.stream(Direction.values()).flatMap(direction -> model.getQuads(state.getBlockState(), direction, RandomSource.create(42L)).stream()).forEach(quad -> builder.putBulkData(matrix.last(), quad, new float[] { 1.0F, 1.0F, 1.0F, 0.8F }, 1.0F, 1.0F, 1.0F, light, OverlayTexture.NO_OVERLAY, false));
            model.getQuads(state.getBlockState(), null, RandomSource.create(42L)).forEach(quad -> builder.putBulkData(matrix.last(), quad, new float[] { 1.0F, 1.0F, 1.0F, 0.8F }, 1.0F, 1.0F, 1.0F, light, OverlayTexture.NO_OVERLAY, false));
        }
    }
    ...
    private void renderNope(PoseStack matrix, MultiBufferSource buffer) {
        ...
        BakedModel model = (BakedModel) Minecraft.getInstance().getModelManager().bakedRegistry.getOrDefault(new ResourceLocation("custommachinery", "block/nope"), Minecraft.getInstance().getModelManager().getMissingModel());
        matrix.translate(-5.0E-4, -5.0E-4, -5.0E-4);
        matrix.scale(1.001F, 1.001F, 1.001F);
        int[] light = new int[4];
        Arrays.fill(light, LightTexture.pack(15, 15));
        Arrays.stream(Direction.values()).flatMap(direction -> model.getQuads(null, direction, RandomSource.create(42L)).stream()).forEach(quad -> builder.putBulkData(matrix.last(), quad, new float[] { 1.0F, 1.0F, 1.0F, 0.8F }, 1.0F, 1.0F, 1.0F, light, OverlayTexture.NO_OVERLAY, false));
        model.getQuads(null, null, RandomSource.create(42L)).forEach(quad -> builder.putBulkData(matrix.last(), quad, new float[] { 1.0F, 1.0F, 1.0F, 0.8F }, 1.0F, 1.0F, 1.0F, light, OverlayTexture.NO_OVERLAY, false));
    }
