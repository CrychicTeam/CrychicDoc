...
@ZenRegister
@Name("mods.custommachinery.requirement.Durability")
public interface DurabilityRequirementCT<T> extends RecipeCTBuilder<T> {
    ...
    default T damageItemTag(MCTag tag, int amount, @Optional IData data, @OptionalString String slot) {
        try {
            return this.addRequirement(new DurabilityRequirement(RequirementIOMode.INPUT, ItemTagIngredient.create(tag.getTagKey()), amount, CTUtils.getNBT(data), true, slot));
    ...
    default T damageItemTagNoBreak(MCTag tag, int amount, @Optional IData data, @OptionalString String slot) {
        try {
            return this.addRequirement(new DurabilityRequirement(RequirementIOMode.INPUT, ItemTagIngredient.create(tag.getTagKey()), amount, CTUtils.getNBT(data), false, slot));
        } catch (IllegalArgumentException var6) {
            return this.error(var6.getMessage(), new Object[0]);
        }
    }
    ...
    default T repairItemTag(MCTag tag, int amount, @Optional IData data, @OptionalString String slot) {
        try {
            return this.addRequirement(new DurabilityRequirement(RequirementIOMode.OUTPUT, ItemTagIngredient.create(tag.getTagKey()), amount, CTUtils.getNBT(data), false, slot));
        } catch (IllegalArgumentException var6) {
            return this.error(var6.getMessage(), new Object[0]);
        }
    }
