...
public class MachineCT {
    ...
    public FluidStack getFluidStored(String tank) {
        return (FluidStack) this.internal.getComponentManager().getComponentHandler((MachineComponentType) Registration.FLUID_MACHINE_COMPONENT.get()).flatMap(handler -> handler.getComponentForID(tank)).map(FluidMachineComponent::getFluidStack).orElse(FluidStack.empty());
    }

    @Method
    public void setFluidStored(String tank, Fluid fluid, long amount, @Optional IData data) {
        this.internal.getComponentManager().getComponentHandler((MachineComponentType) Registration.FLUID_MACHINE_COMPONENT.get()).flatMap(handler -> handler.getComponentForID(tank)).ifPresent(x -> x.setFluidStack(FluidStack.create(fluid, amount, data == null ? null : (CompoundTag) data.getInternal())));
    }
    ...
    public long addFluid(Fluid fluid, long amount, boolean simulate) {
        return (Long) this.internal.getComponentManager().getComponentHandler((MachineComponentType) Registration.FLUID_MACHINE_COMPONENT.get()).map(handler -> (FluidComponentHandler) handler).map(handler -> handler.fill(FluidStack.create(fluid, amount), simulate)).orElse(amount);
    }

    @Method
    public long addFluidToTank(String tank, Fluid fluid, long amount, boolean simulate, @Optional IData data) {
        return (Long) this.internal.getComponentManager().getComponentHandler((MachineComponentType) Registration.FLUID_MACHINE_COMPONENT.get()).flatMap(handler -> handler.getComponentForID(tank)).map(component -> component.insert(fluid, amount, data == null ? null : (CompoundTag) data.getInternal(), simulate)).orElse(amount);
    }

    @Method
    public FluidStack removeFluid(Fluid fluid, int amount, boolean simulate, @Optional IData data) {
        return (FluidStack) this.internal.getComponentManager().getComponentHandler((MachineComponentType) Registration.FLUID_MACHINE_COMPONENT.get()).map(handler -> (FluidComponentHandler) handler).map(handler -> handler.drain(FluidStack.create(fluid, (long) amount, data == null ? null : (CompoundTag) data.getInternal()), simulate)).orElse(FluidStack.empty());
    }

    @Method
    public FluidStack removeFluidFromTank(String tank, int amount, boolean simulate) {
        return (FluidStack) this.internal.getComponentManager().getComponentHandler((MachineComponentType) Registration.FLUID_MACHINE_COMPONENT.get()).flatMap(handler -> handler.getComponentForID(tank)).map(component -> component.extract((long) amount, simulate)).orElse(FluidStack.empty());
    }

    @Method
    public IItemStack getItemStored(String slot) {
        return (IItemStack) this.internal.getComponentManager().getComponentHandler((MachineComponentType) Registration.ITEM_MACHINE_COMPONENT.get()).flatMap(handler -> handler.getComponentForID(slot)).map(component -> Services.PLATFORM.createItemStack(component.getItemStack())).orElse(Services.PLATFORM.createItemStack(ItemStack.EMPTY));
    }
    ...
    public IItemStack addItemToSlot(String slot, IItemStack stackCT, boolean simulate) {
        return (IItemStack) this.internal.getComponentManager().getComponentHandler((MachineComponentType) Registration.ITEM_MACHINE_COMPONENT.get()).flatMap(handler -> handler.getComponentForID(slot)).map(component -> {
    ...
    public IItemStack removeItemFromSlot(String slot, int toRemove, boolean simulate) {
        return (IItemStack) this.internal.getComponentManager().getComponentHandler((MachineComponentType) Registration.ITEM_MACHINE_COMPONENT.get()).flatMap(handler -> handler.getComponentForID(slot)).map(component -> Services.PLATFORM.createItemStack(component.extract(toRemove, simulate, true))).orElse(Services.PLATFORM.createItemStack(ItemStack.EMPTY));
    }
