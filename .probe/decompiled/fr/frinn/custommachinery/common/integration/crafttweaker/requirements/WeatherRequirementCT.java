...
@ZenRegister
@Name("mods.custommachinery.requirement.Weather")
public interface WeatherRequirementCT<T> extends RecipeCTBuilder<T> {

    @Method
    default T requireWeather(String weatherType) {
        return this.addRequirement(new WeatherRequirement(WeatherMachineComponent.WeatherType.value(weatherType), false));
    }

    @Method
    default T requireWeatherOnMachine(String weatherType) {
        return this.addRequirement(new WeatherRequirement(WeatherMachineComponent.WeatherType.value(weatherType), true));
    }
}