...
public class CustomMachineRecipeSerializer implements RecipeSerializer<CustomMachineRecipe> {
    ...
    public CustomMachineRecipe fromJson(ResourceLocation recipeId, JsonObject json) {
        ...
        if (result.result().isPresent()) {
            ...
            throw new JsonParseException("Error while parsing Custom Machine Recipe json: " + recipeId + " error: " + ((PartialResult) result.error().get()).message());
        } else {
    ...
    public CustomMachineRecipe fromNetwork(ResourceLocation recipeId, FriendlyByteBuf buffer) {
        ...
        if (result.result().isPresent()) {
            ...
            ICustomMachineryAPI.INSTANCE.logger().error("Error while parsing recipe json: {}, skipping...\n{}", recipeId, ((PartialResult) result.error().get()).message());
            throw new IllegalArgumentException("Error while receiving Custom Machine Recipe from server: " + recipeId + " error: " + ((PartialResult) result.error().get()).message());
        } else {
    ...
    public void toNetwork(FriendlyByteBuf buffer, CustomMachineRecipe recipe) {
        ...
        if (result.result().isPresent()) {
            ...
            ICustomMachineryAPI.INSTANCE.logger().error("Error while sending recipe: {} to clients.%n{}", recipe.getId(), ((PartialResult) result.error().get()).message());
            throw new IllegalArgumentException("Error while sending Custom Machine Recipe to clients: " + recipe.getId() + " error: " + ((PartialResult) result.error().get()).message());
        } else {
