...
public class MachineRecipeFinder {
    ...
    public void init() {
        if (this.tile.m_58904_() == null) {
            ...
            this.recipes = this.tile.m_58904_().getRecipeManager().getAllRecipesFor((RecipeType) Registration.CUSTOM_MACHINE_RECIPE.get()).stream().filter(recipe -> this.tile.getMachine().getRecipeIds().contains(recipe.getMachineId())).sorted(Comparators.RECIPE_PRIORITY_COMPARATOR.reversed()).map(CustomMachineRecipe::checker).toList();
            this.okToCheck = new ArrayList();
            this.recipeCheckCooldown = this.tile.m_58904_().random.nextInt(this.baseCooldown);
        }
    }
        ...
        if (this.tile.m_58904_() == null) {
            ...
            if (immediately || this.recipeCheckCooldown-- <= 0) {
                ...
                while (iterator.hasNext()) {
                    RecipeChecker<CustomMachineRecipe> checker = (RecipeChecker<CustomMachineRecipe>) iterator.next();
