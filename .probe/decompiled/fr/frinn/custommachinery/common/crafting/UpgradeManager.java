...
public class UpgradeManager implements IMachineUpgradeManager {
    ...
    private void refreshUpgrades() {
        ...
        this.activeModifiers.addAll(this.tile.getComponentManager().getComponentHandler((MachineComponentType) Registration.ITEM_MACHINE_COMPONENT.get()).stream().flatMap(handler -> handler.getComponents().stream()).filter(slot -> slot.getVariant() == UpgradeItemComponentVariant.INSTANCE && !slot.getItemStack().isEmpty()).flatMap(slot -> CustomMachinery.UPGRADES.getUpgradesForItemAndMachine(slot.getItemStack().getItem(), this.tile.getMachine().getId()).stream().flatMap(upgrade -> upgrade.getModifiers().stream().map(modifier -> Pair.of(modifier, Math.min(slot.getItemStack().getCount(), upgrade.getMaxAmount()))))).toList());
    }
