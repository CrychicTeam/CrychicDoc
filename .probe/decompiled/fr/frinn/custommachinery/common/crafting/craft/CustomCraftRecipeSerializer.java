...
public class CustomCraftRecipeSerializer implements RecipeSerializer<CustomCraftRecipe> {
    ...
    public CustomCraftRecipe fromJson(ResourceLocation recipeId, JsonObject json) {
        ...
        if (result.result().isPresent()) {
            ...
            ICustomMachineryAPI.INSTANCE.logger().error("Error while parsing craft recipe json: {}, skipping...\n{}", recipeId, ((PartialResult) result.error().get()).message());
            throw new JsonParseException("Error while parsing Custom Machine Craft Recipe json: " + recipeId + " error: " + ((PartialResult) result.error().get()).message());
        } else {
    ...
    public CustomCraftRecipe fromNetwork(ResourceLocation recipeId, FriendlyByteBuf buffer) {
        ...
        if (result.result().isPresent()) {
            ...
            ICustomMachineryAPI.INSTANCE.logger().error("Error while parsing craft recipe json: {}, skipping...\n{}", recipeId, ((PartialResult) result.error().get()).message());
            throw new IllegalArgumentException("Error while receiving Custom Machine Craft Recipe from server: " + recipeId + " error: " + ((PartialResult) result.error().get()).message());
        } else {
    ...
    public void toNetwork(FriendlyByteBuf buffer, CustomCraftRecipe recipe) {
        ...
        if (result.result().isPresent()) {
            ...
            ICustomMachineryAPI.INSTANCE.logger().error("Error while sending craft recipe: {} to clients.%n{}", recipe.getId(), ((PartialResult) result.error().get()).message());
            throw new IllegalArgumentException("Error while sending Custom Machine Craft Recipe to clients: " + recipe.getId() + " error: " + ((PartialResult) result.error().get()).message());
        } else {
