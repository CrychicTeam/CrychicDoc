...
public class CraftRecipeFinder {
    ...
    public void init() {
        if (this.tile.m_58904_() == null) {
            ...
            this.recipes = this.tile.m_58904_().getRecipeManager().getAllRecipesFor((RecipeType) Registration.CUSTOM_CRAFT_RECIPE.get()).stream().filter(recipe -> this.tile.getMachine().getRecipeIds().contains(recipe.getMachineId())).sorted(Comparators.RECIPE_PRIORITY_COMPARATOR.reversed()).map(CustomCraftRecipe::checker).toList();
        ...
        if (this.tile.m_58904_() == null) {
            ...
            if (immediately || this.recipeCheckCooldown-- <= 0) {
                ...
                while (iterator.hasNext()) {
                    RecipeChecker<CustomCraftRecipe> checker = (RecipeChecker<CustomCraftRecipe>) iterator.next();
