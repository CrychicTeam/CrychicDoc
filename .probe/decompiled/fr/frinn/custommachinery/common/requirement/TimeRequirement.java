...
public class TimeRequirement extends AbstractRequirement<TimeMachineComponent> implements IDisplayInfoRequirement {
    ...
    public static final NamedCodec<TimeRequirement> CODEC = NamedCodec.record(timeRequirementInstance -> timeRequirementInstance.group(IntRange.CODEC.fieldOf("range").forGetter(requirement -> requirement.range)).apply(timeRequirementInstance, TimeRequirement::new), "Time requirement");
    ...
    public RequirementType<TimeRequirement> getType() {
        return (RequirementType<TimeRequirement>) Registration.TIME_REQUIREMENT.get();
    }

    public boolean test(TimeMachineComponent component, ICraftingContext context) {
        return this.range.contains(Integer.valueOf((int) component.getTime()));
    }

    public CraftingResult processStart(TimeMachineComponent component, ICraftingContext context) {
        return this.test(component, context) ? CraftingResult.success() : CraftingResult.error(Component.translatable("custommachinery.requirements.time.error"));
    }
