...
public class RedstoneRequirement extends AbstractRequirement<RedstoneMachineComponent> implements ITickableRequirement<RedstoneMachineComponent>, IDisplayInfoRequirement {
    ...
    public static final NamedCodec<RedstoneRequirement> CODEC = NamedCodec.record(redstoneRequirementInstance -> redstoneRequirementInstance.group(IntRange.CODEC.fieldOf("power").forGetter(requirement -> requirement.powerLevel)).apply(redstoneRequirementInstance, RedstoneRequirement::new), "Redstone requirement");
    ...
    public RequirementType<RedstoneRequirement> getType() {
        return (RequirementType<RedstoneRequirement>) Registration.REDSTONE_REQUIREMENT.get();
    }

    public boolean test(RedstoneMachineComponent component, ICraftingContext context) {
        return this.powerLevel.contains(Integer.valueOf(component.getMachinePower()));
    }

    public CraftingResult processStart(RedstoneMachineComponent component, ICraftingContext context) {
        return this.test(component, context) ? CraftingResult.success() : CraftingResult.error(Component.translatable("custommachinery.requirements.redstone.error", this.powerLevel.toFormattedString()));
    }
    ...
    public void getDisplayInfo(IDisplayInfo info) {
        info.addTooltip(Component.translatable("custommachinery.requirements.redstone.info", this.powerLevel.toFormattedString())).setItemIcon(Items.REDSTONE);
    }
}