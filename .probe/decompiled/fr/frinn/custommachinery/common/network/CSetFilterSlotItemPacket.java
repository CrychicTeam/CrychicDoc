...
public class CSetFilterSlotItemPacket extends BaseC2SMessage {
    ...
    public void handle(NetworkManager.PacketContext context) {
        if (context.getEnvironment() == Env.SERVER) {
            context.queue(() -> {
                if (context.getPlayer().m_9236_().getBlockEntity(this.pos) instanceof CustomMachineTile machine) {
                    machine.getComponentManager().getComponentHandler((MachineComponentType) Registration.ITEM_MACHINE_COMPONENT.get()).flatMap(handler -> handler.getComponentForID(this.slotID)).ifPresent(component -> {
