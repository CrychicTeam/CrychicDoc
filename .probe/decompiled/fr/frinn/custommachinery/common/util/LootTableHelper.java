...
public class LootTableHelper {
    ...
    private static List<Pair<ItemStack, Double>> getLoots(ResourceLocation table, MinecraftServer server, LootContext context) {
        ...
        LootTable lootTable = server.getLootData().m_278676_(table);
        ...
        if (pools == null) {
            ...
            for (LootPool pool : pools) {
                ...
                float total = (float) Arrays.stream(entries).filter(entry -> entry instanceof LootPoolSingletonContainer).mapToInt(entry -> ((LootPoolSingletonContainer) entry).weight).sum();
                Arrays.stream(entries).filter(entry -> entry instanceof LootItem).map(entry -> (LootItem) entry).forEach(entry -> {
                    Consumer<ItemStack> consumer = stack -> loots.add(Pair.of(stack, (double) ((float) entry.f_79675_ / total)));
                    consumer = applyFunctions(consumer, entry.f_79677_, globalFunction, context);
                ...
                Arrays.stream(entries).filter(entry -> entry instanceof TagEntry).map(entry -> (TagEntry) entry).forEach(entry -> {
                    Consumer<ItemStack> consumer = stack -> loots.add(Pair.of(stack, (double) ((float) entry.f_79675_ / total / (float) (entry.expand ? TagUtil.getItems(entry.tag).count() : 1L))));
                    consumer = applyFunctions(consumer, entry.f_79677_, globalFunction, context);
                    entry.createItemStack(consumer, context);
                });
                Arrays.stream(entries).filter(entry -> entry instanceof LootTableReference).map(entry -> (LootTableReference) entry).map(entry -> getLoots(entry.name, server, context)).forEach(loots::addAll);
            }
            return loots;
        }
    }
    ...
    public static List<Pair<ItemStack, Double>> getLootsForTable(ResourceLocation table) {
        return (List<Pair<ItemStack, Double>>) lootsMap.getOrDefault(table, Collections.emptyList());
    }
}