...
public class FluidComponentHandler extends AbstractComponentHandler<FluidMachineComponent> implements ISerializableComponent, ISyncableStuff, ITickableComponent, IDumpComponent {
    ...
    public void deserialize(CompoundTag nbt) {
        if (nbt.contains("fluids", 9)) {
            ...
            componentsNBT.forEach(inbt -> {
                if (inbt instanceof CompoundTag componentNBT && componentNBT.contains("id", 8)) {
    ...
    public FluidStack drain(FluidStack maxDrain, boolean simulate) {
        ...
        return remainingToDrain == maxDrain.getAmount() ? FluidStack.empty() : FluidStack.create(maxDrain.getFluid(), maxDrain.getAmount() - remainingToDrain, maxDrain.getTag());
    }
    ...
    public long getSpaceForFluid(String tank, Fluid fluid, @Nullable CompoundTag nbt) {
        Predicate<FluidMachineComponent> tankPredicate = component -> tank.isEmpty() || component.getId().equals(tank);
        return this.outputs.stream().filter(component -> component.isFluidValid(FluidStack.create(fluid, 1L, nbt)) && tankPredicate.test(component)).mapToLong(FluidMachineComponent::getRecipeRemainingSpace).sum();
    }
    ...
    public void addToOutputs(String tank, FluidStack stack) {
        ...
        Predicate<FluidMachineComponent> tankPredicate = component -> tank.isEmpty() || component.getId().equals(tank);
        this.outputs.stream().filter(component -> component.isFluidValid(stack) && tankPredicate.test(component)).sorted(Comparator.comparingInt(component -> component.getFluidStack().isFluidEqual(stack) ? -1 : 1)).forEach(component -> {
