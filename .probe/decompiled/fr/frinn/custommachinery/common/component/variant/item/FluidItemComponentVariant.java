...
public class FluidItemComponentVariant extends ItemComponentVariant implements ITickableComponentVariant<ItemMachineComponent> {
    ...
    public static final NamedCodec<FluidItemComponentVariant> CODEC = NamedCodec.record(variantInstance -> variantInstance.group(NamedCodec.STRING.listOf().optionalFieldOf("tanks", Collections.emptyList()).forGetter(variant -> variant.tanks)).apply(variantInstance, FluidItemComponentVariant::new), "Fluid item component variant");

    public static final ResourceLocation ID = new ResourceLocation("custommachinery", "fluid");
    ...
    public void tick(ItemMachineComponent component) {
        ...
        if (PlatformHelper.fluid().isFluidHandler(stack)) {
            ...
            if (this.tanks.isEmpty()) {
                tanks.addAll((Collection) component.getManager().getComponentHandler((MachineComponentType) Registration.FLUID_MACHINE_COMPONENT.get()).map(IComponentHandler::getComponents).orElse(Collections.emptyList()));
            } else {
                for (String tank : this.tanks) {
                    component.getManager().getComponentHandler((MachineComponentType) Registration.FLUID_MACHINE_COMPONENT.get()).flatMap(handler -> handler.getComponentForID(tank)).ifPresent(tanks::add);
                }
            }
