...
public class CommandMachineComponent extends AbstractMachineComponent {
    ...
    public void sendCommand(String command, int permissionLevel, boolean log) {
        if (this.getManager().getLevel().getServer() != null) {
            CommandSourceStack source = new CommandSourceStack(log ? COMMAND_SOURCE_LOG : COMMAND_SOURCE_NO_LOG, Vec3.atCenterOf(this.getManager().getTile().m_58899_()), this.getMachineRotation(), (ServerLevel) this.getManager().getLevel(), permissionLevel, "Custom Machinery", this.getManager().getTile().getMachine().getName(), this.getManager().getServer(), null);
    ...
    public MachineComponentType<CommandMachineComponent> getType() {
        return (MachineComponentType<CommandMachineComponent>) Registration.COMMAND_MACHINE_COMPONENT.get();
    }

    private Vec2 getMachineRotation() {
        Direction facing = (Direction) this.getManager().getTile().m_58900_().m_61143_(BlockStateProperties.HORIZONTAL_FACING);
