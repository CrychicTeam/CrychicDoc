...
public class AdditionRecipeModifier extends RecipeModifier {
    ...
    public double apply(double original, int upgradeAmount) {
        return Mth.clamp(original + this.modifier * (double) upgradeAmount, this.min, this.max);
    }

    @Override
    public Component getDefaultTooltip() {
        String tooltip = (this.modifier >= 0.0 ? "+" : "") + this.modifier + " " + this.requirementType.getName().getString() + " " + Component.translatable(this.mode.getTranslationKey()).getString();
