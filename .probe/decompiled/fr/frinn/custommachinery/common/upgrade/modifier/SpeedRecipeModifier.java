...
public class SpeedRecipeModifier extends RecipeModifier {
    ...
    public double apply(double original, int upgradeAmount) {
        double modified = switch(this.operation) {
            case ADDITION ->
                original + this.modifier * (double) upgradeAmount;
            case MULTIPLICATION ->
                original * this.modifier * (double) upgradeAmount;
            case EXPONENTIAL ->
                original * Math.pow(this.modifier, (double) upgradeAmount);
        };
    ...
    public Component getDefaultTooltip() {
        ...
        String tooltip = (tooltipModifier >= 0.0 ? "+" : "") + (this.operation == IRecipeModifier.OPERATION.ADDITION ? tooltipModifier : tooltipModifier + "%") + " " + this.requirementType.getName().getString();
