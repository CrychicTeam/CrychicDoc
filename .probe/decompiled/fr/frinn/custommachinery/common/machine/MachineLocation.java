...
public class MachineLocation {
    ...
    public static final NamedCodec<MachineLocation> CODEC = NamedCodec.record(machineLocationInstance -> machineLocationInstance.group(DefaultCodecs.RESOURCE_LOCATION.fieldOf("id").forGetter(MachineLocation::getId), MachineLocation.Loader.CODEC.fieldOf("loader").forGetter(MachineLocation::getLoader), NamedCodec.STRING.fieldOf("packName").forGetter(MachineLocation::getPackName)).apply(machineLocationInstance, MachineLocation::new), "Machine location");

    private final ResourceLocation id;

    private final MachineLocation.Loader loader;

    private final String packName;

    private MachineLocation(ResourceLocation id, MachineLocation.Loader loader, String packName) {
        this.id = id;
        this.loader = loader;
        this.packName = packName;
    }

    public static MachineLocation fromLoader(MachineLocation.Loader loader, ResourceLocation id, String packName) {
        return switch(loader) {
            case DEFAULT ->
                fromDefault(id, packName);
            case DATAPACK ->
                fromDatapack(id, packName);
            case DATAPACK_ZIP ->
                fromDatapackZip(id, packName);
            case KUBEJS ->
                fromKubeJS(id, packName);
            case KUBEJS_SCRIPT ->
                fromKubeJSScript(id, packName);
        };
    }

    public static MachineLocation fromDefault(ResourceLocation id, String packName) {
        return new MachineLocation(id, MachineLocation.Loader.DEFAULT, packName);
    }
    ...
    public static MachineLocation fromDatapack(ResourceLocation id, String packName) {
        ...
        return new MachineLocation(id, MachineLocation.Loader.DATAPACK, packName);
    }

    public static MachineLocation fromDatapackZip(ResourceLocation id, String packName) {
        return new MachineLocation(id, MachineLocation.Loader.DATAPACK_ZIP, packName);
    }

    public static MachineLocation fromKubeJS(ResourceLocation id, String packName) {
        return new MachineLocation(id, MachineLocation.Loader.KUBEJS, packName);
    }

    public static MachineLocation fromKubeJSScript(ResourceLocation id, String packName) {
        return new MachineLocation(id, MachineLocation.Loader.KUBEJS_SCRIPT, packName);
    }
    ...
    public MachineLocation.Loader getLoader() {
        return this.loader;
    }
    ...
    public File getFile(MinecraftServer server) {
        ...
        return switch(this.loader) {
            case DATAPACK ->
                server.getWorldPath(LevelResource.DATAPACK_DIR).resolve(this.packName + File.separator + pathFromData).normalize().toFile();
            case KUBEJS ->
                new File(kubejsPath);
            default ->
                null;
    ...
    public static enum Loader {
        ...
        DEFAULT(false, ChatFormatting.BLACK), DATAPACK(true, ChatFormatting.DARK_GREEN), DATAPACK_ZIP(false, ChatFormatting.DARK_RED), KUBEJS(true, ChatFormatting.DARK_PURPLE), KUBEJS_SCRIPT(false, ChatFormatting.DARK_RED);

        public static final NamedCodec<MachineLocation.Loader> CODEC = NamedCodec.enumCodec(MachineLocation.Loader.class);
