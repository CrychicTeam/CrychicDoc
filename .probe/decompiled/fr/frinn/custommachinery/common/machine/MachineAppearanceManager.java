...
public class MachineAppearanceManager {
    ...
    public static final NamedCodec<MachineAppearanceManager> CODEC = NamedCodec.record(builder -> builder.group(MachineAppearance.CODEC.forGetter(manager -> manager.defaultProperties), MachineAppearance.CODEC.optionalFieldOf("idle", Maps.newHashMap()).forGetter(manager -> manager.getStatusProperties(MachineStatus.IDLE)), MachineAppearance.CODEC.optionalFieldOf("running", Maps.newHashMap()).forGetter(manager -> manager.getStatusProperties(MachineStatus.RUNNING)), MachineAppearance.CODEC.optionalFieldOf("errored", Maps.newHashMap()).forGetter(manager -> manager.getStatusProperties(MachineStatus.ERRORED)), MachineAppearance.CODEC.optionalFieldOf("paused", Maps.newHashMap()).forGetter(manager -> manager.getStatusProperties(MachineStatus.PAUSED))).apply(builder, (defaults, idle, running, errored, paused) -> {
    ...
    public Map<MachineAppearanceProperty<?>, Object> getStatusProperties(MachineStatus status) {
        ...
        Builder<MachineAppearanceProperty<?>, Object> map = ImmutableMap.builder();
