...
public class LightBulbScenes {
    ...
    public static void lightBulbScenes(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        Vec3 whiteBulbVec = util.vector.blockSurface(whiteBulbPos, Direction.DOWN).add(0.0, 0.75, 0.0);
        Vec3 greenBulbVec = util.vector.blockSurface(greenBulbPos, Direction.DOWN).add(0.0, 0.75, 0.0);
        Vec3 leverVec = util.vector.centerOf(leverPos).add(0.0, -0.25, 0.0);
        ...
        scene.idle(10);
        scene.overlay.showText(80).attachKeyFrame().text("Light Bulbs are an alternative light source to the Redstone Lamp").pointAt(whiteBulbVec).placeNearTarget();
        scene.idle(90);
        scene.overlay.showText(80).attachKeyFrame().text("They emit light proportionally to the strength of the redstone signal received").pointAt(whiteBulbVec).placeNearTarget();
        scene.overlay.showControls(new InputWindowElement(leverVec, Pointing.DOWN).rightClick(), 40);
        scene.idle(10);
        for (int i = 0; i < 8; i++) {
            scene.idle(5);
            int state = i + 1;
            scene.world.modifyBlockEntityNBT(util.select.position(leverPos), AnalogLeverBlockEntity.class, nbt -> nbt.putInt("State", state));
            scene.world.modifyBlock(redstonePos, s -> (BlockState) s.m_61124_(RedStoneWireBlock.POWER, state), false);
            scene.effects.indicateRedstone(redstonePos);
            scene.world.modifyBlock(whiteBulbPos, s -> (BlockState) s.m_61124_(LightBulbBlock.LEVEL, state), false);
        }
        scene.idle(40);
        scene.world.modifyBlockEntityNBT(util.select.position(leverPos), AnalogLeverBlockEntity.class, nbt -> nbt.putInt("State", 0));
        scene.world.modifyBlock(redstonePos, s -> (BlockState) s.m_61124_(RedStoneWireBlock.POWER, 0), false);
        scene.effects.indicateRedstone(redstonePos);
        scene.world.modifyBlock(whiteBulbPos, s -> (BlockState) s.m_61124_(LightBulbBlock.LEVEL, 0), false);
        scene.idle(20);
        ...
        scene.overlay.showText(80).attachKeyFrame().text("Light Bulbs are available in all colors").pointAt(greenBulbVec).placeNearTarget();
        scene.idle(90);
        scene.overlay.showFilterSlotInput(whiteBulbVec, Direction.UP, 100);
        scene.overlay.showText(80).attachKeyFrame().text("Placing items in Address slot can connect others light bulbs in a Network").pointAt(whiteBulbVec).placeNearTarget();
        scene.idle(90);
    }
}