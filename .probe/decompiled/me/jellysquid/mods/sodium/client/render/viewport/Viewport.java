...
public final class Viewport {
    ...
    public Viewport(Frustum frustum, Vector3d position) {
        ...
        this.chunkCoords = SectionPos.of(SectionPos.posToSectionCoord(position.x), SectionPos.posToSectionCoord(position.y), SectionPos.posToSectionCoord(position.z));
        this.blockCoords = BlockPos.containing(position.x, position.y, position.z);
    }

    public boolean isBoxVisible(AABB box) {
        return box.equals(IForgeBlockEntity.INFINITE_EXTENT_AABB) ? true : this.frustum.testAab((float) (box.minX - (double) this.transform.intX) - this.transform.fracX, (float) (box.minY - (double) this.transform.intY) - this.transform.fracY, (float) (box.minZ - (double) this.transform.intZ) - this.transform.fracZ, (float) (box.maxX - (double) this.transform.intX) - this.transform.fracX, (float) (box.maxY - (double) this.transform.intY) - this.transform.fracY, (float) (box.maxZ - (double) this.transform.intZ) - this.transform.fracZ);
    }
