...
public class BiomeSlice {
    ...
    private final BiomeSlice.BiasMap bias = new BiomeSlice.BiasMap();
    ...
    private void copyBiomeData(Level world, ChunkRenderContext context) {
        Holder.Reference<Biome> defaultValue = world.registryAccess().registryOrThrow(Registries.BIOME).getHolderOrThrow(Biomes.PLAINS);
    ...
    private void calculateBias() {
        ...
        for (int cellX = 1; cellX < 11; cellX++) {
            ...
            long seedX = LinearCongruentialGenerator.next(seed, (long) worldCellX);
            for (int cellY = 1; cellY < 11; cellY++) {
                int worldCellY = offsetY + cellY;
                long seedXY = LinearCongruentialGenerator.next(seedX, (long) worldCellY);
                for (int cellZ = 1; cellZ < 11; cellZ++) {
                    int worldCellZ = offsetZ + cellZ;
                    long seedXYZ = LinearCongruentialGenerator.next(seedXY, (long) worldCellZ);
    ...
    private void calculateBias(int index, int x, int y, int z, long seed) {
        seed = LinearCongruentialGenerator.next(seed, (long) x);
        seed = LinearCongruentialGenerator.next(seed, (long) y);
        seed = LinearCongruentialGenerator.next(seed, (long) z);
    ...
    public Holder<Biome> getBiome(int x, int y, int z) {
        ...
        return this.uniform[centerIndex] ? this.biomes[centerIndex] : this.getBiomeUsingVoronoi(relX, relY, relZ);
    }
