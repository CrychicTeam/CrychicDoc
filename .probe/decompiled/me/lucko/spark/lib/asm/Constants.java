...
final class Constants {
    ...
    static boolean isWhitelisted(String internalName) {
        if (!internalName.startsWith("me/lucko/spark/lib/asm/")) {
            return false;
        } else {
            String member = "(Annotation|Class|Field|Method|Module|RecordComponent|Signature)";
            return internalName.contains("Test$") || Pattern.matches("me/lucko/spark/lib/asm/util/Trace" + member + "Visitor(\\$.*)?", internalName) || Pattern.matches("me/lucko/spark/lib/asm/util/Check" + member + "Adapter(\\$.*)?", internalName);
        }
    }
