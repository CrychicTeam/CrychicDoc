...
public class MimicButtonAction extends Action {
    ...
    public void execute(@Nullable String value) {
        if (value != null && value.contains(":") && !WidgetLocatorHandler.invokeWidgetOnClick(value)) {
            LOGGER.error("[FANCYMENU] Failed to mimic button '" + value + "'!", new Exception());
            Screen current = Minecraft.getInstance().screen;
            Minecraft.getInstance().setScreen(NotificationScreen.error(aBoolean -> Minecraft.getInstance().setScreen(current), LocalizationUtils.splitLocalizedLines("fancymenu.actions.mimic_button.error")));
        }
    }
