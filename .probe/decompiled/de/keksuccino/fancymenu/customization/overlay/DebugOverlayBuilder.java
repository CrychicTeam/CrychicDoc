...
public class DebugOverlayBuilder {
    ...
    private static void addScreenCategory(@NotNull DebugOverlay overlay, @NotNull Screen currentScreen, @NotNull ScreenCustomizationLayer currentScreenLayer) {
        if (FancyMenu.getOptions().debugOverlayShowBasicScreenCategory.getValue() || FancyMenu.getOptions().debugOverlayShowAdvancedScreenCategory.getValue()) {
            ...
            if (FancyMenu.getOptions().debugOverlayShowBasicScreenCategory.getValue()) {
                overlay.addLine("screen_identifier", DebugOverlay.LinePosition.TOP_LEFT, consumes -> {
                    ...
                    if (consumes.isHovered()) {
                        c = c.setStyle(Style.EMPTY.withUnderlined(true));
                    }
                    if (consumes.recentlyClicked()) {
                        c = Component.translatable("fancymenu.overlay.debug.screen_identifier.copied").setStyle(Style.EMPTY.withColor(ChatFormatting.GREEN));
                    }
                    return c;
    ...
    private static void addResourcesCategory(@NotNull DebugOverlay overlay) {
        if (FancyMenu.getOptions().debugOverlayShowResourcesCategory.getValue()) {
            ...
            for (IAnimationRenderer ani : AnimationHandler.getAnimations()) {
                ...
                if (ani.getWidth() > 1920 || ani.getHeight() > 1080) {
    ...
    private static void addSystemCategory(@NotNull DebugOverlay overlay) {
        if (FancyMenu.getOptions().debugOverlayShowSystemCategory.getValue()) {
            overlay.addLine("frames_per_second", DebugOverlay.LinePosition.TOP_LEFT, consumes -> {
                ...
                if (fps < 20) {
                    fpsComp = fpsComp.setStyle(Style.EMPTY.withColor(ChatFormatting.GOLD));
                }
                if (fps < 10) {
                    fpsComp = fpsComp.setStyle(Style.EMPTY.withColor(ChatFormatting.RED));
                }
            ...
            overlay.addLine("ram_usage", DebugOverlay.LinePosition.TOP_LEFT, consumes -> {
                ...
                if (percent >= 60L) {
                    percentString = "" + ChatFormatting.GOLD + percent + "%" + ChatFormatting.RESET;
                }
                if (percent >= 80L) {
                    percentString = "" + ChatFormatting.RED + percent + "%" + ChatFormatting.RESET;
                }
            ...
            overlay.addLine("cpu_jvm_usage", DebugOverlay.LinePosition.TOP_LEFT, consumes -> {
                ...
                if (usage > 50.0) {
                    usageString = "" + ChatFormatting.GOLD + usage + "%" + ChatFormatting.RESET;
                }
                if (usage >= 80.0) {
                    usageString = "" + ChatFormatting.RED + usage + "%" + ChatFormatting.RESET;
                }
