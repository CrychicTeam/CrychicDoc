...
public enum Behaviors {
    ...
    public <E extends LivingEntity, T extends LivingEntity> BehaviorControl<E> interactWith(EntityType<? extends T> typeToInteractWith, int interactionRange, Predicate<E> selfFilter, Predicate<T> targetFilter, MemoryModuleType<T> memory, float speedModifier, int maxDistance) {
        return InteractWith.of(typeToInteractWith, interactionRange, selfFilter, targetFilter, memory, speedModifier, maxDistance);
    }
    ...
    public <E extends Mob> LongJumpToPreferredBlock<E> longJumpToPreferredBlock(int minTimeBetweenJumps, int maxTimeBetweenJumps, int maxJumpHeight, int maxJumpWidth, float maxJumpVelocity, Function<E, SoundEvent> jumpSound, ResourceLocation preferredBlockTag, float preferredBlockChance, BiPredicate<E, BlockPos> acceptableLandingSpot) {
        return new LongJumpToPreferredBlock<>(UniformInt.of(minTimeBetweenJumps, maxTimeBetweenJumps), maxJumpHeight, maxJumpWidth, maxJumpVelocity, jumpSound, TagKey.create(ForgeRegistries.BLOCKS.getRegistryKey(), preferredBlockTag), preferredBlockChance, acceptableLandingSpot);
    }
    ...
    public <E extends Mob> LongJumpToRandomPos<E> longJumpToRandomPos(int minTimeBetweenJumps, int maxTimeBetweenJumps, int maxJumpHeight, int maxJumpWidth, float maxJumpVelocity, Function<E, SoundEvent> jumpSound, BiPredicate<E, BlockPos> acceptableLandingSpot) {
    ...
    public TargetingConditions targetingConditions(boolean isForCombat, double range, boolean ignoreLineOfSight, boolean ignoreInvisibilityTesting, @Nullable Predicate<LivingEntity> selector) {
        TargetingConditions conditions = isForCombat ? TargetingConditions.forCombat() : TargetingConditions.forNonCombat();
        conditions.range(range);
        if (ignoreLineOfSight) {
            conditions.ignoreLineOfSight();
        }
        if (ignoreInvisibilityTesting) {
    ...
    public <E extends PathfinderMob> PrepareRamNearestTarget<E> prepareRamNearestTarget(ToIntFunction<E> cooldownOnFall, int minRamDistance, int maxRamDistance, float walkSpeed, TargetingConditions targetingConditions, int ramPrepareTime, Function<E, SoundEvent> prepareRamSound) {
        return new PrepareRamNearestTarget<>(cooldownOnFall, minRamDistance, maxRamDistance, walkSpeed, targetingConditions, ramPrepareTime, prepareRamSound);
    }
    ...
    public OneShot<PathfinderMob> setWalkTargetAwayFrom(MemoryModuleType<? extends Entity> pWalkTargetAwayFromMemory, float pSpeedModifier, int pDesiredDistance, boolean pHasTarget) {
    ...
    public <E extends Mob> BehaviorControl<E> startAttacking(Predicate<E> canAttackPredicate, Function<E, LivingEntity> targetFinder) {
        return StartAttacking.create(canAttackPredicate, e -> Optional.ofNullable((LivingEntity) targetFinder.apply(e)));
    }
    ...
    public BehaviorControl<LivingEntity> stayCloseToTarget(Function<LivingEntity, PositionTracker> targetPositionGetter, Predicate<LivingEntity> pPredicate, int closeEnough, int tooFar, float speedModifier) {
        return StayCloseToTarget.create(e -> Optional.ofNullable((PositionTracker) targetPositionGetter.apply(e)), pPredicate, closeEnough, tooFar, speedModifier);
    }
