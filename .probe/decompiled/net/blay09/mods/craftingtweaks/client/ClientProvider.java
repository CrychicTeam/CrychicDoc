...
public class ClientProvider {
    ...
    public void balanceGrid(Player entityPlayer, AbstractContainerMenu container, CraftingGrid grid) {
        ...
        for (String key : balanceSlots.keySet()) {
            ...
            average = (int) Math.floor((double) ((float) average / (float) slotList.size()));
            for (Slot slotx : slotList) {
                if (slotx.hasItem()) {
                    ItemStack itemStack = slotx.getItem();
                    if (!itemStack.isEmpty() && itemStack.getCount() > average) {
                        int mouseStackSize = itemStack.getCount();
                        this.getController().handleInventoryMouseClick(container.containerId, slotx.index, 0, ClickType.PICKUP, entityPlayer);
                        for (Slot otherSlot : slotList) {
                            if (slotx != otherSlot && otherSlot.hasItem()) {
    ...
    public void spreadGrid(Player player, AbstractContainerMenu menu, CraftingGrid grid) {
        ...
        while (tries < 9) {
            ...
            for (int ix = start; ix < start + size; ix++) {
                if (ix != biggestSlot.index) {
                    ItemStack itemStack = menu.slots.get(ix).getItem();
                    if (itemStack.isEmpty() && biggestSlotSize > 1) {
                        this.getController().handleInventoryMouseClick(menu.containerId, ix, 1, ClickType.PICKUP, player);
    ...
    private boolean rotateGridWithBuffer(Player player, AbstractContainerMenu menu, CraftingGrid grid, boolean counterClockwise) {
        ...
        if (emptyBuffer < 2) {
            ...
            do {
                ...
                this.getController().handleInventoryMouseClick(menu.containerId, bufferSlot[emptyBuffer], 0, ClickType.PICKUP, player);
                this.getController().handleInventoryMouseClick(menu.containerId, currentSlot, 0, ClickType.PICKUP, player);
                currentSlot = startSlot + this.rotateSlotId(currentSlot - startSlot, counterClockwise);
            } while (currentSlot != startSlot);
            emptyBuffer = (emptyBuffer + 1) % 2;
            this.getController().handleInventoryMouseClick(menu.containerId, bufferSlot[emptyBuffer], 0, ClickType.PICKUP, player);
            this.getController().handleInventoryMouseClick(menu.containerId, startSlot, 0, ClickType.PICKUP, player);
    ...
    private void decompress(Player player, AbstractContainerMenu menu, CraftingGrid grid, Slot mouseSlot, CompressType compressType) {
        if (mouseSlot.hasItem()) {
            ...
            for (Slot slot : menu.slots) {
                if ((compressType == CompressType.DECOMPRESS_ALL || slot == mouseSlot) && slot.container instanceof Inventory && slot.hasItem() && ItemStack.isSameItemSameTags(slot.getItem(), mouseSlot.getItem())) {
    ...
    public void compress(LocalPlayer player, AbstractContainerMenu menu, CraftingGrid grid, Slot mouseSlot, CompressType compressType) {
        if (compressType != CompressType.DECOMPRESS_ALL && compressType != CompressType.DECOMPRESS_ONE && compressType != CompressType.DECOMPRESS_STACK) {
            if (mouseSlot.hasItem()) {
                ...
                for (Slot slot : menu.slots) {
                    if ((compressType == CompressType.COMPRESS_ALL || slot == mouseSlot) && slot.container instanceof Inventory && slot.hasItem() && ItemStack.isSameItemSameTags(slot.getItem(), mouseSlot.getItem())) {
                        ItemStack mouseStack = slot.getItem();
                        if (size == 9 && !mouseStack.isEmpty() && mouseStack.getCount() >= 9) {
                            ItemStack result = findMatchingResult(new InventoryCraftingCompress(menu, 3, mouseStack), player);
                            if (!result.isEmpty() && !this.isCompressBlacklisted(result)) {
                                ...
                                this.getController().handleInventoryMouseClick(menu.containerId, -999, getDragSplittingButton(0, 0), ClickType.QUICK_CRAFT, player);
                                for (int ix = start; ix < start + size; ix++) {
                                    this.getController().handleInventoryMouseClick(menu.containerId, ix, getDragSplittingButton(1, 0), ClickType.QUICK_CRAFT, player);
                                }
                                this.getController().handleInventoryMouseClick(menu.containerId, -999, getDragSplittingButton(2, 0), ClickType.QUICK_CRAFT, player);
                                this.getController().handleInventoryMouseClick(menu.containerId, slot.index, 0, ClickType.PICKUP, player);
                            } else {
                                ...
                                this.getController().handleInventoryMouseClick(menu.containerId, slot.index, 0, ClickType.PICKUP, player);
                                this.getController().handleInventoryMouseClick(menu.containerId, -999, getDragSplittingButton(0, 0), ClickType.QUICK_CRAFT, player);
                                this.getController().handleInventoryMouseClick(menu.containerId, start, getDragSplittingButton(1, 0), ClickType.QUICK_CRAFT, player);
                                this.getController().handleInventoryMouseClick(menu.containerId, start + 1, getDragSplittingButton(1, 0), ClickType.QUICK_CRAFT, player);
                                this.getController().handleInventoryMouseClick(menu.containerId, start + 3, getDragSplittingButton(1, 0), ClickType.QUICK_CRAFT, player);
                                this.getController().handleInventoryMouseClick(menu.containerId, start + 4, getDragSplittingButton(1, 0), ClickType.QUICK_CRAFT, player);
                                this.getController().handleInventoryMouseClick(menu.containerId, -999, getDragSplittingButton(2, 0), ClickType.QUICK_CRAFT, player);
                                this.getController().handleInventoryMouseClick(menu.containerId, slot.index, 0, ClickType.PICKUP, player);
                            }
                        } else if (size >= 4 && !mouseStack.isEmpty() && mouseStack.getCount() >= 4) {
                            ItemStack result = findMatchingResult(new InventoryCraftingCompress(menu, 2, mouseStack), player);
                            if (result.isEmpty() || this.isCompressBlacklisted(result)) {
                                return;
                            }
                            this.getController().handleInventoryMouseClick(menu.containerId, slot.index, 0, ClickType.PICKUP, player);
                        ...
                        for (Slot resultSlot : menu.slots) {
                            if (resultSlot instanceof ResultSlot && resultSlot.hasItem()) {
                                this.getController().handleInventoryMouseClick(menu.containerId, resultSlot.index, 0, compressAll ? ClickType.QUICK_MOVE : ClickType.PICKUP, player);
                                break;
                            }
                        }
                        this.dropOffMouseStack(player, menu, slot.index);
                        for (int ix = start; ix < start + size; ix++) {
                            if (menu.slots.get(ix).hasItem()) {
                                this.getController().handleInventoryMouseClick(menu.containerId, ix, 0, ClickType.PICKUP, player);
                                this.getController().handleInventoryMouseClick(menu.containerId, slot.index, 0, ClickType.PICKUP, player);
                            }
                        }
                        this.dropOffMouseStack(player, menu);
            ...
            this.decompress(player, menu, grid, mouseSlot, compressType);
        }
    }
    ...
    private static <T extends CraftingContainer & RecipeHolder> ItemStack findMatchingResult(T craftingInventory, LocalPlayer player) {
        for (RecipeCollection recipeList : player.getRecipeBook().getCollections()) {
            for (Recipe<?> recipe : recipeList.getRecipes()) {
                if (recipe.getType() == RecipeType.CRAFTING && ((Recipe<T>) recipe).matches(craftingInventory, player.m_9236_())) {
                    return ((Recipe<T>) recipe).assemble(craftingInventory, player.m_9236_().registryAccess());
                }
            }
        }
    ...
    public void refillLastCrafted(Player player, AbstractContainerMenu menu, CraftingGrid grid, boolean fullStack) {
        if (this.hasLastCraftedMatrix) {
            ...
            if (menu.slots.size() >= gridStart + gridSize && gridSize == this.lastCraftedMatrix.getContainerSize()) {
                for (int i = 0; i < this.lastCraftedMatrix.getContainerSize(); i++) {
                    ItemStack itemStack = this.lastCraftedMatrix.getItem(i);
                    if (!itemStack.isEmpty()) {
                        for (Slot slot : menu.slots) {
                    ...
                    } else if (menu.slots.get(gridStart + i).hasItem()) {
                ...
                if (fullStack) {
                    for (int ix = 0; ix < this.lastCraftedMatrix.getContainerSize(); ix++) {
                        ItemStack itemStack = this.lastCraftedMatrix.getItem(ix);
                        if (!itemStack.isEmpty() && !menu.slots.get(gridStart + ix).hasItem()) {
                            for (int j = gridStart; j < gridStart + gridSize; j++) {
                                if (j != gridStart + ix) {
                                    ...
                                    if (gridStack.getCount() > 1 && ItemStack.isSameItemSameTags(gridStack, itemStack)) {
                                        ...
                                        this.getController().handleInventoryMouseClick(menu.containerId, gridStart + ix, 1, ClickType.PICKUP, player);
                                        this.getController().handleInventoryMouseClick(menu.containerId, j, 0, ClickType.PICKUP, player);
                                        break;
                                    }
                                }
