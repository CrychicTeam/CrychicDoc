    ...
    public static void initialize(BalmMenus menus) {
        waystoneSelection = menus.registerMenu(id("waystone_selection"), (syncId, inventory, buf) -> {
            ...
            if (warpMode == WarpMode.WAYSTONE_TO_WAYSTONE) {
                ...
                BlockEntity blockEntity = inventory.player.m_9236_().getBlockEntity(pos);
        ...
        sharestoneSelection = menus.registerMenu(id("sharestone_selection"), (syncId, inventory, buf) -> {
            ...
            if (blockEntity instanceof SharestoneBlockEntity) {
                ...
                return new WaystoneSelectionMenu(sharestoneSelection.get(), WarpMode.SHARESTONE_TO_SHARESTONE, fromWaystone, syncId, waystones);
            } else {
                return null;
            }
        });
        warpPlate = menus.registerMenu(id("warp_plate"), (windowId, inv, data) -> {
            ...
            return inv.player.m_9236_().getBlockEntity(pos) instanceof WarpPlateBlockEntity warpPlate ? new WarpPlateContainer(windowId, warpPlate, warpPlate.getContainerData(), inv) : null;
        });
