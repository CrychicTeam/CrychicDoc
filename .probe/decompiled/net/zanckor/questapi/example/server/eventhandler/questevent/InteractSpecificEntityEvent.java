...
public class InteractSpecificEntityEvent {
    ...
    public static void interactWithNPC(PlayerInteractEvent.EntityInteract e) throws IOException {
        if (e.getHand() != InteractionHand.OFF_HAND && !e.getSide().isClient() && e.getTarget() instanceof LivingEntity) {
            if (Timer.canUseWithCooldown(e.getEntity().m_20148_(), "INTERACT_SPECIFIC_EVENT_COOLDOWN", 1.0F)) {
                ServerHandler.questHandler(EnumGoalType.INTERACT_SPECIFIC_ENTITY, (ServerPlayer) e.getEntity(), (LivingEntity) e.getTarget());
                Timer.updateCooldown(e.getEntity().m_20148_(), "INTERACT_SPECIFIC_EVENT_COOLDOWN", 1.0F);
            }
        }
    }
}