...
public abstract class BrewingMixin {
    ...
    private static ItemStack amendments$mixEffects(ItemStack newPot, Potion potion, Operation<ItemStack> original, @Local(ordinal = 1) ItemStack potionStack) {
        if (potion == Potions.EMPTY && potionStack.hasTag()) {
            newPot.setTag(potionStack.getTag().copy());
            return newPot;
        } else {
            return (ItemStack) original.call(new Object[] { newPot, potion });
        }
    }
}