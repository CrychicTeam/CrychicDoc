...
public class RenderUtilImpl {
    ...
    public static void renderBlock(BakedModel model, long seed, PoseStack poseStack, MultiBufferSource buffer, BlockState state, Level level, BlockPos pos, BlockRenderDispatcher dispatcher) {
        for (RenderType renderType : model.getRenderTypes(state, RandomSource.create(seed), ModelData.EMPTY)) {
            ...
            dispatcher.getModelRenderer().tesselateBlock(level, model, state, pos, poseStack, vertexConsumer, false, RandomSource.create(), state.m_60726_(pos), OverlayTexture.NO_OVERLAY, ModelData.EMPTY, renderType);
        }
    }
    ...
    public static void renderGuiItem(BakedModel model, ItemStack stack, ItemRenderer renderer, int combinedLight, int pCombinedOverlay, PoseStack poseStack, MultiBufferSource.BufferSource buffer, boolean flatItem) {
        ...
        if (!model.isCustomRenderer() && (!stack.is(Items.TRIDENT) || flatItem)) {
            ...
            for (BakedModel m : model.getRenderPasses(stack, fabulous)) {
                for (RenderType renderType : m.getRenderTypes(stack, fabulous)) {
            ...
            IClientItemExtensions.of(stack).getCustomRenderer().renderByItem(stack, ItemDisplayContext.GUI, poseStack, buffer, combinedLight, pCombinedOverlay);
        }
        poseStack.popPose();
    }
