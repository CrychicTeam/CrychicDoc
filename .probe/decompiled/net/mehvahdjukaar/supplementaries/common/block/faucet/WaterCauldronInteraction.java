...
class WaterCauldronInteraction implements FaucetTarget.BlState, FaucetSource.BlState {
    ...
    public FluidOffer getProvidedFluid(Level level, BlockPos pos, Direction dir, BlockState source) {
        return source.m_60713_(Blocks.WATER_CAULDRON) ? FluidOffer.of(BuiltInSoftFluids.WATER.getHolder(), (Integer) source.m_61143_(LayeredCauldronBlock.LEVEL)) : null;
    }
    ...
    public Integer fill(Level level, BlockPos pos, BlockState state, SoftFluidStack fluid, int minAmount) {
        ...
        if (state.m_60713_(Blocks.CAULDRON) && fluid.is(BuiltInSoftFluids.WATER.get())) {
            int am = Math.min(minAmount, 3);
            level.setBlockAndUpdate(pos, (BlockState) Blocks.WATER_CAULDRON.defaultBlockState().m_61124_(LayeredCauldronBlock.LEVEL, am));
            return minAmount;
        } else if (state.m_60713_(Blocks.WATER_CAULDRON)) {
            if (fluid.is(BuiltInSoftFluids.WATER.get()) && (Integer) state.m_61143_(LayeredCauldronBlock.LEVEL) < 3) {
