...
public class AddCageMobCommand implements Command<CommandSourceStack> {
    ...
    public static ArgumentBuilder<CommandSourceStack, ?> register(CommandDispatcher<CommandSourceStack> dispatcher, CommandBuildContext context) {
        return ((LiteralArgumentBuilder) Commands.literal("cage").requires(cs -> cs.hasPermission(2))).then(Commands.argument("entity", ResourceArgument.resource(context, Registries.ENTITY_TYPE)).suggests(SuggestionProviders.SUMMONABLE_ENTITIES).executes(new AddCageMobCommand()));
    }
