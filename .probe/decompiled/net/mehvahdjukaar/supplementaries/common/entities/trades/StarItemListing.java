...
public record StarItemListing(ItemStack emeralds, ItemStack priceSecondary, int stars, int maxTrades, int xp, float priceMult, int level) implements ModItemListing {
    ...
    public static final Codec<StarItemListing> CODEC = RecordCodecBuilder.create(instance -> instance.group(ItemStack.CODEC.fieldOf("price").forGetter(StarItemListing::emeralds), StrOpt.of(ItemStack.CODEC, "price_secondary", ItemStack.EMPTY).forGetter(StarItemListing::priceSecondary), Codec.INT.fieldOf("amount").forGetter(StarItemListing::stars), StrOpt.of(ExtraCodecs.POSITIVE_INT, "max_trades", 16).forGetter(StarItemListing::maxTrades), StrOpt.of(ExtraCodecs.POSITIVE_INT, "xp").forGetter(s -> Optional.of(s.xp)), StrOpt.of(ExtraCodecs.POSITIVE_FLOAT, "price_multiplier", 0.05F).forGetter(StarItemListing::priceMult), StrOpt.of(Codec.intRange(1, 5), "level", 1).forGetter(StarItemListing::level)).apply(instance, StarItemListing::createDefault));
    ...
    public static StarItemListing createDefault(ItemStack price, ItemStack price2, int rockets, int maxTrades, Optional<Integer> xp, float priceMult, int level) {
        return new StarItemListing(price, price2, rockets, maxTrades, (Integer) xp.orElse(ModItemListing.defaultXp(false, level)), priceMult, level);
    }

    @Override
    public MerchantOffer getOffer(Entity entity, RandomSource random) {
        ItemStack star = new ItemStack(Items.FIREWORK_STAR, this.stars);
    ...
    public static CompoundTag createRandomFireworkStar(RandomSource random, List<FireworkRocketItem.Shape> usedShapes) {
        ...
        if (possible.isEmpty()) {
            tag.putByte("Type", (byte) FireworkRocketItem.Shape.values()[random.nextInt(FireworkRocketItem.Shape.values().length)].getId());
        } else {
            tag.putByte("Type", (byte) ((FireworkRocketItem.Shape) possible.get(random.nextInt(possible.size()))).getId());
        }
