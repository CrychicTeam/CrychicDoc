...
public class GunpowderBlockColor implements BlockColor {
    ...
    public int getColor(BlockState state, BlockAndTintGetter reader, BlockPos pos, int color) {
        return COLORS[state.m_61143_(GunpowderBlock.BURNING)];
    }
    ...
    static {
        for (int i = 0; i < 9; i++) {
            ...
            int redInt = Mth.clamp(Mth.floor(red * 255.0F), 0, 255);
            int greenInt = Mth.clamp(Mth.floor(green * 255.0F), 0, 255);
            int blueInt = Mth.clamp(Mth.floor(blue * 255.0F), 0, 255);
