        ...
        for (Direction side : SIDES) {
            if (!checkSides || Block.shouldRenderFace(state, level, pos, side, pos.relative(side))) {
                ...
                if (!quads.isEmpty()) {
                    ...
                    for (BakedQuad quadx : quads) {
                        if (smoothLighter && !quadx.hasAmbientOcclusion()) {
                            if (flatLighter == null) {
                                flatLighter = (QuadLighter) renderer.flatLighter.get();
                                flatLighter.setup(level, pos, state);
                            }
                            flatLighter.process(vertexConsumer, pose, quadx, packedOverlay);
                        } else {
                            lighter.process(vertexConsumer, pose, quadx, packedOverlay);
                        }
                    }
                }
            }
