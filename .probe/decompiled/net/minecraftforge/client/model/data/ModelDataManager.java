...
public class ModelDataManager {
    ...
    public void requestRefresh(@NotNull BlockEntity blockEntity) {
        ...
        ((Set) this.needModelDataRefresh.computeIfAbsent(new ChunkPos(blockEntity.getBlockPos()), $ -> Collections.synchronizedSet(new HashSet()))).add(blockEntity.getBlockPos());
    }

    private void refreshAt(ChunkPos chunk) {
        Set<BlockPos> needUpdate = (Set<BlockPos>) this.needModelDataRefresh.remove(chunk);
        if (needUpdate != null) {
            Map<BlockPos, ModelData> data = (Map<BlockPos, ModelData>) this.modelDataCache.computeIfAbsent(chunk, $ -> new ConcurrentHashMap());
    ...
    public Map<BlockPos, ModelData> getAt(ChunkPos pos) {
        ...
        this.refreshAt(pos);
        return (Map<BlockPos, ModelData>) this.modelDataCache.getOrDefault(pos, Collections.emptyMap());
    }
