...
public class CriteriaTriggers {
    ...
    private static <T extends CriterionTrigger<?>> T register(T t0) {
        if (CRITERIA.containsKey(t0.getId())) {
            throw new IllegalArgumentException("Duplicate criterion id " + t0.getId());
        } else {
            CRITERIA.put(t0.getId(), t0);
            return t0;
        }
    }

    @Nullable
    public static <T extends CriterionTriggerInstance> CriterionTrigger<T> getCriterion(ResourceLocation resourceLocation0) {
        return (CriterionTrigger<T>) CRITERIA.get(resourceLocation0);
    }
