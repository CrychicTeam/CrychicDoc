...
public class BlockEntityType<T extends BlockEntity> {
    ...
    @Nullable
    public static ResourceLocation getKey(BlockEntityType<?> blockEntityType0) {
        return BuiltInRegistries.BLOCK_ENTITY_TYPE.getKey(blockEntityType0);
    }

    private static <T extends BlockEntity> BlockEntityType<T> register(String string0, BlockEntityType.Builder<T> blockEntityTypeBuilderT1) {
        if (blockEntityTypeBuilderT1.validBlocks.isEmpty()) {
            LOGGER.warn("Block entity type {} requires at least one valid block to be defined!", string0);
        }
        Type<?> $$2 = Util.fetchChoiceType(References.BLOCK_ENTITY, string0);
        return Registry.register(BuiltInRegistries.BLOCK_ENTITY_TYPE, string0, blockEntityTypeBuilderT1.build($$2));
    }

    public BlockEntityType(BlockEntityType.BlockEntitySupplier<? extends T> blockEntityTypeBlockEntitySupplierExtendsT0, Set<Block> setBlock1, Type<?> type2) {
        this.factory = blockEntityTypeBlockEntitySupplierExtendsT0;
        this.validBlocks = setBlock1;
        this.dataType = type2;
    }

    @Nullable
    public T create(BlockPos blockPos0, BlockState blockState1) {
        return (T) this.factory.create(blockPos0, blockState1);
    }

    public boolean isValid(BlockState blockState0) {
        return this.validBlocks.contains(blockState0.m_60734_());
    }

    @Nullable
    public T getBlockEntity(BlockGetter blockGetter0, BlockPos blockPos1) {
    ...
    public static final class Builder<T extends BlockEntity> {
        ...
        private Builder(BlockEntityType.BlockEntitySupplier<? extends T> blockEntityTypeBlockEntitySupplierExtendsT0, Set<Block> setBlock1) {
            this.factory = blockEntityTypeBlockEntitySupplierExtendsT0;
            this.validBlocks = setBlock1;
        }

        public static <T extends BlockEntity> BlockEntityType.Builder<T> of(BlockEntityType.BlockEntitySupplier<? extends T> blockEntityTypeBlockEntitySupplierExtendsT0, Block... block1) {
            return new BlockEntityType.Builder<>(blockEntityTypeBlockEntitySupplierExtendsT0, ImmutableSet.copyOf(block1));
        }

        public BlockEntityType<T> build(Type<?> type0) {
            return new BlockEntityType<>(this.factory, this.validBlocks, type0);
        }
    }
}