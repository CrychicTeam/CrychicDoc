...
public class BuiltInLootTables {
    ...
    private static ResourceLocation register(String string0) {
        return register(new ResourceLocation(string0));
    }

    private static ResourceLocation register(ResourceLocation resourceLocation0) {
        if (LOCATIONS.add(resourceLocation0)) {
            return resourceLocation0;
        } else {
            throw new IllegalArgumentException(resourceLocation0 + " is already a registered built-in loot table");
        }
    }
