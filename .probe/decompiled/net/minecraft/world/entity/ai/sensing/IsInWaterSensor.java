...
public class IsInWaterSensor extends Sensor<LivingEntity> {
    ...
    protected void doTick(ServerLevel serverLevel0, LivingEntity livingEntity1) {
        if (livingEntity1.m_20069_()) {
            livingEntity1.getBrain().setMemory(MemoryModuleType.IS_IN_WATER, Unit.INSTANCE);
        } else {
            livingEntity1.getBrain().eraseMemory(MemoryModuleType.IS_IN_WATER);
        }
    }
}