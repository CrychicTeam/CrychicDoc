...
public class ChickenModel<T extends Entity> extends AgeableListModel<T> {
    ...
    public ChickenModel(ModelPart modelPart0) {
        this.head = modelPart0.getChild("head");
        this.beak = modelPart0.getChild("beak");
        this.redThing = modelPart0.getChild("red_thing");
    ...
    public static LayerDefinition createBodyLayer() {
        ...
        $$1.addOrReplaceChild("body", CubeListBuilder.create().texOffs(0, 9).addBox(-3.0F, -4.0F, -3.0F, 6.0F, 8.0F, 6.0F), PartPose.offsetAndRotation(0.0F, 16.0F, 0.0F, (float) (Math.PI / 2), 0.0F, 0.0F));
    ...
    public void setupAnim(T t0, float float1, float float2, float float3, float float4, float float5) {
        ...
        this.rightLeg.xRot = Mth.cos(float1 * 0.6662F) * 1.4F * float2;
        this.leftLeg.xRot = Mth.cos(float1 * 0.6662F + (float) Math.PI) * 1.4F * float2;
        this.rightWing.zRot = float3;
        this.leftWing.zRot = -float3;
    }
}