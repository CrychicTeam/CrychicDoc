...
public class MovementTutorialStepInstance implements TutorialStepInstance {
    ...
    public MovementTutorialStepInstance(Tutorial tutorial0) {
    ...
    public void onInput(Input input0) {
        if (input0.up || input0.down || input0.left || input0.right || input0.jumping) {
            this.moved = true;
        }
    }
    ...
    public void onMouse(double double0, double double1) {
        if (Math.abs(double0) > 0.01 || Math.abs(double1) > 0.01) {
            this.turned = true;
        }
    }
}