...
public class Vec3f extends Vec2f {
    ...
    public static float getAngleBetween(Vec3f a, Vec3f b) {
        return (float) Math.acos((double) Math.min(1.0F, dot(a, b) / (a.length() * b.length())));
    }

    public static Quaternionf getRotatorBetween(Vec3f a, Vec3f b) {
        Vec3f axis = cross(a, b, null).normalise();
        float dotDivLength = dot(a, b) / (a.length() * b.length());
        if (!Float.isFinite(dotDivLength)) {
            EpicFightMod.LOGGER.info("Warning : given vector's length is zero");
            new IllegalArgumentException().printStackTrace();
            dotDivLength = 1.0F;
        }
        float radian = (float) Math.acos((double) Math.min(1.0F, dotDivLength));
