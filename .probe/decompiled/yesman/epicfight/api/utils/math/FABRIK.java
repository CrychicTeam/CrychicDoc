...
public class FABRIK {
    ...
    private final List<FABRIK.Chain> chains = Lists.newArrayList();
    ...
    public void addChain(Pose pose, Joint startJoint, Joint endJoint) {
        OpenMatrix4f bindTransform = this.armature.getBindedTransformFor(pose, startJoint);
    ...
    private void addChainInternal(Pose pose, OpenMatrix4f parentTransform, Joint joint, int pathIndex) {
        Joint nextJoint = (Joint) joint.getSubJoints().get(pathIndex % 10 - 1);
        ...
        this.chains.add(new FABRIK.Chain(joint.getName(), parentTransform.toTranslationVector(), result.toTranslationVector()));
    ...
    public void run(Vec3f target, int iteration) {
        ...
        for (FABRIK.Chain chain : this.chains) {
    ...
    private void forward() {
        ...
        for (int i = 0; i < chainNum; i++) {
            FABRIK.Chain chain = (FABRIK.Chain) this.chains.get(i);
    ...
    public List<Vec3f> getChainingPosition() {
        ...
        list.add(((FABRIK.Chain) this.chains.get(this.chains.size() - 1)).head);
        return list;
    }
    ...
    class Chain {
        ...
        Chain(String jointName, Vec3f tail, Vec3f head) {
            ...
            this.length = (float) Math.sqrt((double) tail.distanceSqr(head));
        }
