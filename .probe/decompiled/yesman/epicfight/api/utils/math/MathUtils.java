...
public class MathUtils {
    ...
    public static double getAngleBetween(Vec3f a, Vec3f b) {
        double cos = (double) (a.x * b.x + a.y * b.y + a.z * b.z);
    ...
    public static double getXRotOfVector(Vec3 vec) {
        ...
        return -(Math.atan2(normalized.y, (double) ((float) Math.sqrt(normalized.x * normalized.x + normalized.z * normalized.z))) * (180.0 / Math.PI));
    }

    public static double getYRotOfVector(Vec3 vec) {
        Vec3 normalized = vec.normalize();
        return Math.atan2(normalized.z, normalized.x) * (180.0 / Math.PI) - 90.0;
    }
    ...
    private static Quaternionf getQuaternionFromMatrix(OpenMatrix4f mat) {
        ...
        if (diagonal > 0.0F) {
            ...
            float x4 = (float) (Math.sqrt((double) (1.0F + mat.m00 - mat.m11 - mat.m22)) * 2.0);
            w = (mat.m21 - mat.m12) / x4;
            x = x4 * 0.25F;
            y = (mat.m01 + mat.m10) / x4;
            z = (mat.m02 + mat.m20) / x4;
        } else if (mat.m11 > mat.m22) {
            float y4 = (float) (Math.sqrt((double) (1.0F + mat.m11 - mat.m00 - mat.m22)) * 2.0);
            w = (mat.m02 - mat.m20) / y4;
            x = (mat.m01 + mat.m10) / y4;
            y = y4 * 0.25F;
            z = (mat.m12 + mat.m21) / y4;
        } else {
            float z4 = (float) (Math.sqrt((double) (1.0F + mat.m22 - mat.m00 - mat.m11)) * 2.0);
    ...
    public static Vec3 projectVector(Vec3 from, Vec3 to) {
        ...
        double normalScale = 1.0 / (to.x * to.x + to.y * to.y + to.z * to.z);
