...
public class JsonModelLoader {
    ...
    public JsonModelLoader(ResourceManager resourceManager, ResourceLocation resourceLocation) {
        try {
            if (resourceManager == null) {
                ...
                Resource resource = (Resource) resourceManager.m_213713_(resourceLocation).get();
                JsonReader in = new JsonReader(new InputStreamReader(resource.open(), StandardCharsets.UTF_8));
                in.setLenient(true);
                this.rootJson = Streams.parse(in).getAsJsonObject();
            }
        } catch (Exception var7) {
            EpicFightMod.LOGGER.info("Can't read " + resourceLocation.toString() + " because of " + var7);
    ...
    public Mesh.RenderProperties getRenderProperties() {
        ...
        Mesh.RenderProperties renderProperties = Mesh.RenderProperties.create();
        if (properties != null) {
        ...
        if (parent != null) {
            ...
            if (parts != null) {
                for (Entry<String, JsonElement> e : parts.entrySet()) {
                    meshMap.put((String) e.getKey(), new ModelPart((List<T>) VertexIndicator.create(ParseUtil.toIntArray(((JsonElement) e.getValue()).getAsJsonObject().get("array").getAsJsonArray()))));
                }
            }
            if (indices != null) {
                meshMap.put("noGroups", new ModelPart((List<T>) VertexIndicator.create(ParseUtil.toIntArray(indices.get("array").getAsJsonArray()))));
            }
    ...
    public <T extends AnimatedMesh> T loadAnimatedMesh(Meshes.MeshContructor<VertexIndicator.AnimatedVertexIndicator, T> constructor) {
        ...
        if (parent != null) {
            ...
            if (parts != null) {
                for (Entry<String, JsonElement> e : parts.entrySet()) {
                    meshMap.put((String) e.getKey(), new ModelPart((List<T>) VertexIndicator.createAnimated(ParseUtil.toIntArray(((JsonElement) e.getValue()).getAsJsonObject().get("array").getAsJsonArray()), vcountArray, animationIndexArray)));
                }
            }
            if (indices != null) {
                meshMap.put("noGroups", new ModelPart((List<T>) VertexIndicator.createAnimated(ParseUtil.toIntArray(indices.get("array").getAsJsonArray()), vcountArray, animationIndexArray)));
            }
            return constructor.invoke(arrayMap, null, this.getRenderProperties(), meshMap);
        }
    }
                ...
                for (AttackAnimation.Phase phase : ((AttackAnimation) animation).phases) {
                    ...
                    for (Pair<Joint, Collider> colliderInfo : phase.getColliders()) {
                        ...
                        while (joint != null) {
                            ...
                            if (nextJoint > 0) {
                                pathIndex /= 10;
                                joint = (Joint) joint.getSubJoints().get(nextJoint - 1);
                            } else {
                                joint = null;
                            }
                        }
