...
public class Pose {
    ...
    public JointTransform getOrDefaultTransform(String jointName) {
        return (JointTransform) this.jointTransformData.getOrDefault(jointName, JointTransform.empty());
    }

    public void removeJointIf(Predicate<? super Entry<String, JointTransform>> predicate) {
    ...
    public String toString() {
        ...
        for (Entry<String, JointTransform> entry : this.jointTransformData.entrySet()) {
            str = str + String.format("%s{%s, %s}, ", entry.getKey(), ((JointTransform) entry.getValue()).translation().toString(), ((JointTransform) entry.getValue()).rotation().toString()) + "\n";
        }
        return str;
    }
}