...
public interface ProceduralAnimation {
    ...
    default void setIKInfo(IKInfo[] ikInfos, Map<String, TransformSheet> src, Map<String, TransformSheet> dest, Armature armature, boolean correctY, boolean correctZ) {
        for (IKInfo ikInfo : ikInfos) {
            ...
            while (pathToEnd > 0) {
                start = (Joint) start.getSubJoints().get(pathToEnd % 10 - 1);
                pathToEnd /= 10;
                ikInfo.pathToEndJoint.add(start.getName());
            }
            Keyframe[] keyframes = ((TransformSheet) src.get(ikInfo.endJoint.getName())).getKeyframes();
            Keyframe[] bindedposKeyframes = new Keyframe[keyframes.length];
            int keyframeLength = ((TransformSheet) src.get(ikInfo.endJoint.getName())).getKeyframes().length;
            for (int i = 0; i < keyframeLength; i++) {
                Keyframe kf = keyframes[i];
                Pose pose = new Pose();
                for (String jointName : src.keySet()) {
                    pose.putJointData(jointName, ((TransformSheet) src.get(jointName)).getInterpolatedTransform(kf.time()));
                }
                ...
                if (correctY || correctZ) {
                    JointTransform rootTransform = ((TransformSheet) src.get("Root")).getInterpolatedTransform(kf.time());
    ...
    default TransformSheet clipAnimation(TransformSheet transformSheet, IKInfo ikInfo) {
        return ikInfo.clipAnimation ? transformSheet.copy(ikInfo.startFrame, ikInfo.endFrame) : this.getFirstPart(transformSheet);
    }
    ...
    default Vec3f getRayCastedTipPosition(Vec3f clipStart, OpenMatrix4f toWorldCoord, EnderDragonPatch enderdragonpatch, float maxYDown, float leastHeight) {
        ...
        BlockHitResult clipResult = enderdragonpatch.getOriginal().m_9236_().m_45547_(new ClipContext(new Vec3((double) clipStartWorld.x, (double) clipStartWorld.y, (double) clipStartWorld.z), new Vec3((double) clipStartWorld.x, (double) (clipStartWorld.y - maxYDown), (double) clipStartWorld.z), ClipContext.Block.COLLIDER, ClipContext.Fluid.NONE, enderdragonpatch.getOriginal()));
        float dy = clipResult.getType() != HitResult.Type.MISS ? clipStartWorld.y - (float) clipResult.getBlockPos().m_123342_() - 1.0F : maxYDown;
