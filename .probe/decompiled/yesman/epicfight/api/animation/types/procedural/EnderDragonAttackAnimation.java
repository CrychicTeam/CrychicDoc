...
public class EnderDragonAttackAnimation extends AttackAnimation implements ProceduralAnimation {
    ...
    public Pose getPoseByTime(LivingEntityPatch<?> entitypatch, float time, float partialTicks) {
        ...
        if (entitypatch instanceof EnderDragonPatch enderdragonpatch) {
            float x = (float) entitypatch.getOriginal().m_20185_();
            float y = (float) entitypatch.getOriginal().m_20186_();
            float z = (float) entitypatch.getOriginal().m_20189_();
            float xo = (float) entitypatch.getOriginal().f_19854_;
            float yo = (float) entitypatch.getOriginal().f_19855_;
            float zo = (float) entitypatch.getOriginal().f_19856_;
            ...
            this.correctRootRotation((JointTransform) pose.getJointTransformData().get("Root"), enderdragonpatch, partialTicks);
    ...
    public void begin(LivingEntityPatch<?> entitypatch) {
        MoveCoordFunctions.MoveCoordSetter actionAnimCoordSetter = (MoveCoordFunctions.MoveCoordSetter) this.getProperty(AnimationProperty.ActionAnimationProperty.COORD_SET_BEGIN).orElse((MoveCoordFunctions.MoveCoordSetter) (self, entitypatch$2, transformSheet) -> transformSheet.readFrom((TransformSheet) self.getTransfroms().get("Root")));
        actionAnimCoordSetter.set(this, entitypatch, entitypatch.getArmature().getActionAnimationCoord());
        if (entitypatch instanceof EnderDragonPatch enderdragonpatch) {
            Vec3 entitypos = enderdragonpatch.getOriginal().m_20182_();
            ...
            for (IKInfo ikInfo : this.ikInfos) {
                TransformSheet tipAnim = this.getFirstPart((TransformSheet) this.tipPointTransform.get(ikInfo.endJoint.getName()));
    ...
    public void tick(LivingEntityPatch<?> entitypatch) {
        ...
        if (entitypatch instanceof EnderDragonPatch enderdragonpatch) {
            Vec3 entitypos = enderdragonpatch.getOriginal().m_20182_();
            OpenMatrix4f toWorld = OpenMatrix4f.mul(OpenMatrix4f.createTranslation((float) entitypos.x, (float) entitypos.y, (float) entitypos.z), enderdragonpatch.getModelMatrix(1.0F), null);
            float elapsedTime = entitypatch.<Animator>getAnimator().getPlayerFor(this).getElapsedTime();
            for (IKInfo ikInfo : this.ikInfos) {
                if (ikInfo.clipAnimation) {
                    Keyframe[] keyframes = ((TransformSheet) this.getTransfroms().get(ikInfo.endJoint.getName())).getKeyframes();
                    ...
                    if (startTime <= elapsedTime && elapsedTime < endTime) {
                        ...
                        Vec3f finalTargetpos = ikInfo.clipAnimation && !ikInfo.touchingGround[ikInfo.touchingGround.length - 1] ? OpenMatrix4f.transform3v(toWorld, ikInfo.endpos.multiply(-1.0F, 1.0F, -1.0F), null) : this.getRayCastedTipPosition(clipStart, toWorld, enderdragonpatch, 8.0F, ikInfo.rayLeastHeight);
                        if (tipAnim.isOnWorking()) {
                            tipAnim.newTargetPosition(finalTargetpos);
                        } else {
                            this.startPartAnimation(ikInfo, tipAnim, this.clipAnimation((TransformSheet) this.tipPointTransform.get(ikInfo.endJoint.getName()), ikInfo), finalTargetpos);
                        }
                    }
                }
            }
