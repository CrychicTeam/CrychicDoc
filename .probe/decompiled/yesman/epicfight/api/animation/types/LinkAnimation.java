...
public class LinkAnimation extends DynamicAnimation {
    ...
    public void end(LivingEntityPatch<?> entitypatch, DynamicAnimation nextAnimation, boolean isEnd) {
        if (!isEnd) {
            ...
            entitypatch.<Animator>getAnimator().getPlayerFor(this).setElapsedTime(this.startsAt);
            entitypatch.<Animator>getAnimator().getPlayerFor(this).markToDoNotReset();
            this.startsAt = 0.0F;
        }
    }
    ...
    public Pose getPoseByTime(LivingEntityPatch<?> entitypatch, float time, float partialTicks) {
        ...
        for (Entry<String, JointTransform> entry : nextStartingPose.getJointTransformData().entrySet()) {
