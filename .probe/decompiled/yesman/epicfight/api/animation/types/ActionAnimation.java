...
public class ActionAnimation extends MainFrameAnimation {
    ...
    public void begin(LivingEntityPatch<?> entitypatch) {
        ...
        if (entitypatch.shouldMoveOnCurrentSide(this)) {
            if ((Boolean) this.getProperty(AnimationProperty.ActionAnimationProperty.STOP_MOVEMENT).orElse(false) && entitypatch.shouldMoveOnCurrentSide(this)) {
                entitypatch.getOriginal().m_20334_(0.0, entitypatch.getOriginal().m_20184_().y, 0.0);
            }
            entitypatch.correctRotation();
            MoveCoordFunctions.MoveCoordSetter moveCoordSetter = (MoveCoordFunctions.MoveCoordSetter) this.getProperty(AnimationProperty.ActionAnimationProperty.COORD_SET_BEGIN).orElse(MoveCoordFunctions.RAW_COORD);
    ...
    protected void move(LivingEntityPatch<?> entitypatch, DynamicAnimation animation) {
        if (this.validateMovement(entitypatch, animation)) {
            EntityState state = this.getState(entitypatch, entitypatch.<Animator>getAnimator().getPlayerFor(this).getElapsedTime());
            if (state.inaction()) {
    ...
    protected boolean validateMovement(LivingEntityPatch<?> entitypatch, DynamicAnimation animation) {
        if (!entitypatch.shouldMoveOnCurrentSide(this)) {
            return false;
        } else if (animation instanceof LinkAnimation) {
    ...
    protected boolean shouldMove(float currentTime) {
        if (this.properties.containsKey(AnimationProperty.ActionAnimationProperty.MOVE_TIME)) {
            TimePairList moveTimes = (TimePairList) this.getProperty(AnimationProperty.ActionAnimationProperty.MOVE_TIME).get();
    ...
    public void setLinkAnimation(Pose pose1, float convertTimeModifier, LivingEntityPatch<?> entitypatch, LinkAnimation dest) {
        ...
        if (entitypatch.shouldMoveOnCurrentSide(this) && (Boolean) this.getProperty(AnimationProperty.ActionAnimationProperty.MOVE_ON_LINK).orElse(true)) {
            JointTransform jt = pose.getOrDefaultTransform("Root");
            if (this.getProperty(AnimationProperty.ActionAnimationProperty.COORD).isEmpty()) {
                ...
                TransformSheet coordTransform = (TransformSheet) this.getProperty(AnimationProperty.ActionAnimationProperty.COORD).get();
        ...
        for (String jointName : data2.keySet()) {
            Keyframe[] keyframes = new Keyframe[] { new Keyframe(0.0F, (JointTransform) data1.getOrDefault(jointName, JointTransform.empty())), new Keyframe(totalTime, (JointTransform) data2.get(jointName)) };
    ...
    protected Vec3 getCoordVector(LivingEntityPatch<?> entitypatch, DynamicAnimation animation) {
        ...
        TimePairList coordUpdateTime = (TimePairList) this.getProperty(AnimationProperty.ActionAnimationProperty.COORD_UPDATE_TIME).orElse(null);
        boolean isCoordUpdateTime = coordUpdateTime == null || coordUpdateTime.isTimeInPairs(player.getElapsedTime());
        MoveCoordFunctions.MoveCoordSetter moveCoordsetter = isCoordUpdateTime ? (MoveCoordFunctions.MoveCoordSetter) this.getProperty(AnimationProperty.ActionAnimationProperty.COORD_SET_TICK).orElse(null) : MoveCoordFunctions.RAW_COORD;
        ...
        MoveCoordFunctions.MoveCoordGetter moveGetter = isCoordUpdateTime ? (MoveCoordFunctions.MoveCoordGetter) this.getProperty(AnimationProperty.ActionAnimationProperty.COORD_GET).orElse(MoveCoordFunctions.DIFF_FROM_PREV_COORD) : MoveCoordFunctions.DIFF_FROM_PREV_COORD;
        ...
        this.getProperty(AnimationProperty.ActionAnimationProperty.NO_GRAVITY_TIME).ifPresentOrElse(noGravityTime -> {
            if (noGravityTime.isTimeInPairs(animation instanceof LinkAnimation ? 0.0F : player.getElapsedTime())) {
                livingentity.m_20334_(motion.x, 0.0, motion.z);
            } else {
                move.y = 0.0F;
            }
        }, () -> {
            if (moveVertical && move.y > 0.0F && !hasNoGravity) {
                ...
                livingentity.m_20334_(motion.x, motion.y <= 0.0 ? motion.y + gravity : motion.y, motion.z);
            }
        });
