...
public class AnimationManager extends SimplePreparableReloadListener<Map<Integer, Map<Integer, StaticAnimation>>> {
    ...
    public StaticAnimation findAnimationById(int namespaceId, int animationId) {
        if (this.animationById.containsKey(namespaceId)) {
            Map<Integer, StaticAnimation> map = (Map<Integer, StaticAnimation>) this.animationById.get(namespaceId);
            if (map.containsKey(animationId)) {
                return (StaticAnimation) map.get(animationId);
            }
        }
    ...
    public StaticAnimation findAnimationByPath(String resourceLocation) {
        ...
        if (this.animationByName.containsKey(rl)) {
            return (StaticAnimation) this.animationByName.get(rl);
        } else {
    ...
    public void registerAnimations() {
        ...
        registryMap.entrySet().forEach(entry -> {
            EpicFightMod.LOGGER.info("Start animation registration of " + (String) entry.getKey());
    ...
    protected Map<Integer, Map<Integer, StaticAnimation>> prepare(ResourceManager resourceManager, ProfilerFiller profilerIn) {
        if (EpicFightMod.isPhysicalClient()) {
            this.registerAnimations();
            this.animationById.values().forEach(map -> map.values().forEach(animation -> this.setAnimationProperties(resourceManager, animation)));
        }
    ...
    private void setAnimationProperties(ResourceManager resourceManager, StaticAnimation animation) {
        if (resourceManager != null) {
            ...
            int last = location.getPath().lastIndexOf(47);
            if (last > 0) {
                ...
                if (resourceManager.m_213713_(dataLocation).isPresent()) {
                    AnimationDataReader.readAndApply(animation, resourceManager, (Resource) resourceManager.m_213713_(dataLocation).get());
                }
            }
        }
    }
    ...
    public StaticAnimation refreshAnimation(StaticAnimation oldAnimation) {
        return (StaticAnimation) this.animationByName.get(oldAnimation.getRegistryName());
    }
    ...
    public Map<Integer, StaticAnimation> getIdMap() {
        return (Map<Integer, StaticAnimation>) this.animationById.get(this.namespaceHash);
    }
    ...
    public static AnimationManager.AnimationRegistryCallbacks getCallBack() {
        return AnimationManager.AnimationRegistryCallbacks.INSTANCE;
    }

    public void onRegistryCreated(Supplier<IForgeRegistry<StaticAnimation>> animationRegistry) {
        this.animationRegistry = (IForgeRegistry<StaticAnimation>) animationRegistry.get();
    }
    ...
    private static class AnimationRegistryCallbacks implements IForgeRegistry.BakeCallback<StaticAnimation>, IForgeRegistry.CreateCallback<StaticAnimation> {
        ...
        private static final AnimationManager.AnimationRegistryCallbacks INSTANCE = new AnimationManager.AnimationRegistryCallbacks();

        private static final ResourceLocation ANIMATION_ID_MAP = new ResourceLocation("epicfight", "animationidmap");
        ...
        public void onCreate(IForgeRegistryInternal<StaticAnimation> owner, RegistryManager stage) {
            IdMapper<StaticAnimation> animationIdMap = new IdMapper<>(owner.getKeys().size());
