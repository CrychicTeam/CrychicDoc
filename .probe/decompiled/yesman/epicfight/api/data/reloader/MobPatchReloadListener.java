...
public class MobPatchReloadListener extends SimpleJsonResourceReloadListener {
    ...
    protected void apply(Map<ResourceLocation, JsonElement> objectIn, ResourceManager resourceManagerIn, ProfilerFiller profilerIn) {
        for (Entry<ResourceLocation, JsonElement> entry : objectIn.entrySet()) {
            ...
            if (!ForgeRegistries.ENTITY_TYPES.containsKey(registryName)) {
                ...
                EntityPatchProvider.putCustomEntityPatch(entityType, entity -> () -> ((MobPatchReloadListener.AbstractMobPatchProvider) MOB_PATCH_PROVIDERS.get(entity.getType())).get(entity));
    ...
    public static MobPatchReloadListener.AbstractMobPatchProvider deserialize(EntityType<?> entityType, CompoundTag tag, boolean clientSide) {
        ...
        if (hasBranch) {
            ...
            ((MobPatchReloadListener.BranchProvider) provider).defaultProvider = deserializeMobPatchProvider(entityType, tag, clientSide);
        } else {
        ...
        while (hasBranch) {
            ...
            ((MobPatchReloadListener.BranchProvider) provider).providers.add(Pair.of(deserializePredicate(branchTag.getCompound("condition")), deserialize(entityType, branchTag, clientSide)));
    ...
    public static MobPatchReloadListener.AbstractMobPatchProvider deserializeMobPatchProvider(EntityType<?> entityType, CompoundTag tag, boolean clientSide) {
        ...
        if (disabled) {
            ...
            MobPatchReloadListener.CustomMobPatchProvider provider = (MobPatchReloadListener.CustomMobPatchProvider) (humanoid ? new MobPatchReloadListener.CustomHumanoidMobPatchProvider() : new MobPatchReloadListener.CustomMobPatchProvider());
    ...
    public static Map<WeaponCategory, Map<Style, Set<Pair<LivingMotion, StaticAnimation>>>> deserializeHumanoidWeaponMotions(ListTag tag) {
        ...
        for (int i = 0; i < tag.size(); i++) {
            Builder<Pair<LivingMotion, StaticAnimation>> motions = ImmutableSet.builder();
    ...
    public static CompoundTag filterClientData(CompoundTag tag) {
        ...
        int i = 0;
        for (boolean hasBranch = tag.contains(String.format("branch_%d", i)); hasBranch; hasBranch = tag.contains(String.format("branch_%d", ++i))) {
            CompoundTag branchTag = tag.getCompound(String.format("branch_%d", i));
    ...
    public static class BranchProvider extends MobPatchReloadListener.AbstractMobPatchProvider {
        ...
        protected List<Pair<EpicFightPredicates<Entity>, MobPatchReloadListener.AbstractMobPatchProvider>> providers = Lists.newArrayList();

        protected MobPatchReloadListener.AbstractMobPatchProvider defaultProvider;

        @Override
        public EntityPatch<?> get(Entity entity) {
            for (Pair<EpicFightPredicates<Entity>, MobPatchReloadListener.AbstractMobPatchProvider> provider : this.providers) {
                if (((EpicFightPredicates) provider.getFirst()).test(entity)) {
