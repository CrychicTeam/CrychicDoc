...
public abstract class Mesh<T extends VertexIndicator> {
    ...
    final Mesh.RenderProperties renderProperties;

    public Mesh(Map<String, float[]> arrayMap, Mesh<T> parent, Mesh.RenderProperties renderProperties, Map<String, ModelPart<T>> parts) {
        this.positions = parent == null ? (float[]) arrayMap.get("positions") : parent.positions;
        this.normals = parent == null ? (float[]) arrayMap.get("normals") : parent.normals;
        this.uvs = parent == null ? (float[]) arrayMap.get("uvs") : parent.uvs;
    ...
    public ModelPart<T> getPart(String part) {
        return (ModelPart<T>) this.parts.get(part);
    }
    ...
    public Mesh.RenderProperties getRenderProperty() {
        ...
        for (ModelPart<T> part : this.parts.values()) {
            if (!part.hidden) {
                for (VertexIndicator vi : part.getVertices()) {
                    ...
                    Vector4f posVec = new Vector4f(this.positions[pos], this.positions[pos + 1], this.positions[pos + 2], 1.0F);
                    posVec.mul(matrix4f);
                    builder.vertex((double) posVec.x(), (double) posVec.y(), (double) posVec.z()).color(r, g, b, a).uv(this.uvs[uv], this.uvs[uv + 1]).uv2(packedLightIn).endVertex();
                }
            }
        }
    }
    ...
    public static class RawMesh extends Mesh<VertexIndicator> {
        ...
        public RawMesh(Map<String, float[]> arrayMap, Mesh<VertexIndicator> parent, Mesh.RenderProperties properties, Map<String, ModelPart<VertexIndicator>> parts) {
        ...
        protected ModelPart<VertexIndicator> getOrLogException(Map<String, ModelPart<VertexIndicator>> parts, String name) {
            if (!parts.containsKey(name)) {
                ...
                return (ModelPart<VertexIndicator>) parts.get(name);
            }
        }
    }
    ...
    public static class RenderProperties {
        ...
        public Mesh.RenderProperties customTexturePath(String path) {
            this.customTexturePath = path;
            return this;
        }

        public Mesh.RenderProperties transparency(boolean isTransparent) {
            this.isTransparent = isTransparent;
            return this;
        }

        public static Mesh.RenderProperties create() {
