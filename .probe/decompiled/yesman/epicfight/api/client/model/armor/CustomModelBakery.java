...
public class CustomModelBakery {
    ...
    public static void exportModels(File resourcePackDirectory) throws IOException {
        ...
        for (Entry<ResourceLocation, AnimatedMesh> entry : BAKED_MODELS.entrySet()) {
            ZipEntry zipEntry = new ZipEntry(String.format("assets/%s/animmodels/armor/%s.json", ((ResourceLocation) entry.getKey()).getNamespace(), ((ResourceLocation) entry.getKey()).getPath()));
            Gson gson = new GsonBuilder().create();
            out.putNextEntry(zipEntry);
            out.write(gson.toJson(((AnimatedMesh) entry.getValue()).toJsonObject()).getBytes());
            out.closeEntry();
