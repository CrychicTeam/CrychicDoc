...
public class ClientAnimator extends Animator {
    ...
    public static Animator getAnimator(LivingEntityPatch<?> entitypatch) {
        return (Animator) (entitypatch.isLogicalClient() ? new ClientAnimator(entitypatch) : ServerAnimator.getAnimator(entitypatch));
    }
    ...
    public void playAnimation(StaticAnimation nextAnimation, float convertTimeModifier) {
        Layer layer = (Layer) (nextAnimation.getLayerType() == Layer.LayerType.BASE_LAYER ? this.baseLayer : (Layer) this.baseLayer.compositeLayers.get(nextAnimation.getPriority()));
        layer.paused = false;
    ...
    public void addLivingAnimation(LivingMotion livingMotion, StaticAnimation animation) {
        ...
        Layer layer = (Layer) (layerType == Layer.LayerType.BASE_LAYER ? this.baseLayer : (Layer) this.baseLayer.compositeLayers.get(animation.getPriority()));
        ...
        if (isBaseLayer) {
            animation.getProperty(ClientAnimationProperties.MULTILAYER_ANIMATION).ifPresent(multilayerAnimation -> {
                ...
                if (livingMotion == this.currentCompositeMotion) {
                    EntityState statex = this.getEntityState();
                    if (!statex.inaction()) {
    ...
    public StaticAnimation getLivingMotion(LivingMotion motion) {
        return (StaticAnimation) this.livingAnimations.getOrDefault(motion, (StaticAnimation) this.livingAnimations.get(LivingMotions.IDLE));
    }

    public StaticAnimation getCompositeLivingMotion(LivingMotion motion) {
        return (StaticAnimation) this.compositeLivingAnimations.getOrDefault(motion, Animations.DUMMY_ANIMATION);
    }

    @Override
    public void init() {
        this.entitypatch.initAnimator(this);
        StaticAnimation idleMotion = (StaticAnimation) this.livingAnimations.get(this.currentMotion);
    ...
    public void tick() {
        ...
        if (!this.compareCompositeMotion(this.entitypatch.currentCompositeMotion)) {
            ...
            this.getLivingMotion(this.currentCompositeMotion).getProperty(ClientAnimationProperties.MULTILAYER_ANIMATION).ifPresent(multilayerAnimation -> this.getCompositeLayer(multilayerAnimation.getPriority()).off(this.entitypatch));
            if (this.compositeLivingAnimations.containsKey(this.entitypatch.currentCompositeMotion)) {
                this.playAnimation(this.getCompositeLivingMotion(this.entitypatch.currentCompositeMotion), 0.0F);
            }
        }
        if (!this.compareMotion(this.entitypatch.currentLivingMotion) && this.entitypatch.currentLivingMotion != LivingMotions.DEATH && this.livingAnimations.containsKey(this.entitypatch.currentLivingMotion)) {
            this.baseLayer.playAnimation(this.getLivingMotion(this.entitypatch.currentLivingMotion), this.entitypatch, 0.0F);
        }
    ...
    public void playDeathAnimation() {
        if (!(Boolean) this.getPlayerFor(null).getAnimation().getProperty(AnimationProperty.ActionAnimationProperty.IS_DEATH_ANIMATION).orElse(false)) {
            this.playAnimation((StaticAnimation) this.livingAnimations.get(LivingMotions.DEATH), 0.0F);
            this.currentMotion = LivingMotions.DEATH;
        }
    }

    public StaticAnimation getJumpAnimation() {
        return (StaticAnimation) this.livingAnimations.get(LivingMotions.JUMP);
    }

    public Layer getCompositeLayer(Layer.Priority priority) {
        return (Layer) this.baseLayer.compositeLayers.get(priority);
    }
    ...
    public Pose getComposedLayerPose(float partialTicks) {
        ...
        for (Layer.Priority priority : this.baseLayer.baseLayerPriority.uppers()) {
            Layer compositeLayer = (Layer) this.baseLayer.compositeLayers.get(priority);
            if ((priority != Layer.Priority.LOWEST || !this.baseLayer.animationPlayer.getAnimation().isMainFrameAnimation()) && !compositeLayer.isDisabled()) {
    ...
    public static void applyBindModifier(LivingEntityPatch<?> entitypatch, Pose basePose, Pose result, Joint joint, Map<Layer.Priority, Pair<DynamicAnimation, Pose>> poses) {
        ...
        for (Layer.Priority priority : list) {
            DynamicAnimation nowPlaying = (DynamicAnimation) ((Pair) poses.get(priority)).getFirst();
    ...
    public void playReboundAnimation() {
        if (this.compositeLivingAnimations.containsKey(LivingMotions.SHOT)) {
            this.playAnimation((StaticAnimation) this.compositeLivingAnimations.get(LivingMotions.SHOT), 0.0F);
            this.entitypatch.currentCompositeMotion = LivingMotions.NONE;
    ...
    public <T> Pair<AnimationPlayer, T> findFor(Class<T> animationType) {
        ...
        return animationType.isAssignableFrom(this.baseLayer.animationPlayer.getAnimation().getClass()) ? Pair.of(this.baseLayer.animationPlayer, this.baseLayer.animationPlayer.getAnimation()) : null;
    }
