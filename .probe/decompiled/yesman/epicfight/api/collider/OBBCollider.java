...
public class OBBCollider extends Collider {
    ...
    public OBBCollider(AABB aabbCopy) {
        ...
        double xSize = (aabbCopy.maxX - aabbCopy.minX) / 2.0;
        double ySize = (aabbCopy.maxY - aabbCopy.minY) / 2.0;
        double zSize = (aabbCopy.maxZ - aabbCopy.minZ) / 2.0;
        this.worldCenter = new Vec3(-((double) ((float) aabbCopy.minX) + xSize), (double) ((float) aabbCopy.minY) + ySize, -((double) ((float) aabbCopy.minZ) + zSize));
        this.rotatedVertex = new Vec3[4];
        this.rotatedNormal = new Vec3[3];
    ...
    protected AABB getHitboxAABB() {
        return this.outerAABB.inflate((this.outerAABB.maxX - this.outerAABB.minX) * (double) this.scale.x, (this.outerAABB.maxY - this.outerAABB.minY) * (double) this.scale.y, (this.outerAABB.maxZ - this.outerAABB.minZ) * (double) this.scale.z).move(-this.worldCenter.x, this.worldCenter.y, -this.worldCenter.z);
    }
    ...
    public boolean isCollide(OBBCollider opponent) {
        ...
        for (Vec3 seperateAxisx : opponent.rotatedNormal) {
            if (!collisionDetection(seperateAxisx, toOpponent, this, opponent)) {
                return false;
            }
        }
        return true;
    ...
    private static boolean collisionDetection(Vec3 seperateAxis, Vec3 toOpponent, OBBCollider box1, OBBCollider box2) {
        ...
        double maxDot1 = -1.0;
        double maxDot2 = -1.0;
        Vec3 distance = seperateAxis.dot(toOpponent) > 0.0 ? toOpponent : toOpponent.scale(-1.0);
        for (Vec3 vertexVector : box1.rotatedVertex) {
            Vec3 temp = seperateAxis.dot(vertexVector) > 0.0 ? vertexVector : vertexVector.scale(-1.0);
        ...
        for (Vec3 vertexVectorx : box2.rotatedVertex) {
            Vec3 temp = seperateAxis.dot(vertexVectorx) > 0.0 ? vertexVectorx : vertexVectorx.scale(-1.0);
            double dot = seperateAxis.dot(temp);
            if (dot > maxDot2) {
                maxDot2 = dot;
                maxProj2 = temp;
