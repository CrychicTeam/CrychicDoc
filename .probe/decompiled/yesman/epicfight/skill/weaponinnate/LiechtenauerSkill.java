...
public class LiechtenauerSkill extends WeaponInnateSkill {
    ...
    public void onInitiate(SkillContainer container) {
        container.getExecuter().getEventListener().addEventListener(PlayerEventListener.EventType.DEALT_DAMAGE_EVENT_POST, EVENT_UUID, event -> {
            if (container.isActivated() && !container.isDisabled() && !event.getTarget().isAlive()) {
        ...
        container.getExecuter().getEventListener().addEventListener(PlayerEventListener.EventType.HURT_EVENT_PRE, EVENT_UUID, event -> {
            ...
            if (event.getAmount() > 0.0F && container.isActivated() && !container.isDisabled() && phaseLevel > 0 && phaseLevel < 3 && this.canExecute(event.getPlayerPatch()) && isBlockableSource((DamageSource) event.getDamageSource())) {
    ...
    public boolean canExecute(PlayerPatch<?> executer) {
        if (executer.isLogicalClient()) {
            ...
            return EpicFightCapabilities.getItemStackCapability(itemstack).getInnateSkill(executer, itemstack) == this && executer.getOriginal().m_20202_() == null;
        }
    }
