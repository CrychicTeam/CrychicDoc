...
public class GraspingSpireSkill extends WeaponInnateSkill {
    ...
    private AnimationProvider.AttackAnimationProvider first = () -> (AttackAnimation) Animations.GRASPING_SPIRAL_FIRST;

    private AnimationProvider.AttackAnimationProvider second = () -> (AttackAnimation) Animations.GRASPING_SPIRAL_SECOND;

    public GraspingSpireSkill(Skill.Builder<? extends Skill> builder) {
        super(builder);
    }
    ...
    public void onInitiate(SkillContainer container) {
        ...
        container.getExecuter().getEventListener().addEventListener(PlayerEventListener.EventType.DEALT_DAMAGE_EVENT_PRE, EVENT_UUID, event -> {
            if (this.second.get().equals(event.getDamageSource().getAnimation())) {
                ...
                event.getDamageSource().setImpact(impact + (float) container.getDataManager().getDataValue(SkillDataKeys.LAST_HIT_COUNT.get()).intValue() * 0.4F);
            }
        });
    }
    ...
    public List<Component> getTooltipOnItem(ItemStack itemStack, CapabilityItem cap, PlayerPatch<?> playerCap) {
        ...
        this.generateTooltipforPhase(list, itemStack, cap, playerCap, (Map<AnimationProperty.AttackPhaseProperty<?>, Object>) this.properties.get(0), "Pierce:");
