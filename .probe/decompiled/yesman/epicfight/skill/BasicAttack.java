...
public class BasicAttack extends Skill {
    ...
    public static Skill.Builder<BasicAttack> createBasicAttackBuilder() {
        return new Skill.Builder<BasicAttack>().setCategory(SkillCategories.BASIC_ATTACK).setActivateType(Skill.ActivateType.ONE_SHOT).setResource(Skill.Resource.NONE);
    }
    ...
    public void onInitiate(SkillContainer container) {
        container.getExecuter().getEventListener().addEventListener(PlayerEventListener.EventType.ACTION_EVENT_SERVER, EVENT_UUID, event -> {
            if (!event.getAnimation().isBasicAttackAnimation() && (Boolean) event.getAnimation().getProperty(AnimationProperty.ActionAnimationProperty.RESET_PLAYER_COMBO_COUNTER).orElse(true)) {
                CapabilityItem item = ((ServerPlayerPatch) event.getPlayerPatch()).getHoldingItemCapability(InteractionHand.MAIN_HAND);
                if (item.shouldCancelCombo(event.getPlayerPatch())) {
                    setComboCounterWithEvent(ComboCounterHandleEvent.Causal.ACTION_ANIMATION_RESET, (ServerPlayerPatch) event.getPlayerPatch(), container, event.getAnimation(), 0);
                }
            }
        });
    }
    ...
    public void executeOnServer(ServerPlayerPatch executer, FriendlyByteBuf args) {
        ...
        if (!executer.getEventListener().triggerEvents(PlayerEventListener.EventType.BASIC_ATTACK_EVENT, new BasicAttackEvent(executer))) {
            ...
            if (player.m_20159_()) {
                if (player.m_20202_() instanceof PlayerRideableJumping ridable && ridable.canJump() && cap.availableOnHorse() && cap.getMountAttackMotion() != null) {
