...
public class SkillDataManager {
    ...
    public <T> void registerData(SkillDataKey<T> key) {
        if (this.hasData(key)) {
            ...
            if (key.syncronizeTrackingPlayers() && !this.container.getExecuter().isLogicalClient()) {
                ...
                EpicFightNetworkManager.sendToAllPlayerTrackingThisEntity(new SPAddOrRemoveSkillData(key, this.container.getSlot().universalOrdinal(), key.defaultValue(), SPAddOrRemoveSkillData.AddRemove.ADD, owner.m_19879_()), owner);
            }
        }
    }
    ...
    public <T> void removeData(SkillDataKey<T> key) {
        ...
        if (key.syncronizeTrackingPlayers() && !this.container.getExecuter().isLogicalClient()) {
            Player owner = this.container.getExecuter().getOriginal();
            EpicFightNetworkManager.sendToAllPlayerTrackingThisEntity(new SPAddOrRemoveSkillData(key, this.container.getSlot().universalOrdinal(), null, SPAddOrRemoveSkillData.AddRemove.REMOVE, owner.m_19879_()), owner);
        }
    }
