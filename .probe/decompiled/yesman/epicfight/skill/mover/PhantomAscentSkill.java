...
public class PhantomAscentSkill extends Skill {
    ...
    public void onInitiate(SkillContainer container) {
        ...
        listener.addEventListener(PlayerEventListener.EventType.MOVEMENT_INPUT_EVENT, EVENT_UUID, event -> {
            if (event.getPlayerPatch().getOriginal().m_20202_() == null && event.getPlayerPatch().isBattleMode() && !event.getPlayerPatch().getOriginal().m_150110_().flying && !event.getPlayerPatch().isChargingSkill() && !event.getPlayerPatch().getEntityState().inaction()) {
                ...
                if (jumpPressed && !jumpPressedPrev) {
                    ...
                    if (jumpCounter <= 0 && event.getPlayerPatch().currentLivingMotion != LivingMotions.FALL) {
                        ...
                        Vec3 forwardHorizontal = Vec3.directionFromRotation(new Vec2(0.0F, container.getExecuter().getOriginal().m_5675_(1.0F)));
                        ...
                        Vec3 deltaMove = container.getExecuter().getOriginal().m_20184_();
                        container.getExecuter().getOriginal().m_20334_(deltaMove.x + jumpDir.x, 0.6 + (double) container.getExecuter().getOriginal().m_285755_(), deltaMove.z + jumpDir.z);
