...
public class PlayerModeCommand {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        LiteralArgumentBuilder<CommandSourceStack> builder = (LiteralArgumentBuilder<CommandSourceStack>) Commands.literal("mode").requires(commandSourceStack -> commandSourceStack.hasPermission(2));
        for (PlayerPatch.PlayerMode mode : PlayerPatch.PlayerMode.values()) {
            builder.then(((LiteralArgumentBuilder) Commands.literal(mode.name().toLowerCase(Locale.ROOT)).executes(command -> setMode(command, Collections.singleton(((CommandSourceStack) command.getSource()).getPlayerOrException()), mode))).then(Commands.argument("target", EntityArgument.players()).executes(p_137728_ -> setMode(p_137728_, EntityArgument.getPlayers(p_137728_, "target"), mode))));
        }
    ...
    private static void logGamemodeChange(CommandSourceStack command, ServerPlayer serverPlayer, PlayerPatch.PlayerMode playerMode) {
        ...
        if (command.getEntity() == serverPlayer) {
            ...
            if (command.getLevel().m_46469_().getBoolean(GameRules.RULE_SENDCOMMANDFEEDBACK)) {
                serverPlayer.sendSystemMessage(Component.translatable("gameMode.changed", component));
            }
            command.sendSuccess(() -> Component.translatable("commands.gamemode.success.other", serverPlayer.m_5446_(), component), true);
        }
    }
}