    ...
    public static void handle(CPPlayAnimation msg, Supplier<NetworkEvent.Context> ctx) {
        ((NetworkEvent.Context) ctx.get()).enqueueWork(() -> {
            ...
            if (!msg.isClientSideAnimation) {
                playerpatch.<Animator>getAnimator().playAnimation(msg.namespaceId, msg.animationId, msg.modifyTime);
            }
            EpicFightNetworkManager.sendToAllPlayerTrackingThisEntity(new SPPlayAnimation(msg.namespaceId, msg.animationId, serverPlayer.m_19879_(), msg.modifyTime), serverPlayer);
            if (msg.resendToSender) {
                EpicFightNetworkManager.sendToPlayer(new SPPlayAnimation(msg.namespaceId, msg.animationId, serverPlayer.m_19879_(), msg.modifyTime), serverPlayer);
            }
        });
