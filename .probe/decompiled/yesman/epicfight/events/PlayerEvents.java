...
public class PlayerEvents {
    ...
    public static void rightClickItemServerEvent(PlayerInteractEvent.RightClickItem event) {
        if (event.getSide() == LogicalSide.SERVER) {
            ...
            if (playerpatch != null && (itemstack.getUseAnimation() == UseAnim.NONE || !playerpatch.getHoldingItemCapability(InteractionHand.MAIN_HAND).getStyle(playerpatch).canUseOffhand())) {
                boolean canceled = playerpatch.getEventListener().triggerEvents(PlayerEventListener.EventType.SERVER_ITEM_USE_EVENT, new RightClickItemEvent<>(playerpatch));
    ...
    public static void cloneEvent(PlayerEvent.Clone event) {
        ...
        if (oldCap != null) {
            ...
            if (!event.isWasDeath() || event.getOriginal().m_9236_().getGameRules().getBoolean(EpicFightGamerules.KEEP_SKILLS)) {
    ...
    public static void changeDimensionEvent(PlayerEvent.PlayerChangedDimensionEvent event) {
        ...
        EpicFightNetworkManager.sendToPlayer(new SPChangeGamerule(SPChangeGamerule.SynchronizedGameRules.WEIGHT_PENALTY, player.m_9236_().getGameRules().getInt(EpicFightGamerules.WEIGHT_PENALTY)), (ServerPlayer) player);
        EpicFightNetworkManager.sendToPlayer(new SPChangeGamerule(SPChangeGamerule.SynchronizedGameRules.DIABLE_ENTITY_UI, player.m_9236_().getGameRules().getBoolean(EpicFightGamerules.DISABLE_ENTITY_UI)), (ServerPlayer) player);
    }
    ...
    public static void itemUseStopEvent(LivingEntityUseItemEvent.Stop event) {
        if (event.getEntity().m_9236_().isClientSide()) {
            ...
            ServerPlayerPatch playerpatch = EpicFightCapabilities.getEntityPatch(player, ServerPlayerPatch.class);
            if (playerpatch != null) {
                boolean canceled = playerpatch.getEventListener().triggerEvents(PlayerEventListener.EventType.SERVER_ITEM_STOP_EVENT, new ItemUseEndEvent(playerpatch, event));
                event.setCanceled(canceled);
            }
        }
    }
    ...
    public static void attackEntityEvent(AttackEntityEvent event) {
        boolean var10000;
        label22: {
            if (event.getTarget() instanceof LivingEntity livingEntity && livingEntity.attackable()) {
                var10000 = true;
                break label22;
            }
            var10000 = false;
        }
        boolean isLivingTarget = var10000;
        PlayerPatch<?> playerpatch = EpicFightCapabilities.getEntityPatch(event.getEntity(), PlayerPatch.class);
        if (playerpatch != null && !event.getEntity().m_9236_().getGameRules().getBoolean(EpicFightGamerules.DO_VANILLA_ATTACK) && isLivingTarget && playerpatch.getEpicFightDamageSource() == null) {
            event.setCanceled(true);
        }
    }
}