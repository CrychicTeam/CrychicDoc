...
public class LocalPlayerPatch extends AbstractClientPlayerPatch<LocalPlayer> {
    ...
    public void clientTick(LivingEvent.LivingTickEvent event) {
        ...
        if (renderEngine.isPlayerRotationLocked()) {
            ...
            Vec3 vec3 = this.original.m_20299_(1.0F);
            ...
            AABB aabb = this.original.m_20191_().expandTowards(vec31.scale(pickRange)).inflate(1.0, 1.0, 1.0);
            cameraHitResult = ProjectileUtil.getEntityHitResult(this.original, vec3, vec32, aabb, hit -> !hit.isSpectator() && hit.isPickable() && !hit.is(this.grapplingTarget), pickRange);
        }
        if (cameraHitResult != null && cameraHitResult.getType() == HitResult.Type.ENTITY) {
            ...
            if (hit != this.rayTarget) {
                if (hit instanceof LivingEntity livingentity) {
                    ...
                    if (partEntity.getParent() instanceof LivingEntity parentLivingEntity && !this.targetLockedOn) {
        ...
        if (this.rayTarget != null) {
            if (this.targetLockedOn) {
                Vec3 playerPosition = this.original.m_146892_();
                ...
                if (!this.getEntityState().turningLocked() || this.getEntityState().lockonRotate()) {
                    this.original.m_146926_(lockOnXRotDst);
                    this.original.m_146922_(lockOnYRotDst);
                }
            } else {
            ...
            if (!this.rayTarget.isAlive() || this.getOriginal().m_20280_(this.rayTarget) > 400.0 || this.getAngleTo(this.rayTarget) > 100.0 && !this.targetLockedOn) {
                this.rayTarget = null;
    ...
    public void updateHeldItem(CapabilityItem mainHandCap, CapabilityItem offHandCap) {
        ...
        if (EpicFightMod.CLIENT_CONFIGS.battleAutoSwitchItems.contains(this.original.m_21205_().getItem())) {
            this.toBattleMode(true);
        } else if (EpicFightMod.CLIENT_CONFIGS.miningAutoSwitchItems.contains(this.original.m_21205_().getItem())) {
    ...
    public void correctRotation() {
        if (this.targetLockedOn) {
            if (this.rayTarget != null && !this.rayTarget.m_213877_()) {
                ...
                float yaw = (float) MathUtils.getYRotOfVector(toTarget);
                float pitch = (float) MathUtils.getXRotOfVector(toTarget);
                this.original.m_146922_(yaw);
                this.original.m_146926_(pitch);
            } else {
                this.original.m_146922_(this.lockOnYRot);
                this.original.m_146926_(this.lockOnXRot);
            }
