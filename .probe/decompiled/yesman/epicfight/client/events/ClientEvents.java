...
public class ClientEvents {
    ...
    public static void mouseClickEvent(ScreenEvent.MouseButtonPressed.Pre event) {
        if (event.getScreen() instanceof AbstractContainerScreen) {
            ...
            if (slot != null) {
                CapabilityItem cap = EpicFightCapabilities.getItemStackCapability(minecraft.player.f_36096_.getCarried());
                if (!cap.canBePlacedOffhand() && slot.getNoItemIcon() != null && slot.getNoItemIcon().equals(OFFHAND_TEXTURE)) {
    ...
    public static void presssKeyInGui(ScreenEvent.KeyPressed.Pre event) {
        ...
        if (event.getKeyCode() == minecraft.options.keySwapOffhand.getKey().getValue()) {
            ...
            if (slot != null && slot.getNoItemIcon() != null && slot.getNoItemIcon().equals(OFFHAND_TEXTURE)) {
                itemCapability = EpicFightCapabilities.getItemStackCapability(minecraft.player.m_150109_().getItem(event.getKeyCode() - 49));
                if (!itemCapability.canBePlacedOffhand()) {
                    event.setCanceled(true);
                }
            }
    ...
    public static void rightClickItemClient(PlayerInteractEvent.RightClickItem event) {
        if (event.getSide() == LogicalSide.CLIENT) {
            ...
            if (playerpatch != null && playerpatch.getOriginal().m_21206_().getUseAnimation() == UseAnim.NONE) {
                boolean canceled = playerpatch.getEventListener().triggerEvents(PlayerEventListener.EventType.CLIENT_ITEM_USE_EVENT, new RightClickItemEvent<>(playerpatch));
