...
public class BattleModeGui extends ModIngameGui {
    ...
    public void renderGui(LocalPlayerPatch playerpatch, GuiGraphics guiGraphics, float partialTicks) {
        if (playerpatch.getOriginal().m_6084_() && playerpatch.getOriginal().m_20202_() == null) {
            if (this.sliding <= 28) {
                if (this.sliding > 0) {
                ...
                if (maxStamina > 0.0F && stamina < maxStamina) {
                    ...
                    guiGraphics.blit(EntityIndicator.BATTLE_ICON, pos.x, pos.y, 118, 4, 2.0F, 38.0F, 237, 9, 255, 255);
                    guiGraphics.blit(EntityIndicator.BATTLE_ICON, pos.x, pos.y, (int) (118.0F * ratio), 4, 2.0F, 47.0F, (int) (237.0F * ratio), 9, 255, 255);
                ...
                int passiveX = (Integer) horBasis.positionGetter.apply(width, this.config.passivesX.getValue());
                int passiveY = (Integer) verBasis.positionGetter.apply(height, this.config.passivesY.getValue());
                ...
                for (SkillContainer container : this.skillIcons) {
                    if (!container.isEmpty()) {
                        ...
                        if (skill.shouldDraw(container)) {
                            ...
                            skill.drawOnGui(this, container, guiGraphics, (float) slotCoord.x, (float) slotCoord.y);
    ...
    private void drawWeaponInnateIcon(LocalPlayerPatch playerpatch, SkillContainer container, GuiGraphics guiGraphics, float partialTicks) {
        ...
        poseStack.pushPose();
        poseStack.translate(0.0F, (float) this.sliding, 0.0F);
        boolean creative = playerpatch.getOriginal().m_7500_();
        ...
        for (int j = 0; j < var27; j++) {
            bufferbuilder.m_252986_(poseStack.last().pose(), (float) pos.x + iconSize * CLOCK_POS[j].x, (float) pos.y + iconSize * CLOCK_POS[j].y, 0.0F).uv(CLOCK_POS[j].x, CLOCK_POS[j].y).endVertex();
        }
        bufferbuilder.m_252986_(poseStack.last().pose(), lastVertexX, lastVertexY, 0.0F).uv(lastTexX, lastTexY).endVertex();
        tessellator.end();
        if (canUse) {
        ...
        if (container.getSkill().getMaxStack() > 1) {
            ...
            int stringWidth = (this.font.width(s) - 6) / 3;
            guiGraphics.drawString(this.font, s, pos.x + 25 - stringWidth, pos.y + 22, 16777215, true);
        }
        poseStack.popPose();
    }
