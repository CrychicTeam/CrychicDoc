...
public abstract class EntityIndicator extends ModIngameGui {
    ...
    public void drawTexturedModalRect2DPlane(Matrix4f matrix, VertexConsumer vertexBuilder, float minX, float minY, float maxX, float maxY, float minTexU, float minTexV, float maxTexU, float maxTexV) {
        this.drawTexturedModalRect3DPlane(matrix, vertexBuilder, minX, minY, (float) this.getBlitOffset(), maxX, maxY, (float) this.getBlitOffset(), minTexU, minTexV, maxTexU, maxTexV);
    }
    ...
    public final Matrix4f getMVMatrix(PoseStack poseStack, LivingEntity entity, float x, float y, float z, boolean lockRotation, float partialTicks) {
        float posX = (float) Mth.lerp((double) partialTicks, entity.f_19790_, entity.m_20185_());
        float posY = (float) Mth.lerp((double) partialTicks, entity.f_19791_, entity.m_20186_());
        float posZ = (float) Mth.lerp((double) partialTicks, entity.f_19792_, entity.m_20189_());
        poseStack.pushPose();
