...
public class WearableItemLayer<E extends LivingEntity, T extends LivingEntityPatch<E>, M extends HumanoidModel<E>, AM extends HumanoidMesh> extends PatchedLayer<E, T, M, HumanoidArmorLayer<E, M, M>, AM> {
    ...
    private void renderTrim(PoseStack matStack, MultiBufferSource multiBufferSource, int packedLightIn, boolean hasEffect, AnimatedMesh model, Armature armature, ArmorMaterial armorMaterial, ArmorTrim armorTrim, OpenMatrix4f[] poses) {
        ...
        TextureAtlasSprite textureatlassprite = (TextureAtlasSprite) this.armorTrimAtlas.apply(armorTexture);
                ...
                if (slot != EquipmentSlot.HEAD || !this.firstPersonModel) {
                    ...
                    if (stack.getItem() instanceof ArmorItem armorItem) {
                        ...
                        if (slot == EquipmentSlot.HEAD) {
                            poseStack.translate(0.0, (double) head * 0.055, 0.0);
                        }
                        boolean debuggingMode = ClientEngine.getInstance().isArmorModelDebuggingMode();
                        if (debuggingMode) {
                            poseStack.pushPose();
                            poseStack.scale(-1.0F, -1.0F, 1.0F);
                            poseStack.translate(1.0, -1.501, 0.0);
                            vanillaLayer.render(poseStack, buf, packedLightIn, entityliving, partialTicks, head, (float) packedLightIn, bob, yRot, xRot);
                            poseStack.popPose();
                        }
                        ...
                        ArmorTrim.getTrim(entityliving.m_9236_().registryAccess(), stack).ifPresent(armorTrim -> this.renderTrim(poseStack, buf, packedLightIn, hasEffect, armorMesh, entitypatch.getArmature(), armorItem.getMaterial(), armorTrim, poses));
                        poseStack.popPose();
                    }
                }
            }
    ...
    private AnimatedMesh getArmorModel(HumanoidArmorLayer<E, M, M> originalRenderer, M originalModel, E entityliving, ArmorItem armorItem, ItemStack stack, EquipmentSlot slot, boolean armorDebugging) {
        ...
        if (ARMOR_MODELS.containsKey(registryName) && !armorDebugging) {
            return (AnimatedMesh) ARMOR_MODELS.get(registryName);
        } else {
    ...
    private ResourceLocation getArmorTexture(ItemStack itemstack, LivingEntity entity, AnimatedMesh armorMesh, EquipmentSlot slot, String type, M originalModel) {
        ...
        int idx2 = s1.lastIndexOf(47);
        String s2 = String.format("%s/epicfight/%s", s1.substring(0, idx2), s1.substring(idx2 + 1));
        ResourceLocation resourcelocation2 = (ResourceLocation) EPICFIGHT_OVERRIDING_TEXTURES.get(s2);
        if (resourcelocation2 != null) {
            ...
            if (extensionTexturePath != null) {
                ...
                ResourceLocation resourcelocation = (ResourceLocation) HumanoidArmorLayer.ARMOR_LOCATION_CACHE.get(s1);
