...
public class PatchedHeadLayer<E extends LivingEntity, T extends LivingEntityPatch<E>, M extends EntityModel<E> & HeadedModel, AM extends HumanoidMesh> extends PatchedLayer<E, T, M, CustomHeadLayer<E, M>, AM> {
    ...
    protected void renderLayer(T entitypatch, E entityliving, CustomHeadLayer<E, M> vanillaLayer, PoseStack postStack, MultiBufferSource buffer, int packedLightIn, OpenMatrix4f[] poses, float bob, float yRot, float xRot, float partialTicks) {
        ...
        if (!itemstack.isEmpty()) {
            ModelPart model = ((HeadedModel) vanillaLayer.m_117386_()).getHead();
            ...
            vanillaLayer.render(postStack, buffer, packedLightIn, entity, entity.walkAnimation.position(), entity.walkAnimation.speed(), (float) packedLightIn, (float) entity.f_19797_, yRot, xRot);
            postStack.popPose();
        }
    }
}