...
public class PatchedItemInHandLayer<E extends LivingEntity, T extends LivingEntityPatch<E>, M extends EntityModel<E>, AM extends HumanoidMesh> extends PatchedLayer<E, T, M, RenderLayer<E, M>, AM> {
    ...
    protected void renderLayer(T entitypatch, E entityliving, RenderLayer<E, M> vanillaLayer, PoseStack postStack, MultiBufferSource buffer, int packedLightIn, OpenMatrix4f[] poses, float bob, float yRot, float xRot, float partialTicks) {
        if (entitypatch.getArmature() instanceof HumanoidArmature humanoidArmature) {
            ItemStack var16 = entitypatch.getOriginal().getMainHandItem();
            RenderEngine renderEngine = ClientEngine.getInstance().renderEngine;
            if (var16.getItem() != Items.AIR) {
                if (entitypatch.getOriginal().m_20202_() != null && !entitypatch.getHoldingItemCapability(InteractionHand.MAIN_HAND).availableOnHorse()) {
                    renderEngine.getItemRenderer(var16.getItem()).renderUnusableItemMount(var16, entitypatch, poses, buffer, postStack, packedLightIn);
                    return;
                }
                renderEngine.getItemRenderer(var16.getItem()).renderItemInHand(var16, entitypatch, InteractionHand.MAIN_HAND, humanoidArmature, poses, buffer, postStack, packedLightIn);
            }
