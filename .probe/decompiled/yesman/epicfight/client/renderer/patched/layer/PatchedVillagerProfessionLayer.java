...
public class PatchedVillagerProfessionLayer extends PatchedLayer<ZombieVillager, MobPatch<ZombieVillager>, ZombieVillagerModel<ZombieVillager>, VillagerProfessionLayer<ZombieVillager, ZombieVillagerModel<ZombieVillager>>, HumanoidMesh> {
    ...
    protected void renderLayer(MobPatch<ZombieVillager> entitypatch, ZombieVillager entityliving, VillagerProfessionLayer<ZombieVillager, ZombieVillagerModel<ZombieVillager>> vanillaLayer, PoseStack postStack, MultiBufferSource buffer, int packedLightIn, OpenMatrix4f[] poses, float bob, float yRot, float xRot, float partialTicks) {
        if (!entityliving.m_20145_()) {
            ...
            if (typeHat != VillagerMetaDataSection.Hat.NONE && (typeHat != VillagerMetaDataSection.Hat.PARTIAL || professionHat == VillagerMetaDataSection.Hat.FULL) || !entityliving.m_6844_(EquipmentSlot.HEAD).isEmpty()) {
                this.mesh.head.hidden = true;
                this.mesh.hat.hidden = true;
            }
            if (!entitypatch.getOriginal().m_6844_(EquipmentSlot.LEGS).isEmpty()) {
