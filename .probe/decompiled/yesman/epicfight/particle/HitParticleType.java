...
public class HitParticleType extends SimpleParticleType {
    ...
    public static final BiFunction<Entity, Entity, Vector3d> CENTER_OF_TARGET = (target, attacker) -> {
        ...
        double y = target.getY() + (double) size.width * 0.5;
    ...
    public static final BiFunction<Entity, Entity, Vector3d> RANDOM_WITHIN_BOUNDING_BOX = (target, attacker) -> {
        ...
        double x = target.getX() + (random.nextDouble() - 0.5) * (double) size.width;
        double y = target.getY() + (random.nextDouble() + (double) size.height) * 0.5;
        double z = target.getZ() + (random.nextDouble() - 0.5) * (double) size.width;
        return new Vector3d(x, y, z);
    };
    ...
    public static final BiFunction<Entity, Entity, Vector3d> FRONT_OF_EYES = (target, attacker) -> {
        ...
        Vec3 viewVec = target.getLookAngle().scale(2.0);
    ...
    public static final BiFunction<Entity, Entity, Vector3d> MIDDLE_OF_ENTITIES = (target, attacker) -> {
        Vec3 targetPos = target.position().add(0.0, (double) (target.getBbHeight() * 0.5F), 0.0);
        Vec3 attackerPos = attacker.position().add(0.0, (double) (target.getBbHeight() * 0.5F), 0.0);
        Vec3 to = attackerPos.subtract(targetPos).scale(0.5);
    ...
    public static final BiFunction<Entity, Entity, Vector3d> ATTACKER_XY_ROTATION = (target, attacker) -> new Vector3d((double) attacker.getViewXRot(1.0F), (double) attacker.getViewYRot(1.0F), -1.0);

    public static final BiFunction<Entity, Entity, Vector3d> ATTACKER_Y_ROTATION = (target, attacker) -> new Vector3d(90.0, (double) attacker.getViewYRot(1.0F), -1.0);
    ...
    public void spawnParticleWithArgument(ServerLevel world, BiFunction<Entity, Entity, Vector3d> positionProvider, BiFunction<Entity, Entity, Vector3d> argumentProvider, Entity e1, Entity e2) {
        Vector3d position = positionProvider == null ? (Vector3d) this.positionProvider.apply(e1, e2) : (Vector3d) positionProvider.apply(e1, e2);
        Vector3d arguments = argumentProvider == null ? (Vector3d) this.argumentProvider.apply(e1, e2) : (Vector3d) argumentProvider.apply(e1, e2);
        world.sendParticles(this, position.x, position.y, position.z, 0, arguments.x, arguments.y, arguments.z, 1.0);
    }
    ...
    public void spawnParticleWithArgument(ServerLevel world, double posX, double posY, double posZ, double argX, double argY, double argZ) {
        world.sendParticles(this, posX, posY, posZ, 0, argX, argY, argZ, 1.0);
    }
}