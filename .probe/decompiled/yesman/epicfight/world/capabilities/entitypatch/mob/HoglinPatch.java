...
public class HoglinPatch extends MobPatch<Hoglin> {
    ...
    protected void initAttributes() {
        ...
        this.original.m_21051_(EpicFightAttributes.IMPACT.get()).setBaseValue(5.0);
    }
    ...
    public SoundEvent getWeaponHitSound(InteractionHand hand) {
        return this.original.m_6162_() ? EpicFightSounds.BLUNT_HIT.get() : EpicFightSounds.BLUNT_HIT_HARD.get();
    }

    @Override
    public SoundEvent getSwingSound(InteractionHand hand) {
        return this.original.m_6162_() ? EpicFightSounds.WHOOSH.get() : EpicFightSounds.WHOOSH_BIG.get();
    }

    @Override
    public LivingEntity getTarget() {
        Optional<LivingEntity> opt = this.original.getBrain().getMemory(MemoryModuleType.ATTACK_TARGET);
        return (LivingEntity) opt.orElse(null);
    }
}