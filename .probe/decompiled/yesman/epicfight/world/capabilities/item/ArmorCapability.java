...
public class ArmorCapability extends CapabilityItem {
    ...
    public Multimap<Attribute, AttributeModifier> getAttributeModifiers(EquipmentSlot equipmentSlot, LivingEntityPatch<?> entitypatch) {
        ...
        if (equipmentSlot == this.equipmentSlot) {
            map.put(EpicFightAttributes.WEIGHT.get(), new AttributeModifier(ARMOR_MODIFIERS[equipmentSlot.getIndex()], "Armor modifier", this.weight, AttributeModifier.Operation.ADDITION));
            map.put(EpicFightAttributes.STUN_ARMOR.get(), new AttributeModifier(ARMOR_MODIFIERS[equipmentSlot.getIndex()], "Armor modifier", this.stunArmor, AttributeModifier.Operation.ADDITION));
        }
        return map;
    }
    ...
    public static class Builder extends CapabilityItem.Builder {
        ...
        public ArmorCapability.Builder item(Item item) {
            if (item instanceof ArmorItem armorItem) {
                ...
                if (this.weight < 0.0) {
                    this.weight = (double) ((float) armorMaterial.getDefenseForType(armorType) * 2.5F);
                }
                if (this.stunArmor < 0.0) {
                    this.stunArmor = (double) ((float) armorMaterial.getDefenseForType(armorType) * 0.375F);
                }
            }
            return this;
        }
