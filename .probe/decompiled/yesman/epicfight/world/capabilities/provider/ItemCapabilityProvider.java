...
public class ItemCapabilityProvider implements ICapabilityProvider, NonNullSupplier<CapabilityItem> {
    ...
    public static void registerWeaponTypesByClass() {
        CAPABILITY_BY_CLASS.put(ArmorItem.class, (Function) item -> ArmorCapability.builder().item(item));
        ...
        CAPABILITY_BY_CLASS.put(MapItem.class, (Function) item -> MapCapability.builder());
    }
    ...
    public static CapabilityItem get(Item item) {
        return (CapabilityItem) CAPABILITIES.getOrDefault(item, CAPABILITY_BY_CLASS.containsKey(item.getClass()) ? ((CapabilityItem.Builder) ((Function) CAPABILITY_BY_CLASS.get(item.getClass())).apply(item)).build() : null);
    }
    ...
    public static void addDefaultItems() {
        for (Item item : ForgeRegistries.ITEMS.getValues()) {
            if (!CAPABILITIES.containsKey(item)) {
                ...
                for (capability = null; clazz != null && capability == null; clazz = clazz.getSuperclass()) {
                    if (CAPABILITY_BY_CLASS.containsKey(clazz)) {
                        capability = ((CapabilityItem.Builder) ((Function) CAPABILITY_BY_CLASS.get(clazz)).apply(item)).build();
                    }
                }
                if (capability != null) {
    ...
    public ItemCapabilityProvider(ItemStack itemstack) {
        this.capability = (CapabilityItem) CAPABILITIES.get(itemstack.getItem());
